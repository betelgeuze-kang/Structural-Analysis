...
- Current state: P0/P1/P1 breadth status commands converge on the same release-publication evidence index, PM release milestones M1-M5 pass, and the source structure viewer has a real Playwright browser smoke for the registered optimized/real-drawing 3D preset. The broader release-area gate is still blocked by CI streak, evidence freshness/provenance metadata, human UX observation, and license-status evidence.
...
- Release evidence freshness is now a separate PM release area: `python3 scripts/report_release_evidence_freshness.py` audits `p0_closure_status.json`, `p1_readiness_status.json`, and `p1_benchmark_breadth_status.json` for `generated_at`, source commit, engine version, input checksum, reuse marker, and producer mtime. Current status is blocked: producer mtime passes `3/3`, but those three artifacts still lack the required metadata, yielding 15 local-remediation freshness blockers and PM release areas `11/15` green.
...
- The measured corpus exit status is generated by `python3 implementation/phase1/check_real_project_corpus_measured_status.py`. Current status is blocked by PEER metric values: KR measured rows are `10/10`, measured formats are `2/2`, but PEER metric-bearing groups with values are `1/5`.
...
- Customer shadow evidence is schema-gated by `implementation/phase1/customer_shadow_evidence.schema.json` and `implementation/phase1/validate_customer_shadow_evidence.py`. It records checksums, metrics, reviewer decision, and reproduce bundle id only; raw customer data must remain customer-retained and `redistribution_allowed=false`.
