...
- Release evidence freshness is now a passing PM release area: `python3 scripts/report_release_evidence_freshness.py` audits `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, and `customer_shadow_evidence_status.json` for `generated_at`, source commit, engine version, input checksum, reuse marker, and producer mtime. Current status is PASS with `5/5` rows passing; PM release areas are `12/15` green, with remaining blockers limited to CI streak, human UX observation, and license-status evidence. Freshness pass does not close customer shadow readiness; that gate remains blocked until real completed-project evidence reaches `3/3`.
