...
- Commercial scope is release-facing and intentionally bounded: grade is `Commercial`, `engineer_in_loop_accelerated_coverage_ready=true` for 95-99% accelerated coverage, but `full_commercial_replacement_ready=false` with a 1-5% residual holdout queue tracked as owner/status/work item for licensed engineer review, legacy tool cross-validation, and legal/authority sign-off.
...
- P0-1 remains closed only while the release has exactly the 12 manifest assets, metadata preflight passes, upload-plan SHA/bytes match the promoted manifest, and hydrated published bytes verify cleanly.
...
- Release verification runbook:
  ...
  6. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest <candidate-manifest.json> --assets-json <release-assets.json> --artifact-root <fresh-release-asset-root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --tag-ref-present true --require-all --require-exact --fail-unclosed`.
  7. Overall P0 status: run `python3 scripts/check_p0_closure_status.py --manifest <candidate-manifest.json> --release-assets-json <release-assets.json> --artifact-root <fresh-release-asset-root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --tag-ref-present --fail-open` to combine P0-1 publication evidence with P0-2..P0-6 core evidence against the candidate manifest.
  ...
  9. Published status: `structural-analysis-artifacts-2026-04-26` is published as a GitHub Release with the 12 manifest-listed assets. Repo-local `implementation/phase1/release/` is still not an upload source; verify the release through asset listing, upload plan, metadata preflight, and hydrated published-byte SHA/bytes checks.
