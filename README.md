...
- Current state: P0/P1/P1 breadth status commands converge on the same release-publication evidence index, PM release milestones M1-M5 pass, release evidence freshness passes, and the source structure viewer has a real Playwright browser smoke for the registered optimized/real-drawing 3D preset. The broader release-area gate is still blocked by CI streak, human UX observation, license-status evidence, and read-only GitHub sync pending until feature/main remote refs match local HEAD. GA/Enterprise handoff is separately blocked by independent V&V, family validation manual signoff, customer audit/failure-bundle/SLA evidence, and fresh full-validation lane receipts.
...
- GitHub development sync is checked read-only with `python3 scripts/check_github_development_sync_preflight.py --fetch --json`. This preflight refreshes remote-tracking refs, verifies a clean worktree plus feature/main fast-forward readiness, lists only the refs still pending in `pending_remote_updates`, and intentionally reports `remote_mutation_approval_required` until an explicit R4 approval is given for those remaining remote updates. The PM release gate also evaluates this boundary as the read-only `github_sync` release area; by default it uses live local git state without fetch or remote mutation, and an optional `--github-sync-preflight` JSON can pin the input. Any unsynced feature/main ref remains a release-area blocker until the remote refs actually match local HEAD.
...
- Release evidence freshness is now a passing PM release area: `python3 scripts/report_release_evidence_freshness.py` audits `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, `customer_shadow_evidence_status.json`, `customer_shadow_evidence_intake_packet.json`, `fresh_full_validation_lane_status.json`, `residual_level3_status.json`, `mgt_g1_direct_residual_terminal_gate_report.json`, `mgt_g1_followup387_shell_material_budgeted_continuation_status.json`, and `evidence_console_scope_status.json` for `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime, and declared input-checksum dependency mtime. Current status is PASS with `11/11` rows passing; PM release areas are `12/16` green, with release-area blockers for CI streak, human UX observation, license-status evidence, and the read-only `github_sync` area until feature/main remote refs actually match local HEAD. Freshness pass does not close customer shadow readiness, fresh full-validation readiness, G1 full-mesh nonlinear equilibrium, Evidence Console launch readiness, GitHub remote synchronization, or GA breadth.
- Fresh full-validation lane status is tracked separately with `python3 scripts/build_fresh_full_validation_lane_status.py --json`. Current status is blocked with lane contracts `8/8` visible but fresh receipts `0/8`; each future receipt must satisfy `implementation/phase1/fresh_validation_receipt.schema.json` through `implementation/phase1/validate_fresh_validation_receipt.py`, including `reused_evidence=false`, matching lane/runner identity, green contract fields, provenance, input checksums, artifact checksums, and claim boundary. CPU-required hydrated reports cannot replace torch-capable benchmark, GPU HIP, performance, surface/material/contact, MIDAS, productization heavy-profile, external benchmark, or design-optimization refresh receipts for Level 3 promotion.
- PM blocker handoff is tracked with `python3 scripts/build_pm_release_blocker_action_register.py --json`. Current action register has `19` open blocker handoffs: release-area CI/UX/Security/GitHub-sync `8`, GA external signoff `3`, and fresh full-validation receipt `8`. All `19/19` have handoff commands and acceptance criteria; this does not convert missing evidence, missing R4 approval, or unsynced remote refs into release PASS. The support bundle currently records `51/51` artifacts and PM failure-bundle coverage PASS.
...
- Independent commercial product status: `python3 scripts/check_independent_product_readiness.py --json` aggregates P0/P1, strict EB/RH evidence, runtime packaging, production ops/security, on-prem/air-gapped packaging skeleton, support bundle, static viewer performance budget, local browser performance probe, 11-case visual regression baseline, claim governance, and source-boundary state. Current status is blocked at `80/100`: runtime packaging, production ops/security hardening, on-prem/air-gapped skeleton packaging, support bundle, viewer report/workflow packaging, static viewer performance budget, local browser performance probe, 11-case visual regression baseline, and RH signed closure evidence are ready, while strict EB receipt remains `0/4`.
...
- The measured corpus exit status is generated by `python3 implementation/phase1/check_real_project_corpus_measured_status.py`. Current status is PASS for the initial metadata/value exit gate: KR measured rows are `10/10`, measured formats are `2/2`, and PEER metric-bearing groups with values are `5/5`. The same report keeps the claim boundary visible: PEER official reference-truth metric groups are `1`, measured-run KPI bridge groups are `3`, and raw redistribution remains blocked.
...
- Customer shadow evidence is schema-gated by `implementation/phase1/customer_shadow_evidence.schema.json` and `implementation/phase1/validate_customer_shadow_evidence.py`. The current status gate is `python3 implementation/phase1/check_customer_shadow_evidence_status.py`; it is intentionally blocked at `0/3` completed-project shadow cases until real customer-retained evidence files are attached. `python3 scripts/build_customer_shadow_evidence_intake_packet.py --json` generates five owner-input slots and validation commands for the 3-5 case target, but it does not create customer evidence or close the gate. It records SHA256 reference checksums, engine commit SHA, numeric delta/residual metrics, reviewer decision, and reproduce bundle id only; raw customer data must remain customer-retained and `redistribution_allowed=false`.
