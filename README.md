...
The portal path returns exact local contract results and an epoch-zero checkpoint
chain, but remains a bounded candidate until both independent Level 2 slots and
the remaining promotion gates pass.
...
| Corotational 2D fiber frame | experimental | no | bounded_j1_j5_and_exact_engineering_recovery_candidate | python_api, cli | corotational_one_bay_portal.v1; One-bay, one-story load-control CPU dense portal candidate only; general topology, native sparse assembly, member features, direct displacement control, both independent Level 2 comparisons, and release promotion remain open. |
