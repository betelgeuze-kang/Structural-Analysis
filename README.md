...
- Commercial scope is release-facing and intentionally bounded: grade is `Commercial`, `engineer_in_loop_accelerated_coverage_ready=true` for 95-99% accelerated coverage, but `full_commercial_replacement_ready=false` with a 1-5% residual holdout queue for licensed engineer review, legacy tool cross-validation, and legal/authority sign-off.
