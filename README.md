...
- Fresh full-validation lane status is tracked separately with `python3 scripts/build_fresh_full_validation_lane_status.py --json`. Current status is blocked with lane contracts `8/8` visible but fresh receipts `0/8`; CPU-required hydrated reports cannot replace torch-capable benchmark, GPU HIP, performance, surface/material/contact, MIDAS, productization heavy-profile, external benchmark, or design-optimization refresh receipts for Level 3 promotion.
