...
export async function validateRcLayoutSearch(raw: Uint8Array, read: StudyRead, work: (rows: RcObject[]) => RcObject, coverage: (plan: RcObject, oracle: RcObject | null) => RcObject): Promise<RcSearchReview> {
  ...
  const standalone = report.schema_version === 'experimental-rc-control-layout-strategy.v1'
  ...
  const arms: string[] = standalone ? [report.strategy] : ['price_order', 'learned_order']
  check((standalone || report.schema_version === 'experimental-rc-control-layout-search.v1') && keys(report.arms, arms)
  ...
  check(plan.schema_version === (standalone ? 'experimental-rc-control-layout-strategy-plan.v1' : 'experimental-rc-control-layout-search-plan.v1') && plan.plan_hash === report.plan_hash && plan.source_revision === report.source_revision
  ...
  check(!standalone || (plan.strategy === report.strategy && plan.oracle_after_online_arms === false && report.oracle === null
    && report.timing_scope === 'single_layout_strategy_preparation_ranking_full_reference_and_IO_excluding_final_report_write'), 'layout_standalone_scope_invalid')
  ...
  let policy: RcObject | null = null
  if (priceOnly) {
    check(plan.policy_hash === null && plan.training_report_hash === null && plan.ranking === null && report.historical_training_cost === null && report.ranking_wall_ns === 0, 'layout_price_only_invalid')
  } else {
    const policyBytes = await read('policy.json', MAX), policyDoc = document(policyBytes), loadedPolicy = policyDoc.value
    policy = loadedPolicy
    await selfHash(policyDoc.raw, policy, 'policy_hash')
    check(loadedPolicy.schema_version === 'experimental-rc-control-layout-policy.v1' && loadedPolicy.policy_hash === plan.policy_hash && hash(loadedPolicy.context_hash) && hash(loadedPolicy.label_comparison_hash)
      && same(loadedPolicy.features, features) && same(loadedPolicy.targets, targets) && num(loadedPolicy.ridge) && loadedPolicy.ridge > 0 && num(loadedPolicy.ood_margin) && loadedPolicy.ood_margin >= 0 && loadedPolicy.ood_margin <= 1, 'layout_policy_invalid')
    for (const k of ['mean', 'scale', 'minimum', 'maximum', 'target_scale']) check(Array.isArray(loadedPolicy[k]) && loadedPolicy[k].length === (k === 'target_scale' ? 7 : 163) && loadedPolicy[k].every(num), 'layout_policy_dimensions_invalid')
    check([...loadedPolicy.scale, ...loadedPolicy.target_scale].every((v: number) => v > 0) && loadedPolicy.minimum.every((v: number, i: number) => v <= loadedPolicy.maximum[i])
      && Array.isArray(loadedPolicy.weights) && loadedPolicy.weights.length === 164 && loadedPolicy.weights.every((r: unknown) => Array.isArray(r) && r.length === 7 && r.every(num)), 'layout_policy_weights_invalid')
    ...
    check(training.schema_version === 'experimental-rc-control-layout-training.v1' && training.report_hash === plan.training_report_hash && same(training, report.historical_training_cost)
      && training.labels_report_hash === loadedPolicy.label_comparison_hash && training.policy?.sha256 === await sha256Bytes(policyBytes) && training.policy.byte_length === policyBytes.byteLength
    ...
    for (const k of ['training_model_identities', 'training_sample_hashes']) check(Array.isArray(loadedPolicy[k]) && loadedPolicy[k].length === training.sample_count && loadedPolicy[k].every(hash) && new Set(loadedPolicy[k]).size === training.sample_count, 'layout_training_identities_invalid')
  ...
  check(Array.isArray(plan.predictions) && same(plan.predictions.map((r: RcObject) => r.candidate_id), priceOnly ? [] : ids), 'layout_prediction_pool_invalid')
  ...
  const ranked = priceOnly ? { detail: null, ordering: [] } : candidateRanking(plan.predictions, CHEAPER_BOUNDARY_RANKING)
  ...
  check(same(report.candidate_cost_optimality_audit, cost) && same(report.candidate_coverage_audit, (priceOnly ? null : coverage(plan, designs.exhaustive_oracle?.report ?? null))), 'layout_audit_invalid')
