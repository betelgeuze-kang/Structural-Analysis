  ...
  try {
    if (type === 'initialize' && !initialized) {
      ...
      const read = async (path: string, maximum: number, expected?: number) => {
        ...
        if (path === 'cohort.json' || path.endsWith('/strategy-runtime.json')) originals.set(path, bytes)
        return bytes
      }
    ...
    } else if (type === 'download' && review && (data.path === 'cohort.json' || review.executions.some(e => e.runtimePath === data.path))) {
