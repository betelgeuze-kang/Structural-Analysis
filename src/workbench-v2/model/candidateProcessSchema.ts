...
import { validateDesignComparisonReport, type VerifiedDesignComparison } from './designComparisonSchema'
...
export interface CandidateProcessManifest {
  schema_version: 'rc-fiber-candidate-process-review-bundle.v1'; source_revision: string
...
export function validateCandidateProcessManifest(value: unknown): CandidateProcessManifest {
  ...
  equal(m.schema_version, 'rc-fiber-candidate-process-review-bundle.v1'); equal(m.suite_file, 'suite.json')
...
function validateWorker(run: CandidateObject, declared: CandidateObject, frozenRequest: CandidateObject, manifest: CandidateProcessManifest, artifacts: Map<string, CandidateLoadedArtifact>, previous: CandidateObject[]): void {
  ...
  if (!run.attempted) { equal(run.report_contract_pass, false); equal(run.resource_contract_pass, false); equal(run.report, null); equal(run.resources, null); equal(run.manifest, null); ensure(run.request_identity === undefined, 'unlaunched_request_identity'); return }
  {
    ...
    equal(request.schema_version, 'rc-fiber-candidate-process-worker-request.v1'); equal(request.strategy, strategy)
  ...
  if (run.report_contract_pass) {
    ...
    equal(report.schema_version, strategy === 'oracle' ? 'fiber-frame-candidate-search-oracle.v1' : 'fiber-frame-candidate-search-arm.v1')
    ...
    if (strategy !== 'oracle') {
      ...
      if (selected) { const row = rows.find(item => item.candidate_id === selected.candidate_id); ensure(row, 'selected_candidate_missing'); same(selected, row); equal(selected.full_reference_verification_pass, true); equal(selected.terminal_limit_status, 'pass'); if (input.history_limits !== undefined) { equal(selected.full_history_verification_pass, true); equal(selected.history_limit_status, 'pass') } }
      equal(report.status, selected ? 'ready' : 'blocked')
    } else {
      equal(report.status, rows.every(row => row.full_reference_verification_pass === true && row.full_history_verification_pass !== false) ? 'ready' : 'blocked')
...
function audit(pool: CandidateObject[], shortlist: string[], oracle: CandidateObject[] | null, history: boolean, deterministic: boolean): CandidateObject {
  ...
  if (oracle === null) {
    ...
    for (const candidate of pool) {
      ...
      if (boolean(row.full_reference_verification_pass)) {
        known++; const terminal = row.terminal_limit_status === 'pass'; const verifiedHistory = !history || row.full_history_verification_pass === true
        if (verifiedHistory) combined++
        if (terminal && verifiedHistory && (!history || row.history_limit_status === 'pass') && !shortlist.includes(id)) missed.push(id)
...
function validateSummaries(suite: CandidateObject, cases: CandidateObject[], repetitions: number): void {
  ...
  cases.forEach((declared, index) => {
    ...
    for (let repetition = 0; repetition < repetitions; repetition++) {
      ...
      if (valid) for (const strategy of ['deterministic', 'learned']) audits[strategy] = audit(array(object(object(declared.plans).learned).candidate_pool).map(object), array(object(strategy === 'deterministic' ? detArm : learnedArm).shortlist).map(string), oracleRows, input.history_limits !== undefined, strategy === 'deterministic')
...
export function validateCandidateProcessReview(value: unknown, manifest: CandidateProcessManifest, artifacts: Map<string, CandidateLoadedArtifact>, comparisons: Map<string, CandidateLoadedComparison>): { suite: CandidateProcessSuite; slots: CandidateProcessSlot[] } {
  ...
  equal(suite.schema_version, 'rc-fiber-candidate-process-suite.v1'); equal(hash(suite.report_hash), manifest.suite_report_hash); equal(hash(suite.suite_identity_hash), manifest.suite_identity_hash)
  ...
  equal(originalRequest.schema_version, 'rc-fiber-candidate-process-suite-request.v1')
