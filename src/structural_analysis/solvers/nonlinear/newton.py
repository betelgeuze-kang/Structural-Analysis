            ...
            attempts.append(
                {
                    ...
                    "accepted": False,
                    "invalid_trial": True,
                    "invalid_trial_reason": type(exc).__name__,
                }
            )
            continue
...
def _vector_line_search(
    ...
    *,
    ...
    for alpha in alphas:
        ...
        if accepted:
            return trial_displacement, alpha, attempts
        if trial_norm < _vector_equation_scaling(
            problem,
            cfg,
            residual_kn=best_residual,
            increment_m=np.zeros_like(newton_increment_m),
            tangent_kn_per_m=tangent_before,
        ).scaled_residual_norm:
            best_residual = trial_residual
            best_alpha = alpha
...
def newton_raphson_vector(
    ...
    *,
    ...
    if (
        not np.isfinite(cfg.characteristic_length_m)
        or cfg.characteristic_length_m <= 0.0
    ):
        return _blocked_vector_solution(
            ...
            history=[],
            line_search_history=[],
            detail="invalid_characteristic_length",
        )
    ...
    final_residual, final_tangent = problem.assemble(free_displacements_m)
    ...
    final_scaling = _vector_equation_scaling(
        ...
        tangent_kn_per_m=final_tangent,
    )
