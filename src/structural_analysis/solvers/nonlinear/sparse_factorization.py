...
def validate_sparse_factorization_diagnostic_manifest(
    ...
    """Validate a detached diagnostic against schema, hash, and status rules."""
    ...
    if claimed != canonical_hash(body):
        raise ValueError("sparse factorization diagnostic hash mismatch")
    checks_pass = all(bool(value) for value in normalized["checks"].values())
    expected_pass = normalized["status"] == "pass" and checks_pass
