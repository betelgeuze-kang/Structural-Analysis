...
class NonlinearShearBuilding:
    ...
    def __init__(
        self, mass_matrix_kn_s2_per_m: Sequence[Sequence[float]] | np.ndarray,
        damping_matrix_kn_s_per_m: Sequence[Sequence[float]] | np.ndarray,
        stories: Sequence[BilinearStory], dof_ids: Sequence[str],
        ...
        n = len(ids)
        if n < 2 or mass.shape != (n, n) or damping.shape != (n, n) or len(story_rows) != n:
        ...
        if np.min(np.linalg.eigvalsh(mass)) <= 0.0 or np.min(np.linalg.eigvalsh(damping)) < -1.0e-12:
        ...
        object.__setattr__(self, "mass_matrix_kn_s2_per_m", tuple(map(tuple, mass.tolist())))
        object.__setattr__(self, "damping_matrix_kn_s_per_m", tuple(map(tuple, damping.tolist())))
    ...
    def arrays(self) -> tuple[np.ndarray, np.ndarray]:
        return np.asarray(self.mass_matrix_kn_s2_per_m), np.asarray(self.damping_matrix_kn_s_per_m)

    def to_manifest(self) -> dict[str, Any]:
        return {
            "model_id": self.model_id, "dof_ids": list(self.dof_ids),
            "mass_matrix_kn_s2_per_m": [list(row) for row in self.mass_matrix_kn_s2_per_m],
            "damping_matrix_kn_s_per_m": [list(row) for row in self.damping_matrix_kn_s_per_m],
...
class NonlinearMDOFTransientConfig:
    ...
    def __post_init__(self) -> None:
        for name in ("time_step_s", "residual_relative_tolerance", "residual_absolute_tolerance_kn"):
    ...
    def contract_hash(self) -> str:
        return canonical_hash(asdict(self) | {"profile": NONLINEAR_MDOF_TRANSIENT_PROFILE})
...
def _response(
    system: NonlinearShearBuilding, displacement: np.ndarray,
    committed_states: Sequence[BilinearMaterialState],
) -> tuple[np.ndarray, np.ndarray, tuple[BilinearMaterialState, ...], tuple[bool, ...], float]:
    drift = system.drift_matrix @ displacement
    rows = tuple(
        evaluate_bilinear_restoring_force(story.material_model, float(drift[index]), committed_states[index])
        for index, story in enumerate(system.stories)
    )
    story_force = np.asarray([row.force_kn for row in rows])
    tangent = system.drift_matrix.T @ np.diag([row.tangent_kn_per_m for row in rows]) @ system.drift_matrix
    global_force = system.drift_matrix.T @ story_force
    return global_force, tangent, tuple(row.state for row in rows), tuple(row.yielded for row in rows), float(sum(row.stored_energy_kn_m for row in rows))


def _checkpoint(
    *, system: NonlinearShearBuilding, config: NonlinearMDOFTransientConfig,
    step_index: int, force: np.ndarray, u: np.ndarray, v: np.ndarray, a: np.ndarray,
    states: tuple[BilinearMaterialState, ...], external_work: float,
    damping_dissipation: float, initial_energy: float, parent: str | None,
) -> NonlinearMDOFCheckpoint:
    provisional = NonlinearMDOFCheckpoint(
        schema_version=NONLINEAR_MDOF_CHECKPOINT_SCHEMA_VERSION,
        profile=NONLINEAR_MDOF_TRANSIENT_PROFILE, model_hash=system.model_hash,
        integration_contract_hash=config.contract_hash, step_index=step_index,
        time_s=step_index * config.time_step_s,
        applied_force_kn=tuple(map(float, force)), displacement_m=tuple(map(float, u)),
        velocity_m_per_s=tuple(map(float, v)), acceleration_m_per_s2=tuple(map(float, a)),
        material_states=states, external_work_kn_m=float(external_work),
        damping_dissipation_kn_m=float(damping_dissipation),
        initial_mechanical_energy_kn_m=float(initial_energy),
        parent_checkpoint_hash=parent, checkpoint_hash=_ZERO_HASH,
    )
    payload = provisional.to_dict(); payload.pop("checkpoint_hash")
...
def solve_nonlinear_mdof_transient(
    ...
    *, config: NonlinearMDOFTransientConfig,
    ...
    u = np.asarray(initial_displacement_m if initial_displacement_m is not None else np.zeros(system.dimension), dtype=np.float64)
    v = np.asarray(initial_velocity_m_per_s if initial_velocity_m_per_s is not None else np.zeros(system.dimension), dtype=np.float64)
    ...
    start = _checkpoint(system=system, config=config, step_index=0, force=forces[0], u=u, v=v, a=a, states=states, external_work=0.0, damping_dissipation=0.0, initial_energy=initial_energy, parent=None)
    return _advance(system, config, start, forces[1:])


def resume_nonlinear_mdof_transient(
    system: NonlinearShearBuilding, checkpoint: NonlinearMDOFCheckpoint,
...
def _advance(
    system: NonlinearShearBuilding, config: NonlinearMDOFTransientConfig,
    start: NonlinearMDOFCheckpoint, future_forces: np.ndarray,
) -> NonlinearMDOFSolution:
    mass, damping = system.arrays(); drift_matrix = system.drift_matrix
    dt, beta, gamma = config.time_step_s, config.newmark_beta, config.newmark_gamma
    c0, c1 = 1.0 / (beta * dt * dt), gamma / (beta * dt)
    u = np.asarray(start.displacement_m); v = np.asarray(start.velocity_m_per_s); a = np.asarray(start.acceleration_m_per_s2)
    previous_force = np.asarray(start.applied_force_kn)
    states = start.material_states
    external_work, damping_dissipation = start.external_work_kn_m, start.damping_dissipation_kn_m
    checkpoints = [start]; steps: list[NonlinearMDOFStep] = []
    initial_internal, _, _, _, initial_stored = _response(system, u, states)
    _ = initial_internal
    initial_kinetic = float(0.5 * v @ mass @ v)
    steps.append(NonlinearMDOFStep(
        step_index=start.step_index, time_s=start.time_s, applied_force_kn=start.applied_force_kn,
        displacement_m=start.displacement_m, velocity_m_per_s=start.velocity_m_per_s,
        acceleration_m_per_s2=start.acceleration_m_per_s2,
        story_drift_m=tuple(map(float, drift_matrix @ u)), story_force_kn=tuple(map(float, np.linalg.solve(drift_matrix.T, initial_internal))),
        equilibrium_residual_kn=tuple(0.0 for _ in range(system.dimension)), relative_residual=0.0,
        newton_iterations=0, yielded_story_count=0, kinetic_energy_kn_m=initial_kinetic,
        stored_energy_kn_m=initial_stored, external_work_kn_m=external_work,
        damping_dissipation_kn_m=damping_dissipation,
        plastic_dissipation_kn_m=float(sum(row.plastic_dissipation_kn_m for row in states)),
        energy_balance_error_kn_m=float(external_work + start.initial_mechanical_energy_kn_m - initial_kinetic - initial_stored - damping_dissipation),
    ...
    for offset, force in enumerate(np.asarray(future_forces, dtype=np.float64), start=1):
        ...
        for iteration in range(1, config.maximum_iterations + 1):
            candidate_a = c0 * (candidate - u) - (1.0 / (beta * dt)) * v - (1.0 / (2.0 * beta) - 1.0) * a
            candidate_v = v + dt * ((1.0 - gamma) * a + gamma * candidate_a)
            internal, tangent, trial_states, yielded, stored = _response(system, candidate, parent_states)
            residual = mass @ candidate_a + damping @ candidate_v + internal - force
            scale = max(float(np.linalg.norm(force)), float(np.linalg.norm(mass @ candidate_a) + np.linalg.norm(damping @ candidate_v) + np.linalg.norm(internal)), 1.0)
            relative = float(np.linalg.norm(residual) / scale)
            if float(np.linalg.norm(residual)) <= config.residual_absolute_tolerance_kn or relative <= config.residual_relative_tolerance:
                converged = True
                final = (candidate_a, candidate_v, internal, trial_states, yielded, stored, residual, relative, iteration)
                break
        ...
        if not converged or final is None:
            ...
            raise NonlinearMDOFTransientError(f"Newton failed at step {step_index}; accepted state rolled back exactly")
        new_a, new_v, internal, trial_states, yielded, stored, residual, relative, iterations = final
        ...
        energy_error = external_work + start.initial_mechanical_energy_kn_m - kinetic - stored - damping_dissipation - plastic
        checkpoint = _checkpoint(system=system, config=config, step_index=step_index, force=force, u=candidate, v=new_v, a=new_a, states=trial_states, external_work=external_work, damping_dissipation=damping_dissipation, initial_energy=start.initial_mechanical_energy_kn_m, parent=checkpoints[-1].checkpoint_hash)
        story_force = np.linalg.solve(drift_matrix.T, internal)
        steps.append(NonlinearMDOFStep(
            step_index=step_index, time_s=step_index * dt, applied_force_kn=tuple(map(float, force)),
            displacement_m=tuple(map(float, candidate)), velocity_m_per_s=tuple(map(float, new_v)), acceleration_m_per_s2=tuple(map(float, new_a)),
            story_drift_m=tuple(map(float, drift_matrix @ candidate)), story_force_kn=tuple(map(float, story_force)),
            equilibrium_residual_kn=tuple(map(float, residual)), relative_residual=relative,
            newton_iterations=iterations, yielded_story_count=sum(yielded), kinetic_energy_kn_m=kinetic,
            stored_energy_kn_m=stored, external_work_kn_m=external_work, damping_dissipation_kn_m=damping_dissipation,
            plastic_dissipation_kn_m=plastic, energy_balance_error_kn_m=energy_error,
            checkpoint_hash=checkpoint.checkpoint_hash,
        ))
        checkpoints.append(checkpoint); total_iterations += iterations
        u, v, a, states, previous_force = candidate, new_v, new_a, trial_states, force
    provisional = NonlinearMDOFSolution(
        schema_version=NONLINEAR_MDOF_TRANSIENT_SCHEMA_VERSION, profile=NONLINEAR_MDOF_TRANSIENT_PROFILE,
        ...
        maximum_absolute_energy_balance_error_kn_m=max(abs(row.energy_balance_error_kn_m) for row in steps),
        total_newton_iterations=total_iterations,
        yielded_step_count=sum(row.yielded_story_count > 0 for row in steps), result_hash=_ZERO_HASH,
        exact_checkpoint_resume_supported=True, material_trial_commit_rollback=True,
        regularization_used=False, fallback_used=False, contract_pass=True,
    )
...
def validate_nonlinear_mdof_checkpoint_authority(
    terminal: NonlinearMDOFCheckpoint, *, system: NonlinearShearBuilding,
    config: NonlinearMDOFTransientConfig,
    checkpoint_chain: Sequence[NonlinearMDOFCheckpoint],
    force_history_prefix_kn: Sequence[Sequence[float]] | np.ndarray,
) -> NonlinearMDOFCheckpointAuthority:
    chain = tuple(checkpoint_chain); forces = np.asarray(force_history_prefix_kn, dtype=np.float64)
    parent_complete = bool(chain and chain[-1] == terminal and len(chain) == terminal.step_index + 1 and chain[0].parent_checkpoint_hash is None and all(row.step_index == index for index, row in enumerate(chain)) and all(chain[index].parent_checkpoint_hash == chain[index - 1].checkpoint_hash for index in range(1, len(chain))))
    force_exact = bool(forces.shape == (len(chain), system.dimension) and all(np.array_equal(np.asarray(row.applied_force_kn), forces[index]) for index, row in enumerate(chain)))
    replay = solve_nonlinear_mdof_transient(system, forces, config=config, initial_displacement_m=chain[0].displacement_m, initial_velocity_m_per_s=chain[0].velocity_m_per_s) if parent_complete and force_exact else None
    ...
    material = bool(replay is not None and replay.checkpoints[-1].material_states == terminal.material_states)
    payload = {
        "schema_version": NONLINEAR_MDOF_AUTHORITY_SCHEMA_VERSION, "checkpoint_hash": terminal.checkpoint_hash,
        "source_authenticated_checkpoint": bool(parent_complete and force_exact and deterministic and dynamic and material),
        "parent_chain_complete": parent_complete, "force_history_hash": canonical_hash(forces.tolist()),
        "force_history_sample_count": int(forces.shape[0]) if forces.ndim == 2 else 0,
        "dynamic_equilibrium_replay_pass": dynamic, "material_state_replay_pass": material,
        "deterministic_checkpoint_replay_pass": deterministic,
    }
    return NonlinearMDOFCheckpointAuthority(**payload, receipt_hash=canonical_hash(payload))


__all__ = [
    "BilinearStory", "NONLINEAR_MDOF_AUTHORITY_SCHEMA_VERSION",
    "NONLINEAR_MDOF_CHECKPOINT_SCHEMA_VERSION", "NONLINEAR_MDOF_TRANSIENT_PROFILE",
    "NONLINEAR_MDOF_TRANSIENT_SCHEMA_VERSION", "NonlinearMDOFCheckpoint",
    "NonlinearMDOFCheckpointAuthority", "NonlinearMDOFSolution", "NonlinearMDOFStep",
    "NonlinearMDOFTransientConfig", "NonlinearMDOFTransientError",
    "NonlinearShearBuilding", "resume_nonlinear_mdof_transient",
    "solve_nonlinear_mdof_transient", "validate_nonlinear_mdof_checkpoint_authority",
]
