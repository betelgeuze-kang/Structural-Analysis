...
class ShellStaticModel:
    ...
    def __init__(
        ...
        *,
        ...
        node_coordinates_m: Sequence[Sequence[float]],
        element_ids: Sequence[str],
        element_connectivity: Sequence[Sequence[int]],
        elastic_modulus_pa: float,
        poisson_ratio: float,
        thickness_m: float,
        restrained_dofs: Sequence[int],
        load_global_n_nm: Sequence[float],
    ) -> None:
        ids = tuple(map(str, node_ids))
        ...
        element_names = tuple(map(str, element_ids))
        dof_count = 6 * len(ids)
        ...
        object.__setattr__(
            self, "node_coordinates_m", tuple(map(tuple, coordinates.tolist()))
        )
        object.__setattr__(self, "element_ids", element_names)
        object.__setattr__(self, "element_connectivity", elements)
        object.__setattr__(self, "elastic_modulus_pa", float(elastic_modulus_pa))
        object.__setattr__(self, "poisson_ratio", float(poisson_ratio))
        object.__setattr__(self, "thickness_m", float(thickness_m))
        object.__setattr__(self, "restrained_dofs", restraints)
        object.__setattr__(self, "load_global_n_nm", tuple(map(float, load)))
...
def solve_shell_static(model: ShellStaticModel) -> ShellStaticSolution:
    stiffness, element_matrices = _assemble(model)
    load = np.asarray(model.load_global_n_nm)
    restrained = set(model.restrained_dofs)
    free = np.asarray(
        [index for index in range(model.dof_count) if index not in restrained],
        dtype=np.int64,
    )
    if free.size == 0:
    ...
    if np.linalg.matrix_rank(reduced) != reduced.shape[0]:
        raise ShellStaticError(
            "shell reduced stiffness is singular; no regularization is allowed"
        )
    displacement = np.zeros(model.dof_count)
    displacement[free] = np.linalg.solve(reduced, load[free])
    ...
    reactions = np.zeros(model.dof_count)
    reactions[list(restrained)] = residual[list(restrained)]
    results = []
    for element_id, (matrices, dofs) in zip(
        model.element_ids, element_matrices, strict=True
    ):
    ...
    checkpoint0 = ShellStaticCheckpoint(
        SHELL_CHECKPOINT_SCHEMA_VERSION,
        SHELL_STATIC_PROFILE,
        model.model_hash,
        stiffness_hash,
        load_hash,
        tuple(map(float, displacement)),
        tuple(map(float, reactions)),
        _ZERO_HASH,
    )
    checkpoint_payload = checkpoint0.to_dict()
    checkpoint_payload.pop("checkpoint_hash")
    checkpoint = replace(
        checkpoint0, checkpoint_hash=canonical_hash(checkpoint_payload)
    )
...
def resume_shell_static(
    model: ShellStaticModel, checkpoint: ShellStaticCheckpoint
) -> ShellStaticSolution:
    if (
        checkpoint.schema_version != SHELL_CHECKPOINT_SCHEMA_VERSION
        or checkpoint.profile != SHELL_STATIC_PROFILE
