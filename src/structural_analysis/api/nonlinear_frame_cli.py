...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Linear algebra backend (sparse is supported by the corotational profile).",
    )
    parser.add_argument(
        "--control-mode",
        choices=(LOAD_CONTROL_MODE, DISPLACEMENT_CONTROL_MODE),
        default=LOAD_CONTROL_MODE,
    )
    parser.add_argument("--control-node", help="Displacement-control node ID.")
    parser.add_argument(
        "--control-dof",
        choices=("UX", "UY", "RZ"),
        help="Displacement-control component.",
    )
    parser.add_argument(
        "--terminal-control-displacement",
        type=float,
        help="Full-path control target in metres for UX/UY or radians for RZ.",
    )
    parser.add_argument(
        "--restart-from",
    ...
    try:
        config = NonlinearFrameConfig(
            ...
            control_node_id=args.control_node,
            control_dof=args.control_dof,
            terminal_control_displacement=args.terminal_control_displacement,
        )
