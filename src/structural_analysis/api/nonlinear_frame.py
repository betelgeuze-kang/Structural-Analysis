...
LOAD_CONTROL_MODE: Final[Literal["load_control"]] = "load_control"
DISPLACEMENT_CONTROL_MODE: Final[Literal["displacement_control"]] = (
...
UNIFIED_NONLINEAR_FRAME_CLAIM_BOUNDARY = (
    ...
    "recovery, exact checkpoint-chain replay, end releases, rigid offsets, and uniform "
    "initial-local-axis dead loads plus load or direct single-DOF displacement control, "
    "but remain candidates until two independent Level "
    "2 comparisons pass. No profile grants "
...
NonlinearFrameControlMode = Literal["load_control", "displacement_control"]
...
class NonlinearFrameConfig:
    ...
    control_mode: NonlinearFrameControlMode = LOAD_CONTROL_MODE
    control_node_id: str | None = None
    control_dof: Literal["UX", "UY", "RZ"] | None = None
    terminal_control_displacement: float | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.control_mode not in {LOAD_CONTROL_MODE, DISPLACEMENT_CONTROL_MODE}:
            raise ValueError("control_mode is unsupported")
        if self.profile == FIXED_CHORD_SERIAL_PROFILE and (
            self.control_mode != LOAD_CONTROL_MODE
        ):
            raise ValueError(
                "the fixed-chord profile currently supports only load control"
            )
        ...
        if self.control_mode == LOAD_CONTROL_MODE:
            if (
                self.control_node_id is not None
                or self.control_dof is not None
                or self.terminal_control_displacement is not None
            ):
                raise ValueError(
                    "load control does not accept displacement-control fields"
                )
        else:
            if (
                type(self.control_node_id) is not str
                or _STABLE_ID.fullmatch(self.control_node_id) is None
                or self.control_dof not in _ACTIVE_COMPONENTS
                or self.terminal_control_displacement is None
            ):
                raise ValueError(
                    "displacement control requires a stable node, UX/UY/RZ, and terminal target"
                )
            terminal = _number(
                self.terminal_control_displacement,
                "/config/terminal_control_displacement",
            )
            if terminal == 0.0:
    ...
    @property
    def target_control_displacements(self) -> tuple[float, ...]:
        if (
            self.control_mode != DISPLACEMENT_CONTROL_MODE
            or self.terminal_control_displacement is None
        ):
            return ()
        return tuple(
            (
                self.terminal_control_displacement
                if step == self.load_steps
                else self.terminal_control_displacement * step / self.load_steps
            )
...
def nonlinear_frame_resume_contract_hash(
    ...
    """Hash the exact model/compiler/control path required for checkpoint reuse."""
...
def _analyze_corotational_portal(
    ...
    configuration = {
        ...
        "load_steps": config.load_steps,
        "target_load_factors": (
            list(config.target_load_factors)
            if config.control_mode == LOAD_CONTROL_MODE
            else []
        ),
        "control_mode": config.control_mode,
        "control_node_id": config.control_node_id,
        "control_dof": config.control_dof,
        "terminal_control_displacement": config.terminal_control_displacement,
        "target_control_displacements": list(config.target_control_displacements),
        ...
        try:
            ...
            if execution.path.status != "ready" or not execution.path.contract_pass:
                raise NonlinearFrameError(
                    ...
                    "/solver",
                    "The configured control path did not commit exactly.",
                )
            if isinstance(
    ...
    metrics = {
        ...
        "controlled_global_dof": (
            execution.path.steps[-1].metrics.get("controlled_global_dof")
            if config.control_mode == DISPLACEMENT_CONTROL_MODE
            else None
        ),
        "terminal_control_displacement": config.terminal_control_displacement,
        ...
        "sparse_factorization_count": sum(
            int(step.metrics.get("sparse_factorization_count", 0))
            for step in execution.path.steps
        ),
        ...
        "sparse_factorization_policy_hash": next(
            (
                step.metrics["sparse_factorization_policy_hash"]
                for step in execution.path.steps
                if step.metrics.get("sparse_factorization_policy_hash") is not None
            ),
            None,
        ),
        "member_feature_count": max(
            int(step.metrics.get("member_feature_count", 0))
            for step in execution.path.steps
        ),
        "released_end_count": max(
            int(step.metrics.get("released_end_count", 0))
            for step in execution.path.steps
        ),
        "rigid_offset_end_count": max(
            int(step.metrics.get("rigid_offset_end_count", 0))
            for step in execution.path.steps
        ),
        "distributed_load_member_count": max(
            int(step.metrics.get("distributed_load_member_count", 0))
            for step in execution.path.steps
        ),
        "release_local_iteration_count": sum(
            int(step.metrics.get("release_local_iteration_count", 0))
            for step in execution.path.steps
        ),
        "release_equilibrium_max_abs_kn_m": max(
            float(step.metrics.get("release_equilibrium_max_abs_kn_m", 0.0))
            for step in execution.path.steps
        ),
        "member_feature_response_hashes": [
            response_hash
            for step in execution.path.steps
            for response_hash in step.metrics.get("member_feature_response_hashes", ())
        ],
        "external_level2_attached": False,
    ...
    *,
    ...
    for index, row in enumerate(model.elements):
        ...
        if (
            ...
            or set(row) - required_element_keys - optional_element_keys
        ):
            _fail(
...
def _run_corotational_path(
    ...
    *,
    ...
    displacement_control = config.control_mode == DISPLACEMENT_CONTROL_MODE
    controlled_global_dof = (
        _displacement_control_global_dof(compiled, config)
        if displacement_control
        else None
    )
    targets = (
        config.target_control_displacements
        if displacement_control
        else config.target_load_factors
    )
    ...
    def run_segment(
        ...
        *,
        ...
        if displacement_control:
            assert controlled_global_dof is not None
            assert config.terminal_control_displacement is not None
    ...
    if restart is None:
        segment_targets = (
            targets
            if maximum_new_steps is None
            else targets[:maximum_new_steps]
        )
        ...
        actual_prefix = (
            tuple(
                checkpoint.global_displacements[controlled_global_dof]
                for checkpoint in loaded.checkpoints[1:]
            )
            if displacement_control and controlled_global_dof is not None
            else tuple(checkpoint.load_factor for checkpoint in loaded.checkpoints[1:])
        )
        if actual_prefix != expected_prefix:
            raise NonlinearFrameError(
                (
                    "corotational_restart_control_prefix_mismatch"
                    if displacement_control
                    else "corotational_restart_load_prefix_mismatch"
                ),
                "/restart_checkpoint_chain",
                "Restart targets are not the exact configured control prefix.",
            )
        if prefix_count:
            ...
            prefix_path = StatefulCorotationalFiberFrame2DLoadPathResult(
                status="ready", initial_checkpoint=root, final_checkpoint=root, steps=()
            )
    ...
    return canonical_hash(
        {
            ...
            "control_dof": config.control_dof,
            "terminal_control_displacement": config.terminal_control_displacement,
            "target_control_displacements": list(
                config.target_control_displacements
            ),
            "residual_tolerance": config.residual_tolerance,
            "increment_tolerance_m": config.increment_tolerance_m,
...
def _displacement_control_global_dof(
    compiled: _CompiledPortal,
    config: NonlinearFrameConfig,
) -> int:
    assert config.control_node_id is not None
    assert config.control_dof is not None
    try:
        node_index = compiled.node_ids.index(config.control_node_id)
    except ValueError:
        _fail(
            "corotational_displacement_control_node_missing",
            "/config/control_node_id",
            "The displacement-control node is not declared in the model.",
        )
    component = {"UX": 0, "UY": 1, "RZ": 2}[config.control_dof]
    global_dof = 3 * node_index + component
    if global_dof not in compiled.problem.free_global_dofs:
        _fail(
            "corotational_displacement_control_dof_constrained",
            "/config/control_dof",
            "The displacement-control DOF must be an unconstrained analysis equation.",
        )
    return global_dof
