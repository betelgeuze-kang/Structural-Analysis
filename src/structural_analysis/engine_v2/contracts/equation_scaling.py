    ...
    *,
    ...
    try:
        centroid = np.asarray(
            [
                math.fsum(
                    float(coordinates[row, column]) for row in range(plan.node_count)
                )
                / plan.node_count
    ...
    max_radius = 0.0
    for row in range(plan.node_count):
    ...
    if (
        ...
        or characteristic_length < minimum_length
    ):
        _fail(
            "characteristic_length_invalid",
            "/characteristic_length_m",
    ...
    max_translation = _max_abs(loads[translation_equations])
    max_equivalent_moment = _max_abs(loads[rotation_equations]) / characteristic_length
    reference_force = max(minimum_force, max_translation, max_equivalent_moment)
    ...
    return validate_equation_scaling(scaling, execution_plan=plan)
...
def _validate_equation_scaling_binding_semantics(
    ...
    base_provisional = replace(
        plan, plan_hash=_HASH_ZERO, extensions=_freeze_extensions(extensions)
    )
...
def validate_equation_scaling(
    scaling: EquationScaling, *, execution_plan: ExecutionPlan | None = None
) -> EquationScaling:
