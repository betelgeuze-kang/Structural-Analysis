...
def open_trial_state(
    ...
    *,
    ...
    trial_id = state_id or (
        f"state.trial.e{next_epoch}.s{next_load_step}.i{iteration}"
    )
    return _build_state(
        state_id=trial_id,
    ...
    *,
    ...
    if state.parent_state_hash is not None:
        ...
        if state.parent_state_hash == state.state_hash:
            raise StateIRError(
                "state_parent_cycle", "/parent_state_hash", "State cannot parent itself."
            )
    ...
    if epoch == 0:
        ...
        if load_step != 0 or state.iteration != 0 or load_factor != 0.0 or time_s != 0.0:
            raise StateIRError(
    ...
    if state.role == "trial" and epoch == 0:
        raise StateIRError("trial_epoch_invalid", "/epoch", "A trial epoch must be positive.")
    ...
    expected_state_hash = canonical_hash(_state_payload(state, include_state_hash=False))
    ...
    schema_errors = sorted(
        _state_schema_validator().iter_errors(
            _state_payload(state, include_state_hash=True)
        ),
...
def _build_state(
    *,
    ...
    provisional = StateIR(
        ...
        step=StateStep(
            load_step=normalized_load_step, iteration=normalized_iteration
        ),
...
def _validate_trial_transition(
    ...
    *,
    ...
    if trial.role != "trial":
        raise StateIRError("trial_state_role_invalid", "/role", "Trial state is required.")
...
def _immutable_f64_vector(value: Any, count: int, path: str) -> np.ndarray:
    ...
    if source.dtype.kind not in "iuf":
        raise StateIRError(
            "state_vector_type_invalid", path, "State vectors must contain real numbers."
        )
    try:
        converted = np.asarray(value, dtype="<f8")
    except (TypeError, ValueError, OverflowError) as exc:
        raise StateIRError(
            "state_vector_type_invalid", path, "State vector cannot be converted to FP64."
        ) from exc
...
def _validate_vector(array: np.ndarray, count: int, path: str) -> None:
    ...
    if array.dtype.str != "<f8":
        raise StateIRError(
            "state_vector_dtype_invalid", path, "State vectors must use little-endian FP64."
        )
    if array.shape != (count,) or not array.flags.c_contiguous:
        raise StateIRError(
            "state_vector_shape_invalid", path, f"Expected C-contiguous shape ({count},)."
        )
    ...
    if not np.all(np.isfinite(array)):
        raise StateIRError("state_vector_nonfinite", path, "State vector is non-finite.")
