...
def compare_result_quantity(
    ...
    *,
    ...
    if not math.isfinite(maximum_error) or not math.isfinite(allowed):
        _fail(
            "result_quantity_comparison_nonfinite",
            "/candidate_si",
            "Comparison metrics must remain finite.",
        )
...
def validate_result_quantity_catalog(
    ...
    try:
        ...
        _fail(
            ...
            "/authority_rules",
            "Authority rules must be a mapping fixed by v1.",
        )
    if authority_rules != dict(_AUTHORITY_RULES):
        _fail(
            "result_quantity_authority_rules_invalid",
            "/authority_rules",
            "Authority rules are fixed by v1.",
        )
    if type(catalog.quantities) is not tuple:
        _fail(
            "result_quantity_quantities_type_invalid",
            "/quantities",
            "Typed catalog quantities must be an immutable tuple.",
        )
    ...
    expected_specs = _default_specs()
    for index, (actual, expected) in enumerate(
        zip(catalog.quantities, expected_specs, strict=True)
    ):
        if actual != expected:
            _fail(
                "result_quantity_spec_mismatch",
                f"/quantities/{index}",
                "Every v1 quantity row, order, unit, authority axis, component, "
                "and tolerance is fixed.",
            )
...
def validate_result_quantity_catalog_manifest(
    ...
    try:
        ...
        _fail(
            "result_quantity_manifest_invalid",
            "/",
            "Catalog manifest must be a finite JSON object.",
        )
    errors = sorted(
    ...
    catalog = ResultQuantityCatalog(
        ...
        quantities=tuple(
            ResultQuantitySpec(
                ...
                authority_axis=row["authority_axis"],
                component_labels=tuple(row["component_labels"]),
                absolute_tolerance_si=row["absolute_tolerance_si"],
                relative_tolerance=row["relative_tolerance"],
                comparison_norm=row["comparison_norm"],
                comparison_profile=row["comparison_profile"],
            )
            for row in normalized["quantities"]
        ),
        authority_rules=MappingProxyType(dict(normalized["authority_rules"])),
    )
    validate_result_quantity_catalog(catalog)
    return normalized
...
def _validate_spec(spec: ResultQuantitySpec, *, index: int) -> None:
    ...
    if type(spec.quantity_id) is not str:
        _fail(
            "result_quantity_id_invalid",
            f"{path}/quantity_id",
            "Quantity identifier must be a string.",
        )
    if type(spec.component_labels) is not tuple or not spec.component_labels or any(
        type(label) is not str or not label for label in spec.component_labels
    ):
        _fail(
    ...
    if (
        type(spec.absolute_tolerance_si) not in (int, float)
        or type(spec.relative_tolerance) not in (int, float)
        or not math.isfinite(spec.absolute_tolerance_si)
...
def _finite_vector(values: Sequence[float] | np.ndarray, path: str) -> np.ndarray:
    try:
        ...
        _fail(
            ...
            path,
            "Quantity values must be convertible to finite float64 values.",
        )
