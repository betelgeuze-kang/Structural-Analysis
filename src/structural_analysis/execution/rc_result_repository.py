...
def _validate_local_owner(path: Path, expected: dict[str, str]) -> None:
    """Reject ambiguous or partial ownership files; never replace an existing one."""
    import hmac
    ...
    with os.fdopen(descriptor, "rb") as stream:
        if not stat.S_ISREG(os.fstat(stream.fileno()).st_mode):
            raise ValueError("local owner must be a regular file")
        previous = strict_json_object_bytes(stream.read(4097), maximum_bytes=4096)
    if (
        set(previous) != set(expected)
        or previous.get("schema") != expected["schema"]
        or previous.get("tenant") != expected["tenant"]
    ):
        raise ValueError("local repository ownership mismatch")
    if type(previous.get("token_hash")) is not str or not hmac.compare_digest(
        previous["token_hash"], expected["token_hash"]
    ):
        raise ValueError("local repository authorization failed")


def _initialize_local_owner(root: Path, expected: dict[str, str]) -> None:
    """Publish a complete owner record atomically without replacing a race winner.

    A process death before the hard-link publication can leave an unreferenced
    temporary file, never a partially written authoritative owner record. Another
    initializer must validate the winning credentials before opening the service.
    This assumes a trusted POSIX local filesystem with atomic hard links.
    """
    import tempfile

    owner = root / "rc-local-owner.json"
    try:
        _validate_local_owner(owner, expected)
        return
    except FileNotFoundError:
        pass
    descriptor, temporary = tempfile.mkstemp(prefix=".rc-owner-", dir=root)
    temporary_path = Path(temporary)
    try:
        with os.fdopen(descriptor, "wb") as stream:
            stream.write(_encode(expected))
            stream.flush()
            os.fsync(stream.fileno())
        try:
            os.link(temporary_path, owner, follow_symlinks=False)
...
def open_local_rc_repository(
    ...
    """Dedicated local CLI store, with atomically pinned first-owner credentials.
    ...
    if type(tenant_id) is not str or not re.fullmatch(
        r"[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}", tenant_id
    ):
        raise ValueError("invalid local tenant identifier")
    if type(authorization_token) is not str or not (
        16 <= len(authorization_token.encode("utf-8")) <= 4096
    ):
        raise ValueError("local store token must contain 16 to 4096 UTF-8 bytes")
    if type(scope_id) is not str or not scope_id or len(scope_id) > 128:
        raise ValueError("bounded scope required")
