...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if (
            not 1 <= len(args.experiment) <= 16
            or not 0 <= args.max_new_model_analyses <= 17
        ):
        ...
        session = DurableRCControlResultSession(
            ...
            chunk_target_count=args.chunk_target_count,
        ...
        for index, (candidates, prices, terminal, history, material) in enumerate(
            ...
            if report["new_work"]["unknown_work"]:
                break
        report = {
            "schema_version": "local-durable-rc-search-batch.v1",
            "scenarios": results,
            "requested_scenario_count": len(scenarios),
            "remaining_new_model_budget": budget,
            "claims": {
                "same_host_persistence": True,
                "gpu_execution": False,
        ...
        return (
            ...
            and all(r["status"] == "pool_minimum_confirmed" for r in results)
            else 2
        )
        ...
        print(
            "durable_rc_search_failed: inspect authenticated job records",
            file=sys.stderr,
        )
        return 1
