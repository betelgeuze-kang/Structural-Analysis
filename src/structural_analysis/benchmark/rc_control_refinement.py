...
RESPONSE_UNITS = {
    ...
    "steel_strain": "1",
    "steel_stress": "MPa",
    "steel_plastic_strain": "1",
    "concrete_tensile_envelope": "1",
    "concrete_compressive_envelope": "1",
...
CLAIMS = {
    ...
    "concrete_field_correspondence_verified": False,
...
def _plan(levels, tolerances):
    if (
        ...
        or not 3 <= len(levels) <= 6
        or any(type(n) is not int or not 2 <= n <= 32 for n in levels)
        or any(a >= b for a, b in zip(levels, levels[1:]))
    ):
        raise ValueError(
            "three to six strictly increasing layer counts in [2, 32] required"
        )
    if (
        type(tolerances) is not tuple
        or not 1 <= len(tolerances) <= len(RESPONSE_UNITS)
...
def physical_quantity_basis(quantities: dict) -> dict:
    ...
    return {
        ...
        if k not in {"model_checksum", "quantity_hash"}
    }
...
def refine_sections(model: CanonicalModel, count: int) -> CanonicalModel:
    ...
    if not model.sections or any(
        s.get("type") != "rectangular_rc_fiber_section" for s in model.sections
    ):
        raise ValueError(
            "refinement currently requires only rectangular RC fiber sections"
        )
...
def response_trace(history: list[dict], response: str) -> dict[str, float] | None:
    ...
    for step, row in enumerate(history):
        ...
        if response.startswith("node_"):
            fields = (
                ("UX_m", "UY_m", "UZ_m")
                if response == "node_translation"
                else ("RX_rad", "RY_rad", "RZ_rad")
            )
            points = [
                ((p["node_id"], field), p[field])
                for p in row["node_displacements"]
                for field in fields
            ]
        elif response.startswith("reaction_"):
            unit = "N" if response == "reaction_force" else "N*m"
            points = [
                ((p["node_id"], p["dof"], p["unit"]), p["value_si"])
                for p in row["support_reactions"]
                if p["unit"] == unit
            ]
            ...
            locations = {
                (s["member_id"], s["integration_point_index"]): s["xi"]
                for s in row["section_results"]
            }
            ...
            for p in row["fiber_results"]:
                place = (
                    p["member_id"],
                    locations[(p["member_id"], p["integration_point_index"])],
                )
                if response.startswith("steel_") and p["material_kind"] == "steel":
                    ...
                    value = (
                        p["material_state"]["accumulated_plastic_strain"]
                        if response == "steel_plastic_strain"
                        else p["strain"]
                        if response == "steel_strain"
                        else p["stress_MPa"]
                    )
                    points.append((key, value))
                elif (
                    response.startswith("concrete_")
                    and p["material_kind"] == "concrete"
                ):
                    field = (
                        "tensile_damage"
                        if response == "concrete_tensile_envelope"
                        else "compressive_damage"
                    )
...
def compare_response_traces(left, right, tolerance: RCResponseTolerance) -> dict:
    ...
    base = {
        ...
        "unit": RESPONSE_UNITS[tolerance.response],
    ...
    return base | {
        ...
        "matched_count": len(left),
        "exceeded_count": violations,
...
def run_rc_refinement(
    ...
    *,
    levels: tuple[int, ...],
    tolerances: tuple[RCResponseTolerance, ...],
    ...
    plan = {
        "schema_version": "local-rc-refinement-plan.v1",
        "levels": list(levels),
        "model_checksum": model.canonical_model_checksum,
        "request": request.to_dict(),
        "source_revision": session.source_revision,
        "execution_options": session.execution_options,
        "source_revision_is_attestation": False,
        "tolerances": [asdict(t) for t in tolerances],
        ...
        "required_final_adjacent_pairs": 2,
        "claims": CLAIMS,
    }
    ...
    for count, refined in models.items():
        row = {
            "concrete_layer_count": count,
            "model_checksum": refined.canonical_model_checksum,
            "status": "not_run",
        ...
        work = value["new_work"]
        row.update(
            evaluation_hash=value["report_hash"], new_work=work, mode=value["mode"]
        )
        unknown_work = work["unknown_work"]
        outcome = value["row"]
        if (
            unknown_work
            or outcome["full_reference_verification_pass"] is not True
        ...
        if (
            ...
            or study._sha(raw) != reference["sha256"]
        ):
        ...
        traces[count] = {
            t.response: response_trace(history, t.response) for t in tolerances
        }
        row.update(
            status="verified_original",
            verified_limit_outcome=outcome["selection_eligible"],
    ...
    for left, right in zip(levels, levels[1:]):
        comparisons = [
            compare_response_traces(
                traces.get(left, {}).get(t.response),
                traces.get(right, {}).get(t.response),
                t,
            )
            for t in tolerances
        ]
    ...
    for t in tolerances:
        final = [
            next(r for r in pair["responses"] if r["response"] == t.response)["status"]
            for pair in pairs[-2:]
        ]
        response_status[t.response] = (
            "comparison_criteria_met"
            if all(s == "within_declared_tolerance" for s in final)
            else "comparison_unavailable"
            if "not_comparable" in final
    ...
    stable_screen = (
        all(type(s) is bool for s in final_outcomes) and len(set(final_outcomes)) == 1
    )
    decision = (
        final_outcomes[-1] if criteria and stable_screen and not unknown_work else None
    )
    report = {
        "schema_version": "local-rc-refinement-result.v1",
        "plan_hash": plan["plan_hash"],
        "status": "comparison_criteria_met"
        if criteria and not unknown_work
        else "further_verification_required",
        "rows": rows,
        ...
        "claims": CLAIMS,
        "timing_scope": "whole_refinement_except_final_report_write",
...
def run_refined_candidate_search(
    ...
    *,
    ...
    tolerances = _plan(levels, tolerances)
    if (
        type(max_new_model_analyses) is not int
        or not 0 <= max_new_model_analyses <= 102
    ):
    ...
    if (
        type(session) is not RCControlResultSession
        or type(prices) is not design.FiberFrameMaterialPrices
    ):
    ...
    order = [
        "baseline",
        *sorted((n for n in models if n != "baseline"), key=lambda n: (costs[n], n)),
    ]
    session._check_context(scope_id)
    root = Path(output_directory)
    root.mkdir(parents=True, exist_ok=False)
    plan = {
        "schema_version": "local-rc-refined-candidate-plan.v1",
        "pool": pool,
        "order": order,
        "levels": list(levels),
        "tolerances": [asdict(t) for t in tolerances],
    ...
    for name in order:
        ...
        if (
            name != "baseline"
            and bound["selected_estimate"] is not None
            and costs[name] >= bound["selected_estimate"]
        ):
    ...
    bound = finite_pool_cost_bound(pool, outcomes)
    report = {
        "schema_version": "local-rc-refined-candidate-search.v1",
        "plan_hash": plan["plan_hash"],
        "status": "unknown_work_stop" if stopped else bound["status"],
        "candidate_records": records,
        "refinement_screen_outcomes": outcomes,
        "cost_bound": bound,
        "selection_scope": "finite_declared_pool_and_response_comparison_screens_not_design_approval",
        "new_model_evaluations": used,
        "reused_model_evaluations": hits,
        "total_wall_ns": perf_counter_ns() - started,
        "timing_scope": "whole_candidate_search_except_final_report_write",
        "claims": CLAIMS,
    }
    report["report_hash"] = study._sha(study._bytes(report))
