...
class DurableRCControlResultSession(RCControlResultSession):
    ...
    def __init__(
        ...
        *,
        ...
        # Share the integrated store's no-replace first-owner authorization.
        # Chunk jobs retain isolated per-key databases so claim_next cannot
        # acquire an unrelated candidate. Completed jobs are not injected into
        # the full-result/refinement cache or awarded fresh replay credit.
        from structural_analysis.execution.rc_result_repository import (
            open_local_rc_repository,
        )

        if self._root.is_dir() and not (self._root / "rc-local-owner.json").exists():
            if any(
                not item.name.startswith(".rc-owner-") for item in self._root.iterdir()
            ):
                raise ValueError("legacy chunk store requires explicit owner migration")
        self._owner_repository = open_local_rc_repository(
            self._root,
            tenant_id=scope_id,
    ...
    def _store(self, key):
        self._owner_repository._authorize(self.scope_id)
