...
from structural_analysis.benchmark.rc_control_reuse import (
    NewAnalysisRequired, RCControlResultSession, _inputs,
)
from structural_analysis.execution.job_service import (
    DurableJobService, build_job_completion_evidence,
)
...
from structural_analysis.execution.rc_fiber_job_contract import (
    ...
    validate_rc_fiber_job_request, validate_rc_fiber_job_result,
)
...
class DurableRCControlResultSession(RCControlResultSession):
    ...
    def __init__(self, *, store_root: Path, source_revision: str, scope_id: str,
                 authorization_token: str, worker_token: str,
                 chunk_target_count: int = 255, max_chunks_per_call: int = 255) -> None:
        super().__init__(source_revision=source_revision, scope_id=scope_id)
        for name, number in (("chunk_target_count", chunk_target_count),
        ...
        for token in (authorization_token, worker_token):
            if type(token) is not str or len(token) < 16:
                raise ValueError("out-of-band credentials of at least 16 characters required")
    ...
    def _request(self, model, request, key):
        ...
        value = {
            ...
            "model": model.canonical_payload(), "config": request.to_dict(),
            "execution_config": {"chunk_target_count": chunk,
                                 "maximum_api_invocations": 2 * math.ceil(len(request.targets_m) / chunk)},
            "result_contract": CONSTANT_RC_FIBER_JOB_RESULT_SCHEMA_VERSION
            if request.constant_nodal_loads else RC_FIBER_JOB_RESULT_SCHEMA_VERSION,
        }
    ...
    def _store(self, key):
        ...
        for child in ("jobs.sqlite3", "jobs.sqlite3-wal", "jobs.sqlite3-shm", "blobs", "blobs/sha256"):
            _safe_directory(path / child)
        return DurableJobService(
            path, tenant_tokens={self.scope_id: self._tenant_token},
    ...
    def _evidence(self, service, job):
        ...
        for record in evidence["invocations"]:
            raw = service.read_rc_invocation_artifact(job.job_id, ordinal=record["ordinal"], **auth)
            outcome = strict_json_object_bytes(raw, maximum_bytes=576 * 1024 * 1024)
            payload = outcome["api_result"] if outcome["phase"] == "analysis" else outcome["verification_report"]
            metrics = (payload or {}).get("metrics", {}) if outcome["phase"] == "analysis" else (payload or {})
            work = metrics.get("control_work" if outcome["phase"] == "analysis" else "replay_control_work")
            rows.append({"phase": outcome["phase"], "work": work,
                         "unknown_execution_work": outcome["unavailable_execution_work"],
                         "ordinal": record["ordinal"]})
        for ordinal in evidence["pending_ordinals"]:
            rows.append({"phase": "unrecorded", "work": None,
                         "unknown_execution_work": True, "ordinal": ordinal})
        return evidence, rows

    def evaluate(self, model, request, *, scope_id, output_directory,
                 history_limits, material_limits, prices, terminal_limits=None,
                 fresh=False, allow_new_analysis=True) -> dict[str, Any]:
        ...
        if fresh:
            ...
            return super().evaluate(
                model, request, scope_id=scope_id, output_directory=output_directory,
                history_limits=history_limits, material_limits=material_limits,
                prices=prices, terminal_limits=terminal_limits, fresh=True,
        ...
        key = study._sha(study._bytes({"physics": self._key(model, request),
        ...
        mark = perf_counter_ns()
        completed_chunks = 0
        while allow_new_analysis and job.status in ("queued", "checkpointed", "running") and completed_chunks < self._max_chunks:
            if before["pending_ordinals"]:
                break  # Never silently retry abandoned, unknown numerical work.
            claim = service.claim_next(worker_id="local-rc", authorization_token=self._worker_token,
                                       tenant_id=scope_id, lease_seconds=300)
            if claim is None:
            ...
            reservation_start = service.read_execution_budget(
                job.job_id, worker_id="local-rc", authorization_token=self._worker_token,
                lease_token=claim.lease_token)["reserved_attempts"]
            ...
            job = execute_rc_fiber_direct_control_claim(
                service, claim, worker_id="local-rc", authorization_token=self._worker_token)
            completed_chunks += 1
        ...
        if not new_rows:
            new_work["known_counters"] = {name: 0 for name in (
                "attempted_step_count", "known_linear_solve_count", "known_newton_iteration_count",
                "unknown_solver_work_attempt_count")}
        unknown_origin = _work({"rows": [{"invocations": invocation_rows}]})["unknown_work"]
        raw_artifacts = {"request": service.read_request(job.job_id, **auth)}
        row = {"candidate_id": "baseline", "status": job.status, "quantities": quantities,
               "material_estimate": estimate, "performance": None, "screens": None,
               "full_reference_verification_pass": False, "selection_eligible": False,
               "invocations": new_rows, "artifacts": {}}
        if job.status == "succeeded":
            ...
            checkpoint = service.read_checkpoint(job.job_id, **auth) if job.checkpoint else None
            report = validate_rc_fiber_job_result(
                value, request=job_request, execution_budget=evidence["execution_budget"], checkpoint=checkpoint)
            expected = build_job_completion_evidence(
                job_id=job.job_id, request_hash=job.request.content_hash,
                checkpoint_hash=None if job.checkpoint is None else job.checkpoint.content_hash,
                result_bytes=raw, validation_report=report, validator_id=RC_FIBER_JOB_VALIDATOR_ID)
            if study._bytes(expected) != original_evidence:
                raise ValueError("persisted completion evidence differs from the validated result")
            ...
            row["screens"] = study._screens(row["performance"], history_limits, material_limits, terminal_limits)
            row["full_reference_verification_pass"] = not unknown_origin
            row["status"] = "verified" if not unknown_origin else "unknown_work"
            row["selection_eligible"] = not unknown_origin and all(r["status"] == "pass" for r in row["screens"].values())
        phase_times["integrity_and_rescreen_ns"] = perf_counter_ns() - mark
        mark = perf_counter_ns()
        output.mkdir(parents=True, exist_ok=False)
        row["artifacts"] = {name: study._save(output, name + ".json", raw)
                            for name, raw in raw_artifacts.items()}
        phase_times["original_export_ns"] = perf_counter_ns() - mark
        result = {
            "schema_version": "local-durable-rc-evaluation.v1", "physics_key": key,
            "model_checksum": model.canonical_model_checksum, "request": request.to_dict(),
            "mode": "verified_durable_original_reused" if reused else "durable_execution",
            "source_revision": self.source_revision, "source_revision_is_attestation": False,
            "job": job.to_dict(), "row": row, "integrity": integrity,
            "new_model_evaluation": completed_chunks > 0, "completed_chunks_this_call": completed_chunks,
            "fresh_reference_verification_this_call": bool(new_rows) and row["full_reference_verification_pass"],
            "new_work": new_work, "original_work_not_recharged": _work({"rows": [{"invocations": prior_rows}]}),
            "historical_unknown_work": unknown_origin, "timings_ns": phase_times,
            "total_wall_ns": perf_counter_ns() - started, "total_process_cpu_ns": process_time_ns() - started_cpu,
            ...
            "claims": {"persistent_local_storage": True, "independent_physical_validation": False,
                       "design_authority": False, "performance_improvement": False,
                       "gpu_execution": False, "cross_host_storage": False, "automatic_failed_retry": False},
        }
