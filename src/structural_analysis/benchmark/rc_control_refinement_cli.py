...
from structural_analysis.benchmark.rc_control_refinement import (
    RCResponseTolerance,
    _plan,
    run_refined_candidate_search,
)
...
def read_refinement(path: Path):
    ...
    if (
        set(value) != {"schema_version", "levels", "responses"}
        or value["schema_version"] != "local-rc-refinement-input.v1"
        or type(value["levels"]) is not list
        or type(value["responses"]) is not list
    ):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        report = run_refined_candidate_search(
            ...
            max_new_model_analyses=args.max_new_model_analyses,
