...
from structural_analysis.benchmark.rc_control_candidate_cost import verified_limit_outcome
...
def finite_pool_cost_bound(pool: list[dict], outcomes: dict[str, bool | None]) -> dict:
    ...
    for row in pool:
        ...
        identity = tuple(estimate.get(key) for key in ('currency', 'scope', 'price_table_hash'))
        ...
        common = identity
        costs[name] = value
    if ('baseline' not in costs or type(outcomes) is not dict or set(outcomes) != set(costs)
            or any(v is not None and type(v) is not bool for v in outcomes.values())):
        raise ValueError('complete unique pool and strict three-state outcomes required')
    ...
    cheaper_unknown = None if selected is None else [
        name for name in unknown if costs[name] < costs[selected]]
    confirmed = selected is not None and not cheaper_unknown
    status = ('pool_minimum_confirmed' if confirmed else
              'cheaper_outcomes_unknown' if selected is not None else
              'no_verified_selection' if unknown else 'no_feasible_candidate')
    assert common is not None
    return {
        'schema_version': 'local-rc-finite-pool-cost-bound.v1',
        'status': status, 'baseline_included': True,
        'candidate_denominator': len(costs),
        'currency': common[0], 'quantity_scope': common[1], 'price_table_hash': common[2],
        ...
        'all_minimum_cost_ties_verified': None if not confirmed else
            not any(costs[name] == costs[selected] for name in unknown),
...
def run_rc_control_cost_search(
    baseline, candidates, request, *, session: RCControlResultSession,
    scope_id: str, prices: design.FiberFrameMaterialPrices,
    history_limits: design.FiberFrameHistoryLimits,
    material_limits: design.FiberFrameMaterialHistoryLimits,
    output_directory: Path, max_new_model_analyses: int = 17,
    ...
    candidates = tuple(design.FiberFrameDesignCandidate(c.candidate_id, tuple(
    ...
    if candidate_order is not None and (type(candidate_order) is not tuple
            or len(candidate_order) != len(ids) or any(type(n) is not str for n in candidate_order)
    ...
    models.update({c.candidate_id: design.apply_fiber_frame_section_changes(baseline, c) for c in candidates})
    quantities = {name: design.calculate_fiber_frame_member_quantities(model) for name, model in models.items()}
    pool = [{'candidate_id': name, 'material_estimate': design._estimate(quantities[name], prices)} for name in models]
    outcomes: dict[str, bool | None] = {name: None for name in models}
    finite_pool_cost_bound(pool, outcomes)  # Validates finite comparable prices before output.
    costs = {row['candidate_id']: row['material_estimate']['total'] for row in pool}
    order = ['baseline', *(candidate_order if candidate_order is not None else
                          sorted(ids, key=lambda name: (costs[name], name)))]
    ...
    plan = {
        ...
        'scope_id': scope_id, 'baseline_checksum': baseline.canonical_model_checksum,
        'candidates': [asdict(c) for c in candidates], 'request': request.to_dict(),
        ...
        'prices': asdict(prices), 'price_table_hash': prices.price_table_hash,
        'pool': pool, 'evaluation_order': order,
        ...
        'model_checksums': {name: model.canonical_model_checksum for name, model in models.items()},
    }
    ...
    records = []
    aggregate = {key: 0 for key in ('attempted_step_count', 'known_linear_solve_count',
                'known_newton_iteration_count', 'unknown_solver_work_attempt_count')}
    invocations = 0
    for name in order:
        record: dict[str, Any] = {'candidate_id': name, 'status': 'not_evaluated',
            'verified_limit_outcome': None, 'evaluation': None}
        records.append(record)
        bound = finite_pool_cost_bound(pool, outcomes)
        selected_cost = bound['selected_estimate']
        if name != 'baseline' and selected_cost is not None and costs[name] >= selected_cost:
        ...
        try:
            evaluation = session.evaluate(
                models[name], request, scope_id=scope_id, output_directory=root / name,
                prices=prices, history_limits=history_limits, material_limits=material_limits,
                terminal_limits=terminal_limits, allow_new_analysis=used < max_new_model_analyses)
        ...
        hits += int(not fresh)
        row = evaluation['row']
        if (row['material_estimate'] != next(r['material_estimate'] for r in pool if r['candidate_id'] == name)
    ...
    bound = finite_pool_cost_bound(pool, outcomes)
    report = {
        'schema_version': 'local-rc-cost-search-result.v1', 'plan_hash': plan['plan_hash'],
        'status': 'unknown_work_stop' if unknown_work_stop else bound['status'],
        'evaluation_order': order, 'records': records, 'outcomes': outcomes,
        'cost_bound': bound, 'new_model_evaluations': used, 'reused_model_evaluations': hits,
        'new_work': {'known_counters': aggregate, 'unknown_work': unknown_work_stop,
        ...
        'claims': {'global_design_optimality_proved': False, 'confirmed_currency_savings': False,
                   'independent_physical_validation': False, 'design_authority': False,
                   'performance_improvement': False, 'exhaustive_oracle_executed': False,
                   'strategy_comparison_executed': False, 'release_approved': False},
    }
