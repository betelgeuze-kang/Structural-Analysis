"""Fail-closed persisted ancestor chains for bounded fiber-frame checkpoints."""
...
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_SCHEMA_VERSION = (
    "stateful-fiber-frame2d-checkpoint-chain.v1"
)
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_ROLE = "committed_ancestor_chain"
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_STORAGE_PROFILE = (
    STATEFUL_FIBER_FRAME2D_CHECKPOINT_STORAGE_PROFILE
)
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_MAX_BYTES = 32 * 1024 * 1024
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_MAX_CHECKPOINTS = 256
STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_SCHEMA_RESOURCE = (
    "stateful_fiber_frame2d_checkpoint_chain_v1.schema.json"
)

_CHAIN_HASH_DOMAIN = b"structural-analysis/stateful-fiber-frame2d-checkpoint-chain/v1\0"
...
class StatefulFiberFrame2DCheckpointChainArtifactError(
    StatefulFiberFrame2DCheckpointArtifactError
):
...
class StatefulFiberFrame2DCheckpointChain:
    ...
    def __post_init__(self) -> None:
        ...
        if self.role != STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_ROLE:
        ...
        if (
            ...
            or not all(
                type(checkpoint) is StatefulFiberFrame2DCheckpoint
    ...
    @property
    def root_checkpoint(self) -> StatefulFiberFrame2DCheckpoint:
        return self.checkpoints[0]

    @property
    def terminal_checkpoint(self) -> StatefulFiberFrame2DCheckpoint:
    ...
    def canonical_bytes(self) -> bytes:
        chunks = [
            ...
            _pack_text(STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_STORAGE_PROFILE),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": (STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_SCHEMA_VERSION),
            "role": self.role,
            "storage_profile": (
                STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_STORAGE_PROFILE
            ),
...
def validate_stateful_fiber_frame2d_checkpoint_chain(
    problem: StatefulFiberFrame2DProblem,
    chain: StatefulFiberFrame2DCheckpointChain,
) -> None:
    """Validate a complete chain against one exact frame problem and genesis."""

    if type(chain) is not StatefulFiberFrame2DCheckpointChain:
    ...
    expected_root = initial_stateful_fiber_frame2d_checkpoint(problem)
    if (
...
def make_stateful_fiber_frame2d_checkpoint_chain(
    problem: StatefulFiberFrame2DProblem,
    checkpoints: tuple[StatefulFiberFrame2DCheckpoint, ...],
    ...
    try:
        ...
        validate_stateful_fiber_frame2d_checkpoint_chain(problem, chain)
    except (TypeError, ValueError, OverflowError) as exc:
        raise StatefulFiberFrame2DCheckpointChainArtifactError(
...
def load_stateful_fiber_frame2d_checkpoint_chain_bytes(
    ...
    raw = bytes(data)
    if len(raw) > STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_MAX_BYTES:
        raise StatefulFiberFrame2DCheckpointChainArtifactError(
    ...
    except StatefulFiberFrame2DCheckpointChainArtifactError:
        raise
    ...
    checkpoints: list[StatefulFiberFrame2DCheckpoint] = []
    for index, checkpoint_payload in enumerate(payload["checkpoints"]):
        ...
        except StatefulFiberFrame2DCheckpointArtifactError as exc:
            raise StatefulFiberFrame2DCheckpointChainArtifactError(
...
def dump_stateful_fiber_frame2d_checkpoint_chain_bytes(
    problem: StatefulFiberFrame2DProblem,
    chain: StatefulFiberFrame2DCheckpointChain,
) -> bytes:
    ...
    for index, checkpoint in enumerate(chain.checkpoints):
        try:
            dump_stateful_fiber_frame2d_checkpoint_bytes(problem, checkpoint)
        except StatefulFiberFrame2DCheckpointArtifactError as exc:
            raise StatefulFiberFrame2DCheckpointChainArtifactError(
                f"checkpoint chain entry {index} is invalid: {exc}"
            ) from exc
    ...
    restored = load_stateful_fiber_frame2d_checkpoint_chain_bytes(raw, problem)
...
def stateful_fiber_frame2d_checkpoint_chain_artifact_hash(
    data: bytes | bytearray | memoryview,
) -> str:
...
def write_stateful_fiber_frame2d_checkpoint_chain_artifact(
    problem: StatefulFiberFrame2DProblem,
    chain: StatefulFiberFrame2DCheckpointChain,
    target: str | Path,
) -> Path:
    ...
    raw = dump_stateful_fiber_frame2d_checkpoint_chain_bytes(problem, chain)
    path = Path(target)
    try:
...
def read_stateful_fiber_frame2d_checkpoint_chain_artifact(
    problem: StatefulFiberFrame2DProblem,
    source: str | Path,
) -> StatefulFiberFrame2DCheckpointChain:
    ...
    try:
        with path.open("rb") as stream:
            raw = stream.read(STATEFUL_FIBER_FRAME2D_CHECKPOINT_CHAIN_MAX_BYTES + 1)
    except OSError as exc:
        raise StatefulFiberFrame2DCheckpointChainArtifactError(
    ...
    return load_stateful_fiber_frame2d_checkpoint_chain_bytes(raw, problem)
