"""Global assembly namespace reserved for canonical solver extraction."""

from structural_analysis.assembly.coupled_static import (
    FrameShellMaterialCoupledNewtonAdapter,
    FrameShellMaterialCoupledProblem,
    FrameShellMaterialCoupledState,
    assemble_frame_shell_material_coupled_state,
    default_frame_shell_material_coupled_problem,
    finite_difference_coupled_jacobian_check,
    solve_frame_shell_material_coupled,
)
from structural_analysis.assembly.nonlinear_static import (
    AxialChainAssemblyState,
    AxialChainMeshNewtonAdapter,
    AxialChainMeshProblem,
    CubicSpringAxialMaterialLaw,
    StrainCubicAxialMaterialLaw,
    assemble_axial_chain_state,
    axial_chain_mesh_problem_from_canonical_model,
    default_phase2_axial_chain_mesh_problem,
    finite_difference_assembled_jacobian_check,
    mesh_problem_with_scaled_external_load,
    mesh_series_force_equilibrium_check,
    refined_strain_cubic_axial_chain_mesh_problem,
    single_element_axial_chain_mesh_problem,
    solve_axial_chain_mesh,
)
from structural_analysis.assembly.material_state import (
    StateUpdatedFrameShellCoupledMaterialLoadStep,
    StateUpdatedFrameShellCoupledMaterialLoadStepResult,
    StateUpdatedFrameShellCoupledMaterialLoadStepSpec,
    StateUpdatedFrameShellCoupledMaterialNewtonAdapter,
    StateUpdatedFrameShellCoupledMaterialProblem,
    StateUpdatedFrameShellCoupledMaterialState,
    StateUpdatedMaterialAxialChainMeshNewtonAdapter,
    StateUpdatedMaterialAxialChainMeshProblem,
    StateUpdatedMaterialAxialChainMeshState,
    StateUpdatedMaterialMeshElement,
    StateUpdatedMaterialMeshLoadStep,
    StateUpdatedMaterialMeshLoadStepResult,
    StateUpdatedMaterialPathHistoryResult,
    StateUpdatedMaterialPathHistorySpec,
    StateUpdatedMaterialPathHistoryStep,
    assemble_state_updated_material_axial_chain_mesh_state,
    assemble_state_updated_frame_shell_coupled_material_state,
    check_frame_shell_coupled_material_load_step_replay,
    check_material_mesh_load_step_replay,
    check_state_updated_material_path_history_replay,
    default_state_updated_material_axial_chain_mesh_problem,
    default_state_updated_frame_shell_coupled_material_problem,
    default_state_updated_frame_shell_coupled_material_load_step_spec,
    default_state_updated_material_path_history_specs,
    frame_shell_coupled_material_load_step_checkpoint_payload,
    material_mesh_load_step_checkpoint_payload,
    material_path_history_checkpoint_payload,
    solve_state_updated_material_axial_chain_mesh_newton,
    solve_state_updated_material_mesh_load_step_history,
    solve_state_updated_frame_shell_coupled_material_load_step_history,
    solve_default_state_updated_material_path_histories,
    solve_state_updated_frame_shell_coupled_material_newton,
    solve_state_updated_material_path_history,
)
from structural_analysis.assembly.patch_static import (
    AxialPatchRigidBodyProblem,
    AxialPatchState,
    assemble_axial_patch_state,
    axial_element_strains_and_forces,
    axial_patch_stiffness,
    default_axial_patch_rigidbody_problem,
    rigid_body_nullspace_check,
)

__all__ = [
    "AxialPatchRigidBodyProblem",
    "AxialPatchState",
    "FrameShellMaterialCoupledNewtonAdapter",
    "FrameShellMaterialCoupledProblem",
    "FrameShellMaterialCoupledState",
    "StateUpdatedFrameShellCoupledMaterialLoadStep",
    "StateUpdatedFrameShellCoupledMaterialLoadStepResult",
    "StateUpdatedFrameShellCoupledMaterialLoadStepSpec",
    "StateUpdatedFrameShellCoupledMaterialNewtonAdapter",
    "StateUpdatedFrameShellCoupledMaterialProblem",
    "StateUpdatedFrameShellCoupledMaterialState",
    "StateUpdatedMaterialAxialChainMeshNewtonAdapter",
    "StateUpdatedMaterialAxialChainMeshProblem",
    "StateUpdatedMaterialAxialChainMeshState",
    "StateUpdatedMaterialMeshElement",
    "StateUpdatedMaterialMeshLoadStep",
    "StateUpdatedMaterialMeshLoadStepResult",
    "StateUpdatedMaterialPathHistoryResult",
    "StateUpdatedMaterialPathHistorySpec",
    "StateUpdatedMaterialPathHistoryStep",
    "AxialChainAssemblyState",
    "AxialChainMeshNewtonAdapter",
    "AxialChainMeshProblem",
    "CubicSpringAxialMaterialLaw",
    "StrainCubicAxialMaterialLaw",
    "assemble_frame_shell_material_coupled_state",
    "assemble_state_updated_material_axial_chain_mesh_state",
    "assemble_state_updated_frame_shell_coupled_material_state",
    "assemble_axial_chain_state",
    "assemble_axial_patch_state",
    "axial_chain_mesh_problem_from_canonical_model",
    "axial_element_strains_and_forces",
    "axial_patch_stiffness",
    "check_frame_shell_coupled_material_load_step_replay",
    "check_material_mesh_load_step_replay",
    "check_state_updated_material_path_history_replay",
    "default_phase2_axial_chain_mesh_problem",
    "default_frame_shell_material_coupled_problem",
    "default_state_updated_material_axial_chain_mesh_problem",
    "default_state_updated_frame_shell_coupled_material_load_step_spec",
    "default_state_updated_frame_shell_coupled_material_problem",
    "default_state_updated_material_path_history_specs",
    "default_axial_patch_rigidbody_problem",
    "finite_difference_assembled_jacobian_check",
    "finite_difference_coupled_jacobian_check",
    "mesh_problem_with_scaled_external_load",
    "mesh_series_force_equilibrium_check",
    "frame_shell_coupled_material_load_step_checkpoint_payload",
    "material_mesh_load_step_checkpoint_payload",
    "material_path_history_checkpoint_payload",
    "refined_strain_cubic_axial_chain_mesh_problem",
    "rigid_body_nullspace_check",
    "single_element_axial_chain_mesh_problem",
    "solve_default_state_updated_material_path_histories",
    "solve_frame_shell_material_coupled",
    "solve_state_updated_material_axial_chain_mesh_newton",
    "solve_state_updated_frame_shell_coupled_material_load_step_history",
    "solve_state_updated_frame_shell_coupled_material_newton",
    "solve_state_updated_material_mesh_load_step_history",
    "solve_state_updated_material_path_history",
    "solve_axial_chain_mesh",
]
