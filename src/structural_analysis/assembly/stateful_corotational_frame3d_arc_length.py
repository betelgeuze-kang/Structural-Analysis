...
class StatefulCorotationalFrame3DArcLengthError(StatefulCorotationalFrame3DSparseError):
    ...
    def __init__(
        ...
        message: str,
        *,
        code: str = "frame3d_arc_length_error",
        attempts: tuple[Mapping[str, Any], ...] | list[Mapping[str, Any]] = (),
    ) -> None:
        self.code = str(code)
        self.attempts = tuple(MappingProxyType(dict(row)) for row in attempts)
        super().__init__(
            message,
            reason_code=self.code,
            retryable_convergence_failure=(self.code in _RETRIABLE_ARC_FAILURE_CODES),
        )


@dataclass(frozen=True)
class _ArcLengthEquationScalingObservation:
    """JSON-ready observation using the current source-bound 6DOF scaling."""
    ...
    def to_dict(self) -> dict[str, float | str]:
        return {
            ...
            "rotation_residual_norm": self.rotation_residual_norm,
            "scaled_residual_norm": self.scaled_residual_norm,
            "translation_increment_norm": self.translation_increment_norm,
            "rotation_increment_norm": self.rotation_increment_norm,
            "scaled_increment_norm": self.scaled_increment_norm,
            "scaled_tangent_condition": self.scaled_tangent_condition,
            "scaling_hash": self.scaling_hash,
        }


@dataclass(frozen=True)
class _ArcLengthScalingTransform:
    """Dimensionless arc coordinates bound to the current 6DOF evidence."""

    binding: StatefulCorotationalFrame3DEquationScaling6DOF
    free_dofs: tuple[int, ...]

    def __post_init__(self) -> None:
        self.binding.to_dict()
        if len(self.free_dofs) == 0:
            raise ValueError("arc-length scaling requires free equations")
        equilibration_vectors_6dof(
            self.free_dofs,
            self.binding.characteristic_length_m,
        )

    @property
    def reference_force(self) -> float:
        return self.binding.reference_force_kn

    @property
    def reference_force_kn(self) -> float:
        return self.binding.reference_force_kn

    @property
    def characteristic_length(self) -> float:
        return self.binding.characteristic_length_m
    ...
    def scaling_hash(self) -> str:
        return self.binding.scaling_hash

    @property
    def dof_labels(self) -> tuple[str, ...]:
        labels = ("UX", "UY", "UZ", "RX", "RY", "RZ")
        return tuple(labels[dof % 6] for dof in self.free_dofs)

    def _residual_multipliers(self) -> np.ndarray:
        row_scale, _column_scale = equilibration_vectors_6dof(
            self.free_dofs,
            self.characteristic_length,
        )
        return row_scale / self.reference_force

    def _physical_coordinate_scales(self) -> np.ndarray:
        _row_scale, column_scale = equilibration_vectors_6dof(
            self.free_dofs,
            self.characteristic_length,
        )
        return self.characteristic_length * column_scale

    def scale_residual(self, residual: Any) -> np.ndarray:
        values = _finite_vector(residual, len(self.free_dofs), "residual")
        return self._residual_multipliers() * values

    def scale_increment(self, increment: Any) -> np.ndarray:
        values = _finite_vector(increment, len(self.free_dofs), "increment")
        return values / self._physical_coordinate_scales()
    ...
    def unscale_increment(self, scaled_increment: Any) -> np.ndarray:
        ...
        return self._physical_coordinate_scales() * values

    def scale_tangent(self, tangent: Any) -> csr_matrix:
        matrix = tangent.tocsr(copy=True).astype(np.float64, copy=False)
        size = len(self.free_dofs)
        if matrix.shape != (size, size) or not np.all(np.isfinite(matrix.data)):
            raise StatefulCorotationalFrame3DArcLengthError(
                "arc-length tangent is invalid",
                code="arc_length_invalid_augmented_correction",
            )
        scaled = (
            diags(self._residual_multipliers(), format="csr")
            @ matrix
            @ diags(self._physical_coordinate_scales(), format="csr")
        ).tocsr()
    ...
    def observe(
        ...
        *,
        ...
        increment: Any,
        scaled_tangent_condition: float,
    ) -> _ArcLengthEquationScalingObservation:
        ...
        return _ArcLengthEquationScalingObservation(
            reference_force=self.reference_force,
            characteristic_length=self.characteristic_length,
            translation_residual_norm=residual_metrics["translation"],
            rotation_residual_norm=residual_metrics["rotation"],
            scaled_residual_norm=residual_metrics["scaled"],
            translation_increment_norm=increment_metrics["translation"],
            rotation_increment_norm=increment_metrics["rotation"],
            scaled_increment_norm=increment_metrics["scaled"],
...
class StatefulCorotationalFrame3DArcLengthConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.maximum_corrector_iterations > self.solver_config.maximum_iterations:
            raise ValueError(
...
class StatefulCorotationalFrame3DArcLengthStep:
    ...
    equation_scaling: _ArcLengthEquationScalingObservation
...
def stateful_corotational_frame3d_arc_length_path_contract_hash(
    ...
    scaling = _arc_length_scaling(model, config.solver_config)
    ...
    *,
    ...
    while boundary.attempt_count < config.maximum_attempt_count:
        ...
        try:
            ...
            error_code = _sparse_error_code(error)
            error_attempts = _sparse_error_attempts(error)
            rollback_exact = bool(
                parent_bytes == canonical_json_bytes(parent.to_dict())
            )
            ...
            retriable = error_code in _RETRIABLE_ARC_FAILURE_CODES
            next_arc = (
            ...
            attempt = StatefulCorotationalFrame3DArcLengthAttempt(
                ...
                corrector_iteration_count=len(error_attempts),
                ...
                convergence_trace=tuple(
                    MappingProxyType(dict(row)) for row in error_attempts
                ),
...
def _solve_arc_attempt(
    *,
    ...
    scaling = _arc_length_scaling(model, solver)
    ...
    for iteration in range(config.maximum_corrector_iterations + 1):
        ...
        if residual_gate and constraint_gate and increment_gate and load_increment_gate:
            ...
            checks = MappingProxyType(
                {
                    ...
                    "line_search_step_valid": all(
                        alpha in solver.line_search_alphas for alpha in accepted_alphas
                    ),
            ...
            residual = _checkpoint_residual_inf_norm(final.assembly.residual_free)
...
def _arc_trial(
    *,
    ...
    scaled_coordinates: np.ndarray,
    load_factor: float,
    scaling: _ArcLengthScalingTransform,
) -> _ArcTrial:
    if (
    ...
    try:
        ...
        raise StatefulCorotationalFrame3DArcLengthError(
            str(error),
            code=_sparse_error_code(error),
            attempts=_sparse_error_attempts(error),
        ) from error
...
def _arc_corrector_line_search(
    *,
    ...
    for line_search_iteration, alpha in enumerate(solver.line_search_alphas):
        ...
        attempt: dict[str, Any] = {
            ...
            "required_normalized_merit": base_merit,
            "accepted": False,
        }
        try:
            ...
            accepted = bool(merit < float(attempt["required_normalized_merit"]))
            ...
            attempt["invalid_trial_code"] = _sparse_error_code(error)
            attempts.append(MappingProxyType(attempt))
    invalid_codes = {
...
def _arc_length_scaling(
    model: StatefulCorotationalFrame3DSparseModel,
    config: StatefulCorotationalFrame3DSparseConfig,
) -> _ArcLengthScalingTransform:
    binding = stateful_corotational_frame3d_equation_scaling_6dof(
        model,
        config=config,
    )
    return _ArcLengthScalingTransform(
        binding=binding,
        free_dofs=tuple(model.free_dofs),
    )
...
def _require_parent_unchanged(
    ...
    if _checkpoint_parent_signature(checkpoint) != expected_signature:
        raise StatefulCorotationalFrame3DArcLengthError(
            "arc-length trial mutated its accepted parent",
            code="parent_state_mutated",
        )


def _checkpoint_residual_inf_norm(values: Any) -> float:
    return _linf(values)


def _sparse_error_code(error: StatefulCorotationalFrame3DSparseError) -> str:
    ...
    if isinstance(reason_code, str) and reason_code:
        return reason_code
    return "frame3d_sparse_error"


def _sparse_error_attempts(
    ...
    if not isinstance(attempts, (tuple, list)):
        return ()
    return tuple(
        MappingProxyType(dict(row)) for row in attempts if isinstance(row, Mapping)
    )
