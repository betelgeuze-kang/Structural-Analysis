"""Fail-closed persisted checkpoint I/O for the bounded fiber-frame path."""
...
STATEFUL_FIBER_FRAME2D_CHECKPOINT_MAX_BYTES = 4 * 1024 * 1024
STATEFUL_FIBER_FRAME2D_CHECKPOINT_SCHEMA_RESOURCE = (
    "stateful_fiber_frame2d_checkpoint_v1.schema.json"
)
...
class StatefulFiberFrame2DCheckpointArtifactError(ValueError):
    """Raised when a persisted checkpoint artifact fails closed."""


def _artifact_json_bytes(payload: Any) -> bytes:
    """Return deterministic JSON while preserving the sign of binary64 zero."""
...
def _restore_fiber_state(
    ...
    *,
    ...
    except StatefulFiberFrame2DCheckpointArtifactError:
        raise
...
def _restore_element_state(
    ...
    *,
    ...
    try:
        ...
        raise StatefulFiberFrame2DCheckpointArtifactError(
            f"{path} element state is invalid"
        ) from exc
    _require_roundtrip(payload, restored, path=path)
    return restored


def load_stateful_fiber_frame2d_checkpoint_bytes(
    ...
    raw = bytes(data)
    if len(raw) > STATEFUL_FIBER_FRAME2D_CHECKPOINT_MAX_BYTES:
        raise StatefulFiberFrame2DCheckpointArtifactError(
...
def dump_stateful_fiber_frame2d_checkpoint_bytes(
    problem: StatefulFiberFrame2DProblem,
    checkpoint: StatefulFiberFrame2DCheckpoint,
) -> bytes:
...
def stateful_fiber_frame2d_checkpoint_artifact_hash(
    data: bytes | bytearray | memoryview,
) -> str:
    """Return the SHA-256 identity of exact persisted artifact bytes."""
...
def write_stateful_fiber_frame2d_checkpoint_artifact(
    problem: StatefulFiberFrame2DProblem,
    checkpoint: StatefulFiberFrame2DCheckpoint,
    target: str | Path,
) -> Path:
...
def read_stateful_fiber_frame2d_checkpoint_artifact(
    problem: StatefulFiberFrame2DProblem,
    source: str | Path,
) -> StatefulFiberFrame2DCheckpoint:
    """Read and source-validate one bounded persisted checkpoint artifact."""

    path = Path(source)
    try:
        size = path.stat().st_size
        if size > STATEFUL_FIBER_FRAME2D_CHECKPOINT_MAX_BYTES:
            raise StatefulFiberFrame2DCheckpointArtifactError(
                "checkpoint artifact exceeds the bounded byte limit"
            )
        raw = path.read_bytes()
    ...
    return load_stateful_fiber_frame2d_checkpoint_bytes(raw, problem)
