...
class CorotationalFiberFrameSparseAssembly:
    ...
    _jacobian_csr: csr_matrix

    @property
    def jacobian_csr(self) -> csr_matrix:
        return self._jacobian_csr.copy()
    ...
    def to_manifest(self) -> dict[str, Any]:
        return {
            ...
            "array_hashes": {
                ...
                "residual_load_factor_derivative_kn": array_data_hash(
                    self.residual_load_factor_derivative_kn
                ),
...
def assemble_stateful_corotational_fiber_frame2d_sparse(
    ...
    *,
    ...
    """Scatter element contributions directly to COO and canonical CSR."""
    ...
    physical_displacement = scale * generalized
    physical_displacement[list(problem.fixed_global_dofs)] = prescribed[
    ...
    prescribed_tangent_derivative = np.zeros(problem.global_dof_count, dtype=np.float64)
    terminal_prescribed = problem.prescribed_displacement_vector(1.0)
    ...
    for member, parent in zip(
        ...
        feature_response = integrate_corotational_frame2d_member_features(
            member.element,
            member.features,
            physical_displacement[list(global_dofs)],
            parent,
        ...
        prescribed_tangent_derivative[list(global_dofs)] += (
            tangent @ (terminal_prescribed[list(global_dofs)])
        )
    ...
    residual_load_factor_derivative = (
        scale[list(free_dofs)]
        * (partial_load_factor_derivative + prescribed_tangent_derivative)[
            list(free_dofs)
        ]
    )
    ...
    frozen = {
        ...
        "residual_load_factor_derivative": immutable_array(
            residual_load_factor_derivative, dtype="<f8"
        ),
    ...
    provisional = CorotationalFiberFrameSparseAssembly(
        ...
        residual_load_factor_derivative_kn=frozen["residual_load_factor_derivative"],
...
def compare_corotational_fiber_frame_dense_sparse_assembly(
    ...
    *,
    ...
    """Build both paths and issue a non-promoting numerical parity receipt."""
    ...
    metrics = MappingProxyType(
        {
            ...
            "load_factor_derivative_scaled_linf": _scaled_linf(
    ...
    checks = MappingProxyType(
        {
            ...
            "residual_parity": metrics["residual_scaled_linf"]
            <= COROTATIONAL_FIBER_FRAME_DENSE_SPARSE_PARITY_TOLERANCE,
            "tangent_parity": metrics["tangent_scaled_linf"]
            <= COROTATIONAL_FIBER_FRAME_DENSE_SPARSE_PARITY_TOLERANCE,
            "internal_load_parity": metrics["internal_load_scaled_linf"]
            <= COROTATIONAL_FIBER_FRAME_DENSE_SPARSE_PARITY_TOLERANCE,
            "load_factor_derivative_parity": metrics[
                "load_factor_derivative_scaled_linf"
            ]
            <= COROTATIONAL_FIBER_FRAME_DENSE_SPARSE_PARITY_TOLERANCE,
            "reaction_parity": metrics["reaction_scaled_linf"]
            <= COROTATIONAL_FIBER_FRAME_DENSE_SPARSE_PARITY_TOLERANCE,
