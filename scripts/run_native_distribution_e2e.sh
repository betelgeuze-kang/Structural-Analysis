...
release_linear_model="$e2e_root/frame3d-end-release-model-ir.json"
sed \
  ...
  "$linear_model" > "$release_linear_model"
grep -Fq '"i": ["RY"]' "$release_linear_model"
grep -Fq '"id": "BC2"' "$release_linear_model"
release_linear_request_directory="$e2e_root/frame3d-end-release-request"
env -i PATH="$empty_path" "$active/bin/structural-workbench" \
  model-create-linear-analysis-request "$release_linear_model" \
  --case model-frame-end-release-linear-c5 --load-pattern LC_WEAK \
  --max-iterations 100 --absolute-residual-tolerance 1e-11 \
  --relative-residual-tolerance 1e-13 --maximum-increment 0 \
  --output-dir "$release_linear_request_directory" \
  > "$e2e_root/frame3d-end-release-request.stdout.json"
grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
  "$release_linear_request_directory/request-receipt.json"
release_linear_direct="$e2e_root/frame3d-end-release-direct"
release_linear_partial="$e2e_root/frame3d-end-release-partial"
release_linear_resumed="$e2e_root/frame3d-end-release-resumed"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-run \
  "$release_linear_model" "$release_linear_request_directory/analysis-request.json" \
  --output-dir "$release_linear_direct" \
  > "$e2e_root/frame3d-end-release-direct.stdout.json"
grep -Fq '"status":"completed"' "$release_linear_direct/run-receipt.json"
grep -Fq '"fallback_count":0' "$release_linear_direct/result-ir.json"
grep -Fq '"recovery_element_types":[1]' \
  "$release_linear_direct/result-recovery-ir.json"
grep -Fq '"recovery_offsets":[0,12]' \
  "$release_linear_direct/result-recovery-ir.json"
release_i_my="$(sed -n 's/.*"recovery_values":\[\([^]]*\)\].*/\1/p' \
  "$release_linear_direct/result-recovery-ir.json" | cut -d, -f5)"
test "$release_i_my" = "0"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-run \
  "$release_linear_model" "$release_linear_request_directory/analysis-request.json" \
  --output-dir "$release_linear_partial" --iteration-budget 1 \
  > "$e2e_root/frame3d-end-release-partial.stdout.json"
grep -Fq '"status":"active"' "$release_linear_partial/run-receipt.json"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-resume \
  "$release_linear_model" "$release_linear_request_directory/analysis-request.json" \
  "$release_linear_partial/checkpoint.mlpcp" --output-dir "$release_linear_resumed" \
  > "$e2e_root/frame3d-end-release-resumed.stdout.json"
diff -r "$release_linear_direct" "$release_linear_resumed" \
  > "$e2e_root/frame3d-end-release-restart-diff.txt"

self_weight_linear_model="$e2e_root/frame3d-self-weight-model-ir.json"
sed \
  ...
  "$linear_model" > "$self_weight_linear_model"
grep -Fq '"model_id": "engine-v2-frame-cantilever-self-weight-installed"' \
  "$self_weight_linear_model"
grep -Fq '"self_weight": [0.0, 0.0, -1.0]' "$self_weight_linear_model"
self_weight_linear_request_directory="$e2e_root/frame3d-self-weight-request"
env -i PATH="$empty_path" "$active/bin/structural-workbench" \
  model-create-linear-analysis-request "$self_weight_linear_model" \
  --case model-frame-self-weight-linear-c5 --load-pattern LC_WEAK \
  --max-iterations 100 --absolute-residual-tolerance 1e-11 \
  --relative-residual-tolerance 1e-13 --maximum-increment 0 \
  --output-dir "$self_weight_linear_request_directory" \
  > "$e2e_root/frame3d-self-weight-request.stdout.json"
grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
  "$self_weight_linear_request_directory/request-receipt.json"
self_weight_linear_direct="$e2e_root/frame3d-self-weight-direct"
self_weight_linear_partial="$e2e_root/frame3d-self-weight-partial"
self_weight_linear_resumed="$e2e_root/frame3d-self-weight-resumed"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-run \
  "$self_weight_linear_model" "$self_weight_linear_request_directory/analysis-request.json" \
  --output-dir "$self_weight_linear_direct" \
  > "$e2e_root/frame3d-self-weight-direct.stdout.json"
grep -Fq '"status":"completed"' "$self_weight_linear_direct/run-receipt.json"
grep -Fq '"fallback_count":0' "$self_weight_linear_direct/result-ir.json"
grep -Fq -- '-1539.64405' "$self_weight_linear_direct/result-recovery-ir.json"
grep -Fq -- '-513.2146833333334' "$self_weight_linear_direct/result-recovery-ir.json"
grep -Fq -- '-0.00019245550624999994' "$self_weight_linear_direct/result-recovery-ir.json"
grep -Fq -- '3079.2880999999998' "$self_weight_linear_direct/reaction-result-ir.json"
grep -Fq -- '-3079.2880999999998' "$self_weight_linear_direct/reaction-result-ir.json"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-run \
  "$self_weight_linear_model" "$self_weight_linear_request_directory/analysis-request.json" \
  --output-dir "$self_weight_linear_partial" --iteration-budget 1 \
  > "$e2e_root/frame3d-self-weight-partial.stdout.json"
grep -Fq '"status":"active"' "$self_weight_linear_partial/run-receipt.json"
env -i PATH="$empty_path" "$active/bin/structural-cli" analysis model-linear-resume \
  "$self_weight_linear_model" "$self_weight_linear_request_directory/analysis-request.json" \
  "$self_weight_linear_partial/checkpoint.mlpcp" --output-dir "$self_weight_linear_resumed" \
  > "$e2e_root/frame3d-self-weight-resumed.stdout.json"
...
verify_reaction_view_self_hash() {
  local view="$1"
  local locale="$2"
  ...
  if [[ ! "$declared" =~ ^[0-9a-f]{64}$ ]]; then
    echo "installed reaction view has no valid self-hash: $view" >&2
    exit 1
  fi
  ...
  actual="$(sha256sum "$unsigned" | awk '{print $1}')"
  if [[ "$actual" != "$declared" ]]; then
    echo "installed reaction view self-hash mismatch: $view" >&2
    exit 1
  fi
}

exercise_model_ir_linear_reaction_view_surface() {
  local label="$1"
  local workspace="$2"
  local restarted_workspace="$3"
  local workspace_before="$e2e_root/$label-reaction-view-workspace-before"
  cp -a -- "$workspace" "$workspace_before"
  local locale first second
  for locale in en-US ko-KR; do
    first="$e2e_root/$label-reaction-view-$locale-first.txt"
    second="$e2e_root/$label-reaction-view-$locale-second.txt"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" reaction-view \
      --workspace "$workspace" --locale "$locale" > "$first"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" reaction-view \
      --workspace "$workspace" --locale "$locale" > "$second"
    cmp "$first" "$second"
    grep -Fq 'structural-native-workbench-model-ir-linear-reaction-view.v1' "$first"
    grep -Fq 'fallback 0' "$first"
    grep -Eq $'^000001\t[^\t]+\tUX\t0000000000\t' "$first"
    if LC_ALL=C grep -q $'\033' "$first"; then
      echo "installed reaction view contains an ANSI escape" >&2
      exit 1
    fi
    verify_reaction_view_self_hash "$first" "$locale"
  done
  if cmp -s "$e2e_root/$label-reaction-view-en-US-first.txt" \
    "$e2e_root/$label-reaction-view-ko-KR-first.txt"; then
    echo "installed reaction views must have distinct locale identities" >&2
    exit 1
  fi
  local window="$e2e_root/$label-reaction-view-window.txt"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" reaction-view \
    --workspace "$workspace" --locale en-US --start-row 2 --count 2 > "$window"
  grep -Fq 'Displayed rows: 2-3 of 6' "$window"
  if grep -Eq $'^000001\t|^000004\t' "$window"; then
    echo "installed reaction view window escaped its requested row bounds" >&2
    exit 1
  fi
  grep -Eq $'^000002\t' "$window"
  grep -Eq $'^000003\t' "$window"
  verify_reaction_view_self_hash "$window" en-US
  env -i PATH="$empty_path" "$active/bin/structural-workbench" reaction-view \
    --workspace "$restarted_workspace" --locale en-US \
    > "$e2e_root/$label-reaction-view-restarted.txt"
  cmp "$e2e_root/$label-reaction-view-en-US-first.txt" \
    "$e2e_root/$label-reaction-view-restarted.txt"
  diff -r "$workspace_before" "$workspace" \
    > "$e2e_root/$label-reaction-view-workspace-diff.txt"
}
...
if env -i PATH="$empty_path" "$active/bin/structural-workbench" reaction-view \
  --workspace "$direct" > "$e2e_root/reaction-view-wrong-profile-rejected.json"; then
  echo "installed reaction view accepted the NDTHA profile" >&2
  exit 1
fi
...
verify_nodal_displacement_view_self_hash() {
  local view="$1"
  local locale="$2"
  local label declared unsigned actual
  if [[ "$locale" == "ko-KR" ]]; then
  ...
  declared="$(sed -n "s/^$label//p" "$view")"
  if [[ ! "$declared" =~ ^[0-9a-f]{64}$ ]]; then
    echo "installed nodal displacement view has no valid self-hash: $view" >&2
    exit 1
  fi
  unsigned="$view.unsigned"
  sed '$d' "$view" > "$unsigned"
  actual="$(sha256sum "$unsigned" | awk '{print $1}')"
  if [[ "$actual" != "$declared" ]]; then
    echo "installed nodal displacement view self-hash mismatch: $view" >&2
    exit 1
  fi
}

exercise_model_ir_linear_nodal_displacement_view_surface() {
  local label="$1"
  local workspace="$2"
  local restarted_workspace="$3"
  local workspace_before="$e2e_root/$label-nodal-displacement-view-workspace-before"
  cp -a -- "$workspace" "$workspace_before"
  local locale first second
  for locale in en-US ko-KR; do
    first="$e2e_root/$label-nodal-displacement-view-$locale-first.txt"
    second="$e2e_root/$label-nodal-displacement-view-$locale-second.txt"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      nodal-displacement-view --workspace "$workspace" --locale "$locale" > "$first"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      nodal-displacement-view --workspace "$workspace" --locale "$locale" > "$second"
    cmp "$first" "$second"
    grep -Fq 'structural-native-workbench-model-ir-linear-nodal-displacement-view.v1' \
      "$first"
    grep -Fq 'UX/UY/UZ=m; RX/RY/RZ=rad' "$first"
    grep -Fq 'fallback 0' "$first"
    grep -Eq $'^000001\t[^\t]+\t0000000000\t[-+0-9.e]+\t[-+0-9.e]+\t[-+0-9.e]+\t[-+0-9.e]+\t[-+0-9.e]+\t[-+0-9.e]+$' \
      "$first"
    if LC_ALL=C grep -q $'\033' "$first"; then
      echo "installed nodal displacement view contains an ANSI escape" >&2
      exit 1
    fi
    verify_nodal_displacement_view_self_hash "$first" "$locale"
  done
  if cmp -s "$e2e_root/$label-nodal-displacement-view-en-US-first.txt" \
    "$e2e_root/$label-nodal-displacement-view-ko-KR-first.txt"; then
    echo "installed nodal displacement views must have distinct locale identities" >&2
    exit 1
  fi
  local window="$e2e_root/$label-nodal-displacement-view-window.txt"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    nodal-displacement-view --workspace "$workspace" --locale en-US \
    --start-node 2 --count 1 > "$window"
  grep -Fq 'Displayed nodes: 2-2 of 2' "$window"
  if grep -Eq $'^000001\t' "$window"; then
    echo "installed nodal displacement view window escaped its requested node bounds" >&2
    exit 1
  fi
  grep -Eq $'^000002\t' "$window"
  verify_nodal_displacement_view_self_hash "$window" en-US
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    nodal-displacement-view --workspace "$restarted_workspace" --locale en-US \
    > "$e2e_root/$label-nodal-displacement-view-restarted.txt"
  cmp "$e2e_root/$label-nodal-displacement-view-en-US-first.txt" \
    "$e2e_root/$label-nodal-displacement-view-restarted.txt"
  diff -r "$workspace_before" "$workspace" \
    > "$e2e_root/$label-nodal-displacement-view-workspace-diff.txt"
}
...
if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
  nodal-displacement-view --workspace "$direct" \
  > "$e2e_root/nodal-displacement-view-wrong-profile-rejected.json"; then
  echo "installed nodal displacement view accepted the NDTHA profile" >&2
  exit 1
fi
grep -Fq 'workbench_profile_unsupported' \
...
verify_linear_element_recovery_view_self_hash() {
  local view="$1"
  local locale="$2"
  local label declared unsigned actual
  if [[ "$locale" == "ko-KR" ]]; then
  ...
  declared="$(sed -n "s/^$label//p" "$view")"
  if [[ ! "$declared" =~ ^[0-9a-f]{64}$ ]]; then
    echo "installed ModelIR linear element recovery view has no valid self-hash: $view" >&2
    exit 1
  fi
  unsigned="$view.unsigned"
  sed '$d' "$view" > "$unsigned"
  actual="$(sha256sum "$unsigned" | awk '{print $1}')"
  if [[ "$actual" != "$declared" ]]; then
    echo "installed ModelIR linear element recovery view self-hash mismatch: $view" >&2
    exit 1
  fi
}

exercise_model_ir_linear_element_recovery_view_surface() {
  local label="$1"
  local workspace="$2"
  local restarted_workspace="$3"
  local workspace_before="$e2e_root/$label-element-recovery-view-workspace-before"
  cp -a -- "$workspace" "$workspace_before"
  local locale first second
  for locale in en-US ko-KR; do
    first="$e2e_root/$label-element-recovery-view-$locale-first.txt"
    second="$e2e_root/$label-element-recovery-view-$locale-second.txt"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      element-recovery-view --workspace "$workspace" --locale "$locale" > "$first"
    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      element-recovery-view --workspace "$workspace" --locale "$locale" > "$second"
    cmp "$first" "$second"
    grep -Fq 'structural-native-workbench-model-ir-linear-element-recovery-view.v1' "$first"
    grep -Fq '1 of 1 (terminal linear static)' "$first"
    grep -Fq 'frame3d=element_local; truss3d=element_axis' "$first"
    grep -Fq 'i_FX/i_FY/i_FZ/j_FX/j_FY/j_FZ=N' "$first"
    grep -Fq 'fallback 0' "$first"
    grep -Eq $'^000001\t[^\t]+\t0000000000\tframe_3d\t[^\t]+->[^\t]+\telement_local\ti_FX_N=[-+0-9.e]+;' "$first"
    if LC_ALL=C grep -q $'\033' "$first"; then
      echo "installed ModelIR linear element recovery view contains an ANSI escape" >&2
      exit 1
    fi
    verify_linear_element_recovery_view_self_hash "$first" "$locale"
  done
  if cmp -s "$e2e_root/$label-element-recovery-view-en-US-first.txt" \
    "$e2e_root/$label-element-recovery-view-ko-KR-first.txt"; then
    echo "installed ModelIR linear element recovery views must have distinct locale identities" >&2
    exit 1
  fi
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    element-recovery-view --workspace "$restarted_workspace" --locale en-US \
    > "$e2e_root/$label-element-recovery-view-restarted.txt"
  cmp "$e2e_root/$label-element-recovery-view-en-US-first.txt" \
    "$e2e_root/$label-element-recovery-view-restarted.txt"
  diff -r "$workspace_before" "$workspace" \
    > "$e2e_root/$label-element-recovery-view-workspace-diff.txt"
}
...
if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
  element-recovery-view --workspace "$linear_direct" --start-element 2 \
  > "$e2e_root/linear-element-recovery-view-invalid-window-rejected.json"; then
  echo "installed ModelIR linear element recovery view accepted an invalid window" >&2
  exit 1
fi
grep -Fq 'workbench_element_recovery_view_window_invalid' \
  "$e2e_root/linear-element-recovery-view-invalid-window-rejected.json"
if cmp -s "$e2e_root/model-ir-linear-element-recovery-view-en-US-first.txt" \
  "$e2e_root/mgt-model-ir-linear-element-recovery-view-en-US-first.txt"; then
  echo "installed strict-ModelIR and normalized-MGT element recovery views must differ" >&2
  exit 1
fi
...
exercise_model_ir_linear_localized_pdf_surface() {
  ...
  for locale in en-US ko-KR; do
    ...
    for output_directory in "$first" "$second"; do
      ...
      test -s "$output_directory/report.pdf"
      test -s "$output_directory/pdf-receipt.json"
      grep -Fq '"schema_version":"structural-native-model-ir-linear-engineering-localized-pdf-report-receipt.v3"' \
        "$output_directory/pdf-receipt.json"
      grep -Fq '"profile":"model_ir_linear_cpu_engineering_summary_v1"' \
        "$output_directory/pdf-receipt.json"
      grep -Fq '"source_recovery_hash":"sha256:' \
        "$output_directory/pdf-receipt.json"
      grep -Fq '"source_reaction_hash":"sha256:' \
        "$output_directory/pdf-receipt.json"
      grep -Fq "\"locale\":\"$locale\"" "$output_directory/pdf-receipt.json"
      grep -Fq "\"content_hash\":\"sha256:$localized_report_font_hash\"" \
        "$output_directory/pdf-receipt.json"
...
exercise_frame_section_identity_edit_surface() {
  ...
  local partial_directory resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/frame-section-identity-edit-$label"
    request_directory="$e2e_root/frame-section-identity-edit-$label-request"
    direct_directory="$e2e_root/frame-section-identity-edit-$label-direct"
    partial_directory="$e2e_root/frame-section-identity-edit-$label-partial"
    resumed_directory="$e2e_root/frame-section-identity-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-frame-section-identity "$source_model" \
      --section S2 --new-section S2_RENAMED --output-dir "$edit_directory" \
      > "$e2e_root/frame-section-identity-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"frame_section_identity_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_section_id":"S2"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_section_id":"S2_RENAMED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"section_index":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"family_id":"frame_3d"' "$edit_directory/edit-receipt.json"
    grep -Fq '"parameter_set_version":"1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"area_m2":0.01' "$edit_directory/edit-receipt.json"
    grep -Fq '"iy_m4":4e-05' "$edit_directory/edit-receipt.json"
    grep -Fq '"iz_m4":2.5e-05' "$edit_directory/edit-receipt.json"
    grep -Fq '"torsional_constant_m4":5e-06' "$edit_directory/edit-receipt.json"
    grep -Fq '"shear_area_y_m2":0.008' "$edit_directory/edit-receipt.json"
    grep -Fq '"shear_area_z_m2":0.008' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_source_id":null' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_extensions":{}' "$edit_directory/edit-receipt.json"
    grep -Fq '"cpp_semantic_snapshot_verified":true' \
      "$edit_directory/edit-receipt.json"
    ...
    grep -Eq '"receipt_hash":"sha256:[0-9a-f]{64}"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"id":"S2_RENAMED"' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-frame-section-identity.v1"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/frame-section-identity-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case frame-section-identity-edit-c5 --load-pattern LC_WEAK \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/frame-section-identity-edit-$label-request.stdout.json"
    grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
      "$request_directory/request-receipt.json"
    grep -Fq '"execution_started":false' "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/frame-section-identity-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"active_dof_indices":[6,7,8,9,10,11]' \
      "$direct_directory/result-recovery-ir.json"
    grep -Fq '"active_external_load":[0,-10000,0,0,0,0]' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/frame-section-identity-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/frame-section-identity-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/frame-section-identity-edit-$label-restart-diff.txt"
  done

  local suffix
  for suffix in '' -request -direct -partial -resumed; do
    diff -r "$e2e_root/frame-section-identity-edit-first$suffix" \
      "$e2e_root/frame-section-identity-edit-second$suffix" \
      > "$e2e_root/frame-section-identity-edit$suffix-diff.txt"
  done
  cmp "$e2e_root/frame-section-identity-edit-first.stdout.json" \
    "$e2e_root/frame-section-identity-edit-second.stdout.json"
  cmp "$e2e_root/frame-section-identity-edit-first-validation.json" \
    "$e2e_root/frame-section-identity-edit-second-validation.json"
  for suffix in request direct partial resumed; do
    cmp "$e2e_root/frame-section-identity-edit-first-$suffix.stdout.json" \
      "$e2e_root/frame-section-identity-edit-second-$suffix.stdout.json"
  done
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed frame-section identity edit mutated its source ModelIR" >&2
    exit 1
  fi

  local missing_destination="$e2e_root/frame-section-identity-edit-missing-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity "$source_model" \
    --section S404 --new-section S3 --output-dir "$missing_destination" \
    > "$e2e_root/frame-section-identity-edit-missing-rejected.stdout.json"; then
    echo "installed frame-section identity edit accepted a missing section" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_frame_section_identity_section_missing' \
    "$e2e_root/frame-section-identity-edit-missing-rejected.stdout.json"
  test ! -e "$missing_destination"

  local collision_destination="$e2e_root/frame-section-identity-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity "$source_model" \
    --section S2 --new-section S1 --output-dir "$collision_destination" \
    > "$e2e_root/frame-section-identity-edit-collision-rejected.stdout.json"; then
    echo "installed frame-section identity edit accepted a colliding identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_frame_section_identity_replacement_exists' \
    "$e2e_root/frame-section-identity-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"

  local no_op_destination="$e2e_root/frame-section-identity-edit-no-op-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity "$source_model" \
    --section S2 --new-section S2 --output-dir "$no_op_destination" \
    > "$e2e_root/frame-section-identity-edit-no-op-rejected.stdout.json"; then
    echo "installed frame-section identity edit accepted a no-op" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_no_change' \
    "$e2e_root/frame-section-identity-edit-no-op-rejected.stdout.json"
  test ! -e "$no_op_destination"

  local invalid_destination="$e2e_root/frame-section-identity-edit-invalid-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity "$source_model" \
    --section S2 --new-section 1_INVALID --output-dir "$invalid_destination" \
    > "$e2e_root/frame-section-identity-edit-invalid-rejected.stdout.json"; then
    echo "installed frame-section identity edit accepted an invalid stable identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_frame_section_identity_replacement_invalid' \
    "$e2e_root/frame-section-identity-edit-invalid-rejected.stdout.json"
  test ! -e "$invalid_destination"
}
exercise_frame_section_identity_edit_surface
...
exercise_truss_section_identity_edit_surface() {
  local source_model="$e2e_root/truss-section-delete-source/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local partial_directory resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/truss-section-identity-edit-$label"
    request_directory="$e2e_root/truss-section-identity-edit-$label-request"
    direct_directory="$e2e_root/truss-section-identity-edit-$label-direct"
    partial_directory="$e2e_root/truss-section-identity-edit-$label-partial"
    resumed_directory="$e2e_root/truss-section-identity-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-truss-section-identity "$source_model" \
      --section T2 --new-section T2_RENAMED --output-dir "$edit_directory" \
      > "$e2e_root/truss-section-identity-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"truss_section_identity_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_section_id":"T2"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_section_id":"T2_RENAMED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"section_index":2' "$edit_directory/edit-receipt.json"
    grep -Fq '"family_id":"truss_3d"' "$edit_directory/edit-receipt.json"
    grep -Fq '"parameter_set_version":"1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_parameters_si":{"area_m2":0.0025}' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_source_id":null' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_extensions":{}' "$edit_directory/edit-receipt.json"
    grep -Fq '"cpp_semantic_snapshot_verified":true' \
    ...
    grep -Eq '"receipt_hash":"sha256:[0-9a-f]{64}"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"id":"T2_RENAMED"' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-truss-section-identity.v1"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/truss-section-identity-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case truss-section-identity-edit-c5 --load-pattern LC_WEAK \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/truss-section-identity-edit-$label-request.stdout.json"
    grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
      "$request_directory/request-receipt.json"
    grep -Fq '"execution_started":false' "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/truss-section-identity-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"active_dof_indices":[6,7,8,9,10,11]' \
      "$direct_directory/result-recovery-ir.json"
    grep -Fq '"active_external_load":[0,-10000,0,0,0,0]' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/truss-section-identity-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/truss-section-identity-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/truss-section-identity-edit-$label-restart-diff.txt"
  done

  local suffix
  for suffix in '' -request -direct -partial -resumed; do
    diff -r "$e2e_root/truss-section-identity-edit-first$suffix" \
      "$e2e_root/truss-section-identity-edit-second$suffix" \
      > "$e2e_root/truss-section-identity-edit$suffix-diff.txt"
  done
  cmp "$e2e_root/truss-section-identity-edit-first.stdout.json" \
    "$e2e_root/truss-section-identity-edit-second.stdout.json"
  cmp "$e2e_root/truss-section-identity-edit-first-validation.json" \
    "$e2e_root/truss-section-identity-edit-second-validation.json"
  for suffix in request direct partial resumed; do
    cmp "$e2e_root/truss-section-identity-edit-first-$suffix.stdout.json" \
      "$e2e_root/truss-section-identity-edit-second-$suffix.stdout.json"
  done
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed truss-section identity edit mutated its source ModelIR" >&2
    exit 1
  fi

  local missing_destination="$e2e_root/truss-section-identity-edit-missing-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity "$source_model" \
    --section T404 --new-section T3 --output-dir "$missing_destination" \
    > "$e2e_root/truss-section-identity-edit-missing-rejected.stdout.json"; then
    echo "installed truss-section identity edit accepted a missing section" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_section_missing' \
    "$e2e_root/truss-section-identity-edit-missing-rejected.stdout.json"
  test ! -e "$missing_destination"

  local collision_destination="$e2e_root/truss-section-identity-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity "$source_model" \
    --section T2 --new-section T1 --output-dir "$collision_destination" \
    > "$e2e_root/truss-section-identity-edit-collision-rejected.stdout.json"; then
    echo "installed truss-section identity edit accepted a colliding identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_replacement_exists' \
    "$e2e_root/truss-section-identity-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"

  local referenced_destination="$e2e_root/truss-section-identity-edit-referenced-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity "$source_model" \
    --section T1 --new-section T1_RENAMED --output-dir "$referenced_destination" \
    > "$e2e_root/truss-section-identity-edit-referenced-rejected.stdout.json"; then
    echo "installed truss-section identity edit accepted an element-owned section" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_referenced_by_element' \
    "$e2e_root/truss-section-identity-edit-referenced-rejected.stdout.json"
  test ! -e "$referenced_destination"

  local no_op_destination="$e2e_root/truss-section-identity-edit-no-op-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity "$source_model" \
    --section T2 --new-section T2 --output-dir "$no_op_destination" \
    > "$e2e_root/truss-section-identity-edit-no-op-rejected.stdout.json"; then
    echo "installed truss-section identity edit accepted a no-op" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_no_change' \
    "$e2e_root/truss-section-identity-edit-no-op-rejected.stdout.json"
  test ! -e "$no_op_destination"

  local invalid_destination="$e2e_root/truss-section-identity-edit-invalid-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity "$source_model" \
    --section T2 --new-section 1_INVALID --output-dir "$invalid_destination" \
    > "$e2e_root/truss-section-identity-edit-invalid-rejected.stdout.json"; then
    echo "installed truss-section identity edit accepted an invalid stable identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_replacement_invalid' \
    "$e2e_root/truss-section-identity-edit-invalid-rejected.stdout.json"
  test ! -e "$invalid_destination"
}
exercise_truss_section_identity_edit_surface
...
exercise_node_identity_edit_surface() {
  ...
  local label edit_directory composed_directory request_directory direct_directory
  local partial_directory resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/node-identity-edit-$label"
    composed_directory="$e2e_root/node-identity-edit-$label-composed"
    request_directory="$e2e_root/node-identity-edit-$label-request"
    direct_directory="$e2e_root/node-identity-edit-$label-direct"
    partial_directory="$e2e_root/node-identity-edit-$label-partial"
    resumed_directory="$e2e_root/node-identity-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-node-identity "$source_model" \
      --node N3 --new-node N3_RENAMED --output-dir "$edit_directory" \
      > "$e2e_root/node-identity-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"node_identity_edit"' "$edit_directory/edit-receipt.json"
    grep -Fq '"source_node_id":"N3"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_node_id":"N3_RENAMED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"node_index":2' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_coordinates_m":[4,1,0]' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_source_id":null' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_extensions":{}' "$edit_directory/edit-receipt.json"
    grep -Fq '"cpp_semantic_snapshot_verified":true' "$edit_directory/edit-receipt.json"
    grep -Fq '"analysis_ready":true' "$edit_directory/edit-receipt.json"
    grep -Eq '"receipt_hash":"sha256:[0-9a-f]{64}"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"coordinates_m":[4,1,0],"extensions":{},"id":"N3_RENAMED","index":2,"source_id":null' \
      "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-node-identity.v1"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/node-identity-edit-$label-validation.json"
    grep -Fq '"entity_counts":{"nodes":3,"materials":1,"sections":1,"elements":1,"constraints":1' \
      "$e2e_root/node-identity-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-add-fixed-constraint "$edit_directory/model-ir.json" \
      --constraint BC_N3_RENAMED --node N3_RENAMED --output-dir "$composed_directory" \
      > "$e2e_root/node-identity-edit-$label-composed.stdout.json"
    grep -Fq '"structural-native:model-edit-node-identity.v1"' \
      "$composed_directory/model-ir.json"
    grep -Fq '"id":"BC_N3_RENAMED","index":1,"node_id":"N3_RENAMED"' \
      "$composed_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$composed_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/node-identity-edit-$label-composed-validation.json"
    grep -Fq '"entity_counts":{"nodes":3,"materials":1,"sections":1,"elements":1,"constraints":2' \
      "$e2e_root/node-identity-edit-$label-composed-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$composed_directory/model-ir.json" \
      --case node-identity-edit-c5 --load-pattern LC_WEAK \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/node-identity-edit-$label-request.stdout.json"
    grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
      "$request_directory/request-receipt.json"
    grep -Fq '"execution_started":false' "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$composed_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/node-identity-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"active_dof_indices":[6,7,8,9,10,11]' \
      "$direct_directory/result-recovery-ir.json"
    grep -Fq '"active_external_load":[0,-10000,0,0,0,0]' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$composed_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 > "$e2e_root/node-identity-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$composed_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/node-identity-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/node-identity-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -composed -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/node-identity-edit-first$suffix" \
      "$e2e_root/node-identity-edit-second$suffix" \
      > "$e2e_root/node-identity-edit-$diff_label-diff.txt"
    cmp "$e2e_root/node-identity-edit-first$suffix.stdout.json" \
      "$e2e_root/node-identity-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/node-identity-edit-first-validation.json" \
    "$e2e_root/node-identity-edit-second-validation.json"
  cmp "$e2e_root/node-identity-edit-first-composed-validation.json" \
    "$e2e_root/node-identity-edit-second-composed-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed node identity edit mutated its source ModelIR" >&2
    exit 1
  fi

  local missing_destination="$e2e_root/node-identity-edit-missing-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity "$source_model" \
    --node N404 --new-node N4 --output-dir "$missing_destination" \
    > "$e2e_root/node-identity-edit-missing-rejected.stdout.json"; then
    echo "installed node identity edit accepted a missing node" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_node_identity_node_missing' \
    "$e2e_root/node-identity-edit-missing-rejected.stdout.json"
  test ! -e "$missing_destination"

  local collision_destination="$e2e_root/node-identity-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity "$source_model" \
    --node N3 --new-node N2 --output-dir "$collision_destination" \
    > "$e2e_root/node-identity-edit-collision-rejected.stdout.json"; then
    echo "installed node identity edit accepted a colliding identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_node_identity_replacement_exists' \
    "$e2e_root/node-identity-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"

  local referenced_destination="$e2e_root/node-identity-edit-referenced-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity "$source_model" \
    --node N2 --new-node N2_RENAMED --output-dir "$referenced_destination" \
    > "$e2e_root/node-identity-edit-referenced-rejected.stdout.json"; then
    echo "installed node identity edit accepted an element-owned node" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_node_identity_referenced_by_element' \
    "$e2e_root/node-identity-edit-referenced-rejected.stdout.json"
  test ! -e "$referenced_destination"

  local no_op_destination="$e2e_root/node-identity-edit-no-op-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity "$source_model" \
    --node N3 --new-node N3 --output-dir "$no_op_destination" \
    > "$e2e_root/node-identity-edit-no-op-rejected.stdout.json"; then
    echo "installed node identity edit accepted a no-op" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_no_change' \
    "$e2e_root/node-identity-edit-no-op-rejected.stdout.json"
  test ! -e "$no_op_destination"

  local invalid_destination="$e2e_root/node-identity-edit-invalid-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity "$source_model" \
    --node N3 --new-node 1_INVALID --output-dir "$invalid_destination" \
    > "$e2e_root/node-identity-edit-invalid-rejected.stdout.json"; then
    echo "installed node identity edit accepted an invalid stable identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_node_identity_replacement_invalid' \
    "$e2e_root/node-identity-edit-invalid-rejected.stdout.json"
  test ! -e "$invalid_destination"
}
exercise_node_identity_edit_surface
...
exercise_element_identity_edit_surface() {
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/element-identity-edit-$label"
    request_directory="$e2e_root/element-identity-edit-$label-request"
    direct_directory="$e2e_root/element-identity-edit-$label-direct"
    partial_directory="$e2e_root/element-identity-edit-$label-partial"
    resumed_directory="$e2e_root/element-identity-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-element-identity "$source_model" \
      --element E1 --new-element E1_RENAMED --output-dir "$edit_directory" \
      > "$e2e_root/element-identity-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"element_identity_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_element_id":"E1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_element_id":"E1_RENAMED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"element_index":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_element_without_identity":{' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"formulation":"euler_bernoulli_3d"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"node_ids":["N1","N2"]' "$edit_directory/edit-receipt.json"
    grep -Fq '"material_id":"M1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"section_id":"S1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"cpp_semantic_snapshot_verified":true' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"analysis_ready":true' "$edit_directory/edit-receipt.json"
    grep -Eq '"receipt_hash":"sha256:[0-9a-f]{64}"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"id":"E1_RENAMED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-element-identity.v1"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/element-identity-edit-$label-validation.json"
    grep -Fq '"entity_counts":{"nodes":2,"materials":1,"sections":1,"elements":1,"constraints":1' \
      "$e2e_root/element-identity-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case element-identity-edit-c5 --load-pattern LC_WEAK \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/element-identity-edit-$label-request.stdout.json"
    grep -Fq '"cpp_linear_assembly_preflight_verified":true' \
      "$request_directory/request-receipt.json"
    grep -Fq '"execution_started":false' "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/element-identity-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"active_dof_indices":[6,7,8,9,10,11]' \
      "$direct_directory/result-recovery-ir.json"
    grep -Fq '"active_external_load":[0,-10000,0,0,0,0]' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 > "$e2e_root/element-identity-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/element-identity-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/element-identity-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/element-identity-edit-first$suffix" \
      "$e2e_root/element-identity-edit-second$suffix" \
      > "$e2e_root/element-identity-edit-$diff_label-diff.txt"
    cmp "$e2e_root/element-identity-edit-first$suffix.stdout.json" \
      "$e2e_root/element-identity-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/element-identity-edit-first-validation.json" \
    "$e2e_root/element-identity-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed element identity edit mutated its source ModelIR" >&2
    exit 1
  fi

  local missing_destination="$e2e_root/element-identity-edit-missing-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-element-identity "$source_model" \
    --element E404 --new-element E3 --output-dir "$missing_destination" \
    > "$e2e_root/element-identity-edit-missing-rejected.stdout.json"; then
    echo "installed element identity edit accepted a missing element" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_element_identity_element_missing' \
    "$e2e_root/element-identity-edit-missing-rejected.stdout.json"
  test ! -e "$missing_destination"

  local collision_destination="$e2e_root/element-identity-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-element-identity "$e2e_root/frame3d-member-add-first/model-ir.json" \
    --element E1 --new-element E2 --output-dir "$collision_destination" \
    > "$e2e_root/element-identity-edit-collision-rejected.stdout.json"; then
    echo "installed element identity edit accepted a colliding identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_element_identity_replacement_exists' \
    "$e2e_root/element-identity-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"

  local no_op_destination="$e2e_root/element-identity-edit-no-op-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-element-identity "$source_model" \
    --element E1 --new-element E1 --output-dir "$no_op_destination" \
    > "$e2e_root/element-identity-edit-no-op-rejected.stdout.json"; then
    echo "installed element identity edit accepted a no-op" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_no_change' \
    "$e2e_root/element-identity-edit-no-op-rejected.stdout.json"
  test ! -e "$no_op_destination"

  local invalid_destination="$e2e_root/element-identity-edit-invalid-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-element-identity "$source_model" \
    --element E1 --new-element 1_INVALID --output-dir "$invalid_destination" \
    > "$e2e_root/element-identity-edit-invalid-rejected.stdout.json"; then
    echo "installed element identity edit accepted an invalid stable identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_element_identity_replacement_invalid' \
    "$e2e_root/element-identity-edit-invalid-rejected.stdout.json"
  test ! -e "$invalid_destination"
}
exercise_element_identity_edit_surface
...
exercise_linear_load_combination_identity_edit_surface() {
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/linear-load-combination-identity-edit-$label"
    request_directory="$e2e_root/linear-load-combination-identity-edit-$label-request"
    direct_directory="$e2e_root/linear-load-combination-identity-edit-$label-direct"
    partial_directory="$e2e_root/linear-load-combination-identity-edit-$label-partial"
    resumed_directory="$e2e_root/linear-load-combination-identity-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-linear-load-combination-identity "$source_model" \
      --load-combination COMBO_DIRECT --new-load-combination COMBO_RENAMED \
      --output-dir "$edit_directory" \
      > "$e2e_root/linear-load-combination-identity-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"linear_load_combination_identity_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_load_combination_id":"COMBO_DIRECT"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_load_combination_id":"COMBO_RENAMED"' \
      "$edit_directory/edit-receipt.json"
    ...
    grep -Fq '"retained_combination_without_identity":{' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"cpp_semantic_snapshot_verified":true' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"analysis_ready":true' "$edit_directory/edit-receipt.json"
    ...
    grep -Fq '"structural-native:model-edit-linear-load-combination-identity.v1"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/linear-load-combination-identity-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case linear-load-combination-identity-edit-c5 \
      --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/linear-load-combination-identity-edit-$label-request.stdout.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/linear-load-combination-identity-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/linear-load-combination-identity-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/linear-load-combination-identity-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/linear-load-combination-identity-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/linear-load-combination-identity-edit-first$suffix" \
      "$e2e_root/linear-load-combination-identity-edit-second$suffix" \
      > "$e2e_root/linear-load-combination-identity-edit-$diff_label-diff.txt"
    cmp "$e2e_root/linear-load-combination-identity-edit-first$suffix.stdout.json" \
      "$e2e_root/linear-load-combination-identity-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/linear-load-combination-identity-edit-first-validation.json" \
    "$e2e_root/linear-load-combination-identity-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed load-combination identity edit mutated its source ModelIR" >&2
    exit 1
  fi

  local missing_destination="$e2e_root/linear-load-combination-identity-edit-missing-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$source_model" \
    --load-combination COMBO_MISSING --new-load-combination COMBO_NEW \
    --output-dir "$missing_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-missing-rejected.stdout.json"; then
    echo "installed load-combination identity edit accepted a missing combination" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_combination_identity_combination_missing' \
    "$e2e_root/linear-load-combination-identity-edit-missing-rejected.stdout.json"
  test ! -e "$missing_destination"

  local collision_source="$e2e_root/linear-load-combination-identity-edit-collision-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-linear-load-combination "$source_model" \
    --load-combination COMBO_OTHER --term LC_AXIAL 1 --term LC_TORSION 1 \
    --output-dir "$collision_source" \
    > "$e2e_root/linear-load-combination-identity-edit-collision-source.stdout.json"
  local collision_destination="$e2e_root/linear-load-combination-identity-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$collision_source/model-ir.json" \
    --load-combination COMBO_DIRECT --new-load-combination COMBO_OTHER \
    --output-dir "$collision_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-collision-rejected.stdout.json"; then
    echo "installed load-combination identity edit accepted a colliding identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_combination_identity_replacement_exists' \
    "$e2e_root/linear-load-combination-identity-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"

  local downstream_source="$e2e_root/linear-load-combination-identity-edit-downstream-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-nested-linear-load-combination "$source_model" \
    --load-combination COMBO_DOWNSTREAM \
    --combination-term COMBO_DIRECT 1 --pattern-term LC_TORSION 1 \
    --output-dir "$downstream_source" \
    > "$e2e_root/linear-load-combination-identity-edit-downstream-source.stdout.json"
  local downstream_destination="$e2e_root/linear-load-combination-identity-edit-downstream-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$downstream_source/model-ir.json" \
    --load-combination COMBO_DIRECT --new-load-combination COMBO_RENAMED \
    --output-dir "$downstream_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-downstream-rejected.stdout.json"; then
    echo "installed load-combination identity edit cascaded a downstream reference" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_combination_identity_referenced_by_combination' \
    "$e2e_root/linear-load-combination-identity-edit-downstream-rejected.stdout.json"
  test ! -e "$downstream_destination"

  local no_op_destination="$e2e_root/linear-load-combination-identity-edit-no-op-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$source_model" \
    --load-combination COMBO_DIRECT --new-load-combination COMBO_DIRECT \
    --output-dir "$no_op_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-no-op-rejected.stdout.json"; then
    echo "installed load-combination identity edit accepted a no-op" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_no_change' \
    "$e2e_root/linear-load-combination-identity-edit-no-op-rejected.stdout.json"
  test ! -e "$no_op_destination"

  local invalid_destination="$e2e_root/linear-load-combination-identity-edit-invalid-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$source_model" \
    --load-combination COMBO_DIRECT --new-load-combination 1_INVALID \
    --output-dir "$invalid_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-invalid-rejected.stdout.json"; then
    echo "installed load-combination identity edit accepted an invalid stable identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_combination_identity_replacement_invalid' \
    "$e2e_root/linear-load-combination-identity-edit-invalid-rejected.stdout.json"
  test ! -e "$invalid_destination"

  local ambiguous_destination="$e2e_root/linear-load-combination-identity-edit-ambiguous-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-combination-identity "$source_model" \
    --load-combination COMBO_DIRECT --new-load-combination LC_WEAK \
    --output-dir "$ambiguous_destination" \
    > "$e2e_root/linear-load-combination-identity-edit-ambiguous-rejected.stdout.json"; then
    echo "installed load-combination identity edit accepted an ambiguous pattern identity" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_combination_identity_replacement_ambiguous' \
    "$e2e_root/linear-load-combination-identity-edit-ambiguous-rejected.stdout.json"
  test ! -e "$ambiguous_destination"
}
exercise_linear_load_combination_identity_edit_surface
...
exercise_node_identity_cascade_edit_surface() {
  local source_model="$e2e_root/model-identity-edit-first/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/node-identity-cascade-edit-$label"
    request_directory="$e2e_root/node-identity-cascade-edit-$label-request"
    direct_directory="$e2e_root/node-identity-cascade-edit-$label-direct"
    partial_directory="$e2e_root/node-identity-cascade-edit-$label-partial"
    resumed_directory="$e2e_root/node-identity-cascade-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-node-identity-cascade "$source_model" \
      --node N2 --new-node N2_LINKED \
      --output-dir "$edit_directory" \
      > "$e2e_root/node-identity-cascade-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"node_identity_cascade_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_node_id":"N2"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_node_id":"N2_LINKED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"node_index":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"element_reference_count":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"constraint_reference_count":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"nodal_load_reference_count":4' "$edit_directory/edit-receipt.json"
    ...
    grep -Fq '"node_ids":["N1","N2_LINKED"]' "$edit_directory/model-ir.json"
    test "$(grep -o '"node_id":"N2_LINKED"' "$edit_directory/model-ir.json" | wc -l)" -eq 4
    grep -Fq '"structural-native:model-edit-node-identity-cascade.v2"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/node-identity-cascade-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case node-identity-cascade-edit-c5 --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/node-identity-cascade-edit-$label-request.stdout.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$request_directory/request-receipt.json"
    grep -Fq '"load_combination_id":"COMBO_RENAMED"' \
      "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/node-identity-cascade-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$direct_directory/assembly-receipt.json"
    grep -Fq '"load_pattern_id":"COMBO_RENAMED"' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/node-identity-cascade-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/node-identity-cascade-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/node-identity-cascade-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/node-identity-cascade-edit-first$suffix" \
      "$e2e_root/node-identity-cascade-edit-second$suffix" \
      > "$e2e_root/node-identity-cascade-edit-$diff_label-diff.txt"
    cmp "$e2e_root/node-identity-cascade-edit-first$suffix.stdout.json" \
      "$e2e_root/node-identity-cascade-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/node-identity-cascade-edit-first-validation.json" \
    "$e2e_root/node-identity-cascade-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed node identity cascade edit mutated its source ModelIR" >&2
    exit 1
  fi
  ...
  while IFS='|' read -r name source_id replacement_id expected_code; do
    destination="$e2e_root/node-identity-cascade-edit-$name-rejected"
    if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-node-identity-cascade "$source_model" \
      --node "$source_id" --new-node "$replacement_id" \
      --output-dir "$destination" \
      > "$e2e_root/node-identity-cascade-edit-$name-rejected.stdout.json"; then
      echo "installed node identity cascade edit accepted $name" >&2
      exit 1
    fi
    grep -Fq "$expected_code" \
      "$e2e_root/node-identity-cascade-edit-$name-rejected.stdout.json"
    test ! -e "$destination"
  done <<'EOF'
missing|N404|N4|workbench_model_edit_node_identity_node_missing
collision|N2|N1|workbench_model_edit_node_identity_replacement_exists
no-op|N2|N2|workbench_model_edit_no_change
invalid|N2|1_INVALID|workbench_model_edit_node_identity_replacement_invalid
EOF

  local orphan_source="$e2e_root/node-identity-cascade-edit-orphan-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-node "$source_model" --node N3 --coordinates 4 1 0 \
    --output-dir "$orphan_source" \
    > "$e2e_root/node-identity-cascade-edit-orphan-source.stdout.json"
  local orphan_destination="$e2e_root/node-identity-cascade-edit-orphan-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity-cascade "$orphan_source/model-ir.json" \
    --node N3 --new-node N3_LINKED --output-dir "$orphan_destination" \
    > "$e2e_root/node-identity-cascade-edit-orphan-rejected.stdout.json"; then
    echo "installed node identity cascade edit accepted an unreferenced node" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_node_identity_cascade_unreferenced' \
    "$e2e_root/node-identity-cascade-edit-orphan-rejected.stdout.json"
  test ! -e "$orphan_destination"

  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-node-identity-cascade "$source_model" \
    --node N2 --new-node N2_LINKED \
    --output-dir "$e2e_root/node-identity-cascade-edit-first" \
    > "$e2e_root/node-identity-cascade-edit-existing-rejected.stdout.json"; then
    echo "installed node identity cascade edit replaced an existing destination" >&2
    exit 1
  fi
  grep -Fq 'workbench_stage_destination_exists' \
    "$e2e_root/node-identity-cascade-edit-existing-rejected.stdout.json"
}
exercise_node_identity_cascade_edit_surface

exercise_frame_section_identity_cascade_edit_surface() {
  local source_model="$e2e_root/node-identity-cascade-edit-first/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/frame-section-identity-cascade-edit-$label"
    request_directory="$e2e_root/frame-section-identity-cascade-edit-$label-request"
    direct_directory="$e2e_root/frame-section-identity-cascade-edit-$label-direct"
    partial_directory="$e2e_root/frame-section-identity-cascade-edit-$label-partial"
    resumed_directory="$e2e_root/frame-section-identity-cascade-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-frame-section-identity-cascade "$source_model" \
      --section S1 --new-section S1_LINKED \
      --output-dir "$edit_directory" \
      > "$e2e_root/frame-section-identity-cascade-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"frame_section_identity_cascade_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_section_id":"S1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_section_id":"S1_LINKED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"section_index":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"family_id":"frame_3d"' "$edit_directory/edit-receipt.json"
    grep -Fq '"parameter_set_version":"1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"element_reference_count":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"roundtrip_reference_count":0' "$edit_directory/edit-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case frame-section-identity-cascade-edit-c5 --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-request.stdout.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$request_directory/request-receipt.json"
    grep -Fq '"load_combination_id":"COMBO_RENAMED"' \
      "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$direct_directory/assembly-receipt.json"
    grep -Fq '"load_pattern_id":"COMBO_RENAMED"' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/frame-section-identity-cascade-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/frame-section-identity-cascade-edit-first$suffix" \
      "$e2e_root/frame-section-identity-cascade-edit-second$suffix" \
      > "$e2e_root/frame-section-identity-cascade-edit-$diff_label-diff.txt"
    cmp "$e2e_root/frame-section-identity-cascade-edit-first$suffix.stdout.json" \
      "$e2e_root/frame-section-identity-cascade-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/frame-section-identity-cascade-edit-first-validation.json" \
    "$e2e_root/frame-section-identity-cascade-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed frame-section identity cascade edit mutated its source ModelIR" >&2
    exit 1
  fi

  local name source_id replacement_id expected_code destination
  while IFS='|' read -r name source_id replacement_id expected_code; do
    destination="$e2e_root/frame-section-identity-cascade-edit-$name-rejected"
    if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-frame-section-identity-cascade "$source_model" \
      --section "$source_id" --new-section "$replacement_id" \
      --output-dir "$destination" \
      > "$e2e_root/frame-section-identity-cascade-edit-$name-rejected.stdout.json"; then
      echo "installed frame-section identity cascade edit accepted $name" >&2
      exit 1
    fi
    grep -Fq "$expected_code" \
      "$e2e_root/frame-section-identity-cascade-edit-$name-rejected.stdout.json"
    test ! -e "$destination"
  done <<'EOF'
missing|S404|S_NEW|workbench_model_edit_frame_section_identity_section_missing
no-op|S1|S1|workbench_model_edit_no_change
invalid|S1|1_INVALID|workbench_model_edit_frame_section_identity_replacement_invalid
EOF

  local orphan_source="$e2e_root/frame-section-identity-cascade-edit-orphan-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-frame-section "$source_model" --section S2 --area-m2 0.02 \
    --iy-m4 0.0002 --iz-m4 0.0003 --torsional-constant-m4 0.00004 \
    --shear-area-y-m2 0.015 --shear-area-z-m2 0.016 \
    --output-dir "$orphan_source" \
    > "$e2e_root/frame-section-identity-cascade-edit-orphan-source.stdout.json"
  local collision_destination="$e2e_root/frame-section-identity-cascade-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity-cascade "$orphan_source/model-ir.json" \
    --section S1 --new-section S2 --output-dir "$collision_destination" \
    > "$e2e_root/frame-section-identity-cascade-edit-collision-rejected.stdout.json"; then
    echo "installed frame-section identity cascade edit accepted an identity collision" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_frame_section_identity_replacement_exists' \
    "$e2e_root/frame-section-identity-cascade-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"
  local orphan_destination="$e2e_root/frame-section-identity-cascade-edit-orphan-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity-cascade "$orphan_source/model-ir.json" \
    --section S2 --new-section S2_LINKED --output-dir "$orphan_destination" \
    > "$e2e_root/frame-section-identity-cascade-edit-orphan-rejected.stdout.json"; then
    echo "installed frame-section identity cascade edit accepted an unreferenced section" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_frame_section_identity_cascade_unreferenced' \
    "$e2e_root/frame-section-identity-cascade-edit-orphan-rejected.stdout.json"
  test ! -e "$orphan_destination"

  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-frame-section-identity-cascade "$source_model" \
    --section S1 --new-section S1_LINKED \
    --output-dir "$e2e_root/frame-section-identity-cascade-edit-first" \
    > "$e2e_root/frame-section-identity-cascade-edit-existing-rejected.stdout.json"; then
    echo "installed frame-section identity cascade edit replaced an existing destination" >&2
    exit 1
  fi
  grep -Fq 'workbench_stage_destination_exists' \
    "$e2e_root/frame-section-identity-cascade-edit-existing-rejected.stdout.json"
}
exercise_frame_section_identity_cascade_edit_surface

exercise_linear_material_identity_cascade_edit_surface() {
  local source_model="$e2e_root/frame-section-identity-cascade-edit-first/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/linear-material-identity-cascade-edit-$label"
    request_directory="$e2e_root/linear-material-identity-cascade-edit-$label-request"
    direct_directory="$e2e_root/linear-material-identity-cascade-edit-$label-direct"
    partial_directory="$e2e_root/linear-material-identity-cascade-edit-$label-partial"
    resumed_directory="$e2e_root/linear-material-identity-cascade-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-linear-material-identity-cascade "$source_model" \
      --material M1 --new-material M1_LINKED \
      --output-dir "$edit_directory" \
      > "$e2e_root/linear-material-identity-cascade-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"linear_material_identity_cascade_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_material_id":"M1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_material_id":"M1_LINKED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"material_index":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"law_id":"linear_elastic_isotropic"' "$edit_directory/edit-receipt.json"
    grep -Fq '"parameter_set_version":"1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"element_reference_count":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"roundtrip_reference_count":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"typed_reference_cascade_verified":true' \
    ...
    grep -Fq '"material_id":"M1_LINKED"' "$edit_directory/model-ir.json"
    grep -Fq '"id":"S1_LINKED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"id":"N2_LINKED","index":1' "$edit_directory/model-ir.json"
    grep -Fq '"id":"COMBO_RENAMED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-linear-material-identity-cascade.v2"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case linear-material-identity-cascade-edit-c5 --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-request.stdout.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$request_directory/request-receipt.json"
    grep -Fq '"load_combination_id":"COMBO_RENAMED"' \
      "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$direct_directory/assembly-receipt.json"
    grep -Fq '"load_pattern_id":"COMBO_RENAMED"' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/linear-material-identity-cascade-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/linear-material-identity-cascade-edit-first$suffix" \
      "$e2e_root/linear-material-identity-cascade-edit-second$suffix" \
      > "$e2e_root/linear-material-identity-cascade-edit-$diff_label-diff.txt"
    cmp "$e2e_root/linear-material-identity-cascade-edit-first$suffix.stdout.json" \
      "$e2e_root/linear-material-identity-cascade-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/linear-material-identity-cascade-edit-first-validation.json" \
    "$e2e_root/linear-material-identity-cascade-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed linear-material identity cascade edit mutated its source ModelIR" >&2
    exit 1
  fi

  local name source_id replacement_id expected_code destination
  while IFS='|' read -r name source_id replacement_id expected_code; do
    destination="$e2e_root/linear-material-identity-cascade-edit-$name-rejected"
    if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-linear-material-identity-cascade "$source_model" \
      --material "$source_id" --new-material "$replacement_id" \
      --output-dir "$destination" \
      > "$e2e_root/linear-material-identity-cascade-edit-$name-rejected.stdout.json"; then
      echo "installed linear-material identity cascade edit accepted $name" >&2
      exit 1
    fi
    grep -Fq "$expected_code" \
      "$e2e_root/linear-material-identity-cascade-edit-$name-rejected.stdout.json"
    test ! -e "$destination"
  done <<'EOF'
missing|M404|M_NEW|workbench_model_edit_linear_material_identity_material_missing
no-op|M1|M1|workbench_model_edit_no_change
invalid|M1|1_INVALID|workbench_model_edit_linear_material_identity_replacement_invalid
EOF

  local orphan_source="$e2e_root/linear-material-identity-cascade-edit-orphan-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-linear-material "$source_model" --material M2 \
    --elastic-modulus-pa 70000000000 --poisson-ratio 0.33 --density-kg-m3 2700 \
    --output-dir "$orphan_source" \
    > "$e2e_root/linear-material-identity-cascade-edit-orphan-source.stdout.json"
  local collision_destination="$e2e_root/linear-material-identity-cascade-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-material-identity-cascade "$orphan_source/model-ir.json" \
    --material M1 --new-material M2 --output-dir "$collision_destination" \
    > "$e2e_root/linear-material-identity-cascade-edit-collision-rejected.stdout.json"; then
    echo "installed linear-material identity cascade edit accepted an identity collision" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_material_identity_replacement_exists' \
    "$e2e_root/linear-material-identity-cascade-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"
  local orphan_destination="$e2e_root/linear-material-identity-cascade-edit-orphan-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-material-identity-cascade "$orphan_source/model-ir.json" \
    --material M2 --new-material M2_LINKED --output-dir "$orphan_destination" \
    > "$e2e_root/linear-material-identity-cascade-edit-orphan-rejected.stdout.json"; then
    echo "installed linear-material identity cascade edit accepted an unreferenced material" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_material_identity_cascade_unreferenced' \
    "$e2e_root/linear-material-identity-cascade-edit-orphan-rejected.stdout.json"
  test ! -e "$orphan_destination"

  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-material-identity-cascade "$source_model" \
    --material M1 --new-material M1_LINKED \
    --output-dir "$e2e_root/linear-material-identity-cascade-edit-first" \
    > "$e2e_root/linear-material-identity-cascade-edit-existing-rejected.stdout.json"; then
    echo "installed linear-material identity cascade edit replaced an existing destination" >&2
    exit 1
  fi
  grep -Fq 'workbench_stage_destination_exists' \
    "$e2e_root/linear-material-identity-cascade-edit-existing-rejected.stdout.json"
}
exercise_linear_material_identity_cascade_edit_surface

exercise_truss_section_identity_cascade_edit_surface() {
  local base_model="$e2e_root/linear-material-identity-cascade-edit-first/model-ir.json"
  local section_source="$e2e_root/truss-section-identity-cascade-setup-section"
  local member_source="$e2e_root/truss-section-identity-cascade-setup-member"
  local fixed_source="$e2e_root/truss-section-identity-cascade-setup-fixed"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-truss-section "$base_model" --section T1 --area-m2 0.005 \
    --output-dir "$section_source" \
    > "$e2e_root/truss-section-identity-cascade-setup-section.stdout.json"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-truss3d-member "$section_source/model-ir.json" \
    --node N3 --coordinates 2 1 0 --element E2 --from-node N2_LINKED \
    --material M1_LINKED --section T1 --output-dir "$member_source" \
    > "$e2e_root/truss-section-identity-cascade-setup-member.stdout.json"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-fixed-constraint "$member_source/model-ir.json" \
    --constraint BC_N3 --node N3 --output-dir "$fixed_source" \
    > "$e2e_root/truss-section-identity-cascade-setup-fixed.stdout.json"
  local source_model="$fixed_source/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/truss-section-identity-cascade-edit-$label"
    request_directory="$e2e_root/truss-section-identity-cascade-edit-$label-request"
    direct_directory="$e2e_root/truss-section-identity-cascade-edit-$label-direct"
    partial_directory="$e2e_root/truss-section-identity-cascade-edit-$label-partial"
    resumed_directory="$e2e_root/truss-section-identity-cascade-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-truss-section-identity-cascade "$source_model" \
      --section T1 --new-section T1_LINKED \
      --output-dir "$edit_directory" \
      > "$e2e_root/truss-section-identity-cascade-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"truss_section_identity_cascade_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_section_id":"T1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_section_id":"T1_LINKED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"section_index":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"family_id":"truss_3d"' "$edit_directory/edit-receipt.json"
    grep -Fq '"parameter_set_version":"1"' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_parameters_si":{"area_m2":0.005}' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"element_reference_count":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"roundtrip_reference_count":0' "$edit_directory/edit-receipt.json"
    grep -Fq '"typed_reference_cascade_verified":true' \
    ...
    grep -Fq '"section_id":"T1_LINKED"' "$edit_directory/model-ir.json"
    grep -Fq '"id":"M1_LINKED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"id":"S1_LINKED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"id":"N2_LINKED","index":1' "$edit_directory/model-ir.json"
    grep -Fq '"id":"COMBO_RENAMED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-truss-section-identity-cascade.v2"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case truss-section-identity-cascade-edit-c5 --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-request.stdout.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$request_directory/request-receipt.json"
    grep -Fq '"load_combination_id":"COMBO_RENAMED"' \
      "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$direct_directory/assembly-receipt.json"
    grep -Fq '"load_pattern_id":"COMBO_RENAMED"' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/truss-section-identity-cascade-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/truss-section-identity-cascade-edit-first$suffix" \
      "$e2e_root/truss-section-identity-cascade-edit-second$suffix" \
      > "$e2e_root/truss-section-identity-cascade-edit-$diff_label-diff.txt"
    cmp "$e2e_root/truss-section-identity-cascade-edit-first$suffix.stdout.json" \
      "$e2e_root/truss-section-identity-cascade-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/truss-section-identity-cascade-edit-first-validation.json" \
    "$e2e_root/truss-section-identity-cascade-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed truss-section identity cascade edit mutated its source ModelIR" >&2
    exit 1
  fi

  local name source_id replacement_id expected_code destination
  while IFS='|' read -r name source_id replacement_id expected_code; do
    destination="$e2e_root/truss-section-identity-cascade-edit-$name-rejected"
    if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-truss-section-identity-cascade "$source_model" \
      --section "$source_id" --new-section "$replacement_id" \
      --output-dir "$destination" \
      > "$e2e_root/truss-section-identity-cascade-edit-$name-rejected.stdout.json"; then
      echo "installed truss-section identity cascade edit accepted $name" >&2
      exit 1
    fi
    grep -Fq "$expected_code" \
      "$e2e_root/truss-section-identity-cascade-edit-$name-rejected.stdout.json"
    test ! -e "$destination"
  done <<'EOF'
missing|T404|T_NEW|workbench_model_edit_truss_section_identity_section_missing
no-op|T1|T1|workbench_model_edit_no_change
invalid|T1|1_INVALID|workbench_model_edit_truss_section_identity_replacement_invalid
EOF

  local orphan_source="$e2e_root/truss-section-identity-cascade-edit-orphan-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-truss-section "$source_model" --section T2 --area-m2 0.0025 \
    --output-dir "$orphan_source" \
    > "$e2e_root/truss-section-identity-cascade-edit-orphan-source.stdout.json"
  local collision_destination="$e2e_root/truss-section-identity-cascade-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity-cascade "$orphan_source/model-ir.json" \
    --section T1 --new-section T2 --output-dir "$collision_destination" \
    > "$e2e_root/truss-section-identity-cascade-edit-collision-rejected.stdout.json"; then
    echo "installed truss-section identity cascade edit accepted an identity collision" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_replacement_exists' \
    "$e2e_root/truss-section-identity-cascade-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"
  local orphan_destination="$e2e_root/truss-section-identity-cascade-edit-orphan-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity-cascade "$orphan_source/model-ir.json" \
    --section T2 --new-section T2_LINKED --output-dir "$orphan_destination" \
    > "$e2e_root/truss-section-identity-cascade-edit-orphan-rejected.stdout.json"; then
    echo "installed truss-section identity cascade edit accepted an unreferenced section" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_truss_section_identity_cascade_unreferenced' \
    "$e2e_root/truss-section-identity-cascade-edit-orphan-rejected.stdout.json"
  test ! -e "$orphan_destination"

  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-truss-section-identity-cascade "$source_model" \
    --section T1 --new-section T1_LINKED \
    --output-dir "$e2e_root/truss-section-identity-cascade-edit-first" \
    > "$e2e_root/truss-section-identity-cascade-edit-existing-rejected.stdout.json"; then
    echo "installed truss-section identity cascade edit replaced an existing destination" >&2
    exit 1
  fi
  grep -Fq 'workbench_stage_destination_exists' \
    "$e2e_root/truss-section-identity-cascade-edit-existing-rejected.stdout.json"
}
exercise_truss_section_identity_cascade_edit_surface

exercise_linear_load_pattern_identity_cascade_edit_surface() {
  local source_model="$e2e_root/truss-section-identity-cascade-edit-first/model-ir.json"
  local source_before_hash
  source_before_hash="$(sha256sum "$source_model" | awk '{print $1}')"
  ...
  local resumed_directory
  for label in first second; do
    edit_directory="$e2e_root/linear-load-pattern-identity-cascade-edit-$label"
    request_directory="$e2e_root/linear-load-pattern-identity-cascade-edit-$label-request"
    direct_directory="$e2e_root/linear-load-pattern-identity-cascade-edit-$label-direct"
    partial_directory="$e2e_root/linear-load-pattern-identity-cascade-edit-$label-partial"
    resumed_directory="$e2e_root/linear-load-pattern-identity-cascade-edit-$label-resumed"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-linear-load-pattern-identity-cascade "$source_model" \
      --load-pattern LC_WEAK --new-load-pattern LC_WEAK_LINKED \
      --output-dir "$edit_directory" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label.stdout.json"
    grep -Fq '"schema_version":"structural-native-model-edit-receipt.v1"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"operation":"linear_load_pattern_identity_cascade_edit"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"source_load_pattern_id":"LC_WEAK"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"replacement_load_pattern_id":"LC_WEAK_LINKED"' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"load_pattern_index":1' "$edit_directory/edit-receipt.json"
    grep -Fq '"analysis_type":"linear_static"' "$edit_directory/edit-receipt.json"
    grep -Fq '"retained_self_weight":[0,0,0]' "$edit_directory/edit-receipt.json"
    grep -Fq '"load_combination_reference_count":1' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"construction_stage_reference_count":0' \
      "$edit_directory/edit-receipt.json"
    grep -Fq '"roundtrip_reference_count":0' "$edit_directory/edit-receipt.json"
    ...
    grep -Fq '"terms":[{"factor":0.25,"ref_id":"LC_AXIAL","ref_kind":"load_pattern"},{"factor":1.2,"ref_id":"LC_WEAK_LINKED","ref_kind":"load_pattern"},{"factor":-0.5,"ref_id":"LC_STRONG","ref_kind":"load_pattern"}]' \
      "$edit_directory/model-ir.json"
    grep -Fq '"id":"T1_LINKED","index":1' "$edit_directory/model-ir.json"
    grep -Fq '"id":"M1_LINKED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"id":"S1_LINKED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"id":"N2_LINKED","index":1' "$edit_directory/model-ir.json"
    grep -Fq '"id":"COMBO_RENAMED","index":0' "$edit_directory/model-ir.json"
    grep -Fq '"structural-native:model-edit-linear-load-pattern-identity-cascade.v2"' \
      "$edit_directory/model-ir.json"
    env -i PATH="$empty_path" "$active/bin/structural-cli" model validate \
      "$edit_directory/model-ir.json" --require-analysis-ready \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-validation.json"

    env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-create-linear-analysis-request "$edit_directory/model-ir.json" \
      --case linear-load-pattern-identity-cascade-edit-c5 \
      --load-combination COMBO_RENAMED \
      --max-iterations 100 --absolute-residual-tolerance 1e-11 \
      --relative-residual-tolerance 1e-13 --maximum-increment 0 \
      --output-dir "$request_directory" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-request.stdout.json"
    grep -Fq '"model_id":"engine-v2-frame-cantilever-renamed"' \
      "$request_directory/request-receipt.json"
    grep -Fq '"load_combination_id":"COMBO_RENAMED"' \
      "$request_directory/request-receipt.json"
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-run "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" --output-dir "$direct_directory" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-direct.stdout.json"
    grep -Fq '"status":"completed"' "$direct_directory/run-receipt.json"
    grep -Fq '"load_pattern_id":"COMBO_RENAMED"' \
      "$direct_directory/result-recovery-ir.json"
    grep -Fq '"active_dof_indices":[6,7,8,9,10,11]' \
    ...
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      ...
      "$request_directory/analysis-request.json" --output-dir "$partial_directory" \
      --iteration-budget 0 \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-partial.stdout.json"
    grep -Fq '"status":"active"' "$partial_directory/run-receipt.json"
    test -s "$partial_directory/checkpoint.mlpcp"
    env -i PATH="$empty_path" "$active/bin/structural-cli" analysis \
      model-linear-resume "$edit_directory/model-ir.json" \
      "$request_directory/analysis-request.json" "$partial_directory/checkpoint.mlpcp" \
      --output-dir "$resumed_directory" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-resumed.stdout.json"
    diff -r "$direct_directory" "$resumed_directory" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$label-restart-diff.txt"
  done
  ...
  for suffix in '' -request -direct -partial -resumed; do
    ...
    diff -r "$e2e_root/linear-load-pattern-identity-cascade-edit-first$suffix" \
      "$e2e_root/linear-load-pattern-identity-cascade-edit-second$suffix" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$diff_label-diff.txt"
    cmp "$e2e_root/linear-load-pattern-identity-cascade-edit-first$suffix.stdout.json" \
      "$e2e_root/linear-load-pattern-identity-cascade-edit-second$suffix.stdout.json"
  done
  cmp "$e2e_root/linear-load-pattern-identity-cascade-edit-first-validation.json" \
    "$e2e_root/linear-load-pattern-identity-cascade-edit-second-validation.json"
  if [[ "$(sha256sum "$source_model" | awk '{print $1}')" != "$source_before_hash" ]]; then
    echo "installed load-pattern identity cascade edit mutated its source ModelIR" >&2
    exit 1
  fi

  local name source_id replacement_id expected_code destination
  while IFS='|' read -r name source_id replacement_id expected_code; do
    destination="$e2e_root/linear-load-pattern-identity-cascade-edit-$name-rejected"
    if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
      model-edit-linear-load-pattern-identity-cascade "$source_model" \
      --load-pattern "$source_id" --new-load-pattern "$replacement_id" \
      --output-dir "$destination" \
      > "$e2e_root/linear-load-pattern-identity-cascade-edit-$name-rejected.stdout.json"; then
      echo "installed load-pattern identity cascade edit accepted $name" >&2
      exit 1
    fi
    grep -Fq "$expected_code" \
      "$e2e_root/linear-load-pattern-identity-cascade-edit-$name-rejected.stdout.json"
    test ! -e "$destination"
  done <<'EOF'
missing|LC404|LC_NEW|workbench_model_edit_linear_load_pattern_identity_pattern_missing
no-op|LC_WEAK|LC_WEAK|workbench_model_edit_no_change
invalid|LC_WEAK|1_INVALID|workbench_model_edit_linear_load_pattern_identity_replacement_invalid
EOF

  local orphan_source="$e2e_root/linear-load-pattern-identity-cascade-edit-orphan-source"
  env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-add-linear-load-pattern "$source_model" --load-pattern LC_ORPHAN \
    --load L_ORPHAN_N2 --node N2_LINKED --components 1 0 0 0 0 0 \
    --output-dir "$orphan_source" \
    > "$e2e_root/linear-load-pattern-identity-cascade-edit-orphan-source.stdout.json"
  local collision_destination="$e2e_root/linear-load-pattern-identity-cascade-edit-collision-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-pattern-identity-cascade "$orphan_source/model-ir.json" \
    --load-pattern LC_WEAK --new-load-pattern LC_ORPHAN \
    --output-dir "$collision_destination" \
    > "$e2e_root/linear-load-pattern-identity-cascade-edit-collision-rejected.stdout.json"; then
    echo "installed load-pattern identity cascade edit accepted an identity collision" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_pattern_identity_replacement_exists' \
    "$e2e_root/linear-load-pattern-identity-cascade-edit-collision-rejected.stdout.json"
  test ! -e "$collision_destination"
  local orphan_destination="$e2e_root/linear-load-pattern-identity-cascade-edit-orphan-rejected"
  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-pattern-identity-cascade "$orphan_source/model-ir.json" \
    --load-pattern LC_ORPHAN --new-load-pattern LC_ORPHAN_LINKED \
    --output-dir "$orphan_destination" \
    > "$e2e_root/linear-load-pattern-identity-cascade-edit-orphan-rejected.stdout.json"; then
    echo "installed load-pattern identity cascade edit accepted an unreferenced pattern" >&2
    exit 1
  fi
  grep -Fq 'workbench_model_edit_linear_load_pattern_identity_cascade_unreferenced' \
    "$e2e_root/linear-load-pattern-identity-cascade-edit-orphan-rejected.stdout.json"
  test ! -e "$orphan_destination"

  if env -i PATH="$empty_path" "$active/bin/structural-workbench" \
    model-edit-linear-load-pattern-identity-cascade "$source_model" \
    --load-pattern LC_WEAK --new-load-pattern LC_WEAK_LINKED \
    --output-dir "$e2e_root/linear-load-pattern-identity-cascade-edit-first" \
    > "$e2e_root/linear-load-pattern-identity-cascade-edit-existing-rejected.stdout.json"; then
    echo "installed load-pattern identity cascade edit replaced an existing destination" >&2
    exit 1
  fi
  grep -Fq 'workbench_stage_destination_exists' \
    "$e2e_root/linear-load-pattern-identity-cascade-edit-existing-rejected.stdout.json"
}
exercise_linear_load_pattern_identity_cascade_edit_surface
...
linear_reaction_hash="$(sha256sum "$linear_direct/04-resume/reaction-result-ir.json" | awk '{print $1}')"
...
release_linear_model_hash="$(sha256sum "$release_linear_model" | awk '{print $1}')"
release_linear_request_hash="$(sha256sum "$release_linear_request_directory/analysis-request.json" | awk '{print $1}')"
release_linear_result_hash="$(sha256sum "$release_linear_direct/result-ir.json" | awk '{print $1}')"
release_linear_recovery_hash="$(sha256sum "$release_linear_direct/result-recovery-ir.json" | awk '{print $1}')"
release_linear_checkpoint_hash="$(sha256sum "$release_linear_direct/checkpoint.mlpcp" | awk '{print $1}')"
self_weight_linear_model_hash="$(sha256sum "$self_weight_linear_model" | awk '{print $1}')"
self_weight_linear_request_hash="$(sha256sum "$self_weight_linear_request_directory/analysis-request.json" | awk '{print $1}')"
self_weight_linear_result_hash="$(sha256sum "$self_weight_linear_direct/result-ir.json" | awk '{print $1}')"
...
mgt_linear_element_recovery_view_en_us_hash="$(sha256sum "$e2e_root/mgt-model-ir-linear-element-recovery-view-en-US-first.txt" | awk '{print $1}')"
mgt_linear_element_recovery_view_ko_kr_hash="$(sha256sum "$e2e_root/mgt-model-ir-linear-element-recovery-view-ko-KR-first.txt" | awk '{print $1}')"
...
frame_section_identity_edit_model_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first/model-ir.json" | awk '{print $1}')"
frame_section_identity_edit_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first/edit-receipt.json" | awk '{print $1}')"
frame_section_identity_edit_request_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-request/request-receipt.json" | awk '{print $1}')"
frame_section_identity_edit_request_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-request/analysis-request.json" | awk '{print $1}')"
frame_section_identity_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
frame_section_identity_edit_checkpoint_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
frame_section_identity_edit_result_ir_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-direct/result-ir.json" | awk '{print $1}')"
frame_section_identity_edit_recovery_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
frame_section_identity_edit_report_ir_hash="$(sha256sum "$e2e_root/frame-section-identity-edit-first-direct/report-ir.json" | awk '{print $1}')"
truss_section_identity_edit_model_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first/model-ir.json" | awk '{print $1}')"
truss_section_identity_edit_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first/edit-receipt.json" | awk '{print $1}')"
truss_section_identity_edit_request_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-request/request-receipt.json" | awk '{print $1}')"
truss_section_identity_edit_request_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-request/analysis-request.json" | awk '{print $1}')"
truss_section_identity_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
truss_section_identity_edit_checkpoint_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
truss_section_identity_edit_result_ir_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-direct/result-ir.json" | awk '{print $1}')"
truss_section_identity_edit_recovery_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
truss_section_identity_edit_report_ir_hash="$(sha256sum "$e2e_root/truss-section-identity-edit-first-direct/report-ir.json" | awk '{print $1}')"
node_identity_edit_model_hash="$(sha256sum "$e2e_root/node-identity-edit-first/model-ir.json" | awk '{print $1}')"
node_identity_edit_receipt_hash="$(sha256sum "$e2e_root/node-identity-edit-first/edit-receipt.json" | awk '{print $1}')"
node_identity_edit_request_receipt_hash="$(sha256sum "$e2e_root/node-identity-edit-first-request/request-receipt.json" | awk '{print $1}')"
node_identity_edit_request_hash="$(sha256sum "$e2e_root/node-identity-edit-first-request/analysis-request.json" | awk '{print $1}')"
node_identity_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/node-identity-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
node_identity_edit_checkpoint_hash="$(sha256sum "$e2e_root/node-identity-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
node_identity_edit_result_ir_hash="$(sha256sum "$e2e_root/node-identity-edit-first-direct/result-ir.json" | awk '{print $1}')"
node_identity_edit_recovery_hash="$(sha256sum "$e2e_root/node-identity-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
node_identity_edit_report_ir_hash="$(sha256sum "$e2e_root/node-identity-edit-first-direct/report-ir.json" | awk '{print $1}')"
element_identity_edit_model_hash="$(sha256sum "$e2e_root/element-identity-edit-first/model-ir.json" | awk '{print $1}')"
element_identity_edit_receipt_hash="$(sha256sum "$e2e_root/element-identity-edit-first/edit-receipt.json" | awk '{print $1}')"
element_identity_edit_request_receipt_hash="$(sha256sum "$e2e_root/element-identity-edit-first-request/request-receipt.json" | awk '{print $1}')"
element_identity_edit_request_hash="$(sha256sum "$e2e_root/element-identity-edit-first-request/analysis-request.json" | awk '{print $1}')"
element_identity_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/element-identity-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
element_identity_edit_checkpoint_hash="$(sha256sum "$e2e_root/element-identity-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
element_identity_edit_result_ir_hash="$(sha256sum "$e2e_root/element-identity-edit-first-direct/result-ir.json" | awk '{print $1}')"
element_identity_edit_recovery_hash="$(sha256sum "$e2e_root/element-identity-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
element_identity_edit_report_ir_hash="$(sha256sum "$e2e_root/element-identity-edit-first-direct/report-ir.json" | awk '{print $1}')"
linear_load_combination_identity_edit_model_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first/model-ir.json" | awk '{print $1}')"
linear_load_combination_identity_edit_receipt_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first/edit-receipt.json" | awk '{print $1}')"
linear_load_combination_identity_edit_request_receipt_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first-request/request-receipt.json" | awk '{print $1}')"
linear_load_combination_identity_edit_request_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first-request/analysis-request.json" | awk '{print $1}')"
linear_load_combination_identity_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
linear_load_combination_identity_edit_checkpoint_hash="$(sha256sum "$e2e_root/linear-load-combination-identity-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
...
node_identity_cascade_edit_model_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first/model-ir.json" | awk '{print $1}')"
node_identity_cascade_edit_receipt_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first/edit-receipt.json" | awk '{print $1}')"
node_identity_cascade_edit_request_receipt_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-request/request-receipt.json" | awk '{print $1}')"
node_identity_cascade_edit_request_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-request/analysis-request.json" | awk '{print $1}')"
node_identity_cascade_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
node_identity_cascade_edit_checkpoint_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
node_identity_cascade_edit_result_ir_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-direct/result-ir.json" | awk '{print $1}')"
node_identity_cascade_edit_recovery_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
node_identity_cascade_edit_report_ir_hash="$(sha256sum "$e2e_root/node-identity-cascade-edit-first-direct/report-ir.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_model_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first/model-ir.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first/edit-receipt.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_request_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-request/request-receipt.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_request_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-request/analysis-request.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_checkpoint_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
frame_section_identity_cascade_edit_result_ir_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-direct/result-ir.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_recovery_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
frame_section_identity_cascade_edit_report_ir_hash="$(sha256sum "$e2e_root/frame-section-identity-cascade-edit-first-direct/report-ir.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_model_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first/model-ir.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_receipt_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first/edit-receipt.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_request_receipt_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-request/request-receipt.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_request_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-request/analysis-request.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_checkpoint_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
linear_material_identity_cascade_edit_result_ir_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-direct/result-ir.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_recovery_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
linear_material_identity_cascade_edit_report_ir_hash="$(sha256sum "$e2e_root/linear-material-identity-cascade-edit-first-direct/report-ir.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_model_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first/model-ir.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first/edit-receipt.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_request_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-request/request-receipt.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_request_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-request/analysis-request.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_checkpoint_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
truss_section_identity_cascade_edit_result_ir_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-direct/result-ir.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_recovery_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-direct/result-recovery-ir.json" | awk '{print $1}')"
truss_section_identity_cascade_edit_report_ir_hash="$(sha256sum "$e2e_root/truss-section-identity-cascade-edit-first-direct/report-ir.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_model_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first/model-ir.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_receipt_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first/edit-receipt.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_request_receipt_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first-request/request-receipt.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_request_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first-request/analysis-request.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_assembly_receipt_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first-direct/assembly-receipt.json" | awk '{print $1}')"
linear_load_pattern_identity_cascade_edit_checkpoint_hash="$(sha256sum "$e2e_root/linear-load-pattern-identity-cascade-edit-first-direct/checkpoint.mlpcp" | awk '{print $1}')"
...
v70_receipt_json="${v69_receipt_json/structural-native-distribution-e2e.v69/structural-native-distribution-e2e.v70}"
frame_section_identity_edit_receipt_fields="\"workbench_frame_section_identity_edit_surface_passed\":true,\"workbench_frame_section_identity_edit_model_sha256\":\"sha256:$frame_section_identity_edit_model_hash\",\"workbench_frame_section_identity_edit_receipt_sha256\":\"sha256:$frame_section_identity_edit_receipt_hash\",\"workbench_frame_section_identity_edit_request_receipt_sha256\":\"sha256:$frame_section_identity_edit_request_receipt_hash\",\"workbench_frame_section_identity_edit_request_sha256\":\"sha256:$frame_section_identity_edit_request_hash\",\"workbench_frame_section_identity_edit_assembly_receipt_sha256\":\"sha256:$frame_section_identity_edit_assembly_receipt_hash\",\"workbench_frame_section_identity_edit_checkpoint_sha256\":\"sha256:$frame_section_identity_edit_checkpoint_hash\",\"workbench_frame_section_identity_edit_result_ir_sha256\":\"sha256:$frame_section_identity_edit_result_ir_hash\",\"workbench_frame_section_identity_edit_recovery_sha256\":\"sha256:$frame_section_identity_edit_recovery_hash\",\"workbench_frame_section_identity_edit_report_ir_sha256\":\"sha256:$frame_section_identity_edit_report_ir_hash\",\"workbench_frame_section_identity_edit_restart_passed\":true,"
v70_receipt_json="${v70_receipt_json/\"workbench_result_view_surface_passed\":true,/${frame_section_identity_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v70_receipt_json" > "$temporary_receipt"
v71_receipt_json="${v70_receipt_json/structural-native-distribution-e2e.v70/structural-native-distribution-e2e.v71}"
truss_section_identity_edit_receipt_fields="\"workbench_truss_section_identity_edit_surface_passed\":true,\"workbench_truss_section_identity_edit_model_sha256\":\"sha256:$truss_section_identity_edit_model_hash\",\"workbench_truss_section_identity_edit_receipt_sha256\":\"sha256:$truss_section_identity_edit_receipt_hash\",\"workbench_truss_section_identity_edit_request_receipt_sha256\":\"sha256:$truss_section_identity_edit_request_receipt_hash\",\"workbench_truss_section_identity_edit_request_sha256\":\"sha256:$truss_section_identity_edit_request_hash\",\"workbench_truss_section_identity_edit_assembly_receipt_sha256\":\"sha256:$truss_section_identity_edit_assembly_receipt_hash\",\"workbench_truss_section_identity_edit_checkpoint_sha256\":\"sha256:$truss_section_identity_edit_checkpoint_hash\",\"workbench_truss_section_identity_edit_result_ir_sha256\":\"sha256:$truss_section_identity_edit_result_ir_hash\",\"workbench_truss_section_identity_edit_recovery_sha256\":\"sha256:$truss_section_identity_edit_recovery_hash\",\"workbench_truss_section_identity_edit_report_ir_sha256\":\"sha256:$truss_section_identity_edit_report_ir_hash\",\"workbench_truss_section_identity_edit_restart_passed\":true,"
v71_receipt_json="${v71_receipt_json/\"workbench_result_view_surface_passed\":true,/${truss_section_identity_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v71_receipt_json" > "$temporary_receipt"
v72_receipt_json="${v71_receipt_json/structural-native-distribution-e2e.v71/structural-native-distribution-e2e.v72}"
node_identity_edit_receipt_fields="\"workbench_node_identity_edit_surface_passed\":true,\"workbench_node_identity_edit_model_sha256\":\"sha256:$node_identity_edit_model_hash\",\"workbench_node_identity_edit_receipt_sha256\":\"sha256:$node_identity_edit_receipt_hash\",\"workbench_node_identity_edit_request_receipt_sha256\":\"sha256:$node_identity_edit_request_receipt_hash\",\"workbench_node_identity_edit_request_sha256\":\"sha256:$node_identity_edit_request_hash\",\"workbench_node_identity_edit_assembly_receipt_sha256\":\"sha256:$node_identity_edit_assembly_receipt_hash\",\"workbench_node_identity_edit_checkpoint_sha256\":\"sha256:$node_identity_edit_checkpoint_hash\",\"workbench_node_identity_edit_result_ir_sha256\":\"sha256:$node_identity_edit_result_ir_hash\",\"workbench_node_identity_edit_recovery_sha256\":\"sha256:$node_identity_edit_recovery_hash\",\"workbench_node_identity_edit_report_ir_sha256\":\"sha256:$node_identity_edit_report_ir_hash\",\"workbench_node_identity_edit_restart_passed\":true,"
v72_receipt_json="${v72_receipt_json/\"workbench_result_view_surface_passed\":true,/${node_identity_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v72_receipt_json" > "$temporary_receipt"
v73_receipt_json="${v72_receipt_json/structural-native-distribution-e2e.v72/structural-native-distribution-e2e.v73}"
element_identity_edit_receipt_fields="\"workbench_element_identity_edit_surface_passed\":true,\"workbench_element_identity_edit_model_sha256\":\"sha256:$element_identity_edit_model_hash\",\"workbench_element_identity_edit_receipt_sha256\":\"sha256:$element_identity_edit_receipt_hash\",\"workbench_element_identity_edit_request_receipt_sha256\":\"sha256:$element_identity_edit_request_receipt_hash\",\"workbench_element_identity_edit_request_sha256\":\"sha256:$element_identity_edit_request_hash\",\"workbench_element_identity_edit_assembly_receipt_sha256\":\"sha256:$element_identity_edit_assembly_receipt_hash\",\"workbench_element_identity_edit_checkpoint_sha256\":\"sha256:$element_identity_edit_checkpoint_hash\",\"workbench_element_identity_edit_result_ir_sha256\":\"sha256:$element_identity_edit_result_ir_hash\",\"workbench_element_identity_edit_recovery_sha256\":\"sha256:$element_identity_edit_recovery_hash\",\"workbench_element_identity_edit_report_ir_sha256\":\"sha256:$element_identity_edit_report_ir_hash\",\"workbench_element_identity_edit_restart_passed\":true,"
v73_receipt_json="${v73_receipt_json/\"workbench_result_view_surface_passed\":true,/${element_identity_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v73_receipt_json" > "$temporary_receipt"
v74_receipt_json="${v73_receipt_json/structural-native-distribution-e2e.v73/structural-native-distribution-e2e.v74}"
...
v76_receipt_json="${v75_receipt_json/structural-native-distribution-e2e.v75/structural-native-distribution-e2e.v76}"
node_identity_cascade_edit_receipt_fields="\"workbench_node_identity_cascade_edit_surface_passed\":true,\"workbench_node_identity_cascade_edit_model_sha256\":\"sha256:$node_identity_cascade_edit_model_hash\",\"workbench_node_identity_cascade_edit_receipt_sha256\":\"sha256:$node_identity_cascade_edit_receipt_hash\",\"workbench_node_identity_cascade_edit_request_receipt_sha256\":\"sha256:$node_identity_cascade_edit_request_receipt_hash\",\"workbench_node_identity_cascade_edit_request_sha256\":\"sha256:$node_identity_cascade_edit_request_hash\",\"workbench_node_identity_cascade_edit_assembly_receipt_sha256\":\"sha256:$node_identity_cascade_edit_assembly_receipt_hash\",\"workbench_node_identity_cascade_edit_checkpoint_sha256\":\"sha256:$node_identity_cascade_edit_checkpoint_hash\",\"workbench_node_identity_cascade_edit_result_ir_sha256\":\"sha256:$node_identity_cascade_edit_result_ir_hash\",\"workbench_node_identity_cascade_edit_recovery_sha256\":\"sha256:$node_identity_cascade_edit_recovery_hash\",\"workbench_node_identity_cascade_edit_report_ir_sha256\":\"sha256:$node_identity_cascade_edit_report_ir_hash\",\"workbench_node_identity_cascade_edit_restart_passed\":true,"
v76_receipt_json="${v76_receipt_json/\"workbench_result_view_surface_passed\":true,/${node_identity_cascade_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v76_receipt_json" > "$temporary_receipt"
v77_receipt_json="${v76_receipt_json/structural-native-distribution-e2e.v76/structural-native-distribution-e2e.v77}"
frame_section_identity_cascade_edit_receipt_fields="\"workbench_frame_section_identity_cascade_edit_surface_passed\":true,\"workbench_frame_section_identity_cascade_edit_model_sha256\":\"sha256:$frame_section_identity_cascade_edit_model_hash\",\"workbench_frame_section_identity_cascade_edit_receipt_sha256\":\"sha256:$frame_section_identity_cascade_edit_receipt_hash\",\"workbench_frame_section_identity_cascade_edit_request_receipt_sha256\":\"sha256:$frame_section_identity_cascade_edit_request_receipt_hash\",\"workbench_frame_section_identity_cascade_edit_request_sha256\":\"sha256:$frame_section_identity_cascade_edit_request_hash\",\"workbench_frame_section_identity_cascade_edit_assembly_receipt_sha256\":\"sha256:$frame_section_identity_cascade_edit_assembly_receipt_hash\",\"workbench_frame_section_identity_cascade_edit_checkpoint_sha256\":\"sha256:$frame_section_identity_cascade_edit_checkpoint_hash\",\"workbench_frame_section_identity_cascade_edit_result_ir_sha256\":\"sha256:$frame_section_identity_cascade_edit_result_ir_hash\",\"workbench_frame_section_identity_cascade_edit_recovery_sha256\":\"sha256:$frame_section_identity_cascade_edit_recovery_hash\",\"workbench_frame_section_identity_cascade_edit_report_ir_sha256\":\"sha256:$frame_section_identity_cascade_edit_report_ir_hash\",\"workbench_frame_section_identity_cascade_edit_restart_passed\":true,"
v77_receipt_json="${v77_receipt_json/\"workbench_result_view_surface_passed\":true,/${frame_section_identity_cascade_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v77_receipt_json" > "$temporary_receipt"
v78_receipt_json="${v77_receipt_json/structural-native-distribution-e2e.v77/structural-native-distribution-e2e.v78}"
linear_material_identity_cascade_edit_receipt_fields="\"workbench_linear_material_identity_cascade_edit_surface_passed\":true,\"workbench_linear_material_identity_cascade_edit_model_sha256\":\"sha256:$linear_material_identity_cascade_edit_model_hash\",\"workbench_linear_material_identity_cascade_edit_receipt_sha256\":\"sha256:$linear_material_identity_cascade_edit_receipt_hash\",\"workbench_linear_material_identity_cascade_edit_request_receipt_sha256\":\"sha256:$linear_material_identity_cascade_edit_request_receipt_hash\",\"workbench_linear_material_identity_cascade_edit_request_sha256\":\"sha256:$linear_material_identity_cascade_edit_request_hash\",\"workbench_linear_material_identity_cascade_edit_assembly_receipt_sha256\":\"sha256:$linear_material_identity_cascade_edit_assembly_receipt_hash\",\"workbench_linear_material_identity_cascade_edit_checkpoint_sha256\":\"sha256:$linear_material_identity_cascade_edit_checkpoint_hash\",\"workbench_linear_material_identity_cascade_edit_result_ir_sha256\":\"sha256:$linear_material_identity_cascade_edit_result_ir_hash\",\"workbench_linear_material_identity_cascade_edit_recovery_sha256\":\"sha256:$linear_material_identity_cascade_edit_recovery_hash\",\"workbench_linear_material_identity_cascade_edit_report_ir_sha256\":\"sha256:$linear_material_identity_cascade_edit_report_ir_hash\",\"workbench_linear_material_identity_cascade_edit_restart_passed\":true,"
v78_receipt_json="${v78_receipt_json/\"workbench_result_view_surface_passed\":true,/${linear_material_identity_cascade_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v78_receipt_json" > "$temporary_receipt"
v79_receipt_json="${v78_receipt_json/structural-native-distribution-e2e.v78/structural-native-distribution-e2e.v79}"
truss_section_identity_cascade_edit_receipt_fields="\"workbench_truss_section_identity_cascade_edit_surface_passed\":true,\"workbench_truss_section_identity_cascade_edit_model_sha256\":\"sha256:$truss_section_identity_cascade_edit_model_hash\",\"workbench_truss_section_identity_cascade_edit_receipt_sha256\":\"sha256:$truss_section_identity_cascade_edit_receipt_hash\",\"workbench_truss_section_identity_cascade_edit_request_receipt_sha256\":\"sha256:$truss_section_identity_cascade_edit_request_receipt_hash\",\"workbench_truss_section_identity_cascade_edit_request_sha256\":\"sha256:$truss_section_identity_cascade_edit_request_hash\",\"workbench_truss_section_identity_cascade_edit_assembly_receipt_sha256\":\"sha256:$truss_section_identity_cascade_edit_assembly_receipt_hash\",\"workbench_truss_section_identity_cascade_edit_checkpoint_sha256\":\"sha256:$truss_section_identity_cascade_edit_checkpoint_hash\",\"workbench_truss_section_identity_cascade_edit_result_ir_sha256\":\"sha256:$truss_section_identity_cascade_edit_result_ir_hash\",\"workbench_truss_section_identity_cascade_edit_recovery_sha256\":\"sha256:$truss_section_identity_cascade_edit_recovery_hash\",\"workbench_truss_section_identity_cascade_edit_report_ir_sha256\":\"sha256:$truss_section_identity_cascade_edit_report_ir_hash\",\"workbench_truss_section_identity_cascade_edit_restart_passed\":true,"
v79_receipt_json="${v79_receipt_json/\"workbench_result_view_surface_passed\":true,/${truss_section_identity_cascade_edit_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v79_receipt_json" > "$temporary_receipt"
v80_receipt_json="${v79_receipt_json/structural-native-distribution-e2e.v79/structural-native-distribution-e2e.v80}"
...
v84_receipt_json="${v83_receipt_json/structural-native-distribution-e2e.v83/structural-native-distribution-e2e.v84}"
reaction_result_receipt_fields="\"model_ir_linear_reaction_result_ir_sha256\":\"sha256:$linear_reaction_hash\",\"mgt_model_ir_linear_reaction_result_ir_sha256\":\"sha256:$mgt_linear_reaction_hash\","
v84_receipt_json="${v84_receipt_json/\"workbench_result_view_surface_passed\":true,/${reaction_result_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v84_receipt_json" > "$temporary_receipt"
v85_receipt_json="${v84_receipt_json/structural-native-distribution-e2e.v84/structural-native-distribution-e2e.v85}"
...
v87_receipt_json="${v86_receipt_json/structural-native-distribution-e2e.v86/structural-native-distribution-e2e.v87}"
...
v89_receipt_json="${v88_receipt_json/structural-native-distribution-e2e.v88/structural-native-distribution-e2e.v89}"
...
v94_receipt_json="${v93_receipt_json/structural-native-distribution-e2e.v93/structural-native-distribution-e2e.v94}"
frame3d_end_release_receipt_fields="\"model_ir_frame3d_end_release_linear_cpu_surface_passed\":true,\"model_ir_frame3d_end_release_linear_cpu_restart_bitwise_passed\":true,\"model_ir_frame3d_end_release_linear_cpu_fallback_count\":0,\"model_ir_frame3d_end_release_released_force_exact_zero_passed\":true,\"model_ir_frame3d_end_release_model_sha256\":\"sha256:$release_linear_model_hash\",\"model_ir_frame3d_end_release_request_sha256\":\"sha256:$release_linear_request_hash\",\"model_ir_frame3d_end_release_result_ir_sha256\":\"sha256:$release_linear_result_hash\",\"model_ir_frame3d_end_release_recovery_sha256\":\"sha256:$release_linear_recovery_hash\",\"model_ir_frame3d_end_release_checkpoint_sha256\":\"sha256:$release_linear_checkpoint_hash\","
v94_receipt_json="${v94_receipt_json/\"workbench_result_view_surface_passed\":true,/${frame3d_end_release_receipt_fields}\"workbench_result_view_surface_passed\":true,}"
printf '%s\n' "$v94_receipt_json" > "$temporary_receipt"
v95_receipt_json="${v94_receipt_json/structural-native-distribution-e2e.v94/structural-native-distribution-e2e.v95}"
