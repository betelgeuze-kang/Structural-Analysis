...
def check_native_deployment_cutover(repo_root: Path = ROOT) -> dict[str, object]:
    ...
    for token in (
        ...
        "structural-native-distribution-e2e.v87",
        ...
        "model-edit-element-identity-cascade",
        "--element E_1 --new-element E1_LINKED",
    ...
    for token in (
        ...
        "validate_rootfs_isolation_evidence_v19",
        "inspect_rootfs_frame3d_prescribed_support_surface",
        ...
        "validate_rootfs_isolation_evidence_v21",
        "inspect_rootfs_model_buckling_job_surface",
        ...
        "mgt_model_ir_linear_reaction_audit_en_us_sha256",
        "workbench_reaction_audit_wrong_profile_rejected",
        "inspect_rootfs_reaction_audit_surface",
    ...
    _require_tokens(
        ...
        tokens=(
            ...
            "append-only v9 receipt",
            "append-only v10 receipt",
            ...
            "append-only v14 receipt",
            "frozen v1 through v13 rootfs receipts",
            "isolated durable modal Workbench-session evidence",
            ...
            "pre-v90 receipt",
            "append-only v90",
            "installed Workbench-authored ModelIR modal product authority",
            "pre-v91 receipt",
            "append-only v91",
            "installed model-bound modal restart and result-view authority",
            "pre-v92 receipt",
            "append-only v92",
            "installed durable modal Workbench-session authority",
            "bounded nodal-displacement-view evidence",
            "bounded ModelIR-linear deformed-view evidence",
            "installed Frame3D element-recovery evidence",
    ...
    _require_tokens(
        relative=Path("docs/native/modelir-element-identity-cascade-edit-v2.md"),
        text=element_identity_cascade_edit_doc,
        tokens=(
            "model-edit-element-identity-cascade",
            "single C ABI into C++ semantic",
            "structural-native:model-edit-element-identity-cascade.v2",
    ...
    if not isinstance(element_identity_cascade_edit_capability, dict):
        blockers.append("modelir_element_identity_cascade_edit_capability_missing")
    else:
        for field, expected in (
            ("status", "implemented"),
            ("cutover_gate", "C5"),
            ...
            if element_identity_cascade_edit_capability.get(field) != expected:
                blockers.append(
                    "modelir_element_identity_cascade_edit_capability_"
                    f"field_invalid:{field}"
                )
        element_identity_cascade_edit_claim = str(
            element_identity_cascade_edit_capability.get("claim", "")
        )
        for token in (
            "replaces exactly one existing referenced element ID",
            "atomically updates every construction_stages[].active_element_ids reference",
            "direct element round-trip model_ir_entity_id",
            "exact or canonicalized direct mappings degrade to approximated",
            "at least one typed construction-stage or direct round-trip reference is required",
            "unsupported-feature source_entity_id ownership of either source or replacement",
            "single C ABI into C++ semantic/reference validation",
            "distribution v82 E2E",
            "E_1 with E1_LINKED",
            "exact active DOFs [6,7,8,9,10,11]",
            "active load [200000,0,0,0,0,0]",
            "typed frame recovery [1] with offsets [0,12]",
            ...
            if token not in element_identity_cascade_edit_claim:
                blockers.append(
                    "modelir_element_identity_cascade_edit_capability_"
                    f"claim_missing:{token}"
                )
    ...
    load_pattern_identity_cascade_capability = (
        capabilities.get("modelir_linear_load_pattern_identity_cascade_edit")
        if isinstance(capabilities, dict)
        else None
    )
    if not isinstance(load_pattern_identity_cascade_capability, dict):
        blockers.append(
            "modelir_linear_load_pattern_identity_cascade_edit_capability_missing"
        )
    else:
        for field, expected in (
            ("status", "implemented"),
            ...
            if load_pattern_identity_cascade_capability.get(field) != expected:
                blockers.append(
                    "modelir_linear_load_pattern_identity_cascade_edit_capability_"
                    f"field_invalid:{field}"
                )
        load_pattern_identity_cascade_claim = str(
            load_pattern_identity_cascade_capability.get("claim", "")
        )
        for token in (
            "replaces exactly one existing referenced linear_static pattern ID",
            "atomically updates every typed load-combination",
            "construction-stage load_pattern_ids",
            "direct load_pattern round-trip model_ir_entity_id",
            "exact or canonicalized direct mappings degrade to approximated",
            "unsupported-feature ownership of either identity",
            "single C ABI into C++ semantic/reference validation",
            "distribution v80 E2E",
            "LC_WEAK_LINKED",
            "exact active DOFs [6,7,8,9,10,11]",
            "combined active load [25000,-12000,5000,0,0,0]",
            "typed frame-plus-truss recovery [1,2] with offsets [0,12,15]",
            ...
            if token not in load_pattern_identity_cascade_claim:
                blockers.append(
                    "modelir_linear_load_pattern_identity_cascade_edit_capability_"
                    f"claim_missing:{token}"
                )
    ...
    if not isinstance(capability, dict):
        ...
        if evidence_contract != {
            "latest_rootfs_receipt_schema": "structural-native-rootfs-isolation-e2e.v21",
            "frozen_rootfs_receipts": "v1-v20",
            "required_installed_receipt_schema": "structural-native-distribution-e2e.v99",
        ...
        for token in (
            "local rootfs diagnostic v21",
            "installed distribution v99",
            "append-only durable job store",
            "idempotent submission",
            "separate worker process",
            "nineteen-file export",
            "eighteen-artifact direct-product byte parity",
            "export overwrite and blob-tamper rejection",
            "three durable identities",
            "UID/GID 65532",
            "empty PATH",
            "read-only root and payload",
            "writable workspace",
            "loopback-only isolation",
            "frozen rootfs receipts remain v1-v20",
