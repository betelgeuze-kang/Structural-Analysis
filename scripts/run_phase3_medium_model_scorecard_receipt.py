#!/usr/bin/env python3
"""Run an operator-attached medium-model input and write a scorecard receipt."""
...
def _is_template_like_path(path: Path) -> bool:
    try:
        resolved = path.resolve()
    except Exception:
        resolved = path
    try:
        templates_dir = (ROOT / "docs" / "templates").resolve()
...
def _is_generated_review_evidence_ref_path(path: Path) -> bool:
    try:
        resolved = path.resolve()
    except Exception:
        resolved = path
    for generated_path in GENERATED_REVIEW_EVIDENCE_REF_PATHS:
        if _same_resolved_path(resolved, ROOT / generated_path):
            return True
    try:
        medium_receipt_dir = (ROOT / MEDIUM_RECEIPT_DIR).resolve()
...
def build_medium_model_scorecard_receipt(
    *,
    model_path: Path,
    source_id: str,
    case_id: str,
    ...
    solver: str = "phase3_medium_model_operator_scorecard",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--solver", default="phase3_medium_model_operator_scorecard")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "Phase 3 medium-model scorecard receipt: "
