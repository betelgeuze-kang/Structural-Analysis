#!/usr/bin/env python3
"""Fill the release-surface-first structural-scope owner decision batch."""
...
SCHEMA_VERSION = "structural-scope-release-surface-owner-decision-fill.v1"
PRODUCTIZATION = Path("implementation/phase1/release_evidence/productization")
DEFAULT_TEMPLATE = (
    PRODUCTIZATION / "structural_scope_owner_decisions.release_surface_first.template.csv"
)
DEFAULT_OUT = (
    PRODUCTIZATION / "structural_scope_owner_decisions.release_surface_first.filled.json"
)
...
def _load_template_rows(repo_root: Path, path: Path) -> tuple[list[dict[str, Any]], list[str], list[str]]:
...
def _field_blockers(
    *,
    ...
    if (
        ...
        decision == "extract_to_molecular_or_science_repository"
        and owner_review._is_placeholder_text(external_archive_reference)
    ):
        ...
        **release_evidence_metadata(
        ...
        "validation_commands": [
            (
                ...
                "--owner-decisions <filled-release-surface-first-owner-decisions.json> "
                "--fail-release-surface-first-blocked"
            ),
            (
                "python3 scripts/merge_structural_scope_owner_decision_batch.py "
                "--batch-owner-decisions <filled-release-surface-first-owner-decisions.json> "
        ...
        "claim_boundary": (
            "This helper only fills a release_surface_first owner-decision batch "
            "from explicit owner metadata. It does not delete files, extract files, "
...
def _markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# Structural Scope Release Surface Owner Decision Fill",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "release-surface owner decision fill: "
