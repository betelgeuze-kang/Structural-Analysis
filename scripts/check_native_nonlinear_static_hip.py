#!/usr/bin/env python3
"""Validate the bounded native nonlinear-static HIP implementation and live receipt."""
...
REQUIRED_TOKENS = {
    ...
    "native/cpp/src/solver_cpu/nonlinear_static.hpp": (
        "validate_nonlinear_static_problem",
        "CPU/accelerator nonlinear-static problem contract",
    ),
    ...
    "native/cpp/src/hip/nonlinear_static_hip.hip.cpp": (
        "nonlinear_static_newton_kernel",
        "single_thread_resident_newton_fp64.v1",
        "validate_nonlinear_static_problem",
        "hipStreamSynchronize",
    ),
    "native/cpp/tests/hip/nonlinear_static_hip_parity_test.hip.cpp": (
        "max_displacement_absolute_error",
        "max_residual_absolute_error",
        "device_resident_newton_state",
...
def expected_source_sha256(root: Path) -> str:
    header = (root / "native/cpp/src/hip/nonlinear_static_hip.hpp").read_text(
        encoding="utf-8"
    )
    source = (
        root / "native/cpp/src/hip/nonlinear_static_hip.hip.cpp"
...
def _validate_receipt(
    ...
    for key, expected in exact_values.items():
        if payload.get(key) != expected:
            blockers.append(f"nonlinear_static_hip_receipt_value_invalid:{key}")
    for key in (
        "runtime_version",
        "driver_version",
        "h2d_bytes",
        ...
        if not _positive_integer(payload, key):
            blockers.append(
                f"nonlinear_static_hip_receipt_positive_integer_invalid:{key}"
            )
    for key in (
        "device_name",
        "architecture",
        ...
        if not isinstance(payload.get(key), str) or not payload[key]:
            blockers.append(f"nonlinear_static_hip_receipt_string_invalid:{key}")
    device_id = payload.get("device_id")
    if not isinstance(device_id, int) or isinstance(device_id, bool) or device_id < 0:
        blockers.append("nonlinear_static_hip_receipt_device_id_invalid")
    ...
    if isinstance(architecture, str) and isinstance(compiled, str):
        if architecture.split(":", 1)[0] not in compiled.split(";"):
            blockers.append(
                "nonlinear_static_hip_receipt_runtime_compile_architecture_mismatch"
            )
    tolerances = {
        "max_displacement_absolute_error": 5.0e-11,
        "max_residual_absolute_error": 2.0e-7,
        "max_recovery_absolute_error": 2.0e-8,
    }
    for key, maximum in tolerances.items():
        ...
        if (
            ...
            blockers.append(
                f"nonlinear_static_hip_receipt_parity_tolerance_failed:{key}"
            )
    ...
    if all(isinstance(value, int) for value in (peak, total, free_before, free_after)):
        if peak >= total or free_before > total or free_after > total:
            blockers.append("nonlinear_static_hip_receipt_vram_counters_invalid")
    if payload.get("kernel_source_sha256") != expected_source_sha256(root):
        blockers.append("nonlinear_static_hip_kernel_source_sha256_mismatch")
    try:
        expected_device_lib = expected_device_library_sha256(
            device_lib_path, str(architecture)
        )
    except (OSError, ValueError) as exc:
        blockers.append(f"nonlinear_static_hip_device_library_unreadable:{exc}")
    else:
        if payload.get("device_library_sha256") != expected_device_lib:
            blockers.append("nonlinear_static_hip_device_library_sha256_mismatch")
    return blockers
...
def _approved_runner_context(root: Path) -> tuple[bool, dict[str, str], list[str]]:
    ...
    if fields["github_actions"] != "true":
        blockers.append("nonlinear_static_hip_approval_not_github_actions")
    if fields["runner_environment"] != "self-hosted":
        blockers.append("nonlinear_static_hip_approval_runner_not_self_hosted")
    if fields["approval_environment"] != "native-hip-approved":
        blockers.append("nonlinear_static_hip_approval_environment_mismatch")
    if not fields["github_run_id"].isdigit() or int(fields["github_run_id"] or 0) <= 0:
        blockers.append("nonlinear_static_hip_approval_run_id_invalid")
    if "native-hip-dedicated.yml@" not in fields["github_workflow_ref"]:
        blockers.append("nonlinear_static_hip_approval_workflow_ref_invalid")
    if not fields["runner_name"]:
        blockers.append("nonlinear_static_hip_approval_runner_name_missing")
    try:
        head = subprocess.run(
            ["git", "rev-parse", "HEAD"],
            cwd=root,
        ...
        blockers.append("nonlinear_static_hip_approval_git_head_unreadable")
    else:
        if fields["github_sha"] != head:
            blockers.append("nonlinear_static_hip_approval_sha_mismatch")
    return not blockers, fields, blockers


def check_native_nonlinear_static_hip(
    repo_root: Path = ROOT,
    *,
    ...
    for relative, tokens in REQUIRED_TOKENS.items():
        try:
            ...
            blockers.append(f"nonlinear_static_hip_evidence_missing:{relative}")
            continue
        for token in tokens:
            if token not in text:
                blockers.append(
                    f"nonlinear_static_hip_evidence_token_missing:{relative}:{token}"
                )
    ...
    if receipt_path is not None:
        ...
        try:
            ...
            blockers.append(f"nonlinear_static_hip_receipt_invalid:{exc}")
        else:
    ...
    if require_approved_runner:
        ...
        if receipt_path is None:
            blockers.append("nonlinear_static_hip_approved_receipt_required")
    authoritative_c2 = receipt_valid and approved and require_approved_runner
    blockers = sorted(set(blockers))
    return {
        "schema_version": "native-nonlinear-static-hip-validation.v1",
        ...
        "claim_boundary": (
            ...
            "nonlinear-static Newton C2 candidate. Authoritative C2 additionally requires "
            "the protected native-hip-approved self-hosted workflow context; this does not "
            "promote the product ABI/backend selector, arbitrary ModelIR assembly, transient "
            "Newton, package authority, or C6."
        ),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = check_native_nonlinear_static_hip(
        args.root,
    ...
    if args.json:
        ...
        print(f"Native nonlinear-static HIP contract: {report['status']}")
