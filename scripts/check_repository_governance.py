...
def _read_if_present(root: Path, relative: Path) -> str:
    path = root / relative
...
def build_report(repo_root: Path) -> dict[str, Any]:
    ...
    normalized_license = " ".join(_read_if_present(root, Path("LICENSE")).split())
    ...
    security_text = _read_if_present(root, Path("SECURITY.md"))
    ...
    normalized_contributing = " ".join(
        _read_if_present(root, Path("CONTRIBUTING.md")).split()
    )
    ...
    codeowners_text = _read_if_present(root, Path(".github/CODEOWNERS"))
