...
async function main() {
  ...
  try {
    ...
    await page.locator('[data-native-frame-load-id]').fill('LC_WEAK')
    await page.locator('[data-native-frame-result-id]').fill('result.browser.LC_WEAK')
    ...
    const resultArtifact = requireObject(
      requireObject(bundle.artifacts, 'bundle_artifacts_invalid').result_ir,
    ...
    if (
      ...
      || result.result_id !== 'result.browser.LC_WEAK'
