...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated wind attack angles in degrees (default 0,22.5,45,67.5,90,112.5,135,157.5).",
    )
    parser.add_argument("--output-json", type=Path, required=True)
    args = parser.parse_args()
    angle_sweep: tuple[float, ...] | None = None
    if args.angle_sweep_deg:
        angle_sweep = tuple(float(x.strip()) for x in str(args.angle_sweep_deg).split(",") if x.strip())
    ...
    sweep_count = len(wd.get("angle_sweep") or [])
    print(
        ...
        f"gov_angle={wd.get('governing_angle_deg')} envelope_V={wd.get('envelope_max_base_shear_kN', 0):.0f}kN "
        f"angles={sweep_count} "
