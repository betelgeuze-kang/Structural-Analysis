#!/usr/bin/env python3
"""Build a ChEMBL positive-source snapshot for the GPCR hard-decoy suite."""
...
DEFAULT_OUT = PRODUCTIZATION / "gpcr_hard_decoy_positive_source_snapshot.json"
DEFAULT_OUT_MD = DEFAULT_OUT.with_suffix(".md")
SCHEMA_VERSION = "gpcr-hard-decoy-positive-source-snapshot.v1"
...
def _payload_for_target(
    *,
    ...
    if fixture_payload is not None:
        fixture_rows = fixture_payload.get(target_id) or fixture_payload.get(chembl_target_id)
        if isinstance(fixture_rows, dict):
            return fixture_rows, query_url
        if isinstance(fixture_rows, list):
            return {"activities": fixture_rows, "page_meta": {"total_count": len(fixture_rows)}}, query_url
...
def _activity_sort_key(row: dict[str, Any]) -> tuple[float, str]:
    ...
    return (value if value is not None else float("inf"), str(row.get("molecule_chembl_id") or ""))
...
def _candidate_rows(
    *,
    ...
    activities = [
        ...
        and row.get("molecule_chembl_id")
        ...
        candidates.append(
            {
                **source_row,
                ...
                "closure_role": "positive_ligand_candidate_only",
            }
        )
...
def _target_snapshot(
    *,
    ...
    minimum_positive = int(RAW_ROW_QUALITY_CRITERIA["min_positive_count_per_target"])
    blockers = []
    if len(candidates) < minimum_positive:
        blockers.append(
            f"{target_id}:chembl_positive_candidate_count_below_minimum"
        )
    return {
        **target_source,
        "status": "positive_candidates_ready" if not blockers else "positive_candidates_incomplete",
        "contract_pass": not blockers,
        "positive_candidate_count": len(candidates),
        "minimum_positive_rows_required": minimum_positive,
        ...
        "blockers": blockers,
        "claim_boundary": (
            "These ChEMBL rows identify target-linked positive ligand candidates only; "
            "they are not target-specific hard decoys and do not include a scoring run."
        ),
    }


def build_gpcr_hard_decoy_positive_source_snapshot(
    *,
    repo_root: Path = ROOT,
    ...
    target_candidate_counts = {
        str(target["target_id"]): int(target["positive_candidate_count"])
        for target in target_snapshots
    }
    minimum_positive = int(RAW_ROW_QUALITY_CRITERIA["min_positive_count_per_target"])
    ...
    return {
        ...
        **release_evidence_metadata(
            ...
            reuse_policy="gpcr_hard_decoy_positive_source_snapshot_from_chembl",
            repo_root=repo_root,
        ),
        "status": "positive_sources_ready" if not blockers else "positive_sources_incomplete",
        "contract_pass": not blockers,
        "positive_source_ready": not blockers,
        ...
        "minimum_positive_rows_per_target": minimum_positive,
        "total_positive_candidate_count": total_candidate_count,
        ...
        "claim_boundary": (
            "This snapshot is a positive-ligand source receipt over official ChEMBL "
            "activity rows for DRD2, HTR2A, and OPRM1. It does not provide hard "
            "decoys, docking scores, or Phase 3 closure rows."
        ),
    }


def render_gpcr_hard_decoy_positive_source_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# GPCR Hard-Decoy Positive Source Snapshot",
        "",
        f"- `status`: `{payload['status']}`",
        f"- `contract_pass`: `{payload['contract_pass']}`",
        f"- `positive_source_ready`: `{payload['positive_source_ready']}`",
        f"- `actual_closure_ready`: `{payload['actual_closure_ready']}`",
        f"- `total_positive_candidate_count`: `{payload['total_positive_candidate_count']}`",
        "",
    ...
    for row in payload["target_snapshots"]:
        lines.append(
            ...
            f"{row['positive_candidate_count']} | {row['activity_total_count']} |"
        )
    lines.extend(["", str(payload["claim_boundary"]), ""])
    return "\n".join(lines)


def write_gpcr_hard_decoy_positive_source_snapshot(
    *,
    repo_root: Path = ROOT,
    ...
    payload = build_gpcr_hard_decoy_positive_source_snapshot(
        repo_root=repo_root,
    ...
    resolved_md.write_text(
        render_gpcr_hard_decoy_positive_source_markdown(payload),
        encoding="utf-8",
    )
    return payload
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_gpcr_hard_decoy_positive_source_snapshot(
    ...
    if args.json:
        ...
        print(
            "gpcr-hard-decoy-positive-source-snapshot: "
            f"{payload['status']} | candidates={payload['total_positive_candidate_count']} | "
