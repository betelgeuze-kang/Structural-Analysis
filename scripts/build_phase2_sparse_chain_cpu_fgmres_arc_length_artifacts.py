#!/usr/bin/env python3
"""Build sparse-chain state-operator CPU FGMRES arc-length artifacts."""
...
from structural_analysis.benchmark.sparse_chain_arc_length import (  # noqa: E402
...
DEFAULT_RESULT_OUT = (
    PRODUCTIZATION / "phase2_sparse_chain_cpu_fgmres_arc_length_result.json"
)
DEFAULT_SUMMARY_OUT = (
...
def build_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
    *,
    repo_root: Path = ROOT,
    ...
    result_payload = build_sparse_chain_cpu_fgmres_arc_length_seed()
    ...
    summary_payload = {
        ...
        "input_checksums": input_checksums(
            [
                ...
                Path(
                    "scripts/"
                    "build_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts.py"
                ),
                Path("tests/test_sparse_chain_cpu_fgmres_arc_length.py"),
                Path(
                    "tests/"
                    "test_build_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts.py"
                ),
            ],
            repo_root=repo_root,
        ),
...
def check_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
    *,
    repo_root: Path = ROOT,
    result_out: Path = DEFAULT_RESULT_OUT,
    summary_out: Path = DEFAULT_SUMMARY_OUT,
) -> tuple[bool, str]:
    expected = build_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
        repo_root=repo_root,
    ...
    for key, relative in (("result", result_out), ("summary", summary_out)):
        ...
        if not path.is_file():
            return False, f"phase2_sparse_chain_arc_length_missing:{relative}"
        try:
            existing = _read_json(path)
        except Exception as exc:
            return False, (
                f"phase2_sparse_chain_arc_length_unreadable:{relative}:"
                f"{exc.__class__.__name__}"
            )
        if _strip_volatile(existing) != _strip_volatile(expected[key]):
            return False, f"phase2_sparse_chain_arc_length_mismatch:{key}"
    return True, "phase2_sparse_chain_arc_length_consistent"


def write_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
    *,
    repo_root: Path = ROOT,
    result_out: Path = DEFAULT_RESULT_OUT,
    summary_out: Path = DEFAULT_SUMMARY_OUT,
) -> dict[str, dict[str, Any]]:
    payloads = build_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
        repo_root=repo_root,
        result_out=result_out,
        summary_out=summary_out,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ok, message = check_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
            repo_root=ROOT,
    ...
    payloads = write_phase2_sparse_chain_cpu_fgmres_arc_length_artifacts(
        repo_root=ROOT,
        result_out=args.result_out,
        summary_out=args.summary_out,
    )
    ...
    print(
        f"{summary['status']} | "
        f"equations={summary['equation_count']} | "
