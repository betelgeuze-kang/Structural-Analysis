...
MDOF = _load("f3_mdof_linear_runner", ROOT / "implementation/phase1/run_f3_mdof_linear_transient_vertical_evidence.py")
...
DEFAULT_OUT = Path("implementation/phase1/release_evidence/productization/f3_nonlinear_mdof_vertical_evidence.json")
PREDECESSOR_RECEIPT = Path("implementation/phase1/release_evidence/productization/f3_mdof_linear_transient_vertical_evidence.json")
SOLVER_ID = "newmark.consistent-newton.bilinear-shear-mdof.v1"
SOURCE_PATHS = (
    MODEL_PATH, FORCE_PATH, Path("src/structural_analysis/schemas/model_ir_v2.schema.json"),
    Path("src/structural_analysis/solvers/nonlinear/mdof_transient.py"),
    Path("src/structural_analysis/engine_v2/contracts/nonlinear_mdof_transient_result.py"),
    Path("src/structural_analysis/schemas/nonlinear_mdof_transient_result_ir_v1.schema.json"),
...
def _inputs() -> tuple[Any, NonlinearShearBuilding, NonlinearMDOFTransientConfig, np.ndarray]:
    document = load_model_ir_v2(ROOT / MODEL_PATH); dynamics = document.to_dict()["dynamics"]
    ...
    forces_kn = np.asarray([[float(row["floor_1_ux_n"]), float(row["floor_2_ux_n"])] for row in rows]) / 1000.0
    stories = tuple(BilinearStory(
        row["story_id"], float(row["elastic_stiffness_n_per_m"]) / 1000.0,
        float(row["yield_force_n"]) / 1000.0, float(row["post_yield_stiffness_ratio"]),
    ...
    system = NonlinearShearBuilding(
        ...
        stories, dynamics["dof_ids"], model_id=document.model_id,
    )
    return document, system, NonlinearMDOFTransientConfig(dt), forces_kn


def _result(document: Any, system: Any, config: Any, forces: np.ndarray, solution: Any, authority: Any) -> dict[str, Any]:
    samples = []
    for step in solution.steps:
        row = step.to_dict()
        for key in ("applied_force_kn", "story_force_kn", "equilibrium_residual_kn"):
            row[key.removesuffix("_kn") + "_n"] = [value * 1000.0 for value in row.pop(key)]
    ...
    result = create_nonlinear_mdof_result_ir(
        result_id="f3.nonlinear-mdof", model_ir_content_hash=document.content_hash,
        force_history_hash=LINEAR._sha_payload(forces.tolist()), solver_id=SOLVER_ID,
        solver_result_hash=solution.result_hash, integration_contract_hash=config.contract_hash,
        terminal_checkpoint_hash=solution.checkpoints[-1].checkpoint_hash,
        checkpoint_authority_receipt_hash=authority.receipt_hash,
        dof_ids=system.dof_ids, story_ids=tuple(row.story_id for row in system.stories),
        time_step_s=config.time_step_s,
        residual_relative_tolerance=config.residual_relative_tolerance,
        samples=samples, terminal_story_material_states=terminal_states,
    )
    manifest = result.to_manifest(); validate_nonlinear_mdof_result_ir_manifest(manifest)
    return manifest
...
def _predecessor(source_commit: str) -> tuple[F3StageGateReceipt, str, dict[str, Any]]:
    ...
    receipt = F3StageGateReceipt(
        schema="f3-vertical-evidence-gate.v1", stage="mdof_linear_transient", stage_index=6,
        source_commit_sha=source_commit, required_surfaces=tuple(stage["required_surfaces"]),
        verified_surfaces=tuple(stage["verified_surfaces"]), evidence_artifact_sha256=tuple(sorted(stage["evidence_artifact_sha256"].items())),
        predecessor_stage="sdof_authenticated_transient", predecessor_receipt_sha256=stage["predecessor_receipt_sha256"],
...
def _elastic_parity(system: NonlinearShearBuilding, config: Any, forces: np.ndarray) -> dict[str, Any]:
    elastic_stories = tuple(BilinearStory(row.story_id, row.elastic_stiffness_kn_per_m, 1.0e12, row.post_yield_stiffness_ratio) for row in system.stories)
    nonlinear_elastic = NonlinearShearBuilding(system.mass_matrix_kn_s2_per_m, system.damping_matrix_kn_s_per_m, elastic_stories, system.dof_ids, "elastic-limit-nonlinear")
    nonlinear_solution = solve_nonlinear_mdof_transient(nonlinear_elastic, forces, config=config)
    stiffness = system.drift_matrix.T @ np.diag([row.elastic_stiffness_kn_per_m for row in system.stories]) @ system.drift_matrix
    linear = LinearMDOFSystem(np.asarray(system.mass_matrix_kn_s2_per_m) * 1000.0, np.asarray(system.damping_matrix_kn_s_per_m) * 1000.0, stiffness * 1000.0, system.dof_ids, "elastic-limit-linear")
    linear_solution = solve_linear_mdof_transient(linear, forces * 1000.0, config=LinearMDOFTransientConfig(config.time_step_s))
    ...
    return {
        ...
        "maximum_absolute_displacement_difference_m": float(np.max(np.abs(nonlinear_u - linear_u))),
        "tolerance_m": 2.0e-14, "yielded_step_count": nonlinear_solution.yielded_step_count,
    }
...
def build_receipt(*, source_commit_sha: str | None = None) -> dict[str, Any]:
    ...
    split = 6; prefix = solve_nonlinear_mdof_transient(system, forces[:split], config=config)
    resumed = resume_nonlinear_mdof_transient(system, prefix.checkpoints[-1], forces[split:], config=config, checkpoint_chain=prefix.checkpoints, force_history_prefix_kn=forces[:split])
    exact_restart = (*prefix.checkpoints, *resumed.checkpoints[1:]) == solution.checkpoints
    authority = validate_nonlinear_mdof_checkpoint_authority(solution.checkpoints[-1], system=system, config=config, checkpoint_chain=solution.checkpoints, force_history_prefix_kn=forces)
    ...
    accepted = prefix.checkpoints[-1]; rollback_exact = False; trial_failure_observed = False
    try:
        ...
        trial_failure_observed = True; rollback_exact = accepted == prefix.checkpoints[-1]
    displacement = np.asarray([row.displacement_m for row in solution.steps])
    histories = [EXPLORER.evaluate_time_history(
    ...
    all_pass = bool(
        document.analysis_ready and solution.contract_pass and solution.yielded_step_count > 0
        and sum(row.plastic_dissipation_kn_m for row in terminal_states) > 0.0
        and exact_restart and deterministic and authority.source_authenticated_checkpoint
        and authority.material_state_replay_pass and trial_failure_observed and rollback_exact
        and solution.material_trial_commit_rollback and not solution.fallback_used
        and not solution.regularization_used and solution.maximum_relative_residual <= config.residual_relative_tolerance
        and parity["maximum_absolute_displacement_difference_m"] <= parity["tolerance_m"]
        and parity["yielded_step_count"] == 0
    )
    surfaces: dict[str, Any] = {
        "model_ir": {"content_hash": document.content_hash, "model_id": document.model_id, "capability_profile": document.capability_profile, "analysis_ready": document.analysis_ready, "story_count": len(system.stories), "force_history_source_sha256": document.to_dict()["dynamics"]["force_history_sha256"]},
        "solver": {"schema_version": solution.schema_version, "profile": solution.profile, "solver_id": SOLVER_ID, "step_count": len(solution.steps), "total_newton_iterations": solution.total_newton_iterations, "yielded_step_count": solution.yielded_step_count, "maximum_relative_residual": solution.maximum_relative_residual, "material_trial_commit_rollback": solution.material_trial_commit_rollback, "fallback_used": solution.fallback_used, "regularization_used": solution.regularization_used},
        "result_ir": {"schema_version": manifest["schema_version"], "manifest": manifest, "manifest_valid": True},
        "recovery": {"maximum_absolute_displacement_by_dof_m": dict(zip(system.dof_ids, np.max(np.abs(displacement), axis=0).tolist(), strict=True)), "terminal_story_material_states": manifest["terminal_story_material_states"], "plastic_dissipation_j": sum(row.plastic_dissipation_kn_m for row in terminal_states) * 1000.0},
        "checkpoint": {"terminal_checkpoint": solution.checkpoints[-1].to_dict(), "source_authority_receipt": authority.to_dict(), "exact_restart": exact_restart, "forced_failure_trial_observed": trial_failure_observed, "forced_failure_rollback_exact": rollback_exact},
        "workbench": {"schema_version": "f3-nonlinear-mdof-workbench-payload.v1", "time_history_cards": [row.__dict__ for row in histories], "summary": EXPLORER.build_results_summary(time_histories=tuple(histories))},
        ...
        "external_vv": {"reference_profile": "independent-linear-matrix-elastic-limit-parity.v1", "verification_mode": "local_self_verification_user_authorized", "maximum_absolute_displacement_difference_m": parity["maximum_absolute_displacement_difference_m"], "signature_verifier_waived": True},
    }
    ...
    gate = evaluate_f3_stage_gate(stage="nonlinear_mdof", source_commit_sha=source_commit, evidence=evidence, external_vv_signature=ExternalVVSignatureVerification(status="waived", authority="user_authorized_signature_verifier_waiver", waiver_reason="User authorized signature-verifier omission for F3 self-verification."), predecessor_receipt=predecessor, predecessor_receipt_sha256=predecessor_hash)
    return {
        "schema_version": "f3-nonlinear-mdof-vertical-evidence.v1", "source_commit_sha": source_commit,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if recorded.get("source_input_checksums") != payload["source_input_checksums"] or out.read_text(encoding="utf-8") != text:
            print("f3_nonlinear_mdof_vertical_evidence_mismatch"); return 1
        print("f3_nonlinear_mdof_vertical_evidence_consistent"); return 0
    ...
    print(f"{payload['status']} | surfaces={len(payload['stage_gate']['verified_surfaces'])}/9 | yielded_steps={solver['yielded_step_count']}")
