...
- Release artifact integrity: `Green` (`structural-analysis-artifacts-2026-04-26` publication evidence must cover the current 22/22 manifest assets, metadata preflight, upload-plan evidence, and hydrated post-publish SHA/bytes round-trip verification)
...
목표: source repo/CI의 manifest 구조 검증, release asset listing preflight, fresh GitHub Release asset root의 current manifest assets(현재 22개) SHA/bytes 무결성을 분리하고, 정식 P0-1 close 기준을 후자로 고정한다. 이 항목은 source-boundary 갭이 아니라 release-publication 갭이다.

현재 `structural-analysis-artifacts-2026-04-26`는 GitHub Release로 published 상태이며, close evidence는 current manifest asset set(현재 22개) 업로드와 metadata/published-byte SHA 검증을 요구한다. 로컬 `implementation/phase1/release/`는 stale state일 수 있으므로, fresh candidate root가 아닌 이 경로를 업로드 소스로 쓰면 mismatched/missing asset에서 실패한다.
...
9. repo-local `implementation/phase1/release/`는 wildcard upload 금지 대상으로 두고, freshly regenerated asset root에서 manifest-listed assets(current set, 현재 22개)만 업로드한다.
...
2. current manifest asset set(현재 22개)을 업로드하고 metadata preflight와 SHA/bytes verification을 통과시켜 `P0-1 Release / Review Chain Stabilization`을 닫는다.
