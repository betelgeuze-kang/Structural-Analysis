...
| `release_area.github_sync` read-only GitHub sync preflight is clean or explicitly approved | `release_area` | `blocked_external_owner_input_ready` | `github_sync::github_sync_preflight::worktree_not_clean`, `github_sync::github_sync_preflight::main_remote_not_ancestor_of_head`, `github_sync::github_sync_preflight::remote_mutation_approval_required`, `github_sync::github_sync_remote_sync_pending`, `github_sync::github_sync_preflight_not_synced` | 5 blockers: Obtain explicit R4 approval phrase `feature push + main fast-forward 승인`, then run the pending remote-update commands from `check_github_development_sync_preflight.py --fetch --json`. |
