...
- `summary_line`: `PM release gate completion audit: BLOCKED | requirements=51 | pass=43 | blocked=8`
...
|---|---|---|---|---|
| `release_area.basic_ci` PR/nightly 30 consecutive PASS evidence | `release_area` | `blocked_external_owner_input_ready` | `basic_ci::pr_ci_30_consecutive_pass_evidence_missing`, `basic_ci::nightly_ci_30_consecutive_pass_evidence_missing` | basic_ci::pr_ci_30_consecutive_pass_evidence_missing: Resolve the pr GitHub Actions job-start blocker shown in github_actions_ci_streak_evidence.json, rerun the workflow, and then collect 30 additional consecutive successful CI run(s) before release signoff.; basic_ci::nightly_ci_30_consecutive_pass_evidence_missing: Collect 30 additional consecutive successful nightly CI run(s); keep the scheduled/nightly lane green and refresh github_actions_ci_streak_evidence before release signoff. |
...
| `release_area.support` known issues, failure bundle, and rollback evidence | `release_area` | `blocked_local_remediation_ready` | `support::pm_release_reproduction_command_audit_not_green` | support::pm_release_reproduction_command_audit_not_green: Resolve `pm_release_reproduction_command_audit_not_green` in Support evidence, regenerate PM release reports, and attach the updated evidence. |
...
| `m5_viewer_mode` reviewer/customer viewer preset | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_pdf_or_reviewer_package` PDF/report or reviewer package generated | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_audit_trail` audit trail has action and source row trace | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_signed_release_registry` release registry is signed | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_support_bundle_export` support bundle one-click export passes | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_validation_manual` validation manual is present and complete | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
| `m5_limitation_manual` limitation manual is present and complete | `milestone` | `pass` | `pm_release_reproduction_command_audit_not_green` | none |
