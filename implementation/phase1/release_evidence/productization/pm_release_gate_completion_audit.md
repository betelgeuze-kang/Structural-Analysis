...
- `summary_line`: `PM release gate completion audit: BLOCKED | requirements=52 | pass=44 | blocked=8`
...
| `release_area.support` known issues, failure bundle, and rollback evidence | `release_area` | `blocked_local_remediation_ready` | `support::pm_blocker_closure_board_count_mismatch` | support::pm_blocker_closure_board_count_mismatch: Resolve `pm_blocker_closure_board_count_mismatch` in Support evidence, regenerate PM release reports, and attach the updated evidence. |
...
| `release_area.github_sync` read-only GitHub sync preflight is clean or explicitly approved | `release_area` | `pass` | none | none |
...
| `m5_viewer_mode` reviewer/customer viewer preset | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_pdf_or_reviewer_package` PDF/report or reviewer package generated | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_audit_trail` audit trail has action and source row trace | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_signed_release_registry` release registry is signed | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_support_bundle_export` support bundle one-click export passes | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_validation_manual` validation manual is present and complete | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
| `m5_limitation_manual` limitation manual is present and complete | `milestone` | `pass` | `pm_blocker_closure_board_count_mismatch` | none |
