...
- `summary_line`: `PM release gate reviewer handoff: PASS | open_blockers=72 | incomplete=0 | release_tiers=0/4`
...
| `release_tier.paid_pilot_scope_guard_pass` Paid Pilot Scope Guard | `blocked` | `paid_pilot_scope_guard_missing_or_failed` | Regenerate the paid pilot scope guard and keep all customer-facing use constrained to the scope guard until release-area blockers close. | Paid pilot status is a constrained customer PoC scope only; it does not imply Limited, GA, or engineer-of-record replacement readiness. |
