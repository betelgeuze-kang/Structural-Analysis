...
- `summary_line`: `PM release gate reviewer handoff: PASS | open_blockers=21 | incomplete=0 | release_tiers=2/4`
...
- Current false audit check(s): `github_sync_main_fast_forward_possible`, `github_sync_preflight_clean`, `github_sync_worktree_clean`
...
Verdict change conditions:
- `release_area.github_sync` status is `pass` in `pm_release_gate_completion_audit.json`
- `github_sync::github_sync_preflight::main_remote_not_ancestor_of_head` is absent from `pm_release_gate_report.json.release_area_blockers`
- The owning release-area row has no blocker-specific false check in the PM report.
- Current false audit check(s): `github_sync_main_fast_forward_possible`, `github_sync_preflight_clean`, `github_sync_worktree_clean`
