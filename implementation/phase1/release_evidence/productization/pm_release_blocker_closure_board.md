...
| `github_sync::github_sync_preflight::local_head_mismatch` | `release_owner` | `external_owner_input_ready` | `approval_required` | Obtain explicit R4 approval phrase `feature push + main fast-forward 승인`, then run the pending remote-update commands from `check_github_development_sync_preflight.py --fetch --json`. |
