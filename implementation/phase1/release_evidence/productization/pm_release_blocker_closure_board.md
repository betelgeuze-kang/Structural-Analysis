...
- `summary_line`: `PM release blocker closure board: BLOCKED | open=10 | external_owner_ready=10 | handoff_not_ready=0`
...
| `github_sync::github_sync_preflight::local_head_mismatch` | `release_owner` | `external_owner_input_ready` | `synced` | Obtain explicit R4 approval phrase `feature push + main fast-forward 승인`, then run the pending remote-update commands from `check_github_development_sync_preflight.py --fetch --json`. |
