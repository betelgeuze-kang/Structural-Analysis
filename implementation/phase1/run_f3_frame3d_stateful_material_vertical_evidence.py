...
def _result_ir(
    *, document: Any, model: Any, reference_load_n: np.ndarray, result: Any, path_history: Any
) -> dict[str, Any]:
    operator_hash = LINEAR._sha_payload(
        {"model_hash": model.model_hash, "solver_contract_hash": result.solver_contract_hash}
    )
    base_plan = create_execution_plan(
        ...
        solver_numeric_buffer_hash=LINEAR._sha_payload(
            {"model_hash": model.model_hash, "reference_load": LINEAR._sha_bytes(reference_load_n.tobytes())}
        ),
        solver_entity_mapping_hash=LINEAR._sha_payload({"nodes": ["N1", "N2"], "elements": ["E1"]}),
    ...
    plan = bind_equation_scaling_to_execution_plan(
        base_plan, scaling, node_coordinates_m=coordinates, reference_equation_load_si=reference_load_n
    )
    reduced = create_execution_plan_reduced_csr(plan, operator_numeric_values_hash=operator_hash)
    ...
    rejected = sum(row.outcome == "rolled_back" for row in result.attempts)
    terminal = create_nonlinear_terminal_receipt(
        ...
        source_solution_data_hash=array_data_hash(immutable_array(state.displacement_si[6:], dtype="<f8")),
    ...
    numerical = create_nonlinear_numerical_result_ir(
        ...
        full_residual_receipt_hash=LINEAR._sha_payload({"final_residual_linf_n": final_residual_n}),
        ...
        backend_receipt_hash=LINEAR._sha_payload({"fallback_count": 0, "regularization_count": 0}),
    )
...
def build_receipt(*, source_commit_sha: str | None = None) -> dict[str, Any]:
    ...
    result = solve_stateful_corotational_frame3d_sparse_load_path(model, LOAD_FACTORS, config=config)
    prefix = solve_stateful_corotational_frame3d_sparse_load_path(model, LOAD_FACTORS[:2], config=config)
    ...
    repeated = solve_stateful_corotational_frame3d_sparse_load_path(model, LOAD_FACTORS, config=config)
    exact_restart = bool(
    ...
    viewer = build_linear_static_viewer_payload(
        ...
        member_forces=[{
            ...
            "nodes": ["N1", "N2"],
            "local_end_forces": {
                label: float(end_forces_n[index]) for index, label in enumerate(FRAME_END_FORCE_LABELS)
            },
        }],
    ...
    expected_displacement_m = 2.0 * (
        float(response["stress_mpa"]) / 200_000.0 + float(material_payload["plastic_strain"])
    )
    ...
    surface_artifacts: dict[str, Any] = {
        ...
        "benchmark": {
            ...
            "plastic_history_nonzero": float(material_payload["accumulated_plastic_strain"]) > 0.0,
            "dissipated_energy_nonzero": float(material_payload["dissipated_energy_density_mj_per_m3"]) > 0.0,
