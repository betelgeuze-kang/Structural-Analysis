...
LINEAR_RUNNER_PATH = ROOT / "implementation/phase1/run_f3_frame3d_linear_vertical_evidence.py"
LINEAR_SPEC = importlib.util.spec_from_file_location("f3_linear_runner", LINEAR_RUNNER_PATH)
...
def _model_and_load() -> tuple[dict[str, Any], CorotationalFrame3DModel, np.ndarray]:
    ...
    weak_pattern = next(row for row in payload["load_patterns"] if row["id"] == "LC_WEAK")
...
def _nonlinear_result_ir(
    *,
    ...
    operator_hash = LINEAR._sha_payload(
        {"model_hash": model.model_hash, "solver_contract": "corotational-load-control.v1"}
    )
    base_plan = create_execution_plan(
        ...
        solver_artifact_hash=LINEAR._sha_payload({"profile": "dense_elastic_corotational_timoshenko_frame3d_load_control.v1"}),
...
def build_receipt(*, source_commit_sha: str | None = None) -> dict[str, Any]:
    ...
    solution = solve_corotational_frame3d_global_load_path(model, load_factors, config=config)
    prefix = solve_corotational_frame3d_global_load_path(model, (0.25, 0.5), config=config)
    ...
    full_residual_kn = assembly.internal_force - np.asarray(model.reference_load_kn, dtype=np.float64)
    final_residual_n = float(np.max(np.abs(full_residual_kn[list(model.free_dofs)])) * 1000.0)
    path_history = [
        {
    ...
    one_shot_repeat = solve_corotational_frame3d_global_load_path(model, load_factors, config=config)
    ...
    all_pass = bool(
        ...
        and all(final.convergence_checks.values())
    )
    surface_artifacts: dict[str, Any] = {
        ...
        "benchmark": {
            ...
            "load_factor_monotonic": [step.load_factor for step in solution.steps] == list(load_factors),
            "full_load_reached": solution.final_checkpoint.load_factor == 1.0,
            "all_convergence_checks_pass": all(final.convergence_checks.values()),
        },
        "platform": {
            "source_commit_sha": source_commit,
