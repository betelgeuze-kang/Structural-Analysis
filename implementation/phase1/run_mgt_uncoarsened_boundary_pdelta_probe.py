...
def _iteration_row(
    *,
    ...
    translations = previous_u.reshape((-1, DOF_PER_NODE))[:, :3] if previous_u.size else np.zeros((0, 3))
    ...
    *,
    ...
    try:
        ...
        payload = {
            ...
            "solve_scope": (
                "uncoarsened_frame_shell_boundary_tangent_deformed_state_pdelta_probe"
            ),
            "unit_policy": {
                "mgt_force_unit": "kN",
                "mgt_length_unit": "m",
            ...
            "claim_boundary": (
                ...
                "tangent into a deformed-coordinate frame-shell P-Delta fixed-point loop. The frame tangent "
                "uses geometric stiffness from frame gravity axial forces and the shell tangent is rebuilt "
                "on the deformed nodal coordinates. It is a nonlinear path diagnostic only; it is not a "
