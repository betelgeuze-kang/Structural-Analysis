#!/usr/bin/env python3
"""Generate native authoring writeback breadth coverage report."""
...
DEFAULT_OUT = DEFAULT_OUT_DIR / "native_authoring_writeback_breadth_report.json"

REASONS = {
    "PASS": "native authoring writeback breadth report generated",
    "CHECK": "native authoring writeback breadth report generated with partial coverage",
...
def build_native_authoring_writeback_breadth_report(
    *,
    ...
    for family_id in family_ids:
        ...
        solver_combo_status = _derive_combo_status(
            solver_combo_count,
            _first_text(runtime_row.get("solver_combo_status"), track_row.get("solver_combo_status")),
        )
        ...
        mesh_breadth_status = _derive_mesh_status(
            ...
            _first_text(runtime_row.get("mesh_breadth_status"), track_row.get("mesh_breadth_status")),
        )
        ...
        full_breadth_ready = bool(
            broad_writeback_ready
            and mesh_breadth_status == "broad"
            and active_family_count >= 2
        )
        writeback_breadth_status = (
            "broad"
            if broad_writeback_ready
            else "targeted"
            if writeback_ready and mesh_coverage_ready
            else "check"
        )
        coverage_axes_label = " | ".join(
            [
        ...
        emitted_rows.append(
            {
                ...
                "writeback_breadth_status": writeback_breadth_status,
                ...
                "broad_writeback_ready": broad_writeback_ready,
                "full_breadth_ready": full_breadth_ready,
                ...
                "coverage_axes_label": coverage_axes_label,
                "summary_line": (
                    f"{family_id}: {writeback_breadth_status.upper()} | "
                    f"combo={solver_combo_status}:{solver_combo_count} | "
                    f"mesh={mesh_breadth_status}:{solver_mesh_request_count} | "
                    f"palette={palette_family_count} | active={active_family_count} | "
                    f"writeback={'yes' if writeback_ready else 'no'}"
                ),
                "artifacts": artifacts,
            }
    ...
    runtime_summary = runtime_submission_report.get("summary") if isinstance(runtime_submission_report, dict) and isinstance(runtime_submission_report.get("summary"), dict) else {}
    queued_submission_count = _first_int(
        runtime_summary.get("queued_submission_count"),
        runtime_summary.get("queue_count"),
    )
    broad_ready_family_count = sum(1 for row in emitted_rows if bool(row.get("broad_writeback_ready", False)))
    full_breadth_family_count = sum(1 for row in emitted_rows if bool(row.get("full_breadth_ready", False)))
    writeback_ready_family_count = sum(1 for row in emitted_rows if bool(row.get("writeback_ready", False)))
    runtime_ready_family_count = sum(1 for row in emitted_rows if bool(row.get("runtime_ready", False)))
    submission_ready_family_count = sum(1 for row in emitted_rows if bool(row.get("submission_ready", False)))
    ...
    mesh_broad_family_count = sum(1 for row in emitted_rows if str(row.get("mesh_breadth_status", "")).strip().lower() == "broad")
    palette_breadth_family_count = sum(1 for row in emitted_rows if int(row.get("palette_family_count", 0) or 0) >= 3)
    active_multi_family_count = sum(1 for row in emitted_rows if int(row.get("active_family_count", 0) or 0) >= 2)
    family_count = len(emitted_rows)
    contract_pass = bool(family_count > 0 and broad_ready_family_count >= family_count and queued_submission_count == 0)
    ...
    summary = {
        ...
        "submission_ready_family_count": int(submission_ready_family_count),
        "writeback_ready_family_count": int(writeback_ready_family_count),
        ...
        "family_status_label": ", ".join(
            f"{row['family_id']}:{row['writeback_breadth_status']}" for row in emitted_rows
        ),
        "writeback_breadth_ready": bool(contract_pass),
    }
    payload = {
        "schema_version": "1.0",
        "run_id": "phase1-native-authoring-writeback-breadth-report",
        ...
        "artifacts": {
            ...
            "native_authoring_writeback_breadth_report_json": str(out),
        },
        "summary_line": (
            f"Native authoring writeback breadth: {'PASS' if contract_pass else 'CHECK'} | "
            f"families={family_count} | broad_ready={broad_ready_family_count} | "
            f"full_breadth={full_breadth_family_count} | combos_broad={combo_broad_family_count} | "
            f"mesh_coverage={mesh_coverage_family_count} | mesh_broad={mesh_broad_family_count} | "
...
def main() -> None:
    ...
    payload = build_native_authoring_writeback_breadth_report(
