#!/usr/bin/env python3
"""Project group-local connection-detailing payloads from active optimization changes."""
...
def _resolve_connection_detailing_element_ids(
    *,
    row: dict[str, Any],
...
def main() -> int:
    p = argparse.ArgumentParser(description="Project group-local connection-detailing payloads from optimization deltas.")
    ...
    reason_code = "PASS"
    reason = "projected internal group-local connection-detailing payloads were generated"
    ...
    if reason_code == "PASS":
        ...
        for row in changes:
            ...
            if action_family != "connection_detailing":
                continue
            ...
            group_id = str(row.get("group_id", "") or "")
            member_type = str(row.get("member_type", "") or "").strip().lower()
            element_ids, mapping_source = _resolve_connection_detailing_element_ids(
                row=row,
    ...
    payload = {
        ...
        "source_kind": "internal_group_local_connection_detailing_payload_projection",
        "summary": {
            "group_local_connection_detailing_payload_row_count": int(len(projected_rows)),
            ...
            "blocked_group_local_connection_detailing_payload_row_count": int(len(blocked_rows)),
        ...
        "reason_code": "PASS" if projected_rows else ("ERR_NO_PROJECTABLE_CONNECTION_DETAILING_GROUPS" if reason_code == "PASS" else reason_code),
        "reason": (
            reason
            if projected_rows
            else ("no connection-detailing groups were projectable" if reason_code == "PASS" else reason)
        ),
        "group_local_connection_detailing_payloads": projected_rows,
        "blocked_group_local_connection_detailing_payload_rows": blocked_rows,
    }
