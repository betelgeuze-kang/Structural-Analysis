...
- Generated at: `2026-03-29T04:23:49.294617+00:00`
...
- Measured accelerated chain wall-clock (comparable rolling N=14): `1.50 min` (range `0.59-5.36 min`)
- Current measured chain wall-clock: `0.91 min`
...
- Solver breadth: `Solver breadth: PASS | shell=yes(elems=5,cases=31) | wall=yes(rows=2,cases=14,material=rc_composite) | interface=yes(ssi_nonlinear_boundary) | contact=full_structural_contact | general_fe_contact=yes(10/10) | surface_interaction=yes(7/7) | interaction_family=yes(211/211) | interaction_sources=10/10 | groups=shell-shell=63/63,shell-wall=64/64,footing-soil=64/64,ssi=5/5,soil-tunnel=4/4,direct-contact=11/11`
...
- Constitutive/interaction families: `material constitutive and surface interaction are surfaced explicitly across the release, committee, and external reports; the summary lines are reused as-is.`
- Material constitutive gate: `Material constitutive gate: PASS | concrete_damage=yes(matrix=43/43,max=1.000) | cyclic_degradation=yes(matrix=42/42,residual_max=1.914%) | bond_interface=yes(matrix=44/44,bond_max=0.980) | matrix=129/129 | groups=concrete_damage=43/43,cyclic_degradation=42/42,bond_interface=44/44 | coverage=cd[t=2,h=2,s=2,sf=14],cyc[t=2,h=2,store=1,sf=14],bond[t=2,h=2,s=2,sf=14]`
- MIDAS KDS row provenance export: `MIDAS KDS row provenance export: PASS | combos=6 | rows=144 | members=12 | clauses=6 | exact_rows=144 | clause_filters=6 | member_filters=12 | reverse_jump=viewer_subset_reverse_jump_v1`
...
- Surface interaction benchmark: `Surface interaction benchmark: PASS | ready=7/7 | family_matrix=211/211 | source_families=10/10 | shell_surface=yes | interface_transfer=yes | interface_gap=yes | foundation=yes | ssi=yes | soil_tunnel=yes | direct_contact=6/6 | groups=shell-shell=63/63,shell-wall=64/64,footing-soil=64/64,ssi=5/5,soil-tunnel=4/4,direct-contact=11/11`
...
- `material_constitutive_summary_line`: `Material constitutive gate: PASS | concrete_damage=yes(matrix=43/43,max=1.000) | cyclic_degradation=yes(matrix=42/42,residual_max=1.914%) | bond_interface=yes(matrix=44/44,bond_max=0.980) | matrix=129/129 | groups=concrete_damage=43/43,cyclic_degradation=42/42,bond_interface=44/44 | coverage=cd[t=2,h=2,s=2,sf=14],cyc[t=2,h=2,store=1,sf=14],bond[t=2,h=2,s=2,sf=14]`
- `midas_kds_row_provenance_export_summary_line`: `MIDAS KDS row provenance export: PASS | combos=6 | rows=144 | members=12 | clauses=6 | exact_rows=144 | clause_filters=6 | member_filters=12 | reverse_jump=viewer_subset_reverse_jump_v1`
...
- `surface_interaction_benchmark_summary_line`: `Surface interaction benchmark: PASS | ready=7/7 | family_matrix=211/211 | source_families=10/10 | shell_surface=yes | interface_transfer=yes | interface_gap=yes | foundation=yes | ssi=yes | soil_tunnel=yes | direct_contact=6/6 | groups=shell-shell=63/63,shell-wall=64/64,footing-soil=64/64,ssi=5/5,soil-tunnel=4/4,direct-contact=11/11`
...
- `mgt_export_audit_review_followup_reference_time_utc`: `2026-03-29T04:07:15.152015+00:00`
...
- Measured accelerated chain wall-clock (comparable rolling N=14): `1.50 min` (range `0.59-5.36 min`)
- Comparable run selection: `current_pipeline_comparable_full_chain_pass` | `full_chain_samples=25` | `comparable_samples=14` | `reference_steps=19` | `overlap_threshold=0.90` | `deployment_model=engineer_in_the_loop_accelerated_coverage` | `strict_smoke=True`
- Current measured chain wall-clock: `0.91 min`
...
- Empirical smoke runtime reduction: `95.93-96.14%` (mean `96.03%`)
- `measured_chain_breakdown_min`: parser/screening `0.16`, analysis/optimization `0.39`, nonseismic/construction `0.12`, authority/crosscheck `0.17`, release/packaging `0.07`
- `measured_chain_breakdown_mean_min`: parser/screening `0.16`, analysis/optimization `0.38`, nonseismic/construction `0.75`, authority/crosscheck `0.15`, release/packaging `0.06`
...
- `runtime_drift`: baseline `1.4129s -> 1.4035s` (`-0.0094s`), trial `0.0565s -> 0.0541s` (`-0.0024s`)
...
- `summary`: `MIDAS KDS row provenance export: PASS | combos=6 | rows=144 | members=12 | clauses=6 | exact_rows=144 | clause_filters=6 | member_filters=12 | reverse_jump=viewer_subset_reverse_jump_v1`
...
- `row-provenance sync`: `the Review surface and appendix stay bidirectionally aligned on the same Hazard and Rule Family slices; appendix rows carry viewer_row_url and viewer_slice_url reverse-sync links back to the matching viewer row and slice.`
