#!/usr/bin/env python3
"""Project registry service scaffold for technically signed project packages.

The registry signature and workflow approvals in this module establish package
integrity only. They are not rights-holder signatures and never grant license,
commercial-use, redistribution, third-party-clearance, or release authority.
"""
...
REASONS = {
    "PASS": "technically signed project registry package generated",
...
SIGNATURE_CLAIM_BOUNDARY = (
    "The project-registry Ed25519 signature verifies package bytes only. The key may be generated by this "
    "tool and is not an approved rights-holder identity or release signing authority."
)
LEGAL_CLAIM_BOUNDARY = (
    "The bundled repository LICENSE is an all-rights-reserved, no-grant notice. No legal, commercial, "
    "redistribution, third-party-clearance, or release authority is established without a cryptographically "
...
def _read_repository_license() -> bytes:
    if REPOSITORY_LICENSE.is_symlink() or not REPOSITORY_LICENSE.is_file():
        raise FileNotFoundError(f"repository LICENSE missing or unsafe: {REPOSITORY_LICENSE}")
    payload = REPOSITORY_LICENSE.read_bytes()
    if not payload:
        raise ValueError("repository LICENSE is empty")
    return payload


def _build_rights_status(*, license_bytes: bytes, generated_at: str) -> dict[str, Any]:
    return {
        "schema_version": "project-package-rights-status.v1",
        ...
        "rights_holder_decision": {
            "canonical_status_path": "implementation/phase1/release/support_bundle/license_status.json",
            "closure_report_path": (
                "implementation/phase1/release_evidence/productization/license_status_closure_report.json"
            ),
            "verified": False,
            "signature_verified": False,
            "exact_source_binding_verified": False,
            "decision_id": "",
            "status": "not_provided",
        },
        "third_party_review": {
            "notice_inventory_complete": False,
            "redistribution_conditions_verified": False,
            "status": "not_established",
        },
        "technical_contract_semantics": TECHNICAL_CONTRACT_SEMANTICS,
        "signature_claim_boundary": SIGNATURE_CLAIM_BOUNDARY,
...
def _build_package_manifest(
    *,
    ...
    return {
        "schema_version": "project-package-manifest.v2",
        "project_id": project_id,
        ...
        "legal_claim_boundary": LEGAL_CLAIM_BOUNDARY,
        "authority": _no_legal_authority(),
        "legal_and_third_party_artifacts": [
            {
                "path": "LICENSE",
                "sha256": _sha256_bytes(license_bytes),
                "bytes": len(license_bytes),
    ...
    *,
    ...
    with zipfile.ZipFile(buffer, mode="w", compression=zipfile.ZIP_STORED) as zf:
        entries: list[tuple[str, bytes]] = [
            ...
            (PACKAGE_RIGHTS_STATUS_NAME, rights_status_bytes),
    ...
    *,
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(package_bytes)) as archive:
            ...
            return len(names) == len(set(names)) and archive.read(name) == expected_bytes
...
def build_project_registry(
    *,
    ...
    rights_status = _build_rights_status(license_bytes=license_bytes, generated_at=timestamp)
    rights_status_bytes = _canonical_bytes(rights_status)
    ...
    registry_body = {
        ...
        "approval_semantics": "technical_project_workflow_only",
        "technical_contract_semantics": TECHNICAL_CONTRACT_SEMANTICS,
        "signature_claim_boundary": SIGNATURE_CLAIM_BOUNDARY,
        "legal_claim_boundary": LEGAL_CLAIM_BOUNDARY,
        "authority": _no_legal_authority(),
    ...
    checks = {
        ...
        "rights_status_packaged_pass": _package_entry_matches(
            written_package_bytes,
            name=PACKAGE_RIGHTS_STATUS_NAME,
            expected_bytes=rights_status_bytes,
        ),
        "legal_authority_fail_closed_pass": (
            package_manifest.get("authority") == _no_legal_authority()
            and rights_status.get("authority") == _no_legal_authority()
            and rights_status.get("rights_holder_decision", {}).get("verified") is False
    ...
    technical_contract_pass = bool(reason_code == "PASS" and all(checks.values()))
    ...
    payload = {
        ...
        "contract_pass": technical_contract_pass,
...
def build_project_registry_index(
    *,
    ...
    payload = {
        ...
        "technical_contract_pass": bool(contract_pass),
    ...
    if workspace_out is not None:
        workspace_payload = {
            ...
            "technical_contract_pass": payload["technical_contract_pass"],
