...
from implementation.phase1.pdf_rendering import (  # noqa: E402
...
def _registry_accelerated_coverage_summary(snapshot_dir: Path) -> dict:
    ...
    accel = (
        body.get("accelerated_coverage_provenance")
    ...
    panel_zone_validation_boundary = str(
        summary.get("panel_zone_validation_boundary")
        or accel.get("panel_zone_validation_boundary")
        or ""
    )
    panel_zone_validation_advisory_only = bool(
        ...
        if "panel_zone_validation_advisory_only" in summary
            ...
            "panel_zone_validation_advisory_only",
    ...
    panel_zone_validation_advisory_label = str(
        ...
        or (
            "panel_zone_external_validation_only_boundary"
            if panel_zone_validation_advisory_only
            else "none"
        )
    )
    ...
    return {
        ...
        "measured_chain_comparable_reference_strict_design_opt_cost_smoke": bool(
            ...
            if "measured_chain_comparable_reference_strict_design_opt_cost_smoke"
            in summary
        ...
        "mgt_export_support_mode": str(
            summary.get("mgt_export_support_mode")
            or accel.get("mgt_export_support_mode")
            or ""
        ),
        ...
        "mgt_export_audit_review_packet_file_count": int(
            ...
            else accel.get("mgt_export_audit_review_packet_file_count", 0) or 0
        ),
        ...
        "mgt_export_audit_review_queue_item_count": int(
            ...
            else accel.get("mgt_export_audit_review_queue_item_count", 0) or 0
        ),
        "mgt_export_audit_review_queue_pending_count": int(
            ...
            else accel.get("mgt_export_audit_review_queue_pending_count", 0) or 0
        ),
        "mgt_export_audit_review_queue_acknowledged_count": int(
            ...
            else accel.get("mgt_export_audit_review_queue_acknowledged_count", 0) or 0
        ),
        ...
        "mgt_export_audit_review_followup_item_count": int(
            ...
            else accel.get("mgt_export_audit_review_followup_item_count", 0) or 0
        ),
        "mgt_export_audit_review_followup_open_item_count": int(
            ...
            else accel.get("mgt_export_audit_review_followup_open_item_count", 0) or 0
        ),
        "mgt_export_audit_review_followup_closed_item_count": int(
            ...
            else accel.get("mgt_export_audit_review_followup_closed_item_count", 0) or 0
        ),
        ...
        "mgt_export_audit_review_resolution_item_count": int(
            ...
            else accel.get("mgt_export_audit_review_resolution_item_count", 0) or 0
        ),
        ...
        "audit_review_decision_batch_template_item_count": int(
            ...
            else accel.get("audit_review_decision_batch_template_item_count", 0) or 0
        ),
        ...
        "audit_review_decision_batch_attested_example_count": int(
            ...
            else accel.get("audit_review_decision_batch_attested_example_count", 0) or 0
        ),
        ...
        "external_benchmark_submission_preview_approve_all_pending_count": int(
            ...
            if "external_benchmark_submission_preview_approve_all_pending_count"
            in summary
            else accel.get(
        ...
        "external_benchmark_submission_preview_approve_all_open_revision_count": int(
            ...
            if "external_benchmark_submission_preview_approve_all_open_revision_count"
            in summary
            else accel.get(
                "external_benchmark_submission_preview_approve_all_open_revision_count",
                0,
        ...
        "external_benchmark_submission_preview_reject_one_pending_count": int(
            summary.get(
                "external_benchmark_submission_preview_reject_one_pending_count"
            )
            if "external_benchmark_submission_preview_reject_one_pending_count"
            in summary
            else accel.get(
        ...
        "external_benchmark_submission_preview_reject_one_open_revision_count": int(
            summary.get(
                "external_benchmark_submission_preview_reject_one_open_revision_count"
            )
            if "external_benchmark_submission_preview_reject_one_open_revision_count"
            in summary
            else accel.get(
                "external_benchmark_submission_preview_reject_one_open_revision_count",
                0,
        ...
        "external_benchmark_submission_preview_reject_one_blocker_label": str(
            summary.get(
                "external_benchmark_submission_preview_reject_one_blocker_label"
            )
            or accel.get(
                "external_benchmark_submission_preview_reject_one_blocker_label"
            )
        ...
        "audit_review_decision_batch_runner_preview_open_revision_count": int(
            summary.get(
                "audit_review_decision_batch_runner_preview_open_revision_count"
            )
            if "audit_review_decision_batch_runner_preview_open_revision_count"
            in summary
            else accel.get(
        ...
        "mgt_export_rebar_payload_material_level_namespace_present": bool(
            ...
            if "mgt_export_rebar_payload_material_level_namespace_present" in summary
                ...
                "mgt_export_rebar_payload_material_level_namespace_present", False
            )
        ),
        ...
        "mgt_export_group_local_connection_detailing_payload_row_count": int(
            ...
            if "mgt_export_group_local_connection_detailing_payload_row_count"
            in summary
            else accel.get(
                "mgt_export_group_local_connection_detailing_payload_row_count", 0
            )
        ),
        "mgt_export_group_local_connection_detailing_payload_available_count": int(
            ...
            if "mgt_export_group_local_connection_detailing_payload_available_count"
            in summary
            else accel.get(
                "mgt_export_group_local_connection_detailing_payload_available_count", 0
            )
        ),
        ...
        "mgt_export_group_local_detailing_payload_available_count": int(
            ...
            if "mgt_export_group_local_detailing_payload_available_count" in summary
                ...
                "mgt_export_group_local_detailing_payload_available_count", 0
            )
        ),
        ...
        "mgt_export_connection_detailing_payload_group_local_namespace_present": bool(
            summary.get(
                "mgt_export_connection_detailing_payload_group_local_namespace_present"
            )
            if "mgt_export_connection_detailing_payload_group_local_namespace_present"
            in summary
            else accel.get(
                "mgt_export_connection_detailing_payload_group_local_namespace_present",
                False,
        ...
        "mgt_export_detailing_payload_group_local_namespace_present": bool(
            ...
            if "mgt_export_detailing_payload_group_local_namespace_present" in summary
                ...
                "mgt_export_detailing_payload_group_local_namespace_present", False
            )
        ),
        "mgt_export_connection_detailing_structured_payload_mapped_change_count": int(
            ...
            if "mgt_export_connection_detailing_structured_payload_mapped_change_count"
            in summary
            else accel.get(
                "mgt_export_connection_detailing_structured_payload_mapped_change_count",
                0,
        ...
        "mgt_export_connection_detailing_direct_patch_eligible_change_count": int(
            summary.get(
                "mgt_export_connection_detailing_direct_patch_eligible_change_count"
            )
            if "mgt_export_connection_detailing_direct_patch_eligible_change_count"
            in summary
            else accel.get(
                "mgt_export_connection_detailing_direct_patch_eligible_change_count", 0
            )
        ),
        "mgt_export_detailing_structured_payload_mapped_change_count": int(
            ...
            if "mgt_export_detailing_structured_payload_mapped_change_count" in summary
                ...
                "mgt_export_detailing_structured_payload_mapped_change_count", 0
            )
        ),
        ...
        "pbd_hinge_state_mode": str(
            summary.get("pbd_hinge_state_mode")
            or accel.get("pbd_hinge_state_mode")
            or ""
        ),
        "pbd_hinge_refresh_reason": str(
            summary.get("pbd_hinge_refresh_reason")
            or accel.get("pbd_hinge_refresh_reason")
            or ""
        ),
        ...
        "pbd_hinge_refresh_artifact_kind": str(
            summary.get("pbd_hinge_refresh_artifact_kind")
            or accel.get("pbd_hinge_refresh_artifact_kind")
            or ""
        ),
        "pbd_hinge_refresh_source_mode": str(
            summary.get("pbd_hinge_refresh_source_mode")
            or accel.get("pbd_hinge_refresh_source_mode")
            or ""
        ),
        ...
        "pbd_resolved_ndtha_report": str(
            summary.get("pbd_resolved_ndtha_report")
            or accel.get("pbd_resolved_ndtha_report")
            or ""
        ),
        ...
        "panel_zone_constructability_mode": str(
            summary.get("panel_zone_constructability_mode")
            or accel.get("panel_zone_constructability_mode")
            or ""
        ),
        "panel_zone_constructability_reason": str(
            summary.get("panel_zone_constructability_reason")
            or accel.get("panel_zone_constructability_reason")
            or ""
        ),
        ...
        "panel_zone_source_artifact_kind": str(
            summary.get("panel_zone_source_artifact_kind")
            or accel.get("panel_zone_source_artifact_kind")
            or ""
        ),
        "panel_zone_source_artifact_path": str(
            summary.get("panel_zone_source_artifact_path")
            or accel.get("panel_zone_source_artifact_path")
            or ""
        ),
        "panel_zone_source_contract_mode": str(
            summary.get("panel_zone_source_contract_mode")
            or accel.get("panel_zone_source_contract_mode")
            or ""
        ),
        ...
        "panel_zone_source_valid_row_counts": dict(
            ...
            else accel.get("panel_zone_source_valid_row_counts", {}) or {}
        ),
        "panel_zone_source_overlap_member_counts": dict(
            ...
            else accel.get("panel_zone_source_overlap_member_counts", {}) or {}
        ),
        "panel_zone_source_candidate_scan_modes": dict(
            ...
            else accel.get("panel_zone_source_candidate_scan_modes", {}) or {}
        ),
        ...
        "foundation_optimization_mode": str(
            summary.get("foundation_optimization_mode")
            or accel.get("foundation_optimization_mode")
            or ""
        ),
        "foundation_optimization_reason": str(
            summary.get("foundation_optimization_reason")
            or accel.get("foundation_optimization_reason")
            or ""
        ),
        "foundation_scope_source": str(
            summary.get("foundation_scope_source")
            or accel.get("foundation_scope_source")
            or ""
        ),
        "foundation_artifact_scan_mode": str(
            summary.get("foundation_artifact_scan_mode")
            or accel.get("foundation_artifact_scan_mode")
            or ""
        ),
        ...
        "upstream_foundation_provenance_mode": str(
            summary.get("upstream_foundation_provenance_mode")
            or accel.get("upstream_foundation_provenance_mode")
            or ""
        ),
        ...
        "wind_tunnel_mapping_mode": str(
            summary.get("wind_tunnel_mapping_mode")
            or accel.get("wind_tunnel_mapping_mode")
            or ""
        ),
        "wind_tunnel_mapping_reason": str(
            summary.get("wind_tunnel_mapping_reason")
            or accel.get("wind_tunnel_mapping_reason")
            or ""
        ),
    }


def _load_committee_hold_context(
    snapshot_dir: Path, release_dir: Path
) -> tuple[dict, dict]:
    snapshot_summary = (
    ...
    diff = (
        summary.get("authority_catalog_routing_diff")
        if isinstance(summary.get("authority_catalog_routing_diff"), dict)
        else {}
    )
    if not diff:
        snapshot_diff = (
            ...
            / "release"
            / "committee_review"
            / "authority_catalog_routing_diff.json"
        )
        live_diff = (
            release_dir / "committee_review" / "authority_catalog_routing_diff.json"
        )
...
def _review_packet_rows(
    authority_catalog_diff: dict, residual_holdout_matrix_rows: list[dict]
) -> list[dict]:
    diff_rows = [
        row
...
def _write_hold_review_packet_markdown(path: Path, payload: dict) -> None:
    ...
    rows = [
        row
        for row in (payload.get("review_packet_rows") or [])
        if isinstance(row, dict)
    ]
    checklist = [
        row for row in (payload.get("review_checklist") or []) if isinstance(row, str)
    ]
    signoff = (
        payload.get("licensed_engineer_signoff")
        if isinstance(payload.get("licensed_engineer_signoff"), dict)
        else {}
    )
    evidence = (
        payload.get("clearance_evidence")
        if isinstance(payload.get("clearance_evidence"), dict)
        else {}
    )
    lines = [
        "# Hold Review Packet",
...
def _write_hold_review_packet_pdf(path: Path, payload: dict) -> None:
    ...
    with PdfPages(path) as pdf:
        ...
        header_rows = [
            ...
            (
                "Authority diff count",
                str(payload.get("authority_catalog_diff_change_count", 0)),
            ),
            (
                "Residual matrix rows",
                str(payload.get("residual_holdout_matrix_row_count", 0)),
            ),
        ...
        for key, value in header_rows:
            ...
            ax.text(
                0.33, y, str(value), fontsize=10.7, color="#1f1a16", va="top", wrap=True
            )
            y -= 0.055
        ...
        if checklist:
            ...
            ax.axis("off")
            ax.text(
                0.03, 0.96, "Review Checklist", fontsize=18, weight="bold", va="top"
            )
            y = 0.88
            for item in checklist:
        ...
        if rows:
            ...
            ax.axis("off")
            ax.text(
                0.03, 0.96, "Review Packet Rows", fontsize=18, weight="bold", va="top"
            )
            y = 0.90
            for row in rows:
                ax.text(
                ...
                ax.text(
                    0.06,
                    y,
                    f"owner={row.get('owner', '')} | why={row.get('why', '')}",
                    fontsize=8.8,
                    va="top",
                    wrap=True,
                )
...
def _build_hold_review_ack(
    *,
    ...
    prior_engineer_ack = (
        prior.get("engineer_ack") if isinstance(prior.get("engineer_ack"), dict) else {}
    )
    prior_clearance = (
        prior.get("hold_clearance")
        if isinstance(prior.get("hold_clearance"), dict)
        else {}
    )
    prior_signoff = (
        prior.get("licensed_engineer_signoff")
        if isinstance(prior.get("licensed_engineer_signoff"), dict)
        else {}
    )
    prior_evidence = (
        prior.get("clearance_evidence")
        if isinstance(prior.get("clearance_evidence"), dict)
        else {}
    )
    ...
    event = {
        ...
        "authority_catalog_diff_change_count": int(
            effective_authority_catalog_diff.get("change_count", 0)
        ),
    }
    ...
    if review_required:
        ...
        prior_pending_clearance = str(prior_clearance.get("status", "")) in {
            "pending_clearance",
            "clear_ready_for_record",
        }
        engineer_ack = {
            "status": "not_required"
            if not prior_engineer_ack
            else str(prior_engineer_ack.get("status", "not_required")),
            "acknowledged_by": str(prior_engineer_ack.get("acknowledged_by", "")),
            "acknowledged_at": str(prior_engineer_ack.get("acknowledged_at", "")),
            "notes": str(prior_engineer_ack.get("notes", "")),
        }
        hold_clearance = {
            "status": (
                ...
                if prior_pending_clearance
                    ...
                    "clear_not_required"
                    if not prior_clearance
                    else str(prior_clearance.get("status", "clear_not_required"))
                )
            ),
            "cleared_by": str(prior_clearance.get("cleared_by", "")),
            "cleared_at": str(prior_clearance.get("cleared_at", "")),
        ...
        licensed_engineer_signoff = {
            "status": "not_required"
            if not prior_signoff
            else str(prior_signoff.get("status", "not_required")),
            "signer_name": str(prior_signoff.get("signer_name", "")),
            "signer_license_id": str(prior_signoff.get("signer_license_id", "")),
            "signed_at": str(prior_signoff.get("signed_at", "")),
            "signature_reference": str(prior_signoff.get("signature_reference", "")),
...
def main() -> None:
    ...
    p.add_argument(
        "--nightly-pointer",
        default="implementation/phase1/release/phase3_nightly_latest.json",
    )
    p.add_argument("--pr-ci", default="implementation/phase1/ci_gate_report.json")
    p.add_argument(
        "--out",
        default="implementation/phase1/release/release_candidate_promotion_report.json",
    )
    p.add_argument(
        "--latest-rc-pointer",
        default="implementation/phase1/release/release_candidate_latest.json",
    )
    p.add_argument(
        "--hold-review-manifest",
        default="implementation/phase1/release/hold_review_manifest.json",
    )
    p.add_argument(
        "--hold-review-packet-md",
        default="implementation/phase1/release/hold_review_packet.md",
    )
    p.add_argument(
        "--hold-review-packet-pdf",
        default="implementation/phase1/release/hold_review_packet.pdf",
    )
    p.add_argument(
        "--hold-review-ack-json",
        default="implementation/phase1/release/hold_review_ack.json",
    )
    args = p.parse_args()
    ...
    nightly_commercial_readiness_ok, nightly_commercial_readiness_reason = (
        _report_green(nightly_commercial_readiness)
    )
    nightly_real_source_multi_ok, nightly_real_source_multi_reason = _report_green(
        nightly_real_source_multi
    )
    nightly_nonlinear_engine_ok, nightly_nonlinear_engine_reason = _report_green(
        nightly_nonlinear_engine
    )
    nightly_pushover_stress_ok, nightly_pushover_stress_reason = _report_green(
        nightly_pushover_stress
    )
    nightly_ndtha_stress_ok, nightly_ndtha_stress_reason = _report_green(
        nightly_ndtha_stress
    )
    ...
    committee_summary, authority_catalog_diff = _load_committee_hold_context(
    ...
    if snapshot_manifest.exists():
        ...
        policy = (
            ...
            if isinstance(manifest.get("release_policy"), dict)
            else {}
        )
    ...
    authority_hold_for_review = bool(
        int(accelerated_coverage.get("authority_catalog_diff_change_count", 0) or 0) > 0
    )
    ...
    report = {
        ...
        "pbd_response_source": {
            "resolved_ndtha_report": str(
                accelerated_coverage.get("pbd_resolved_ndtha_report", "") or ""
            ),
            "resolved_ndtha_response_npz": str(
                accelerated_coverage.get("pbd_resolved_ndtha_response_npz", "") or ""
            ),
            "fallback_used": bool(
                accelerated_coverage.get("pbd_ndtha_response_fallback_used", False)
            ),
            "coverage_count": int(
                accelerated_coverage.get("pbd_ndtha_response_coverage_count", 0) or 0
            ),
            "label": str(
                accelerated_coverage.get("pbd_response_source_label", "") or ""
            ),
        },
    ...
    effective_authority_catalog_diff = _effective_authority_catalog_diff(
        authority_catalog_diff, accelerated_coverage
    )
    residual_holdout_matrix_rows = [
        row
        for row in (committee_summary.get("residual_holdout_matrix_rows") or [])
        if isinstance(row, dict)
    ]
    review_packet_rows = _review_packet_rows(
        effective_authority_catalog_diff, residual_holdout_matrix_rows
    )
    hold_manifest = {
        "schema_version": "1.0",
        ...
        "recommended_next_step": "hold_release_candidate"
        if authority_hold_for_review
        else "promotion_may_proceed",
        "reason_code": reason_code,
        "reason": reason,
        "pbd_response_source": {
            "resolved_ndtha_report": str(
        ...
        "authority_catalog_diff_added_count": int(
            effective_authority_catalog_diff.get("added_count", 0)
        ),
        "authority_catalog_diff_removed_count": int(
            effective_authority_catalog_diff.get("removed_count", 0)
        ),
        "authority_catalog_diff_baseline_seeded": bool(
            effective_authority_catalog_diff.get("baseline_seeded", False)
        ),
        "committee_summary_path": str(
            (snapshot_dir / "release" / "committee_review" / "committee_summary.json")
        ),
        "review_checklist": _review_checklist(bool(authority_hold_for_review)),
        "review_rows": [
            row
            for row in (effective_authority_catalog_diff.get("diff_rows") or [])
            if isinstance(row, dict)
        ],
        "review_packet_rows": review_packet_rows,
    ...
    hold_review_manifest_path.write_text(
        json.dumps(hold_manifest, indent=2), encoding="utf-8"
    )
    ...
    hold_review_ack_path.write_text(
        json.dumps(hold_review_ack, indent=2), encoding="utf-8"
    )
    hold_review_packet_payload = {
        **hold_manifest,
        "licensed_engineer_signoff": hold_review_ack.get(
            "licensed_engineer_signoff", {}
        ),
        "clearance_evidence": hold_review_ack.get("clearance_evidence", {}),
    }
    _write_hold_review_packet_markdown(
        hold_review_packet_md_path, hold_review_packet_payload
    )
    _write_hold_review_packet_pdf(
        hold_review_packet_pdf_path, hold_review_packet_payload
    )
    ...
    if contract_pass:
        latest_rc = {
            ...
            "pbd_response_source": {
                ...
                "resolved_ndtha_response_npz": str(
                    accelerated_coverage.get("pbd_resolved_ndtha_response_npz", "")
                    or ""
                ),
                "fallback_used": bool(
                    accelerated_coverage.get("pbd_ndtha_response_fallback_used", False)
                ...
                "coverage_count": int(
                    accelerated_coverage.get("pbd_ndtha_response_coverage_count", 0)
                    or 0
                ),
                "label": str(
                    accelerated_coverage.get("pbd_response_source_label", "") or ""
