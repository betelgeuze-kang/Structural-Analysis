...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="implementation/phase1/soil_tunnel_ssi_report.json",
    )
    parser.add_argument(
        "--soil-impedance-table",
        default="implementation/phase1/soil_impedance_table.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out", default="implementation/phase1/foundation_soil_link_gate_report.json"
    )
    ...
    try:
        validate_input_contract(
            input_payload, INPUT_SCHEMA, label="phase1.run_foundation_soil_link_gate"
        )
        ...
        structural_contact_validation_path = Path(
            args.structural_contact_validation_report
        )
        ...
        foundation_checks = (
            foundation_report.get("checks")
            if isinstance(foundation_report.get("checks"), dict)
            else {}
        )
        foundation_summary = (
            foundation_report.get("summary")
            if isinstance(foundation_report.get("summary"), dict)
            else {}
        )
        foundation_artifact_summary = (
            foundation_artifact.get("summary")
            if isinstance(foundation_artifact.get("summary"), dict)
            else {}
        )
        ssi_checks = (
            ssi_boundary.get("checks")
            if isinstance(ssi_boundary.get("checks"), dict)
            else {}
        )
        soil_tunnel_checks = (
            soil_tunnel.get("checks")
            if isinstance(soil_tunnel.get("checks"), dict)
            else {}
        )
        ...
        missing_foundation_support_models = [
            name
            for name in required_foundation_support_models
            if name not in foundation_support_model_types
        ]
        ...
        for row in list(foundation_support_catalog.values()) + list(
            device_catalog.values()
        ):
            ...
            if not family:
                continue
            derived_search_family_counts[family] = int(
        ...
        support_search_family_types = _sorted_string_union(
            ...
            [
                ...
                for row in list(foundation_support_catalog.values())
        ...
        for row in list(foundation_support_catalog.values()) + list(
            ...
            if mode:
                derived_search_surface_mode_counts[mode] = int(
        ...
        try:
            summary_support_depth_score = int(
                structural_contact_summary.get("support_depth_score", 0) or 0
            )
        except Exception:
            summary_support_depth_score = 0
        support_depth_score = max(
            summary_support_depth_score, derived_support_depth_score
        )
        ...
        try:
            explicit_contact_family_count = int(
                structural_contact_summary.get("contact_family_count", 0) or 0
            )
        except Exception:
        ...
        missing_link_models = [
            name for name in required_link_models if name not in link_model_types
        ]
        ...
        foundation_artifact_ready = bool(
            ...
            and int(
                foundation_artifact_summary.get("optimized_foundation_group_count", 0)
                or 0
            )
            > 0
        )
        ...
        impedance_schema_ready = bool(
            soil_impedance_text
            and all(token in soil_impedance_text for token in impedance_schema_tokens)
        )
        ...
        checks = {
            ...
            "foundation_support_model_surface_ready": bool(
                not missing_foundation_support_models
            ),
            ...
            "contact_family_surface_ready": bool(
                structural_contact_checks.get(
                    "contact_family_surface_pass",
                    contact_family_count >= len(required_link_models),
                )
            ),
            ...
            "node_to_surface_proxy_ready": bool(
                structural_contact_checks.get(
                    "node_to_surface_proxy_pass",
                    bool(node_to_surface_proxy_model_types),
                )
            ),
        ...
        if (
            not checks["foundation_scope_ready"]
            or not checks["foundation_artifact_ready"]
        ):
        ...
        summary = {
            "foundation_member_type_count": int(
                foundation_summary.get("foundation_member_type_count", 0) or 0
            ),
            ...
            "foundation_scope_source": str(
                foundation_summary.get("foundation_scope_source", "") or ""
            ),
            "foundation_optimization_mode": str(
                foundation_summary.get("optimization_mode", "") or ""
            ),
        ...
        payload = {
            ...
            "inputs": {
                **input_payload,
                "input_sha256": {
                    "foundation_optimization_report": _sha256(foundation_report_path)
                    if foundation_report_path.exists()
                    else "",
                    "foundation_optimization_artifact": _sha256(
                        foundation_artifact_path
                    )
                    if foundation_artifact_path.exists()
                    else "",
                    "ssi_boundary_report": _sha256(ssi_boundary_path)
                    if ssi_boundary_path.exists()
                    else "",
                    "soil_tunnel_ssi_report": _sha256(soil_tunnel_path)
                    if soil_tunnel_path.exists()
                    else "",
                    "soil_impedance_table": _sha256(soil_impedance_path)
                    if soil_impedance_path.exists()
                    else "",
                    "structural_contact_validation_report": _sha256(
                        structural_contact_validation_path
                    )
