...
def _model() -> tuple[Any, StatefulCorotationalFrame3DSparseModel, np.ndarray]:
    ...
    steel = BilinearCombinedHardeningSteel(
        elastic_modulus_mpa=float(
            payload["materials"][0]["parameters"]["elastic_modulus_pa"]
        )
        / 1.0e6,
...
def _accepted_checkpoint(step: Any) -> Any:
    checkpoint = step.accepted_checkpoint
    if not step.committed or checkpoint is None:
        raise RuntimeError("f3_direct_control_step_not_committed")
    return checkpoint
...
def _convergence_checks(step: Any) -> dict[str, bool]:
    ...
    alphas = _accepted_line_search_alphas(step)
    return {
        "scaled_residual_gate": bool(step.residual_gate_passed),
        "scaled_control_gate": bool(step.control_gate_passed),
        "scaled_increment_gate": bool(
            metrics["scaled_increment"] <= metrics["scaled_increment_tolerance"]
        ),
        "load_factor_increment_gate": bool(
            abs(metrics["load_factor_increment"])
            <= metrics["load_factor_increment_tolerance"]
        ),
        "line_search_step_valid": bool(
            alphas and all(0.0 < alpha <= 1.0 for alpha in alphas)
        ),
        "material_admissibility": bool(
            metrics["accepted_trial_material_admissibility_passed"]
        ),
        "final_reassembled_equilibrium": bool(step.final_reassembly_binding),
        "parent_state_immutable": bool(step.parent_state_immutable),
        "augmented_sparse_diagnostic_pass": bool(step.sparse_diagnostic_passed),
        "regularization_not_used": not bool(metrics["regularization_used"]),
        "fallback_not_used": not bool(metrics["fallback_used"]),
    }


def _member_recovery(step: Any) -> dict[str, Any]:
    responses = step.solution.final_assembly.frame_assembly.member_responses
    if len(responses) != 1:
        raise RuntimeError("f3_direct_control_member_recovery_invalid")
    return responses[0].recovery_manifest()
    ...
    *,
    ...
    operator_hash = LINEAR._sha_payload(
        {
            ...
            "direct_control_contract_hash": result.solver_contract_hash,
        }
    )
    base_plan = create_execution_plan(
        ...
        solver_artifact_hash=LINEAR._sha_payload(
            {"profile": result.profile, "contract_hash": result.solver_contract_hash}
        ),
    ...
    bundle = create_initial_material_state_bundle(
        ...
        entries=tuple(
            _material_entry(row) for row in result.initial_checkpoint.material_states
        ),
    )
    for index, step in enumerate(result.steps, start=1):
        checkpoint = _accepted_checkpoint(step)
        displacement = np.asarray(checkpoint.displacement, dtype="<f8")
        trial_state = open_trial_state(
            state,
            displacement,
            load_step=index,
            iteration=max(int(checkpoint.converged_iterations), 1),
        ...
        trial_bundle = open_trial_material_state_bundle(
            ...
            entries=tuple(_material_entry(row) for row in checkpoint.material_states),
        )
...
def build_receipt(*, source_commit_sha: str | None = None) -> dict[str, Any]:
    ...
    exact_restart = bool(
        ...
        and _member_recovery(result.steps[-1]) == _member_recovery(resumed.steps[-1])
    )
    deterministic = repeated.result_hash == result.result_hash
    path_history = [
        {
            "target_control_displacement_m": step.target_control_coordinate,
            "solved_load_factor": _accepted_checkpoint(step).load_factor,
            "checkpoint_hash": _accepted_checkpoint(step).checkpoint_hash,
            "scaled_control_error": (
                step.solution.final_assembly.equivalent_control_error_m
                / step.solution.final_assembly.control_reference_m
            ),
    ...
    reactions_n = (
        np.asarray(
            final.solution.final_assembly.frame_assembly.reactions,
            dtype="<f8",
        )
        * 1000.0
    )
    residual_n = np.zeros(12, dtype="<f8")
    member = _member_recovery(final)
    ...
    factor_error = abs(
        float(result.final_checkpoint.load_factor) - expected_final_factor
    )
    final_residual_n = float(result.final_checkpoint.residual_inf_norm_kn * 1000.0)
    all_pass = bool(
        ...
        and exact_restart
        and deterministic
        and all(step.parent_state_immutable for step in result.steps)
    ...
    surface_artifacts: dict[str, Any] = {
        ...
        "checkpoint": {
            ...
            "parent_state_immutability_enforced": all(
                step.parent_state_immutable for step in result.steps
            ),
        },
        "workbench": {
        ...
        "benchmark": {
            ...
            "exact_reversal_path": result.final_checkpoint.displacement[
                CONTROL_GLOBAL_DOF
            ]
            == TARGETS_M[-1],
        },
