...
from implementation.phase1.special_link_library import (  # noqa: E402
    BearingLink,
    CompressionOnlyLink,
    FrictionLink,
    GapLink,
...
def _validate_gap() -> dict[str, object]:
    ...
    return {
        ...
        "checks": {
            "open_gap_zero_force": bool(
                not open_state.engaged and _close(open_state.force, 0.0)
            ),
        ...
        "sample_points": {
            "open": {
                "displacement": 0.002,
                "force": open_state.force,
                "state": open_state.state_label,
            },
            "closed": {
                "displacement": 0.006,
                "force": closed_state.force,
                "state": closed_state.state_label,
            },
        },
...
def _validate_uplift() -> dict[str, object]:
    ...
    return {
        ...
        "implementation_class": type(link).__name__,
        "checks": {
            "uplift_releases_contact": bool(
                not uplifted.engaged and _close(uplifted.force, 0.0)
            ),
            "seating_restores_compression": bool(
                seated.engaged and _close(seated.force, expected_force)
            ),
        },
        "sample_points": {
            "uplifted": {
                "displacement": 0.003,
                "force": uplifted.force,
                "state": uplifted.state_label,
            },
            "seated": {
                "displacement": -0.002,
                "force": seated.force,
                "state": seated.state_label,
            },
        },
...
def _validate_compression_only() -> dict[str, object]:
    ...
    return {
        ...
        "implementation_class": type(link).__name__,
        "checks": {
            "tension_releases": bool(
                not released.engaged and _close(released.force, 0.0)
            ),
            "compression_carries_force": bool(
                compressed.engaged and _close(compressed.force, expected_force)
            ),
        },
        "sample_points": {
            "released": {
                ...
                "state": released.state_label,
            },
            "compressed": {
                "displacement": 0.003,
                "force": compressed.force,
                "state": compressed.state_label,
            },
        },
...
def _validate_bearing() -> dict[str, object]:
    link = BearingLink(
        elastic_stiffness=1.2e5, yield_force=180.0, post_yield_ratio=0.12
    )
    ...
    return {
        ...
        "checks": {
            ...
            "post_yield_branch_active": bool(
                post.state_label == "post-yield" and _close(post.force, expected_post)
            ),
        },
        "sample_points": {
            "elastic": {
                "displacement": 0.001,
                "force": elastic.force,
                "state": elastic.state_label,
            },
            "post_yield": {
                "displacement": 0.004,
                "force": post.force,
                "state": post.state_label,
            },
        },
...
def _validate_friction() -> dict[str, object]:
    link = FrictionLink(
        tangential_stiffness=6.0e4, friction_coefficient=0.42, normal_force=320.0
    )
    ...
    return {
        ...
        "checks": {
            ...
            "slip_branch_capped": bool(
                _close(abs(slip.force), limit) and _close(slip.tangent, 0.0)
            ),
        },
        "sample_points": {
            "stick": {
                "displacement": 0.001,
                "force": stick.force,
                "state": stick.state_label,
            },
            "slip": {
                "displacement": 0.004,
                "force": slip.force,
                "state": slip.state_label,
            },
        },
...
def _validate_pounding() -> dict[str, object]:
    ...
    return {
        ...
        "implementation_class": type(link).__name__,
        "checks": {
            "pre_impact_open": bool(
                not separated.engaged and _close(separated.force, 0.0)
            ),
        ...
        "sample_points": {
            "separated": {
                "displacement": 0.002,
                "velocity": 0.4,
                "force": separated.force,
                "state": separated.state_label,
            },
            "impact": {
                "displacement": 0.006,
                "velocity": 0.5,
                "force": impact.force,
                "state": impact.state_label,
            },
        },
...
def _validate_event_sequence() -> dict[str, object]:
    ...
    trace = [
        {
            ...
            "impact_disp": 0.002,
            "impact_vel": 0.2,
            "expected_uplift": "uplifted",
            "expected_pounding": "separated",
        },
        {
            "step": 2,
            "seat_disp": -0.001,
    ...
    for row in trace:
        ...
        pounding_state = pounding.evaluate(
            row["impact_disp"], velocity=row["impact_vel"]
        ).state_label
        row_mismatch = int(uplift_state != row["expected_uplift"]) + int(
            pounding_state != row["expected_pounding"]
        )
...
def _build_support_search_surface(
    *,
    ...
    foundation_rows = [
        row for row in foundation_catalog.values() if isinstance(row, dict)
    ]
    ...
    search_ready_model_types = sorted(
        {
            ...
            if bool(row.get("support_search_ready", False))
            and str(row.get("link_name", "")).strip()
        }
    )
    ...
    for row in support_rows:
        ...
        if mode:
            search_surface_mode_counts[mode] = int(
                search_surface_mode_counts.get(mode, 0) + 1
            )
        if family:
            ...
            evidence_rows.append(
                {
                    ...
                    "node_to_surface_proxy": bool(
                        row.get("node_to_surface_proxy", False)
                    ),
                    "support_search_ready": bool(
                        row.get("support_search_ready", False)
                    ),
                    "sample_probe_state": str(row.get("sample_probe_state", "")),
                    "sample_probe_engaged": bool(
                        row.get("sample_probe_engaged", False)
                    ),
                }
            )

    contact_search_surface_types = sorted(set(link_model_types))
    search_surface_ready = bool(
        search_ready_model_types and contact_search_surface_types
    )
...
def build_structural_contact_validation_payload(out_path: str) -> dict[str, object]:
    ...
    validated_count = sum(
        1 for row in categories.values() if bool(row.get("validated", False))
    )
    ...
    foundation_support_model_types = sorted(
        str(item).strip() for item in foundation_catalog if str(item).strip()
    )
    device_model_types = sorted(
        str(item).strip() for item in device_catalog if str(item).strip()
    )
    support_link_model_types = sorted(
    ...
    checks = {
        ...
        "event_sequence_pass": bool(
            int(event_sequence["contact_uplift_event_sequence_mismatch"]) == 0
        ),
        "support_search_surface_pass": bool(
            support_search_surface["support_search_surface_pass"]
        ),
        "node_to_surface_proxy_pass": bool(
            support_search_surface["node_to_surface_proxy_pass"]
        ),
        "contact_family_surface_pass": bool(
            support_search_surface["contact_family_surface_pass"]
        ),
        "support_search_family_surface_pass": bool(
            support_search_surface["support_search_family_surface_pass"]
        ),
        "node_to_surface_proxy_family_surface_pass": bool(
    ...
    return {
        ...
        "summary": {
            ...
            "contact_uplift_event_sequence_mismatch": int(
                event_sequence["contact_uplift_event_sequence_mismatch"]
            ),
            "link_model_types": link_model_types,
            ...
            "support_search_model_types": support_search_surface[
                "support_search_model_types"
            ],
            "node_to_surface_proxy_model_types": support_search_surface[
                "node_to_surface_proxy_model_types"
            ],
            "contact_search_surface_types": support_search_surface[
                "contact_search_surface_types"
            ],
            "contact_family_count": int(support_search_surface["contact_family_count"]),
            "search_surface_mode_counts": support_search_surface[
                "search_surface_mode_counts"
            ],
            "search_family_counts": support_search_surface["search_family_counts"],
            "support_search_family_types": support_search_surface[
                "support_search_family_types"
            ],
            "node_to_surface_proxy_family_types": support_search_surface[
                "node_to_surface_proxy_family_types"
            ],
            "support_search_family_requirements": support_search_surface[
                "support_search_family_requirements"
            ],
            "search_ready_group_counts": support_search_surface[
                "search_ready_group_counts"
            ],
            "support_depth_score": support_search_surface["support_depth_score"],
            "support_search_evidence_rows": support_search_surface[
        ...
        "reason_code": "PASS"
        if contract_pass
        else "ERR_STRUCTURAL_CONTACT_VALIDATION_FAIL",
        "reason": "validated special-link constitutive library"
        if contract_pass
        else "one or more structural-contact link validations failed",
    }
...
def main() -> None:
    ...
    try:
        validate_input_contract(
            input_payload,
            INPUT_SCHEMA,
            label="phase1.run_structural_contact_validation",
        )
