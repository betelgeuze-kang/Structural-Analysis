...
- 모바일 정적 테스트 추가: `tests/test_phase1_mobile_static_contracts.py`는 Step5 RCA missing timing subfield, `host_copy_share` 범위 위반, NaN/inf/음수/비숫자 timing, LF→GNN fallback reporting을 workstation/CI에서 검증할 수 있게 한다
...
2) `tests/test_phase1_ci_gate.py` 또는 새 gate integration test에서 `_validate_inputs()`가 `host_copy_share < 0`, `host_copy_share > 1`, `NaN/inf timing`, `missing timing subfield`를 같은 reason/detail 어휘로 반환하는지 확인
