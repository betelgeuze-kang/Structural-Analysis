...
SOURCE_PATHS = (
    ...
    Path("src/structural_analysis/schemas/nonlinear_transient_checkpoint_v1.schema.json"),
...
def _inputs() -> tuple[Any, BilinearOscillator, NonlinearTransientConfig, tuple[float, ...]]:
    document = load_model_ir_v2(ROOT / MODEL_PATH)
    payload = document.to_dict()
    dynamics = payload["dynamics"]
    source = ROOT / dynamics["force_history_source_path"]
    if LINEAR._file_sha(Path(dynamics["force_history_source_path"])) != dynamics["force_history_sha256"]:
    ...
    function = next(
        row for row in payload["time_functions"] if row["id"] == dynamics["force_time_function_id"]
    )
    ...
    if any(not math.isclose(time_s, index * dt, rel_tol=0.0, abs_tol=1.0e-12) for index, (time_s, _) in enumerate(csv_points)):
    ...
    model = BilinearOscillator(
        ...
        elastic_stiffness_kn_per_m=float(dynamics["elastic_stiffness_n_per_m"]) / 1000.0,
...
def _result_ir(
    *, document: Any, config: Any, forces_kn: tuple[float, ...], solution: Any, authority: Any
) -> dict[str, Any]:
    samples = [
        {
...
def build_receipt(*, source_commit_sha: str | None = None) -> dict[str, Any]:
    ...
    linear_error_m = abs(linear_solution.steps[-1].displacement_m - exact_linear_displacement)
    ...
    max_acceleration_m_per_s2 = max(abs(step.acceleration_m_per_s2) for step in solution.steps)
    max_residual_n = max(abs(step.equilibrium_residual_kn) * 1000.0 for step in solution.steps)
    ...
    workbench = {
        ...
        "summary": EXPLORER.build_results_summary(time_histories=(displacement_history, force_history)),
    }
    all_pass = bool(
    ...
    surface_artifacts: dict[str, Any] = {
        ...
        "checkpoint": {
            ...
            "source_authenticated_resume": resumed.source_authenticated_resume,
        },
        "workbench": workbench,
        "benchmark": {
            ...
            "linear_observed_terminal_displacement_m": linear_solution.steps[-1].displacement_m,
            ...
            "cyclic_plastic_history_nonzero": state.cumulative_plastic_displacement_m > 0.0,
