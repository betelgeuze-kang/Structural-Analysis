...
`run_stateful_corotational_frame3d_displacement_control_path` solves one free
UX/UY/UZ/RX/RY/RZ coordinate and the proportional nodal-load factor together.
Multiple-coordinate control remains unsupported.

The augmented system uses the current source-bound 6DOF equilibration and an
equivalent-length coordinate for rotations and load factor:

```text
residual = [D_R^-1 R; c_control (x_control - x_target)]
jacobian = [D_R^-1 K D_u, D_R^-1 (-P) / L_char;
            c_control e_control^T, 0]
```
