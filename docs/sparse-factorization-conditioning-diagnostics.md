...
Its bounded nonlinear-3D integration claim is true only for the experimental
`CorotationalFrame3DGraphModel` path, never for the public corotational API.
...
The policy is wired into the bounded experimental stateful 3D graph solver. A
44-node/43-member chain exercises 258 free equations, nine 32-column inverse-solve blocks,
canonical native CSR, one Newton correction, and a finite exact condition receipt without
regularization or fallback. The graph contract is capped at 768 free equations; the
diagnostic remains capped at 1536.

This remains a quadratic-work exact diagnostic and is not wired into the public
corotational API. A production-scale path still requires a separately reviewed scalable
estimator/policy, representative performance and memory receipts, and external validation.
