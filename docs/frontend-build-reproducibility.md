...
- `npm run install:browser-runtime`
  - Invokes Rust-native `structural-frontend-contract playwright-install` instead of direct `npx` resolution. Rust validates the frontend contract, hashes the installed Playwright CLI, removes inherited `NODE_OPTIONS`, and owns the exact `node .../cli.js install --with-deps chromium` child used by hosted browser workflows.
