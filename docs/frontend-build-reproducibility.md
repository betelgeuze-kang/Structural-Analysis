...
- `npm run install:dependencies`
  - Is a local convenience for Rust-native `structural-frontend-contract frontend-install`; hosted workflows call the Cargo command directly so npm lifecycle hooks cannot run before Rust validation. Rust validates the package/lock/source-map contract, removes inherited `NODE_OPTIONS`, owns the exact `npm ci` direct child, rejects contract mutation, and records its terminal exit.
  - The direct Cargo command with `--dry-run` requires no `node_modules`, network or filesystem mutation, resolves neither npm nor Node, and spawns no child. Using the optional `npm run` convenience still requires its outer npm/Node launcher, which is not counted by the Rust receipt.
  - Live npm registry/cache access, lifecycle scripts, configuration/environment, npm/Node executable identity, transitive processes, extracted package bytes, `node_modules` contents, and rollback remain uninstrumented. This is C0 orchestration, not clean-machine publication, C5, or C6 evidence.
- `cargo run --quiet --locked --manifest-path native/Cargo.toml -p structural-frontend-contract -- frontend-audit --root .`
  - Is the hosted frontend lane's direct Rust-owned entrypoint for the exact `npm audit --audit-level high` child. Rust validates and freezes the package/lock/source-map contract, removes inherited `NODE_OPTIONS`, rejects repository-contract mutation, records the numeric child exit, and emits a canonical self-hashed receipt.
  - The prior workflow was non-blocking for every numeric audit exit. That policy is preserved deliberately: exit zero becomes `audit_clean_exit`, while any numeric nonzero becomes only `advisory_or_tool_failure` and the Rust command exits successfully. The receipt does not parse npm output or claim whether the cause was a vulnerability, registry, network, configuration, or tool failure.
  - `scripts/verify_quality_gate.py` uses the same command with `--fail-on-nonzero`: Rust first publishes the identical classified receipt and then exits nonzero, preserving the quality gate's prior strict stop-on-failure behavior. The Python gate now calls every frontend install/audit/check/build/browser verifier through direct Cargo entrypoints and contains no npm or npm package-script launcher.
  - `--dry-run` resolves neither npm nor Node and spawns no child. Live registry/advisory/cache access, npm configuration and environment, executable/transitive-process identities, external-cache mutation, finding counts and identities, dependency/license clearance, clean-machine publication, C5, and C6 remain outside this C0 orchestration receipt.
- `python3 scripts/build_frontend_dependency_audit_report.py --out FILE`
  - Is now a compatibility launcher for the direct Cargo `structural-frontend-contract frontend-audit-report --root . --out FILE` command. The wrapper contains no direct npm launcher or audit-result classification; it strictly verifies the native self-hashed receipt and the identity of the published report before displaying its summary.
  - Rust owns the exact `npm audit --json` child, removes inherited `NODE_OPTIONS`, drains stdout and stderr concurrently with bounded retained memory, rejects duplicate-key, non-finite, oversized, structurally invalid, or metadata/finding-count-inconsistent JSON as blocked evidence, projects the existing `frontend-dependency-audit-report.v1` schema, and safely publishes it through bounded staging, backup rename, and rollback. `--fail-blocked` changes only the final command exit after the blocked report and receipt have been published.
  - npm remains the external advisory oracle. Registry/advisory/cache behavior, npm configuration and executable identity, independent advisory verification, license/SBOM/secrets clearance, clean-machine publication, C5, and C6 remain outside this C0 evidence boundary.
- `npm run build`
...
- `npm run preview`
  - Invokes Rust-native `structural-frontend-contract frontend-preview` instead of `vite preview`. It first validates the pinned frontend contract and already-built delivery, then serves only `dist/` through the confined SPA router on fixed IPv4 loopback (default port 4173).
  - `npm run preview -- --dry-run` requires a valid existing `dist/` tree but binds no listener. The server accepts bounded GET/HEAD requests, rejects traversal, dotfiles and symlinks, emits no-cache and nosniff responses, and spawns no Node, Vite, browser, Python, or other child process.
  - This is local delivery serving only. It does not execute or validate browser behavior and is not clean-machine publication, native UI parity, C5, or C6 evidence.
- `npm run install:browser-runtime`
  - Is a local convenience for Rust-native `structural-frontend-contract playwright-install` instead of direct `npx` resolution. Hosted workflows call the Cargo command directly so npm lifecycle hooks cannot run before Rust validation. Rust validates the frontend contract, hashes the installed Playwright CLI, removes inherited `NODE_OPTIONS`, and owns the exact `node .../cli.js install --with-deps chromium` child.
  - `npm run install:browser-runtime -- --dry-run` requires no `node_modules`, network, host-package mutation, browser, or child process. Live success rejects package/source-map/CLI mutation and records the direct child exit.
  - The retained Playwright installer still owns browser and operating-system dependency resolution, downloads, caches, elevation, and host-package mutation. Network bytes, downloaded artifact identities and rollback are uninstrumented, so this is C0 orchestration rather than clean-machine publication, C5, or C6 evidence.
...
- `structural-frontend-contract phase5-task-browser-smoke`
  - Replaces the three direct `npm run build`, `npm run preview`, and `npx playwright` process boundaries previously owned by `scripts/run_phase5_task_based_ux_browser_smoke.py`. Rust freezes the complete frontend build plan, exact developer-preview Playwright specification, fixed five-step workflow vocabulary, `127.0.0.1:4173` SPA route, and direct Node command; it owns the two build children unless `--skip-build` is explicit, validates delivery, owns the loopback listener and Playwright child, and rejects input or delivery mutation.
  - `cargo run --quiet --locked --manifest-path native/Cargo.toml -p structural-frontend-contract -- phase5-task-browser-smoke --root . --dry-run` emits a deterministic self-hashed plan with no process, listener, `dist` requirement, or browser. The Python script now launches this one direct Cargo command, strictly verifies the canonical success receipt, and retains only legacy release-receipt metadata/publication compatibility.
  - Node, TypeScript, Vite, Playwright, Chromium, React/TypeScript rendered behavior, browser page requests, the Python compatibility wrapper, human usability observation, and clean-machine live evidence remain transitional. The local sandbox denies loopback bind, so it cannot supply the live browser receipt and this C0 slice does not close C5 or C6.
...
- `npm run verify:viewer-sample-workflow`
  ...
  - `scripts/build_ux_release_readiness_report.py --run-browser-smoke` enters through the direct Cargo `structural-frontend-contract viewer-sample-workflow` command by default, with no outer npm package-script launcher, then consumes and re-hashes the canonical Rust receipt from stdout; a zero exit without that exact four-step receipt fails closed, and a removed temporary artifact is not represented as fresh retained evidence. Python still owns the readiness-report assembly and publication boundary.
