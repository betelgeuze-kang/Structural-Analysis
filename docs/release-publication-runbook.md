...
```bash
python3 scripts/dispatch_release_publish_workflow.py --allow-gh-auth-token --dry-run --json --out <dispatch-plan.json>
```

2. Dispatch the workflow:

```bash
python3 scripts/dispatch_release_publish_workflow.py --allow-gh-auth-token --json
```

3. Check recent workflow runs:

```bash
python3 scripts/dispatch_release_publish_workflow.py --allow-gh-auth-token --status --json --out <workflow-runs.json>
```
...
6. Use `GITHUB_TOKEN=<token>` instead of `--allow-gh-auth-token` when the runner has no authenticated `gh` CLI session.
7. If the dispatch command reports missing token, either set `GITHUB_TOKEN` or `GH_TOKEN`, or run the workflow manually in the GitHub Actions UI.
...
```bash
python3 scripts/hydrate_github_release_assets.py --repo <owner/name> --manifest <candidate-manifest.json> --artifact-root <hydrated-release-root> --write --out <post-publish-roundtrip.json>
...
```bash
python3 scripts/check_release_p0_closure.py --manifest <candidate-manifest.json> --assets-json <release-assets.json> --artifact-root <fresh-root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --post-publish-roundtrip-json <post-publish-roundtrip.json> --tag-ref-present true --require-all --require-exact --fail-unclosed
```

9. Confirm overall P0 status:

```bash
python3 scripts/check_p0_closure_status.py --manifest <candidate-manifest.json> --release-assets-json <release-assets.json> --artifact-root <fresh-root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --post-publish-roundtrip-json <post-publish-roundtrip.json> --tag-ref-present --fail-open
```
...
- `check_release_p0_closure.py --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --post-publish-roundtrip-json <post-publish-roundtrip.json> --tag-ref-present true --require-all --require-exact --fail-unclosed` reports closed.
- `check_p0_closure_status.py --manifest <candidate-manifest.json> --release-assets-json <release-assets.json> --artifact-root <fresh-root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --post-publish-roundtrip-json <post-publish-roundtrip.json> --tag-ref-present --fail-open` reports overall P0 closed. The `Publish Release Assets` workflow also writes `structural-p0-closure-status.json` and `structural-p0-closure-status.md` into the publication evidence artifact.
...
1. Preview batch review updates with `python3 implementation/phase1/preview_external_benchmark_submission_after_review_updates.py --queue-manifest <queue-manifest.json> --batch-updates-json <batch-updates.json> --release-gap-report <release-gap-report.json> --commercial-readiness-report <commercial-readiness-report.json> --tpu-hffb-benchmark-report <tpu-hffb-benchmark-report.json> --peer-spd-hinge-benchmark-report <peer-spd-hinge-benchmark-report.json> --peer-spd-hinge-fixture-regression-report <peer-spd-hinge-fixture-regression-report.json> --peer-spd-hinge-alignment-report <peer-spd-hinge-alignment-report.json> --out <external_benchmark_submission_readiness_preview.json>`.
2. Treat the preview JSON/Markdown as sidecar-only evidence: it predicts whether the external benchmark queue stays at `draft_ready_final_review_pending` or reaches `ready_for_full_submission` and keeps `submission_receipt` / `receipt_status` explicit.
3. When an EB receipt/update sidecar exists, merge it with `python3 implementation/phase1/generate_external_benchmark_submission_readiness.py --submission-updates <external_benchmark_submission_updates.json>` so `receipt_url`, `submitted_at_utc`, `last_checked_at_utc`, and `closure_evidence_status` stay machine-readable. Keep the RH counterpart named `residual_holdout_closure_updates.json` so the residual holdout closure packets stay evidence-backed instead of hand-edited.
4. `scripts/build_release_publication_candidate.py` forwards `external_benchmark_submission_updates.json` and `residual_holdout_closure_updates.json` into the signed project package when those sidecars exist. `scripts/materialize_clean_checkout_evidence_chain.py` can hydrate those sidecars back from `project_package.zip` during clean-checkout reproduction and records both `p1_evidence_sidecar_structure_preflight` and the strict `p1_evidence_sidecar_preflight`. The structure preflight may pass while strict EB/RH evidence remains pending. Do not mark EB/RH rows closed unless the sidecar points to actual receipt, signed review, cross-validation, authority signoff, or formal hold evidence.
5. Materialize the operational backlog with `python3 scripts/materialize_p1_operational_queues.py --p1-benchmark-breadth-status <p1-benchmark-breadth-status.json> --artifact-root <artifact-root> --json --out <p1-operational-queues.json> --out-md <p1-operational-queues.md> --fail-open`.
6. Before mutating EB/RH sidecars from a filled intake, run `python3 scripts/validate_p1_evidence_intake_manifest.py --intake-manifest <p1-evidence-intake.json> --json --fail-open`; it rejects placeholders, missing local evidence files, and invalid UTC timestamps while recording local evidence SHA/bytes for review.
7. The materializer writes `external_benchmark_submission_queue/*.receipt_template.json` for submission receipts and `residual_holdout_queue/*.closure_packet_template.json` for residual holdout closure packets. Keep RH closure updates in `residual_holdout_closure_updates.json` so the materialized packets stay evidence-backed.
