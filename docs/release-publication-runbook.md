...
Commercial scope stays bounded in this handoff path: `Commercial`, `engineer_in_loop_accelerated_coverage_ready=true`, `full_commercial_replacement_ready=false`.
...
3. When a receipt/update sidecar exists, merge it with `python3 implementation/phase1/generate_external_benchmark_submission_readiness.py --submission-updates <external_benchmark_submission_updates.json>` so `receipt_url`, `submitted_at_utc`, `last_checked_at_utc`, and `closure_evidence_status` stay machine-readable.
...
5. The materializer writes `external_benchmark_submission_queue/*.receipt_template.json` for submission receipts and `residual_holdout_queue/*.closure_packet_template.json` for residual holdout closure packets.
