...
4. Materialize the operational backlog with `python3 scripts/materialize_p1_operational_queues.py --p1-benchmark-breadth-status <p1-benchmark-breadth-status.json> --artifact-root <artifact-root> --json --out <p1-operational-queues.json> --out-md <p1-operational-queues.md> --fail-open`.
5. The materializer writes `external_benchmark_submission_queue/*.receipt_template.json` for submission receipts and `residual_holdout_queue/*.closure_packet_template.json` for residual holdout closure packets. If the hard implementation emits `residual_holdout_closure_updates.json`, keep that as the RH closure-update sidecar name.
