...
- GitHub sync: read-only preflight는 현재 feature/main remote refs가 local HEAD와 일치하는 `synced` 상태다. `github_sync` release area는 현재 pass지만, 이후 feature/main ref가 다시 벌어지면 remote sync evidence가 들어오기 전까지 blocked로 돌아간다.
