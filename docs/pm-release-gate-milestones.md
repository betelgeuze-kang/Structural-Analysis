...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 tracked preflight는 synced state를 기록하며, 최신 release claim 전에는 post-push ref check로 feature ref와 `origin/main`이 같은 pushed HEAD를 가리키는지 다시 확인한다. PM `github_sync` release area는 현재 pass이며, 다음 source/evidence commit이 생기면 release claim 전에 preflight를 다시 갱신해야 한다.
...
현재 action register open blocker는 총 `12`개다. Release-area blocker는 CI/UX/Security `5`개이며 external owner input이 필요한 상태로 분류된다. Structural scope cleanup owner-review handoff `1`개는 quarantined PocketMD/GPCR/MD3Bead-family 경로의 owner delete/extract 결정을 요구한다. GA/Enterprise handoff에는 독립 V&V, family validation manual signoff, customer audit/failure-bundle/SLA, completed-project customer shadow 및 fresh validation 후속 blocker가 함께 노출된다. 이는 intake packet, acceptance criteria, reproduction/verification command가 준비됐다는 뜻이며, 실제 CI streak, human UX observation, product/legal license approval, external signoff, customer-retained shadow metadata, fresh-validation receipts, owner delete/extract evidence를 대체하지 않는다.
...
`pm_release_gate_completion_audit.json`은 PM release-area 16개와 M1-M5 세부 요구사항을 requirement-level row로 펼친다. 현재 audit는 milestone 세부 요구사항은 모두 pass, release-area 요구사항은 `13/16` pass, Basic CI/UX/Security top-level row가 external owner input ready 상태로 blocked임을 기록한다.
