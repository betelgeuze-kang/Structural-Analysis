...
- Fresh Full-Validation Lanes: `fresh_full_validation_lane_status.json`은 CPU-required publication runner가 hydrate/materialize하는 evidence와 Level 3 fresh validation receipt를 분리한다. 현재 lane contract는 `8/8`, fresh receipt도 `8/8`이며 status는 ready다. 각 receipt는 `implementation/phase1/fresh_validation_receipt.schema.json`과 `implementation/phase1/validate_fresh_validation_receipt.py`를 통과하고, `docs/templates/fresh_validation_receipt.template.json`은 owner 입력용 placeholder 예시일 뿐 release evidence가 아니다. 이 ready 상태는 CI streak, UX observation, license status, customer shadow, G1 full-mesh nonlinear equilibrium, 독립 V&V/GA breadth blocker를 대신 닫지 않는다.
...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 tracked preflight는 explicit remote-mutation approval을 기록한 `ready_to_push`이고 PM `github_sync` release area는 blocked다. Feature ref는 local release HEAD보다 `2`커밋 뒤, `origin/main`은 `74`커밋 뒤 상태이므로 승인된 feature push, `git push origin HEAD:main` fast-forward, post-push preflight refresh가 필요하다.
