...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 tracked preflight는 post-push synced state를 기록하며, feature ref와 `origin/main`이 모두 `f0f079546445cb9d70e5c40d34a75c9f91e7aa17` local release HEAD와 일치한다. PM `github_sync` release area는 현재 pass이며, 다음 source/evidence commit이 생기면 release claim 전에 preflight를 다시 갱신해야 한다.
