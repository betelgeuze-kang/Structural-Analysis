...
- Evidence Freshness: `release_evidence_freshness_report.json`은 구조 해석 제품 범위의 release evidence 14개를 감사한다. 현재 추적 report는 `3/14` pass, blocker `19`개이므로 freshness gate는 닫히지 않았다. 이 gate는 heavy validation 재실행을 대체하지 않고 P0/P1, real-project/customer-shadow, full-validation lane, residual Level 3, G1 residual, Evidence Console, Developer Preview, 구조 벤치마크 정확도 및 AI contract receipt의 provenance/freshness를 고정한다. Metadata freshness는 실제 completed-project shadow case, G1 full-mesh nonlinear equilibrium, Evidence Console launch readiness, Developer Preview RC final gates 및 독립 V&V/GA breadth blocker를 대신 해제하지 않는다.
...
- UX: automated browser rehearsal는 sample workflow가 30분 예산 안에 끝난다는 workflow evidence로만 인정한다. `--run-browser-smoke` 경로는 Rust-native sample-workflow receipt의 self-hash와 정확한 4-step 결과를 소비하며, zero exit만 있거나 retained artifact가 없는데 과거 artifact를 가리키는 경우에는 실패한다. PM UX release-area pass는 실제 신규 사용자 human observation record, completion minutes, blocker count, observer, evidence reference, accepted decision이 들어간 `ux_new_user_observation_report.json.contract_pass=true`를 요구한다.
