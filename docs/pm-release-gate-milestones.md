...
현재 action register open blocker는 총 `19`개다. Release-area blocker는 CI/UX/Security/GitHub-sync `8`개이며 external owner/R4 input이 필요한 상태로 분류된다. GA/Enterprise handoff에는 독립 V&V, family validation manual signoff, customer audit/failure-bundle/SLA `3`개 external owner blocker와 fresh full-validation receipt `8`개 local-remediation blocker가 추가로 노출된다. 이는 intake packet, acceptance criteria, reproduction/verification command가 준비됐다는 뜻이며, 실제 CI streak, human UX observation, product/legal license approval, explicit GitHub remote-mutation approval, external signoff, fresh validation receipt evidence를 대체하지 않는다.
...
`customer_shadow_evidence_intake_packet.json`은 completed-project shadow evidence 3~5개 목표를 위해 5개 owner-input slot, template path, per-slot validator command를 고정한다. validator는 SHA256 reference checksum, engine commit SHA, numeric delta/residual metrics, reviewer decision, `raw_data_retained_by_customer=true`, `redistribution_allowed=false`를 검사한다. 이 packet은 customer shadow evidence를 생성하거나 고객 raw data를 ingest하지 않으며, 실제 customer-retained metadata file이 validator를 통과하기 전까지 `0/3` blocker를 닫지 않는다.
