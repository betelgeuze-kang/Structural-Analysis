...
- Fresh Full-Validation Lanes: `fresh_full_validation_lane_status.json`은 CPU-required publication runner가 hydrate/materialize하는 evidence와 Level 3 fresh validation receipt를 분리한다. 현재 lane contract는 `8/8`, lane pass는 `7/8`, fresh receipt present는 `8/8`이며 status는 blocked다. 남은 blocker는 GPU/HIP solver lane의 artifact/input checksum mismatch이고, fresh rerun은 solver rows가 HIP-resident production kernel이 아니라 CPU runtime임을 보여 green receipt를 발행하지 않았다. `docs/templates/fresh_validation_receipt.template.json`은 owner 입력용 placeholder 예시일 뿐 release evidence가 아니다. 이 상태는 CI streak, UX observation, license status, customer shadow, G1 full-mesh nonlinear equilibrium, 독립 V&V/GA breadth blocker를 대신 닫지 않는다.
...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 tracked preflight는 explicit remote-mutation approval을 기록했지만 evidence refresh가 아직 uncommitted라 `worktree_not_clean`으로 blocked이고 PM `github_sync` release area도 blocked다. Feature ref는 local release HEAD보다 `3`커밋 뒤, `origin/main`은 `75`커밋 뒤 상태이므로 로컬 evidence commit, 승인된 feature push, `git push origin HEAD:main` fast-forward, post-push preflight refresh가 필요하다.
...
현재 action register open blocker는 총 `16`개다. Release-area blocker는 CI/UX/Security/GitHub sync `8`개이며 external owner input이 필요한 상태로 분류된다. Structural scope cleanup owner-review handoff `1`개는 quarantined PocketMD/GPCR/MD3Bead-family 경로의 owner delete/extract 결정을 요구한다. GA/Enterprise handoff에는 독립 V&V, family validation manual signoff, customer audit/failure-bundle/SLA, completed-project customer shadow 및 fresh validation 후속 blocker가 함께 노출된다. 이는 intake packet, acceptance criteria, reproduction/verification command가 준비됐다는 뜻이며, 실제 CI streak, human UX observation, product/legal license approval, GitHub remote-sync evidence, external signoff, customer-retained shadow metadata, owner delete/extract evidence를 대체하지 않는다.
