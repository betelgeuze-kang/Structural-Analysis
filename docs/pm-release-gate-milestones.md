...
- Implementation orchestration: Cursor Agent와 OpenCode worker bridge 파일/CLI preflight가 구성되어 있고, Cursor는 scoped implementation/focused edit/test-fix loop에 더 적극적으로 활용하며, 현재 OpenCode task assignment는 OpenCode 실행 대신 Cursor `composer-2.5`로 라우팅된다. Model availability evidence는 실행 환경의 writable `XDG_DATA_HOME` 계정 store에 의존하므로, sandbox가 `/tmp` fallback registry만 볼 때는 blocked로 남을 수 있다. 이 evidence는 release pass/fail을 대체하지 않으며, Codex가 goal tracking, diff review, verification, final acceptance를 계속 담당한다.

남은 M1-M5 milestone blocker는 없다.

전체 PM release-area blocker는 다음과 같다.

- Basic CI: local artifacts는 PR `2`회, nightly `230`회 연속 PASS를 보여주지만 release streak credit은 GitHub Actions tracked evidence를 요구한다. 현재 GitHub Actions PR/nightly streak evidence가 `0/30`이므로 두 lane 모두 `30`회 연속 PASS release evidence가 아직 없다. PR lane은 GitHub Actions job-start blocker `5`건이 잡혀 있고 `github_actions_ci_streak_evidence.json`에 billing/spending-limit annotation으로 기록된다. 이 external account blocker가 풀리고 workflow가 실제 runner step까지 진입한 뒤에야 PR streak를 다시 수집할 수 있다. Nightly lane은 workflow state가 active지만 tracked successful nightly streak evidence가 아직 없다.
- Evidence Freshness: `release_evidence_freshness_report.json`은 `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, `customer_shadow_evidence_status.json`, `customer_shadow_evidence_intake_packet.json`, `fresh_full_validation_lane_status.json`, `residual_level3_status.json`, `mgt_g1_direct_residual_terminal_gate_report.json`, `mgt_g1_followup387_shell_material_budgeted_continuation_status.json`, `evidence_console_scope_status.json`, `developer_preview_rc_status.json`, `public_benchmark_source_of_truth.json`, `real_accuracy_validation_report.json`, `ai_engine_productization_contracts.json` 15개 release evidence를 감사한다. 현재 15개 artifact 모두 `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime recency, declared input-checksum dependency mtime recency를 갖고 `15/15` pass한다. 이 gate는 heavy validation 재실행을 대체하지 않고, P0/P1/real-project/customer-shadow/full-validation-lane/residual Level 3/G1 residual/Evidence Console/Developer Preview/public benchmark/accuracy/AI contract receipt의 provenance/freshness를 release-area 증거로 고정한다. Metadata freshness pass는 실제 completed-project shadow case `0/3`, G1 full-mesh nonlinear equilibrium, Evidence Console launch readiness, Developer Preview RC final gates, public benchmark source material, 독립 V&V/GA breadth blocker를 해제하지 않는다.
- Fresh Full-Validation Lanes: `fresh_full_validation_lane_status.json`은 CPU-required publication runner가 hydrate/materialize하는 evidence와 Level 3 fresh validation receipt를 분리한다. 현재 lane contract는 `8/8`, lane pass는 `7/8`, fresh receipt present는 `8/8`, latest result pass는 `7/8`이며 status는 blocked다. GPU/HIP solver fresh rerun은 `ERR_ROCM_RUNTIME_UNAVAILABLE` / `validation_command_exit_1`로 실패했다. 이전 PASS receipt의 artifact/input checksum drift는 raw diagnostic으로 보존되지만 최신 failed execution result가 blocker attribution을 supersede하므로 별도 release blocker로 중복 계산하지 않는다. `docs/templates/fresh_validation_receipt.template.json`은 owner 입력용 placeholder 예시일 뿐 release evidence가 아니다. 이 상태는 CI streak, UX observation, license status, customer shadow, G1 full-mesh nonlinear equilibrium, 독립 V&V/GA breadth blocker를 대신 닫지 않는다.
...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 PM `github_sync` release area는 `github_sync_preflight::local_head_mismatch`로 blocked이며, commit/push, remote ref comparison, refreshed preflight가 같은 HEAD를 가리킨 뒤에만 release claim으로 이동할 수 있다.
...
현재 action register open blocker는 총 `13`개다. Release-area blocker는 CI/UX/Security/GitHub sync `6`개이며 external owner input이 필요한 상태로 분류된다. Structural scope cleanup owner-review handoff `1`개는 quarantined PocketMD/GPCR/MD3Bead-family 경로의 owner delete/extract 결정을 요구한다. GA/Enterprise handoff에는 독립 V&V, family validation manual signoff, customer audit/failure-bundle/SLA, completed-project customer shadow 및 fresh validation 후속 blocker가 함께 노출된다. 이는 intake packet, acceptance criteria, reproduction/verification command가 준비됐다는 뜻이며, 실제 CI streak, human UX observation, product/legal license approval, GitHub remote sync verification, external signoff, customer-retained shadow metadata, fresh-validation receipts, owner delete/extract evidence를 대체하지 않는다.
...
`pm_release_gate_completion_audit.json`은 PM release-area 16개와 M1-M5 세부 요구사항을 requirement-level row로 펼친다. 현재 audit는 milestone 세부 요구사항은 모두 pass, release-area 요구사항은 `12/16` pass, Basic CI/UX/Security/GitHub sync top-level row가 external owner input ready 상태로 blocked임을 기록한다.
...
`license_status_closure_report.json`과 `docs/templates/license_status.template.json`도 support bundle에 포함된다. closure report는 실제 제품/법무 승인 evidence를 검증하는 gate 산출물이며 그 자체가 승인 evidence는 아니다. template은 입력 예시일 뿐 release evidence가 아니다.
...
`customer_shadow_evidence_intake_packet.json`은 completed-project shadow evidence 3~5개 목표를 위해 5개 owner-input slot, template path, per-slot validator command를 고정한다. validator는 SHA256 reference checksum, engine commit SHA, 필수 numeric `delta_metrics.max_relative_error_pct`와 `residual_metrics.normalized_equilibrium_residual`, reviewer decision, `raw_data_retained_by_customer=true`, `redistribution_allowed=false`를 검사한다. `implementation/phase1/customer_shadow_evidence/`는 derived metadata JSON landing zone이며 raw customer data를 저장하지 않는다. 이 packet은 customer shadow evidence를 생성하거나 고객 raw data를 ingest하지 않으며, 실제 customer-retained metadata file이 validator를 통과하기 전까지 `0/3` blocker를 닫지 않는다.
...
`support_bundle_manifest.json`은 redacted support bundle directory뿐 아니라 `implementation/phase1/release/support_bundle_export.zip`의 path, sha256, member count, archive roundtrip check를 고정한다. 현재 support bundle은 `53/53` artifact, redaction self-test, archive roundtrip, PM failure-bundle coverage를 PASS로 기록한다. Developer Preview final-gate owner packet도 redacted bundle에 포함되며, 남은 selected medium models, Windows replay, human new-user observation evidence를 owner handoff로 고정할 뿐 RC나 release readiness를 승격하지 않는다. zip 자체는 ignored runtime artifact이고 manifest가 one-click export evidence다.
