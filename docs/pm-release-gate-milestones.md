...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 현재 tracked preflight는 `synced`이고 PM `github_sync` release area는 pass다. 새 local release commit이 생기면 release claim 전에 feature/main push와 preflight refresh를 다시 수행해야 한다.
