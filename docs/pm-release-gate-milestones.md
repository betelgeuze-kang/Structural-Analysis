...
- Evidence Freshness: `release_evidence_freshness_report.json`은 `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, `customer_shadow_evidence_status.json`, `fresh_full_validation_lane_status.json` 6개 release evidence를 감사한다. 현재 6개 artifact 모두 `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime recency를 갖고 `6/6` pass한다. 이 gate는 heavy validation 재실행을 대체하지 않고, P0/P1/real-project/customer-shadow/full-validation-lane status receipt의 provenance/freshness를 release-area 증거로 고정한다. Customer shadow/full-validation freshness pass는 metadata freshness만 의미하며, 실제 completed-project shadow case `0/3` 또는 fresh validation receipt `0/8` blocker를 해제하지 않는다.
