...
`customer_shadow_evidence_intake_packet.json`은 completed-project shadow evidence 3~5개 목표를 위해 5개 owner-input slot, template path, per-slot validator command를 고정한다. 이 packet은 customer shadow evidence를 생성하거나 고객 raw data를 ingest하지 않으며, 실제 customer-retained metadata file이 validator를 통과하기 전까지 `0/3` blocker를 닫지 않는다.
