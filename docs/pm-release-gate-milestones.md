...
- GitHub sync: read-only preflight는 feature/main remote refs가 tracked release HEAD와 일치하는지 확인한다. 새 local release commit이 생기면 `github_sync` release area는 `local_head_mismatch`로 blocked가 되며, feature branch/main push와 preflight refresh 후에만 다시 pass로 돌아간다.
...
현재 action register open blocker는 총 `21`개다. Release-area blocker는 CI/UX/Security/GitHub-sync `6`개이며 external owner/R4 input이 필요한 상태로 분류된다. Structural scope cleanup owner-review handoff `1`개는 quarantined PocketMD/GPCR/MD3Bead-family 경로의 owner delete/extract 결정을 요구한다. GA/Enterprise handoff에는 독립 V&V, family validation manual signoff, customer audit/failure-bundle/SLA, completed-project customer shadow 및 fresh validation 후속 blocker가 함께 노출된다. 이는 intake packet, acceptance criteria, reproduction/verification command가 준비됐다는 뜻이며, 실제 CI streak, human UX observation, product/legal license approval, explicit GitHub remote-mutation approval, external signoff, customer-retained shadow metadata, owner delete/extract evidence를 대체하지 않는다.
