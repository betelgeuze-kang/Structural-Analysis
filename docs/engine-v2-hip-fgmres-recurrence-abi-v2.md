...
- 상태: v0.2.26 accepted ABI; global fixed schedule, later column/restart kernel, terminal padding, exact full-final-cycle active final-guard handoff와 sealed-continuation-consuming suffix owner 및 `O(L)` fixed-suffix host-control gate까지 `contract_only` 구현, authoritative outcome/full parity는 unavailable
...
7. completion-only export, explicit terminal-outcome observation, model-family CPU/HIP parity 및 iteration host-copy-zero gate
