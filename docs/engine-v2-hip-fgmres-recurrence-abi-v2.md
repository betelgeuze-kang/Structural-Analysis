...
- 상태: v0.2.24 accepted ABI; global fixed schedule, later column/restart kernel, terminal padding, final guard와 sealed-continuation-consuming suffix owner 및 `O(L)` fixed-suffix host-control gate까지 `contract_only` 구현, authoritative outcome/full parity는 unavailable
...
v0.2.23의 [global recurrence owner v1](engine-v2-hip-fgmres-global-recurrence-v1.md)은 `initial + R*M columns + FINAL_GUARD`를 full/sealed-prefix/continuation으로 분할하고 conditional continuation capability를 single-use consume해 suffix만 제출한다. Actual integrated `gfx1030` chain은 active later column 하나, suffix resource/copy/intermediate-sync 0과 one-fence completion을 관찰했지만 product receipt는 terminal outcome을 읽지 않는다. 따라서 active later restart·active final-guard fallthrough, authoritative solver/solution, full parity와 iteration host-copy-zero로 승격하지 않는다.
...
현재 1~5번의 raw fixed recurrence kernel과 schedule, 6번의 caller-attested owner, live resource owner, canonical predecessor, sealed transaction 및 conditional global suffix owner가 구현되었다. Raw actual evidence는 later restart, terminal padding과 final guard를 포함하고 integrated owner evidence는 active later column과 restart `1 -> 2 -> 3`의 active later restart를 포함한다. 그러나 product receipt는 actual mask/verdict/commit/terminal outcome을 host에 노출하지 않고 completion export도 아직 없다.
...
6. live allocation/context, strict completion/export receipt, poison/retry cleanup; conditional global suffix fence owner까지 구현, outcome observation·completion export는 미완료
7. integrated active final-guard fallthrough, model-family CPU/HIP parity 및 iteration host-copy-zero gate
...
- actual integrated active later column/restart; active final-guard fallthrough은 별도 gate
