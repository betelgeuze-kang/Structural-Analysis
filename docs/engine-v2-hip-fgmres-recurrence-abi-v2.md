...
- 상태: v0.2.22 accepted ABI; plan/schema, valid-predecessor raw checkpoint slice, scoped invalid-source failure atomicity, caller-attested transaction context, live resource owner, device-sealed canonical first-column predecessor producer와 capability-consuming sealed checkpoint transaction은 구현, later recurrence와 full recurrence는 unavailable
...
현재 sealed child는 checkpoint 네 row에서 추가 allocation/device bytes/borrow/checkpoint-owner/module/H2D/D2H/intermediate sync/fallback 0을 유지하고 transaction-owned final fence 한 번만 사용한다. 다만 upstream canonical prefix가 이미 final fence 한 번을 사용했으므로 canonical+transaction 전체 chain은 총 두 fence다.
...
현재 1~4번, 5번 중 valid-predecessor restart-1/column-0 checkpoint raw numerical slice와 scoped invalid-source atomicity, 6번 중 caller-attested transaction owner, live parent3+owned8 resource owner, canonical first-column device producer·validator와 그 capability를 소비하는 still-open sealed transaction child까지 구현되었다. Native raw 결과는 device priority predicate, `x_scale_l2`, source preflight, pure-copy commit, finalizer-only publish, masks 0/1792/7936과 poison 보존을 독립 GPU-tree oracle와 비교한 unsigned non-promoting evidence다. Canonical producer hardware gate는 actual HIP chain에서 owned8 zero initialization, exact `27+14S` prefix, validator의 armed snapshot과 one-fence completion을 verification-only D2H로 관찰했다. Sealed child는 fixed four-row program과 별도 final fence를 추가하지만 product receipt는 actual mask/verdict/commit/device outcome을 host에 노출하지 않는다.
...
6. live allocation/context, strict completion/export receipt, poison/retry cleanup; 현재 caller-attested checkpoint owner, resource-only live parent/allocator context, single-use canonical predecessor child와 non-owning sealed transaction child까지 partial 구현
...
현재 `plan_schema_implemented`, `candidate_spmv_residual_l2_raw_linf_implemented`, `device_scale_metrics_priority_predicate_implemented`, `trial_committed_x_norms_implemented`, `raw_x_scale_l2_implemented`, `raw_checkpoint_decide_preflight_gated_commit_finalize_implemented`, `native_first_column_checkpoint_raw_slice_observed`, `caller_attested_checkpoint_range_lease_enqueue_fence_context_implemented`, `live_krylov_parent_integrated`, `allocator_provenance_bound`, `resource_owner_ready`는 true다. Exact registered nonoverlap allocation, same stream, exclusive source ownership과 fixed four-row owner sequence에 한해 `invalid_source_destination_atomicity_proven`도 true다. Canonical producer의 fenced receipt에 한해 `owned_content_initialized`, `canonical_producer_prefix_fenced`, `device_mask_domain_gate_bound`, exact16 projection과 same-runtime/device/stream binding도 true다. Sealed child의 fenced receipt에서는 `canonical_predecessor_capability_consumed`, `direct11_physical16_continuity_bound`, `fixed_four_row_transaction_fenced`, `sealed_predecessor_checkpoint_transaction_integration`과 outcome-unobserved conditional continuation 발행도 true다.

Caller-attested context에서는 range/shift overlap, same-buffer four-launch enqueue/state tracking, ambiguity poison과 exact-runtime fence가 true다. Live resource context에서는 parent3+owned8 exact11, same runtime/device/stream, internal RTC owner와 allocator provenance가 true다. Canonical child는 CSR3+scratch2 delegated projection, sealed memset과 device validator gate를 더하고, sealed transaction child가 그 conditional capability를 single-use consume한다. 하지만 product path는 actual mask, validator outcome, commit gate, commit과 numerical outcome을 host에서 관찰하지 않는다. 따라서 `actual_mask_host_observed`, `device_validation_outcome_host_observed`, `authoritative_predecessor_proven`, `checkpoint_transaction_ready`, `authoritative_checkpoint_transaction`, `authoritative_numerical_transaction_proven`, `later_columns_and_restarts_implemented`, `full_device_recurrence_implemented`, `live_solver_ready`, `solution_ready`, `iteration_host_copy_zero_proven`, `native_full_recurrence_parity`, O(N), speedup, ResultIR 통합, SPD/PCG, commercial readiness는 모두 false다. Arbitrary raw duplicate COMMIT, external writer/DMA/device fault 범위의 전역 atomicity도 false다. 다음 우선순위는 later column/restart global control과 final guard다.
