...
- [Engine v2 ADR index](adr/README.md)와
  [상용 솔버 갭 원장](commercial-structural-solver-product-gap-ledger.md): 솔버,
  CPU/HIP, 수치 계약 및 authoritative closure의 세부 상태
- capability registry와 현재 HEAD에서 생성한 readiness snapshot: 현재 구현·지원·승격
  상태
- [제품 로드맵 폐쇄 매트릭스](product-roadmap-closure-matrix.md): PR 1-18의 현재
  구현 후보, 검증 및 차단 사유
