...
source boundary, P0-2~P0-6 core evidence, release P0-1 publication이 닫혔다. 그래서 P0 closed, P1 now unblocked이고 PM release milestones M1-M5는 pass다. 다만 full release-area gate는 CI 30회 연속 evidence, release evidence freshness/provenance metadata, human UX observation, license status 때문에 blocked이며, 독립 상용제품 claim은 EB receipt 0/4 때문에 아직 blocked다.
...
- Real-project corpus는 seed-only에서 일부 measured 상태로 전환됐다. `real_project_row_provenance_report.json`은 measured local KR artifact row `10`개와 `.ifc/.mgt` measured format `2`개를 기록하고, 모든 measured row에 checksum, stable row pointer, manual review status, release eligibility를 둔다. 다만 `check_real_project_corpus_measured_status.py`는 PEER metric-bearing groups with values가 `1/5`라서 아직 blocked다.
