...
- P0는 아직 열려 있다. source 경계 정리는 generated/build 산출물 기준으로 닫혔고, 상용 납품 가능 상태로 보기에는 release 검증과 25 MiB+ 대형 파일 외부화/allowlist 결정이 남았다.
...
- 25 MiB를 넘는 tracked files 8개는 externalization vs allowlist 결정을 먼저 내려야 한다.
...
3. 25 MiB 초과 tracked 파일 8개의 externalization vs allowlist 결정을 기록하고, 결정에 따라 별도 커밋으로 처리한다.
4. fresh artifact root를 GitHub Release로 올린 뒤 manifest metadata preflight와 SHA/bytes verification을 통과시킨다.
