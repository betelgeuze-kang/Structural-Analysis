...
- `scripts/report_release_evidence_freshness.py`는 productization release evidence의 `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json` 3개 artifact가 `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime을 갖는지 감사한다. 현재 `3/3` artifact가 통과해 freshness gate는 pass이고, PM release areas는 `12/15` green이다.
