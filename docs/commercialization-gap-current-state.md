...
- Customer shadow evidence는 schema/validator/intake packet이 준비됐고 `check_customer_shadow_evidence_status.py`로 3~5개 completed-project shadow case 목표를 추적한다. `build_customer_shadow_evidence_intake_packet.py`는 5개 owner-input slot과 검증 command만 고정하며 evidence를 만들지 않는다. 현재는 실제 customer-retained evidence file이 없어 `0/3` blocked이며, synthetic/placeholder case로 닫으면 안 된다.
...
- `scripts/report_release_evidence_freshness.py`는 productization release evidence의 `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, `customer_shadow_evidence_status.json`, `fresh_full_validation_lane_status.json` 6개 artifact가 `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime을 갖는지 감사한다. 현재 `6/6` artifact가 통과해 freshness gate는 pass이고, PM release areas는 `12/15` green이다. Customer shadow/full-validation freshness pass는 metadata freshness만 의미하며, 실제 completed-project shadow case `0/3` 또는 fresh validation receipt `0/8` blocker를 해제하지 않는다.
- `scripts/build_fresh_full_validation_lane_status.py`는 release freshness와 별개로 Level 3 fresh full-validation lane을 추적한다. 현재 CPU-required publication hydrate/materialize 경계는 `8/8` lane에서 보이지만 fresh receipt는 `0/8`이라 blocked다. 따라서 hydrated commercial/GPU/performance/surface/MIDAS/productization evidence는 publication 재현성을 보강할 뿐, torch-capable benchmark, GPU HIP, heavy surface/material/contact, MIDAS exact refresh, productization heavy profile, external benchmark, design optimization refresh의 fresh validation receipt를 대체하지 않는다.
...
4. `materialize_p1_operational_queues.py`로 `external_benchmark_submission_queue/*.receipt_template.json`와 `residual_holdout_queue/*.closure_packet_template.json` sidecar를 생성하고, RH closure-update sidecar인 `residual_holdout_closure_updates.json`의 `signed_attached` 상태를 attached evidence로 유지한다.
5. `scripts/generate_p1_evidence_intake_template.py --p1-operational-queues <p1-operational-queues.json> --out <p1-evidence-intake.template.json> --out-md <p1-evidence-intake.template.md>`로 EB evidence intake template를 만든다. 제품 개발 중에는 `scripts/preflight_p1_evidence_sidecar_intake.py --external-benchmark-submission-updates <external_benchmark_submission_updates.json> --residual-holdout-closure-updates <residual_holdout_closure_updates.json> --structure-only --json --fail-open`을 내부 준비도 gate로 쓴다. 실제 EB evidence를 채운 뒤에는 `scripts/validate_p1_evidence_intake_manifest.py --intake-manifest <p1-evidence-intake.json> --json --fail-open`으로 no-write lint를 먼저 통과시키고, `scripts/build_p1_evidence_sidecar_updates.py --intake-manifest <p1-evidence-intake.json> --require-complete --fail-open --json`로 EB receipt URL/path를 sidecar로 변환한다. `scripts/preflight_p1_evidence_sidecar_intake.py --external-benchmark-submission-updates <external_benchmark_submission_updates.json> --residual-holdout-closure-updates <residual_holdout_closure_updates.json> --json --fail-open` strict mode가 통과한 뒤에만 상용화 claim을 L4/L5 쪽으로 올린다. release package, committee package, README가 `Commercial` grade와 `full_commercial_replacement_ready=false`를 계속 같이 노출하는지도 회귀 테스트로 고정한다.
