...
- `scripts/report_release_evidence_freshness.py`는 productization release evidence의 `p0_closure_status.json`, `p1_readiness_status.json`, `p1_benchmark_breadth_status.json`, `real_project_corpus_measured_status.json`, `customer_shadow_evidence_status.json` 5개 artifact가 `generated_at`, source commit, engine version, input checksum, reuse marker, producer mtime을 갖는지 감사한다. 현재 `5/5` artifact가 통과해 freshness gate는 pass이고, PM release areas는 `12/15` green이다. Customer shadow freshness pass는 metadata freshness만 의미하며, 실제 completed-project shadow case `0/3` blocker를 해제하지 않는다.
