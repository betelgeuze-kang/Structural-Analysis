...
- Version: 0.2.42 HIP FGMRES model-family audited parity unpublished candidate milestone
- 기준일: 2026-07-15
...
- [HIP FGMRES model-family audited parity v2](engine-v2-hip-fgmres-model-family-audited-parity-v2.md): v0.2.42 feature-branch milestone은 exact package `gfx1030` 10-slot fixed suite의 retained family/model-case parity, host-transfer audit, RTC launch/fence ordinal audit authority를 하나의 additive process-local receipt로 결속한다. Source host-transfer composition을 먼저 재생해 parity와 transfer가 동일 completion-export context/result 객체를 사용했는지 보존하고, 10개 ordinal result를 각각 retained `expected_context`로 검증한다. Canonical/sealed/global/completion, recurrence plan/ABI, kernel/source, full/prefix/continuation schedule, direct-generation/physical projection, architecture/device와 F/R/full-launch count를 교차검증하며 caller order는 registry slot 순서로 canonicalize하고 missing/duplicate/foreign/cross-run splice를 거부한다. Suite 합계는 local `10/10`(matrix `10/20`), recurrence copy attempt `0`, completion D2H `30/30/0`·`4,408` bytes, owned memset `80/80`, fence `30/30`, launch는 retained full-program exact 합이다. Factory는 solve/export/seal/native API를 호출하지 않는다. Process-wide/native 전체 관찰, pre-window/setup/teardown/case-between transfer zero, device semantic/content truth, broad iteration-host-copy-zero, persistent/signed provenance, external `gfx1100`, ResultIR/reaction/recovery/energy, O(N), speedup, commercial/promotion은 false이며 `origin/main` inclusion도 주장하지 않는다.
- v0.2.42 현재 검증은 focused `15 passed in 135.58s (0:02:15)`, public/capability `16 passed in 1.78s`다. Required local `gfx1030` 10-slot gate는 real gfx agent 미검출로 slot 실행 전 `1 failed in 1.65s`였고 CPU fallback은 없었다. 따라서 current-snapshot actual 10-slot triple-composition claim은 false며 historical v0.2.40/v0.2.41 hardware 결과를 대체 증거로 사용하지 않는다.
...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: local `gfx1030` fixed-suite parity와 external signed verifier, v0.2.34 artifact identity replay, v0.2.35 single-ledger durable replay, v0.2.36 identity receipt direct-signed envelope v2, v0.2.37 trust-lifecycle contract → 실제 reviewer root bootstrap/migration과 isolated runner/HSM key 등록·활성 → actual `gfx1100` `10/10` → 동일 final artifact local `gfx1030` 재실행 → historical resolver/외부 monotonic anchor → iteration host-copy-zero → ResultIR integration → certificate-bound SPD-gated PCG 상태기계. Completion-only raw export는 v0.2.27에 exact three blocking D2H로 닫혔고 v0.2.28 observer가 exact process-local terminal record를 해석하지만, raw global/export receipt는 outcome-free로 유지된다. Active final-guard integrated coverage는 v0.2.26, consumed/pending owner-loss recovery는 v0.2.25의 process-local 계약으로만 닫혔다. Durable ledger receipt, artifact identity receipt, 합성 signed verifier test 또는 local unsigned 10/10을 solution-ready, multiarchitecture 또는 promotion 증거로 재분류하지 않는다
...
v0.2.42의 current-snapshot actual local hardware gate는 real gfx agent 미검출로
slot 실행 전 중단되었다. 따라서 아래 0.2.42 행은 미발행 contract/test-double
구현 candidate를 의미하며 actual 10-slot hardware 통과를 의미하지 않는다.

| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| 0.2.42 unpublished candidate milestone | 2026-07-15 | [HIP FGMRES model-family audited parity v2](engine-v2-hip-fgmres-model-family-audited-parity-v2.md)를 추가했다. Exact package `gfx1030` 10-slot family/model-case parity, host-transfer audit, RTC launch/fence ordinal audit의 retained authority를 한 process-local receipt로 결속하고 각 source validator와 ordinal `expected_context`를 먼저 재생한다. Same completion-export 객체 identity와 canonical/sealed/global/completion, plan/ABI, kernel/source, schedule, architecture/device lineage를 교차검증하며 입력 순서를 registry order로 canonicalize하고 missing/duplicate/foreign/cross-run splice를 거부한다. Contract/test-double 합계는 `10/10`(matrix `10/20`), recurrence copy attempt `0`, completion D2H `30/30/0`·`4,408` bytes, owned memset `80/80`, fence `30/30`, launch exact 합이다. Factory는 solve/export/seal/native API를 호출하지 않는다. Current-snapshot actual local 10-slot hardware gate는 pending이다. Detached standalone provenance, broad iteration-host-copy-zero, process-wide visibility, external `gfx1100`, ResultIR/reaction/recovery/energy, O(N), speedup, commercial/promotion은 false이고 `origin/main` inclusion도 주장하지 않는다. |
