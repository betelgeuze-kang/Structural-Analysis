...
- Version: 0.2.45 HIP FGMRES DiagnosticIR v1 unpublished candidate milestone
- 기준일: 2026-07-15
...
- v0.2.45 current-source focused/public 4개 파일은 `41 passed in 378.12s`, capability matrix는 `9 passed`, v0.2.43~v0.2.45 인접 통합 suite는 `137 passed in 496.50s`, hardware harness는 `1 test collected`를 확인했다. Real gfx agent가 없는 환경에서 non-required gate는 `1 skipped`, required gate는 skip 없이 `1 failed`했으므로 actual local token-hardened `gfx1030` single/family gate는 pending이다. External `gfx1100`, 별도 all-converged registry의 10/10 ResultIRV2 ready, broad host-copy-zero, GPU recovery, standalone/signed provenance, end-to-end O(N), speedup, commercial/promotion도 pending 또는 false다.
...
- v0.2.44 focused adversarial `23 passed`, identity-token model-case/ResultIR·disposition·capability·public API 최종 통합 `91 passed in 116.59s`, capability/FGMRES-public/ResultIRV2-public 부분 집합 `19 passed`는 각 nonconverged bridge 자체 거부/forced injection, missing/duplicate/foreign, non-recycled token 기반 serially-identical cross-run splice, clone/coherent rehash/plan·provenance·issuance transplant, final source race, detached no-live-replay, schema-alone exact truth table와 AST operation guard를 통과했다. Hardware harness는 `1 test collected`를 확인했다. Aggregate projection은 추가 native/device solve가 없지만 fixed-registry validation의 deterministic CPU reference replay zero는 false다. 이 v0.2.44 receipt 자체의 DiagnosticIR은 false이고 additive v0.2.45 companion에서 별도로 완료했다. Actual local `gfx1030` 10-slot disposition hardware gate, all-converged registry, 10/10 ResultIRV2 ready, external `gfx1100`, broad host-copy-zero, GPU recovery, standalone/signed provenance, O(N), speedup, commercial/promotion은 pending 또는 false다.
...
- v0.2.43 generic contract `16 passed`, bridge contract `13 passed`, public API `3 passed`, capability `9 passed`, model-case downstream authority `24 passed`, combined `65 passed in 29.31s`를 확인했다. Exact clone/coherent rehash/registry transplant 및 unissued exact-case clone의 identity-token 거부도 회귀로 고정했다. Current-source hardware gate는 real gfx agent 부재에서 non-required `1 skipped`, required `1 failed`로 pending이다. Identity-token 패치 전 source의 required local `gfx1030` 단일 모델 ResultIR gate는 CPU fallback 없이 `1 passed in 169.10s (0:02:49)`로 통과했지만 unsigned 비영속 역사 관찰이며 token-hardened current-source hardware gate는 pending이다. v0.2.44에서 fixed package의 contract-level 7-ready/3-nonconverged disposition은 추가됐지만 actual aggregate hardware gate와 external `gfx1100`은 pending이며 process-wide/broad iteration-host-copy-zero, GPU recovery, standalone/signed provenance, promotion/commercial, nonlinear/dynamic/shell/solid/contact claim은 false다.
...
- v0.2.42 token-hardened current-source 검증은 focused `16 passed in 137.54s (0:02:17)`, public/capability `16 passed`, 독립 적대적 감사 `BLOCKER/P1/P2 = 0/0/0`이다. Identity-token 패치 전 source의 required local `gfx1030` 10-slot gate는 CPU fallback 없이 `1 passed in 3171.31s (0:52:51)`로 actual triple composition과 exact totals를 확인했지만 unsigned 비영속 역사 관찰이며 current-source hardware gate는 pending이다.
...
- v0.2.45에서 intentional `max_iterations` 3건의 partial iterate·true residual·solve record를 별도 DiagnosticIR로 보존하는 contract와 additive family companion을 완료했다. Token-hardened current-source single-case/aggregate actual `gfx1030` gate와 실제 peak RSS 측정, 별도 all-converged registry의 10/10 ResultIRV2 ready gate는 후속 계약으로 분리
...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: v0.2.45 DiagnosticIR 의미론과 additive 7-result/3-diagnostic disposition contract는 완료했으므로 token-hardened current-source single-case 및 aggregate actual `gfx1030` 재검증 → 별도 all-converged registry의 10/10 ResultIRV2 gate → 실제 reviewer root bootstrap/migration과 isolated runner/HSM key 등록·활성 → actual `gfx1100` `10/10` → 동일 final artifact local `gfx1030` 재실행 → historical resolver/외부 monotonic anchor → broad iteration host-copy-zero → certificate-bound SPD-gated PCG 상태기계. Completion-only raw export는 v0.2.27에 exact three blocking D2H로 닫혔고 v0.2.43 ResultIR 및 v0.2.45 DiagnosticIR은 retained-source factory direct-call contract에서 추가 D2H/solve/export 0으로 그 bytes를 재사용하지만 transitive/process-wide operation 계측값은 아니다. Raw global/export receipt의 historical false claim은 변경하지 않는다. Active final-guard integrated coverage는 v0.2.26, consumed/pending owner-loss recovery는 v0.2.25의 process-local 계약으로만 닫혔다. Durable ledger receipt, artifact identity receipt, 합성 signed verifier test, local unsigned 10/10 또는 detached ResultIR/DiagnosticIR을 solution-ready, multiarchitecture, standalone provenance나 promotion 증거로 재분류하지 않는다
- broad iteration-host-copy-zero convergence 정책, all-converged 10/10 ResultIRV2, GPU reaction/recovery/energy 및 authoritative hardware/signed 결속
...
역사 관찰이며 token-hardened current-source hardware gate, standalone hardware
provenance, signature 또는 promotion 증거가 아니다.
