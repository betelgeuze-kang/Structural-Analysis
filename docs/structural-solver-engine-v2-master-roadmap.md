...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: 완료된 still-open live sealed transaction 위에 later column/restart global control·final guard 구현 → full CPU/HIP parity·iteration host-copy-zero; 이후 certificate-bound SPD-gated PCG 상태기계. D2H 없이 host가 exact mask scalar, validator verdict, commit 또는 device outcome을 관찰했다고 쓰지 않으며 conditional program continuity와 authoritative numerical transaction receipt를 계속 분리한다
