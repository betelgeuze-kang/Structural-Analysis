...
- Version: 0.2.34 working milestone
- 기준일: 2026-07-14
- 감사 기준: 마지막 pushed audit HEAD `b2284e7a640932f9e21f5d78ed141097c721d4dd`의 external signed evidence verifier v1; v0.2.34 external release identity v1은 이 HEAD 이후의 local unpushed working milestone이며 `origin/main` 포함으로 해석하지 않음
...
- v0.2.34의 독립 artifact identity 재생 후에도 남은 durable run ledger, identity receipt의 serialized signed binding, package trust-anchor active key 등록, 독립 runner 운영과 외부 actual `gfx1100` signed cell `0/10`; 현재 working milestone은 실제 hardware 또는 signed promotion evidence가 아니며, local unsigned `gfx1030` fixed-suite `10/10`도 동일 final key-bearing artifact에서 재실행하기 전에는 two-architecture claim으로 승격하지 않음
...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| 0.2.34 working | 2026-07-14 | [HIP FGMRES external release identity v1](engine-v2-hip-fgmres-external-release-identity-v1.md)을 local unpushed working milestone로 추가했다. Candidate wheel의 same-FD/ZIP/full `RECORD`, 설치 distribution `RECORD`/declared scripts, clean Git manifest/exact `git archive`/runner·build·lock hash, exact declared-target-environment runtime dependency wheel closure와 fixed recipe policy를 독립 재생한다. 핵심 입력을 두 번 순차 replay하고 challenge 발급·signed verify 직전에 fresh replay하며, 검증된 identity에서 기존 v1 release binding을 파생해 process-local verified release/signed-evidence capability로 결합한다. Pushed audit HEAD는 `b2284e7a640932f9e21f5d78ed141097c721d4dd`로 유지한다. Atomic multi-artifact snapshot, build 실행/재현성, remote commit authenticity, build-system dependency closure, dependency 설치 실행/current-interpreter wheel-tag 호환성, bounded total source-artifact memory, hostile same-process mint isolation, identity receipt hash의 serialized signed binding, durable ledger, runner honesty, hardware attestation은 false다. Active key `0`, external actual `gfx1100` `0/10`, promotion/commercial, ResultIR, host-copy-zero, O(N), speedup claim도 false다. |
