...
- v0.2.45 current-source focused/public 4개 파일은 `41 passed in 378.12s`, capability matrix는 `9 passed`, hardware harness는 `1 test collected`를 확인했다. Real gfx agent가 없는 환경에서 non-required gate는 `1 skipped`, required gate는 skip 없이 `1 failed`했으므로 actual local token-hardened `gfx1030` single/family gate는 pending이다. External `gfx1100`, 별도 all-converged registry의 10/10 ResultIRV2 ready, broad host-copy-zero, GPU recovery, standalone/signed provenance, end-to-end O(N), speedup, commercial/promotion도 pending 또는 false다.
