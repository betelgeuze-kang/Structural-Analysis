...
- [HIP FGMRES recurrence launch/fence rolling ordinal audit v1](engine-v2-hip-fgmres-recurrence-launch-fence-audit-v1.md): v0.2.41 feature-branch publication milestone은 기존 canonical/sealed/global/transfer-audit receipt를 변경하지 않는 additive companion contract다. Exact process-local `HipRtcFgmresV2Kernel`의 `hipMemsetAsync`, `hipModuleLaunchKernel`, `hipStreamSynchronize` choke point에 native call 직전 monotonic operation ordinal과 fixed-binary SHA-256 predecessor chain을 기록하고 exact status `0`/nonzero/예외·`BaseException`을 success/rejected/ambiguous로 보존한다. Canonical enqueue 전 start에서 owned8 memset `8`회, canonical fixed launch descriptor, 첫 fence, sealed checkpoint `4` launch, 두 번째 fence, global continuation, terminal fence의 descriptor chain을 retained full schedule에서 재생해 completion-export child open 전 end head와 exact 일치해야 receipt를 발행한다. Same kernel/loaded runtime/checkpoint token/device/stream과 canonical/sealed/global receipt·schedule·ABI identity를 expected-context로 교차결속하고 raw pointer/token/object identity는 직렬화하지 않는다. Event 배열 없이 head/counter/tail만 유지해 ledger event당 메모리는 `O(1)`, 전체 host-control 계측/replay는 `O(L)`이지만 이는 FE solver end-to-end `O(N)` 증거가 아니다. Focused/public/capability `22 passed in 154.71s`와 actual local `gfx1030` single-case required gate `1 passed in 37.24s`를 통과했고, latter는 owned memset `8/8`, full launch attempt/success exact 일치, fence `3/3`, ordinal-seal-before-export를 확인했다. 이 actual 결과는 비영속 작업 세션 관찰이다. Raw/fresh binding, process-wide ROCm, 모든 device operation, hostile same-process mutation, GPU semantic execution/device content/numerical parity, query-recovered ambiguous fence의 clean relabel, broad iteration-host-copy-zero, external `gfx1100`, signed truth, ResultIR, speedup, commercial/promotion은 false이며 `origin/main` inclusion도 주장하지 않는다.
...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| 0.2.41 feature-branch publication milestone | 2026-07-15 | [HIP FGMRES recurrence launch/fence rolling ordinal audit v1](engine-v2-hip-fgmres-recurrence-launch-fence-audit-v1.md)을 추가했다. Exact process-local FGMRES-v2 kernel의 memset/launch/fence native choke point에서 call 전 ordinal과 rolling predecessor hash를 기록하고, canonical enqueue 전부터 completion export open 전 terminal fence까지 fixed `8` memset + full recurrence descriptors + `3` fences를 replay하는 additive non-promoting receipt다. Same kernel/runtime/token/device/stream 및 schedule/ABI lineage를 retained expected-context로 검증한다. Focused/public/capability `22 passed in 154.71s`와 actual local `gfx1030` single-case required gate `1 passed in 37.24s`에서 owned memset `8/8`, full launch attempt/success exact 일치, fence `3/3`, ordinal-seal-before-export를 확인했다. 이 actual 결과는 비영속 작업 세션 관찰이다. Detached receipt 단독 provenance, process-wide/all-device visibility, GPU execution/content/parity, broad iteration-host-copy-zero, external `gfx1100`, O(N), speedup, commercial/promotion은 false이며 `origin/main` inclusion도 주장하지 않는다. |
