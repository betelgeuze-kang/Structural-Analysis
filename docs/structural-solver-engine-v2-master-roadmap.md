...
- Version: 0.2.44 HIP FGMRES model-family ResultIR disposition v1 unpublished candidate milestone
- 기준일: 2026-07-15
...
- v0.2.44 focused adversarial `23 passed`, identity-token model-case/ResultIR·disposition·capability·public API 최종 통합 `91 passed in 116.59s`, capability/FGMRES-public/ResultIRV2-public 부분 집합 `19 passed`는 각 nonconverged bridge 자체 거부/forced injection, missing/duplicate/foreign, non-recycled token 기반 serially-identical cross-run splice, clone/coherent rehash/plan·provenance·issuance transplant, final source race, detached no-live-replay, schema-alone exact truth table와 AST operation guard를 통과했다. Hardware harness는 `1 test collected`를 확인했다. Aggregate projection은 추가 native/device solve가 없지만 fixed-registry validation의 deterministic CPU reference replay zero는 false다. Actual local `gfx1030` 10-slot disposition hardware gate, all-converged registry, nonconverged DiagnosticIR, 10/10 ResultIRV2 ready, external `gfx1100`, broad host-copy-zero, GPU recovery, standalone/signed provenance, O(N), speedup, commercial/promotion은 pending 또는 false다.
- [HIP FGMRES ResultIR v2](engine-v2-hip-fgmres-result-ir-v2.md): v0.2.43 unpublished candidate는 retained converged HIP single-model-case completion의 live process-local authority를 factory에서 검증하고 이미 export된 `solution_x`/`true_residual` raw bytes를 exact sparse `ExecutionPlanV2`에 결속한다. Free solution과 constrained `+0.0`으로 full displacement를 만들고 `plan.residual()`로 full `K*u-F`, constrained reaction, plan recovery arrays로 local member force와 element energy를 복원한 뒤 trial→committed StateIR과 descriptor-only ResultIRV2를 발행한다. Exact scoped result에서만 `result_ir_ready=true`이며 upstream receipt의 historical false claim은 불변이다. Global dense matrix와 추가 solve/device operation/D2H/export/fallback은 없고 upstream completion export의 기존 blocking D2H `3`회는 그대로다. Live authority는 factory 경계에서만 성립하며 weak-registry token은 exact factory-issued case를 결속하되 live context를 보유하지 않는다. Retained value seal/plan/states의 post-close validation은 standalone provenance가 아니다. Materialization은 `O(G+nnz+144E)`, ResultIR 여섯 raw array는 `24G+104E+8F` bytes이며 detached seal raw HIP payload `16F`는 별도다. 이는 end-to-end O(N) 또는 speedup 증거가 아니다.
- v0.2.43 generic/bridge contract `25 passed`, public API `3 passed`, capability `8 passed`, combined `44 passed in 27.67s`, model-case downstream authority `24 passed in 1.85s`를 확인했다. Exact clone/coherent rehash/registry transplant 및 unissued exact-case clone의 identity-token 거부도 회귀로 고정했다. Identity-token 패치 전 source의 required local `gfx1030` 단일 모델 ResultIR gate는 CPU fallback 없이 `1 passed in 169.10s (0:02:49)`로 통과했지만 unsigned 비영속 역사 관찰이며 token-hardened current-source hardware gate는 pending이다. v0.2.44에서 fixed package의 contract-level 7-ready/3-nonconverged disposition은 추가됐지만 actual aggregate hardware gate와 external `gfx1100`은 pending이며 process-wide/broad iteration-host-copy-zero, GPU recovery, standalone/signed provenance, promotion/commercial, nonlinear/dynamic/shell/solid/contact claim은 false다.
...
- v0.2.44에서 scoped sparse StateIR/ResultIR bridge 7개와 intentional `max_iterations` 3개의 비발행 10-slot disposition은 contract-level로 완료했다. Token-hardened current-source single-case/aggregate actual `gfx1030` gate와 실제 peak RSS 측정, 별도 all-converged registry의 10/10 ready gate 및 nonconverged DiagnosticIR은 후속 계약으로 분리
...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: v0.2.44 exact package 7-ready/3-nonconverged disposition contract는 완료했으므로 token-hardened current-source single-case 및 aggregate actual `gfx1030` 재검증 → 별도 all-converged registry와 DiagnosticIR 의미론 확정 → 실제 reviewer root bootstrap/migration과 isolated runner/HSM key 등록·활성 → actual `gfx1100` `10/10` → 동일 final artifact local `gfx1030` 재실행 → historical resolver/외부 monotonic anchor → broad iteration host-copy-zero → certificate-bound SPD-gated PCG 상태기계. Completion-only raw export는 v0.2.27에 exact three blocking D2H로 닫혔고 v0.2.43 ResultIR projection은 추가 D2H/solve/export 0으로 그 bytes를 재사용하지만, raw global/export receipt의 historical false claim은 변경하지 않는다. Active final-guard integrated coverage는 v0.2.26, consumed/pending owner-loss recovery는 v0.2.25의 process-local 계약으로만 닫혔다. Durable ledger receipt, artifact identity receipt, 합성 signed verifier test, local unsigned 10/10 또는 detached ResultIR을 solution-ready, multiarchitecture, standalone provenance나 promotion 증거로 재분류하지 않는다
- broad iteration-host-copy-zero convergence 정책, all-converged 10/10 ResultIRV2, nonconverged DiagnosticIR, GPU reaction/recovery/energy 및 authoritative hardware/signed 결속
