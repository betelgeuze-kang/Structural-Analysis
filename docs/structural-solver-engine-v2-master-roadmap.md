...
- Version: 0.2.27
- 기준일: 2026-07-14
- 감사 기준: local v0.2.26 base commit `e0bd0231` (`origin/main` 포함으로 해석하지 않음)
...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: 명시적 terminal-outcome observation contract → model-family·multi-architecture CPU/HIP full parity·iteration host-copy-zero; 이후 ResultIR integration과 certificate-bound SPD-gated PCG 상태기계. Completion-only raw export는 v0.2.27에 exact three blocking D2H로 닫혔지만 opaque record/outcome을 해석하지 않고, active final-guard integrated coverage는 v0.2.26, consumed/pending owner-loss recovery는 v0.2.25의 process-local 계약으로만 닫혔다. Verification-only D2H나 raw exported bytes를 authoritative terminal/numerical receipt로 재분류하지 않는다
