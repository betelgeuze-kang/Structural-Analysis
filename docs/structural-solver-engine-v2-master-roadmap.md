...
| 버전 | 날짜 | 변경 |
| --- | --- | --- |
| 0.2.40 working-tree milestone | 2026-07-15 | [HIP FGMRES model-family host-transfer audit composition v1](engine-v2-hip-fgmres-model-family-host-transfer-audit-v1.md)을 추가했다. Exact registry `gfx1030` 10-slot family-v2 result와 10개 live audit context/result를 같은 completion-export 객체 identity로 결합하고 completion/global/recurrence/kernel/device/F/R을 교차검증한다. Slot별 recurrence copy API attempt `0`, export `3/3/0`; suite 합계 `30/30/0`, `4,408` bytes를 고정한다. Shared hardware helper의 canonical-ready hook과 기존 10-slot loop의 audit exporter 재사용으로 추가 GPU solve/export를 만들지 않는다. 합성 adversarial/public/capability 검증은 진행 중이며 actual composed 10-slot hardware 실행은 pending이다. Detached receipt provenance, external `gfx1100`, process-wide/pre-window DMA, broad iteration-host-copy-zero, full family/multiarchitecture, signed evidence, ResultIR, O(N), speedup, commercial/promotion은 false다. Commit/push는 완료로 주장하지 않는다. |
