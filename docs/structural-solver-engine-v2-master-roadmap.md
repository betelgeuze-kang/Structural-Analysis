...
- Version: 0.2.28
- 기준일: 2026-07-14
- 감사 기준: local v0.2.27 base commit `fcebfcf9` (`origin/main` 포함으로 해석하지 않음)
...
- [FGMRES recurrence ABI v2](engine-v2-hip-fgmres-recurrence-abi-v2.md)의 다음 순서: model-family·multi-architecture CPU/HIP full parity → iteration host-copy-zero → ResultIR integration → certificate-bound SPD-gated PCG 상태기계. Completion-only raw export는 v0.2.27에 exact three blocking D2H로 닫혔고 v0.2.28 observer가 exact process-local terminal record를 해석하지만, raw global/export receipt는 outcome-free로 유지된다. Active final-guard integrated coverage는 v0.2.26, consumed/pending owner-loss recovery는 v0.2.25의 process-local 계약으로만 닫혔다. Observer receipt를 numerical parity, solution-ready 또는 promotion 증거로 재분류하지 않는다
