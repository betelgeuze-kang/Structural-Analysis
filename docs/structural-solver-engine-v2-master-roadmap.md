...
- Version: 0.2.36 feature-branch publication milestone
- 기준일: 2026-07-15
- 감사 기준: pushed audit baseline `59a09585884585cff29b0c6a35667b4502334646`의 external durable replay ledger v1; v0.2.36 signed release-identity binding v2는 이 기준 이후의 `codex/engine-v2-v0.2.27-completion-export-20260714` feature-branch publication milestone이며 `origin/main` 병합으로 해석하지 않음
...
- v0.2.36의 full identity receipt serialized signed binding 뒤에도 남은 package trust-anchor active key 등록, 검토된 rotation/revocation lifecycle, 격리 독립 runner/HSM key 운영과 외부 actual `gfx1100` signed cell `0/10`; 현재 v2 contract는 exactly-once·cross-host/ledger·rollback-resistant transparency log 또는 실제 hardware/signed promotion evidence가 아니며, local unsigned `gfx1030` fixed-suite `10/10`도 동일 final key-bearing artifact에서 재실행하기 전에는 two-architecture claim으로 승격하지 않음
