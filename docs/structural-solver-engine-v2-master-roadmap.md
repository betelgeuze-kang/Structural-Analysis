...
- Version: 0.2.35 working milestone
- 기준일: 2026-07-15
- 감사 기준: 마지막 pushed audit HEAD `7a46bfbd732e07291bc166ac6ff4266e1277e238`의 external release identity v1; v0.2.35 external durable replay ledger v1은 이 HEAD 이후의 branch publication candidate이며 `origin/main` 포함으로 해석하지 않음
...
- v0.2.35의 단일 local durable replay ledger 뒤에도 남은 identity receipt의 serialized signed binding, package trust-anchor active key 등록, 격리 독립 runner/key 운영과 외부 actual `gfx1100` signed cell `0/10`; 현재 ledger working milestone은 exactly-once·cross-host/ledger·rollback-resistant transparency log 또는 실제 hardware/signed promotion evidence가 아니며, local unsigned `gfx1030` fixed-suite `10/10`도 동일 final key-bearing artifact에서 재실행하기 전에는 two-architecture claim으로 승격하지 않음
