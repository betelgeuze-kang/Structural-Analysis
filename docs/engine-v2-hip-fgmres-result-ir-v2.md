...
다섯 파일 combined `65 passed in 29.31s`를 확인했다. Current source hardware harness는 real
gfx agent 부재에서 non-required `1 skipped`, required `1 failed`로 hardware-pending을
유지했다. 첫 actual run은 production global context가 매 검증마다
...
3. **DiagnosticIR 분리는 v0.2.45에서 완료:** 미수렴 partial iterate는
   solution-ready ResultIRV2와 분리된 additive DiagnosticIR로 보존한다. 10/10
   solution-ready 검증은 별도 all-converged fixture registry에서 수행한다.
