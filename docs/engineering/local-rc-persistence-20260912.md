...
rotation/recovery UI; protect and back up local credentials. Initialization writes
and fsyncs a private temporary file, then publishes a complete owner by atomic
no-replace hard link. Concurrent initializers validate the winner rather than
replacing it. A crash before publication may leave an unreferenced temporary file;
it is never treated as authority. Existing malformed owner files still reject and
require explicit local repair. Nonregular/symlink owner files reject without
blocking. Tokens are bounded to 16..4096 UTF-8 bytes; unsupported platforms and
invalid credentials reject before creating the repository.
...
The 107 existing local cases were rerun successfully before this continuation.
Fourteen additional initialization regressions now cover same/different-credential
races, failed fsync, actual child death before owner publication, malformed and
nonregular owner files, token bounds and unsupported-platform preflight. The
combined local run passes **121 tests**, without deselections or skipped cases.
It still uses the base CI wheel plus exact affected source files, not the entire
repository. These cases validate software behavior, not independent physics or
AMD performance. Static formatting and full hosted CI remain separate checks.
