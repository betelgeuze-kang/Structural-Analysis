...
The application production build is multi-entry: `vite.config.ts` emits Workbench at `dist/index.html` and Static Viewer at `dist/src/structure-viewer/index.html`. This preserved nested path is part of the Workbench/Viewer delivery contract, not a development-server convenience. The Rust-native `structural-frontend-contract delivery` checker validates both entries, every emitted local asset reference, the Viewer shell marker, and the Workbench bundle's Viewer target without executing Node or a browser; browser E2E additionally proves the iframe did not resolve to the Workbench SPA fallback.
