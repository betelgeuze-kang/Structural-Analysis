...
- token-hardened current-source actual local `gfx1030` single/family gate
...
1. Current-source single-case·3-slot family DiagnosticIR hardware harness를 actual local
   `gfx1030`에서 재실행한다.
2. 현 종료-회귀 registry를 유지하고, 10개 고유 ModelIR·실제 해석
   tolerance를 갖는 별도 all-converged registry와 10/10 ResultIR vertical
   slice를 구현한다.
