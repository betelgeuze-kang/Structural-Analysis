...
values, incomplete or duplicate quantities, duplicate components, reordered
rows, hash tampering, and rehashed changes to any fixed v1 semantic fail closed.

The capability registry exposes this catalog as `bounded_public` because its
Python types and packaged JSON Schema are stable consumer contracts. That
status applies only to the comparison vocabulary; it grants no numerical,
engineering, nonlinear, release, or commercial authority to a result.
