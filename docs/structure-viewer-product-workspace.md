...
- Frontend dependency-install authority: hosted workflows now invoke Rust-native `structural-frontend-contract frontend-install` directly; `npm run install:dependencies` remains only a local convenience. Rust validates the package/lock/source-map contract, removes inherited `NODE_OPTIONS`, owns the exact `npm ci` direct child and rejects contract mutation. npm registry/cache access, lifecycle scripts, configuration/environment, transitive processes, extracted package bytes, `node_modules` contents and rollback remain transitional and uninstrumented.
- Frontend dependency-audit authority: frontend-web CI now invokes Rust-native `structural-frontend-contract frontend-audit` directly for the exact `npm audit --audit-level high` child. Rust freezes the frontend contract, removes inherited `NODE_OPTIONS`, rejects repository-contract mutation and records every numeric exit. To preserve the existing non-blocking policy, numeric nonzero is reported only as `advisory_or_tool_failure`; Rust does not parse findings or distinguish vulnerabilities from registry/network/configuration/tool failure, and no dependency or license clearance is claimed.
- Quality-gate frontend authority: `scripts/verify_quality_gate.py` retains Python ownership of the overall repository gate sequence, but all frontend install, strict audit, check, build, Viewer manifest, browser smoke, sample workflow, PDF smoke, performance and visual-regression entries now call `structural-frontend-contract` directly through Cargo. It contains no npm package-script launcher; `--fail-on-nonzero` makes strict audit publish its canonical receipt before returning failure.
- Frontend development authority: `npm run dev` now enters through Rust-native `structural-frontend-contract frontend-dev`. Rust hashes the installed Vite CLI, removes inherited `NODE_OPTIONS`, fixes the loopback/strict-port launch, and owns the direct Node child while deliberately preserving source mutation for Vite HMR. Vite listener readiness, plugins, environment loading and browser behavior remain uninstrumented.
...
- Browser runtime installation authority: hosted workflows now invoke Rust-native `structural-frontend-contract playwright-install` directly; `npm run install:browser-runtime` remains only a local convenience. Rust hashes the installed Playwright CLI and owns the exact Chromium plus host-dependency installation child, while Playwright still owns downloads, caches, elevation and host-package mutation; downloaded bytes and rollback remain uninstrumented.
- Hosted frontend/browser entrypoint authority: frontend web, nightly full, runtime-input Viewer, and Viewer-browser workflows call `structural-frontend-contract` through Cargo directly and contain no `npm run`, `npx`, direct Node, or direct `npm audit` product entrypoint. Native catalog/evidence Bash wrappers remain for root/timestamp projection, and Node/npm/Playwright still perform the explicitly retained inner work.
- Phase 5 task-smoke authority: `scripts/run_phase5_task_based_ux_browser_smoke.py` now launches one direct Cargo `structural-frontend-contract phase5-task-browser-smoke` command and contains no direct npm, npx, Node, preview-server, or socket-readiness process authority. Rust owns the bounded frontend build, fixed `127.0.0.1:4173` SPA listener, exact developer-preview specification and Playwright child, while Python strictly verifies the canonical receipt and retains legacy release-evidence assembly. Node/TypeScript/Vite/Playwright/Chromium, rendered React behavior, human usability observation, hosted live execution, C5, and C6 remain open.
