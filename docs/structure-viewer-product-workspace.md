...
- Hosted frontend/browser entrypoint authority: frontend web, nightly full, runtime-input Viewer, and Viewer-browser workflows call `structural-frontend-contract` through Cargo directly and contain no `npm run`, `npx`, or direct Node product entrypoint. Native catalog/evidence Bash wrappers remain for root/timestamp projection, and Node/npm/Playwright still perform the explicitly retained inner work.
