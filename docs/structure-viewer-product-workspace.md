...
- Browser runtime installation authority: hosted workflows now use Rust-native `structural-frontend-contract playwright-install` through `npm run install:browser-runtime`. Rust hashes the installed Playwright CLI and owns the exact Chromium plus host-dependency installation child, while Playwright still owns downloads, caches, elevation and host-package mutation; downloaded bytes and rollback remain uninstrumented.
