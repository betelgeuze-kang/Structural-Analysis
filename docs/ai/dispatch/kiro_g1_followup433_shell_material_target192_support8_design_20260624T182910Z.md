# Kiro Design Slice: G1 Shell-Material Target192 Support8 Boundary
...
Goal: Design the next bounded G1 CPU-diagnostic continuation after target128/support8 became the latest counted frontier and target128/support16 failed to improve it.

Current blocker: G1 remains partial. The latest counted frontier is `1.3092276661494922 N`, about `2618.46x` above the `5e-4 N` direct-residual gate, from `mgt_shell_material_rowcorr_budget_controller_followup431_target128_support8_cpu_continuation_children/mgt_shell_material_rowcorr_budget_controller_followup431_target128_support8_cpu_continuation_candidate1_target128_support8.json`. Followup432 target128/support16 recorded no descent from that checkpoint.

Scope: Propose a compact design for one bounded target192/support8 continuation from the followup431 compact checkpoint. Keep it CPU diagnostic, receipt-visible, and non-promoting unless the residual/increment gates truly close. Do not recommend full-load/full-mesh/material Newton or ROCm/HIP closure claims from this receipt.
...
- The followup433 receipt records the selected target/support counts, source checkpoint, stop reason, final residual, and promotion count.
- If target192/support8 improves the counted frontier, update the frontier chain and tests with exact residual/ratio values.
- If target192/support8 does not improve, record it as non-promoting boundary evidence and keep followup431 as the counted frontier.
