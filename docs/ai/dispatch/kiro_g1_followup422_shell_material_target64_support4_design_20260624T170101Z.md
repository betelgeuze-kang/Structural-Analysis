...
Goal: Design the next bounded G1 shell-material row-correction continuation evidence slice after followup421 while preserving the current readiness claim boundary.

Current blocker: G1 remains partial. The latest counted CPU-diagnostic frontier is followup421 target64/support4 at `2.0315225271909085 N`, still about `4.06e3x` above the `5e-4 N` residual gate. Followup421 continued target64/support4 with `row_correction_promotion_count=4` and `row_correction_stop_reason=max_promotions_exhausted`, so this operator has not yet reached a no-descent boundary.

Scope: Design only a narrow followup422 continuation from the followup421 compact checkpoint. Prefer the same target64/support4 `largest_rows`, `row_strongest`, and `current_tangent` CPU-diagnostic probe because followup421 still exhausted the promotion budget through accepted descent. The slice may create a new receipt, update the G1 shell-material budgeted continuation status chain, update the two gap ledgers, update focused tests, and refresh status/readiness artifacts. Non-goals: do not claim G1 closure, do not broaden to external G6/G7 evidence, do not run push/merge/deploy/release, do not hide proxy/partial/external-blocked states.

Candidate files:

- `implementation/phase1/release_evidence/productization/mgt_shell_material_rowcorr_budget_controller_followup421_target64_support4_cpu_continuation_compact_checkpoint.npz`
...
- New followup422 receipt is counted frontier evidence only if it improves the residual; otherwise it stays visible as non-promoting/counter evidence.
