...
Goal: Design the next bounded G1 shell-material row-correction continuation evidence slice after followup423 while preserving the current readiness claim boundary.

Current blocker: G1 remains partial. The latest counted CPU-diagnostic frontier is followup423 target64/support4 at `1.7909880486798002 N`, still about `3.58e3x` above the `5e-4 N` residual gate. Followup423 continued target64/support4 with `row_correction_promotion_count=4` and `row_correction_stop_reason=max_promotions_exhausted`, so this operator has not yet reached a no-descent boundary.

Scope: Design only a narrow followup424 continuation from the followup423 compact checkpoint. Prefer the same target64/support4 `largest_rows`, `row_strongest`, and `current_tangent` CPU-diagnostic probe because followup423 still exhausted the promotion budget through accepted descent. The slice may create a new receipt, update the G1 shell-material budgeted continuation status chain, update the two gap ledgers, update focused tests, and refresh status/readiness artifacts. Non-goals: do not claim G1 closure, do not broaden to external G6/G7 evidence, do not run push/merge/deploy/release, do not hide proxy/partial/external-blocked states.
...
- New followup424 receipt is counted frontier evidence only if it improves the residual; otherwise it stays visible as non-promoting/counter evidence.
