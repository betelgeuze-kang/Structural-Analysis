...
Goal: Design the next bounded G1 shell-material CPU diagnostic continuation after followup423.

Current blocker: G1 remains partial. The latest counted shell-material frontier is `1.7909880486798002 N` from `mgt_shell_material_rowcorr_budget_controller_followup423_target64_support4_cpu_continuation_children/mgt_shell_material_rowcorr_budget_controller_followup423_target64_support4_cpu_continuation_candidate1_target64_support4.json`, still about `3.58e3x` above the `5e-4 N` direct-residual gate. The child stopped at `max_promotions_exhausted`, but `same_operator_exhausted_at_latest_checkpoint=false`; full-load/full-mesh/material Newton/production ROCm-HIP gates remain open.

Scope: Recommend whether Codex should continue the same target64/support4 current-tangent row-correction operator from the followup423 compact checkpoint with one bounded child, or first run a non-promoting replay/operator-change diagnostic. Keep the result CPU-diagnostic only. Do not propose broad refactors, external submissions, readiness closure, or edits outside receipt/status/ledger refreshes.

Candidate files:

- `implementation/phase1/release_evidence/productization/mgt_shell_material_rowcorr_budget_controller_followup423_target64_support4_cpu_continuation_compact_checkpoint.npz`
