...
Goal: Design the next bounded G1 shell-material CPU diagnostic continuation after followup428.

Current blocker: G1 remains partial. Followup428 continued target128/support4 from the followup427 compact checkpoint and lowered the counted frontier from `1.482511519898516 N` to `1.391983647766473 N`, still about `2.78e3x` above the `5e-4 N` direct-residual gate. The latest target128/support4 child stopped at `max_promotions_exhausted`; no target128/support4 no-descent receipt exists at the latest checkpoint.

Scope: Recommend whether Codex should continue target128/support4 current-tangent row correction from the followup428 compact checkpoint with one bounded child, or first run a different changed-operator diagnostic. Keep the result CPU-diagnostic only. Do not propose broad refactors, external submissions, readiness closure, or edits outside receipt/status/ledger refreshes.
...
- If target128/support4 improves, count it as the latest CPU-diagnostic frontier; if it does not, record it as non-promoting boundary evidence without replacing followup428.
