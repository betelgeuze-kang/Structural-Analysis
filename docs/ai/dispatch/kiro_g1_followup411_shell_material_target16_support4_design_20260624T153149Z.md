...
Goal: Design the next bounded G1 shell-material row-correction continuation evidence slice after followup410, preserving the current readiness claim boundary.

Current blocker: G1 remains partial. The latest counted CPU-diagnostic frontier is followup410 target16/support4 at `3.696008025377786 N`, still about `7.39e3x` above the `5e-4 N` residual gate and still blocked by full-load, full-mesh nonlinear equilibrium, material Newton breadth, and production ROCm/HIP residency.

Scope: Design only a narrow followup411 target16/support4 continuation from the followup410 compact checkpoint. The slice may create a new receipt, update the G1 shell-material budgeted continuation status chain, update the two gap ledgers, update focused tests, and refresh status/readiness artifacts. Non-goals: do not claim G1 closure, do not broaden to external G6/G7 evidence, do not run push/merge/deploy/release, do not hide proxy/partial/external-blocked states.

Candidate files:

- `implementation/phase1/release_evidence/productization/mgt_shell_material_rowcorr_budget_controller_followup410_target16_support4_cpu_continuation_compact_checkpoint.npz`
...
- New followup411 receipt is either counted frontier evidence only if it improves the residual, or visible counter/non-promoting evidence if it does not.
