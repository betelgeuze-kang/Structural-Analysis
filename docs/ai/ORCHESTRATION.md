...
이 저장소에서는 별도 자동 실행기나 로컬 runner를 실행하지 않는다. 반복 진행은 Codex의 goal 기능이 맡고, Cursor auto와 OpenCode worker는 필요한 구현 조각만 처리하는 worker로 사용한다. OpenCode wrapper의 기본 모델은 MiniMax M3이며 registry id는 `opencode-go/minimax-m3`이다.
...
OpenCode worker가 필요하면 다음 형태로 실행한다. `scripts/build_ai_orchestration_preflight_report.py`는 wrapper 기본 모델이 현재 OpenCode model registry에 있는지도 별도 evidence로 기록한다.
