...
이 저장소에서는 별도 자동 실행기나 로컬 runner를 실행하지 않는다. 반복 진행은 Codex의 goal 기능이 맡고, Cursor auto와 OpenCode worker는 필요한 구현 조각만 처리하는 worker로 사용한다. 현재 OpenCode task assignment는 OpenCode 실행 대신 같은 prompt file을 Cursor `composer-2.5`에 바로 넘긴다.
