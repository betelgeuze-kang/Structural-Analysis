...
- C3 implementation: ABI v1.9 consumes the two former 144-byte table reserved slots for distinct
  ...
  package consumption cover this integration. Because C2 remains open, this implementation does
  not advance the sequential capability beyond C1.
...
The current sequential promotion is therefore C1 only; the v1.9/Rust work is retained as a C3
implementation candidate behind the still-open C2 gate.
