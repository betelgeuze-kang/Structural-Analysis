...
HIP C2, arbitrary ModelIR-to-analysis adaptation, distributed durable jobs/API, tenant
authorization, native Workbench or C6 removal. Separate bounded C5 slices now own the exact
fixed-guided frame3d ModelIR profile, local durable-job submit/poll/cancel/crash reconciliation, a
loopback single-tenant static-role HTTP facade, three global external-comparison quantities and a
single-page native PDF. They do not broaden this synchronous product claim or close arbitrary
topology, distributed/multi-tenant service, live external-solver/same-mesh, PDF/A or accessibility
gates.
