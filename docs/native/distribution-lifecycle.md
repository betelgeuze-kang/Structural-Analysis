...
16. runs the installed existing-`frame_3d`-element local-axis rotation edit twice, proves
    ...
    deterministic topology rendering with an empty `PATH`;
17. renders all four closed NDTHA response channels twice from the installed Workbench, proves
...
18. renders all four fixed-guided original/deformed projections twice plus an explicit selected
...
19. renders the Korean top-displacement response and Korean isometric fixed-guided deformed view
...
20. exports the installed ModelIR-linear sparse report twice in each exact `en-US` and `ko-KR`
...
21. imports the exact normalized cantilever MGT profile into the installed ModelIR-linear path,
