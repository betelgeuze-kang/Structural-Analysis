...
22. creates the bounded ModelIR linear CPU analysis request twice, proves byte-identical typed
...
23. renders all four closed NDTHA response channels twice from the installed Workbench, proves
...
24. renders all four fixed-guided original/deformed projections twice plus an explicit selected
...
25. renders the Korean top-displacement response and Korean isometric fixed-guided deformed view
...
26. exports the installed ModelIR-linear sparse report twice in each exact `en-US` and `ko-KR`
...
27. imports the exact normalized cantilever MGT profile into the installed ModelIR-linear path,
...
28. installs an immutable update, rolls back and re-verifies activation;
29. emits an append-only v26 hash-bound receipt with ModelIR/MGT result, report, MGT source,
   ...
   The receipt checker continues to accept frozen v1 through v25 receipts; v1 through v19 are not
   ...
   atomic linear-static-pattern creation plus exact recovered-external-load authority.
