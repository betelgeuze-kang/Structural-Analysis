...
- Visual identity v2 is layered as `design-tokens.css` → `design-theme.css` → `viewer-visual-identity.css` → `viewer-product-shell.css` → `commercial-cockpit-polish.css`. Default shell is **product** (`data-si-shell="product"`): viewport-first layout, glass HUD, collapsed rails; **workspace** (`data-si-shell="workspace"` or top-bar **Workspace** toggle) restores cockpit tabs, stage header, left overlay controls, and analysis chart strip. Viewport graphics use `viewer-visual-scene.js` (IBL/PMREM, infinite grid, selection outline, ViewCube, quality tiers) and `viewer-viewport-hud.js` (HUD, info overlay, contour legend). See `src/structure-viewer/vendor/VISUAL_ADDONS.md`.
