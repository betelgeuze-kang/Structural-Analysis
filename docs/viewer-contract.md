...
- Release verification runbook:
  ...
  6. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest <candidate-manifest.json> --assets-json <release-assets.json> --artifact-root <root> --tag-ref-present true --require-all --require-exact --fail-unclosed`.
  7. Overall P0 status: run `python3 scripts/check_p0_closure_status.py --release-assets-json <release-assets.json> --artifact-root <root> --tag-ref-present --fail-open` to combine P0-1 release publication with P0-2..P0-6 core evidence.
  ...
  9. Current blocker: `structural-analysis-artifacts-2026-04-26` now exists as a Git tag, but the GitHub Release object and its 12 manifest-listed assets are not published yet. The repo-local `implementation/phase1/release/` tree is stale and must not be used as the upload source. This is a release-publication gap, not a source-boundary gap. P0-1 closes only after a fresh artifact root is regenerated, the manifest is updated if needed, the release is created, exactly 12 manifest assets are uploaded, and metadata preflight plus SHA/bytes verification pass.
- Verification split: manifest structure, release asset listing, and SHA/bytes checks are scriptable; fresh artifact root regeneration and GitHub Release publication require a token-backed publishing step.
