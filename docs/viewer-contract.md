...
- Release verification runbook:
  ...
  7. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest <candidate-manifest.json> --assets-json <release-assets.json> --artifact-root <root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --tag-ref-present true --require-all --require-exact --fail-unclosed`.
  8. Overall P0 status: run `python3 scripts/check_p0_closure_status.py --manifest <candidate-manifest.json> --release-assets-json <release-assets.json> --artifact-root <root> --upload-plan-json <release-upload-plan.json> --metadata-preflight-json <metadata-preflight.json> --tag-ref-present --fail-open` to combine P0-1 release publication with P0-2..P0-6 core evidence.
  ...
  10. Published status: `structural-analysis-artifacts-2026-04-26` is published as a GitHub Release with exactly the 12 manifest-listed assets. Keep repo-local `implementation/phase1/release/` out of the upload path; use the release asset listing, upload plan, metadata preflight, and hydrated published-byte SHA/bytes checks as the delivery contract.
- P1 handoff: with P0-1 closed, run `python3 scripts/check_p1_readiness_status.py --p0-status <p0-status.json> --json --out <p1-readiness-status.json> --out-md <p1-readiness-status.md> --fail-blocked` and then `python3 scripts/check_p1_benchmark_breadth_status.py --p1-readiness-status <p1-readiness-status.json> --json --out <p1-benchmark-breadth-status.json> --out-md <p1-benchmark-breadth-status.md> --fail-blocked` to move into the P1 quality/fallback/benchmark breadth slice.
