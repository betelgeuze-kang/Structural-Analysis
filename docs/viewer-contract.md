...
- The 3D source viewer keeps reusable browser/selection state helpers in ESM sidecars such as `viewer-real-drawing-browser-state.js` and `viewer-shared-selection-state.js`; release generators must inline those helpers for single-file delivery.
