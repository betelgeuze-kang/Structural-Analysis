...
- Release verification runbook:
  ...
  4. Full integrity: run `python3 scripts/verify_release_artifacts_manifest.py --manifest <candidate-manifest.json> --artifact-root <root> --require-artifacts` to verify SHA/bytes integrity for the 12 manifest-listed assets.
  ...
  6. Published-byte hydrate: run `python3 scripts/hydrate_github_release_assets.py --repo betelgeuze-kang/Structural-Analysis --manifest <candidate-manifest.json> --artifact-root <hydrated-root> --write`, then `python3 scripts/verify_release_artifacts_manifest.py --manifest <candidate-manifest.json> --artifact-root <hydrated-root>` to verify the actual GitHub Release bytes.
  7. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest <candidate-manifest.json> --assets-json <release-assets.json> --artifact-root <root> --tag-ref-present true --require-all --require-exact --fail-unclosed`.
  8. Overall P0 status: run `python3 scripts/check_p0_closure_status.py --release-assets-json <release-assets.json> --artifact-root <root> --tag-ref-present --fail-open` to combine P0-1 release publication with P0-2..P0-6 core evidence.
  ...
  10. Current blocker: `structural-analysis-artifacts-2026-04-26` now exists as a Git tag, but the GitHub Release object and its 12 manifest-listed assets are not published yet. The repo-local `implementation/phase1/release/` tree is stale and must not be used as the upload source. This is a release-publication gap, not a source-boundary gap. P0-1 closes only after a fresh artifact root is regenerated, the manifest is updated if needed, the release is created, exactly 12 manifest assets are uploaded, and metadata preflight plus source-root and published-byte SHA/bytes verification pass.
