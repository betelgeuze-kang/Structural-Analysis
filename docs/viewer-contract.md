...
- Release verification runbook:
  ...
  5. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest implementation/phase1/release_artifacts_manifest.json --assets-json <release-assets.json> --artifact-root <root> --tag-ref-present true --require-all --fail-unclosed`.
  6. Current blocker: `structural-analysis-artifacts-2026-04-26` does not exist yet in the GitHub API or `git ls-remote`, the repo-local `implementation/phase1/release/` tree is stale, and `prepare_release_upload_plan.py` fails on mismatched or missing assets. P0-1 closes only after a fresh artifact root is regenerated, the manifest is updated if needed, the tag/release is created, exactly 12 manifest assets are uploaded, and metadata preflight plus SHA/bytes verification pass.
