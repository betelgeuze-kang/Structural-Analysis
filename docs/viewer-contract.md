...
- Release verification runbook:
  ...
  3. Flat asset root: after regenerating local release outputs, run `python3 scripts/materialize_release_asset_root.py --manifest implementation/phase1/release_artifacts_manifest.json --artifact-root <root> --write` to copy the 12 manifest-listed assets into a flat upload root.
  4. Full integrity: run `python3 scripts/verify_release_artifacts_manifest.py --manifest implementation/phase1/release_artifacts_manifest.json --artifact-root <root> --require-artifacts` to verify SHA/bytes integrity for the 12 manifest-listed assets.
  5. Upload plan: run `python3 scripts/prepare_release_upload_plan.py --manifest implementation/phase1/release_artifacts_manifest.json --artifact-root <root> --out <release-upload-plan.json>` and upload only the `upload_assets` entries.
  6. Closure gate: run `python3 scripts/check_release_p0_closure.py --manifest implementation/phase1/release_artifacts_manifest.json --assets-json <release-assets.json> --artifact-root <root> --tag-ref-present true --require-all --fail-unclosed`.
  7. Current blocker: `structural-analysis-artifacts-2026-04-26` does not exist yet in the GitHub API or `git ls-remote`, the repo-local `implementation/phase1/release/` tree is stale, and `prepare_release_upload_plan.py` fails on mismatched or missing assets. This is a release-publication gap, not a source-boundary gap. P0-1 closes only after a fresh artifact root is regenerated, the manifest is updated if needed, the tag/release is created, exactly 12 manifest assets are uploaded, and metadata preflight plus SHA/bytes verification pass.
- Verification split: manifest structure, release asset listing, and SHA/bytes checks are scriptable; fresh artifact root regeneration, GitHub tag/release publication, and the actual asset upload are the external/publishing steps.
