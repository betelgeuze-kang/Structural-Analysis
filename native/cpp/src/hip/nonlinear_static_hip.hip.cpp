...
__device__ void update_derived_state(
    ...
    output->residual_inf = device_detail::vector_norm_inf(residual, config.story_count);
    output->residual_l2 = device_detail::vector_norm_l2(residual, config.story_count);
...
__global__ void nonlinear_static_newton_kernel(
    ...
    for (std::uint32_t iteration = 1U; iteration <= config.max_iter; ++iteration) {
        ...
        const double residual_inf = device_detail::vector_norm_inf(residual, config.story_count);
        ...
        if (!device_detail::solve_tridiagonal(
                config.story_count,
                lower,
                diagonal,
                upper,
        ...
        while (scale >= config.line_search_min) {
            ...
            const double trial_norm = device_detail::vector_norm_inf(residual, config.story_count);
