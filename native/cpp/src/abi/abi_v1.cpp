...
static_assert(sizeof(sa_api_v1) == 224U);
...
static_assert(offsetof(sa_api_v1, model_ir_linear_buckling_assemble) == 216U);
...
static_assert(sizeof(sa_model_ir_linear_buckling_assembly_config_v1) == 96U);
static_assert(offsetof(sa_model_ir_linear_buckling_assembly_config_v1, load_pattern_id) == 8U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_config_v1, equilibrium_displacement) == 24U);
static_assert(offsetof(sa_model_ir_linear_buckling_assembly_config_v1, flags) == 72U);
static_assert(offsetof(sa_model_ir_linear_buckling_assembly_config_v1, reserved) == 80U);
static_assert(sizeof(sa_model_ir_linear_buckling_assembly_outputs_v1) == 456U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_outputs_v1, active_dof_indices) == 8U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_outputs_v1, frame_stable_indices) == 200U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_outputs_v1, model_content_hash) == 296U);
static_assert(offsetof(sa_model_ir_linear_buckling_assembly_outputs_v1, reserved) == 440U);
static_assert(sizeof(sa_model_ir_linear_buckling_assembly_result_v1) == 80U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_result_v1, equilibrium_residual_inf_n) == 48U);
static_assert(
    offsetof(sa_model_ir_linear_buckling_assembly_result_v1, execution_backend) == 56U);
static_assert(offsetof(sa_model_ir_linear_buckling_assembly_result_v1, reserved) == 64U);
...
static_assert(offsetof(sa_sparse_linear_state_v1, initial_residual_inf) == 32U);
static_assert(offsetof(sa_sparse_linear_state_v1, vector_length) == 64U);
static_assert(offsetof(sa_sparse_linear_state_v1, solution) == 72U);
static_assert(offsetof(sa_sparse_linear_state_v1, diagonal_inverse) == 216U);
static_assert(offsetof(sa_sparse_linear_state_v1, reserved) == 264U);
...
[[nodiscard]] sa_status_code_v1 model_ir_linear_buckling_assemble_boundary(
    const sa_model_ir_handle_v1* const handle,
    const sa_model_ir_linear_buckling_assembly_config_v1* const config,
    const sa_model_ir_linear_buckling_assembly_outputs_v1* const outputs,
    sa_model_ir_linear_buckling_assembly_result_v1* const result,
    sa_error_buffer_v1* const error) noexcept {
    return contain_boundary(error, [handle, config, outputs, result, error]() -> sa_status_code_v1 {
        if (!pointer_is_aligned(config) || !pointer_is_aligned(outputs)
            || !pointer_is_aligned(result)) {
            return report_error(
                error,
                SA_ERR_INVALID_ARGUMENT,
                "ModelIR buckling assembly descriptor is null or misaligned");
        }
        if (config->abi_version != SA_ABI_V1_15 || outputs->abi_version != SA_ABI_V1_15
            || result->abi_version != SA_ABI_V1_15) {
            return report_error(
                error,
                SA_ERR_ABI_VERSION_MISMATCH,
                "ModelIR buckling assembly descriptors require ABI v1.15");
        }
        if (config->struct_size < sizeof(sa_model_ir_linear_buckling_assembly_config_v1)
            || outputs->struct_size < sizeof(sa_model_ir_linear_buckling_assembly_outputs_v1)
            || result->struct_size < sizeof(sa_model_ir_linear_buckling_assembly_result_v1)) {
            return report_error(
                error,
                SA_ERR_STRUCT_SIZE,
                "ModelIR buckling assembly descriptor struct_size is too small");
        }
        const auto reserved_nonzero = config->flags != 0U
            || std::any_of(
                std::begin(config->reserved), std::end(config->reserved), [](const auto value) {
        ...
        if (reserved_nonzero) {
            return report_error(
                error,
                SA_ERR_INVALID_ARGUMENT,
                "ModelIR buckling assembly reserved fields are not zero");
        }
        if (!valid_adapter_selector(config->load_pattern_id)) {
            return report_error(
                error,
                SA_ERR_INVALID_ARGUMENT,
                "ModelIR buckling assembly load selector is invalid");
        }
        ...
        const auto structural_entry_count =
            static_cast<std::uint64_t>(native_sizes.structural_entry_count);
        const auto frame_prestress_count =
            static_cast<std::uint64_t>(native_sizes.recovery_record_count);
        const auto model_identity_length =
            static_cast<std::uint64_t>(native_sizes.model_identity_length);

        auto status = validate_model_ir_linear_input_view(
            config->equilibrium_displacement,
            global_dof_count,
            SA_ELEMENT_TYPE_F64,
            SA_ABI_V1_15,
            "ModelIR buckling equilibrium displacement metadata is invalid",
            error);
        if (status != SA_OK) {
            return status;
        }
        const std::array output_views {
            ...
            &outputs->row_offsets,
            &outputs->column_indices,
            &outputs->geometric_stiffness,
            &outputs->frame_stable_indices,
            &outputs->frame_axial_compression_n,
            &outputs->model_content_hash,
            &outputs->model_semantic_hash,
            &outputs->model_provenance_hash,
        };
        ...
        for (std::size_t index = 0U; index < output_views.size(); ++index) {
            status = validate_model_ir_linear_output_view(
                *output_views[index],
                ...
                output_types[index],
                SA_ABI_V1_15,
                "ModelIR buckling assembly output metadata is invalid",
                error);
            if (status != SA_OK) {
                return status;
            }
            ...
            for (std::size_t right = left + 1U; right < descriptor_regions.size(); ++right) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly descriptors overlap");
                }
            }
        }
        const std::array input_regions {
            MemoryRegion {config->load_pattern_id.data, config->load_pattern_id.length},
            MemoryRegion {
                config->equilibrium_displacement.data,
                global_dof_count * sizeof(double),
            },
        };
        for (std::size_t index = 0U; index < input_regions.size(); ++index) {
            for (const auto& descriptor : descriptor_regions) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly input overlaps descriptor storage");
                }
            }
            for (std::size_t other = index + 1U; other < input_regions.size(); ++other) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly input buffers overlap");
                }
            }
        }
        std::array<MemoryRegion, 9> output_regions {};
        for (std::size_t index = 0U; index < output_regions.size(); ++index) {
            output_regions[index] = {
                output_views[index]->data,
                output_lengths[index] * element_size(output_types[index]),
            ...
            for (const auto& descriptor : descriptor_regions) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly output overlaps descriptor storage");
                }
            }
            for (const auto& input : input_regions) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly output overlaps input data");
                }
            }
        }
        for (std::size_t left = 0U; left < output_regions.size(); ++left) {
            for (std::size_t right = left + 1U; right < output_regions.size(); ++right) {
                if (ranges_overlap(
                    ...
                    return report_error(
                        error,
                        SA_ERR_INVALID_ARGUMENT,
                        "ModelIR buckling assembly output buffers overlap");
                }
            }
        }

        const auto displacement = std::span<const double> {
            static_cast<const double*>(config->equilibrium_displacement.data),
            static_cast<std::size_t>(global_dof_count),
        };
        const auto load_pattern_id = std::string_view {
            config->load_pattern_id.data,
        ...
        const auto native =
            structural::assembly::assemble_model_ir_linear_buckling_reference(
                *model, load_pattern_id, displacement);
        const auto& assembled = native.operator_result;
        const auto shape_valid = assembled.global_dof_count == native_sizes.global_dof_count
            && assembled.active_dof_indices.size() == native_sizes.active_dof_count
            && assembled.row_offsets.size() == native_sizes.row_offset_count
            && assembled.column_indices.size() == native_sizes.structural_entry_count
            && assembled.tangent.size() == native_sizes.structural_entry_count
            && native.frame_prestress.size() == native_sizes.recovery_record_count
            && native.model_content_hash.size() == native_sizes.model_identity_length
            && native.model_semantic_hash.size() == native_sizes.model_identity_length
            && native.model_provenance_hash.size() == native_sizes.model_identity_length
            && native.load_pattern_id == load_pattern_id
            && std::isfinite(native.equilibrium_residual_inf_n)
            && native.equilibrium_residual_inf_n >= 0.0
            && !assembled.active_dof_indices.empty()
            && !assembled.row_offsets.empty()
            && assembled.row_offsets.front() == 0U
            && assembled.row_offsets.back() == native_sizes.structural_entry_count
            && std::is_sorted(
                assembled.active_dof_indices.begin(), assembled.active_dof_indices.end());
        if (!shape_valid) {
            return report_error(
                error,
                SA_ERR_INTERNAL,
                "ModelIR buckling assembly native result shape invariant failed");
        }
        for (std::size_t row = 0U; row < assembled.active_dof_indices.size(); ++row) {
            if (assembled.active_dof_indices[row] >= native_sizes.global_dof_count
                || assembled.row_offsets[row] > assembled.row_offsets[row + 1U]) {
                return report_error(
                    error,
                    SA_ERR_INTERNAL,
                    "ModelIR buckling assembly CSR invariant failed");
            }
            const auto begin = assembled.column_indices.begin()
                + static_cast<std::ptrdiff_t>(assembled.row_offsets[row]);
            const auto end = assembled.column_indices.begin()
        ...
        std::vector<std::uint64_t> stable_indices;
        std::vector<double> axial_compression;
        stable_indices.reserve(native.frame_prestress.size());
        axial_compression.reserve(native.frame_prestress.size());
        auto positive_count = std::size_t {0U};
        for (const auto& frame : native.frame_prestress) {
            if ((!stable_indices.empty() && frame.stable_index <= stable_indices.back())
                || !std::isfinite(frame.axial_compression_n)
                || frame.axial_compression_n < 0.0) {
                return report_error(
                    error,
                    SA_ERR_INTERNAL,
                    "ModelIR buckling assembly prestress invariant failed");
            }
            positive_count += frame.axial_compression_n > 0.0 ? 1U : 0U;
            stable_indices.push_back(frame.stable_index);
            axial_compression.push_back(frame.axial_compression_n);
        }
        if (positive_count == 0U
            || std::any_of(assembled.tangent.begin(), assembled.tangent.end(), [](const auto value) {
                   return !std::isfinite(value);
               })) {
            return report_error(
                error,
                SA_ERR_INTERNAL,
                "ModelIR buckling assembly numerical invariant failed");
        }
        ...
        const std::array source_data {
            ...
            static_cast<const void*>(assembled.column_indices.data()),
            static_cast<const void*>(assembled.tangent.data()),
            static_cast<const void*>(stable_indices.data()),
            static_cast<const void*>(axial_compression.data()),
            static_cast<const void*>(native.model_content_hash.data()),
            static_cast<const void*>(native.model_semantic_hash.data()),
            static_cast<const void*>(native.model_provenance_hash.data()),
        };
        ...
        *result = {
            ...
            static_cast<std::uint32_t>(
                sizeof(sa_model_ir_linear_buckling_assembly_result_v1)),
            global_dof_count,
            active_dof_count,
            structural_entry_count,
            frame_prestress_count,
            native.load_pattern_index,
            native.equilibrium_residual_inf_n,
            SA_EXECUTION_BACKEND_CPU,
            0U,
            {0U, 0U},
        };
...
[[nodiscard]] sa_status_code_v1 validate_sparse_input_view(
    ...
    if (view.abi_version != expected_abi) {
        return report_error(
            error, SA_ERR_ABI_VERSION_MISMATCH, "sparse linear input ABI is incompatible");
    }
    if (view.struct_size < sizeof(sa_buffer_view_v1)) {
        return report_error(
...
[[nodiscard]] sa_status_code_v1 validate_sparse_output_view(
    ...
    if (view.abi_version != expected_abi) {
        return report_error(
            error, SA_ERR_ABI_VERSION_MISMATCH, "sparse linear output ABI is incompatible");
    }
    if (view.struct_size < sizeof(sa_mut_buffer_view_v1)) {
        return report_error(
...
[[nodiscard]] sa_status_code_v1 sparse_linear_solve_boundary(
    ...
    return contain_boundary(
        ...
        const auto output_status = validate_sparse_output_view(
            *solution, order, SA_ABI_V1_8, "sparse linear solution metadata is invalid", error);
        if (output_status != SA_OK) {
            return output_status;
        }
        const std::array descriptor_regions {
...
[[nodiscard]] bool sparse_terminal_status_is_valid(const std::uint32_t status) noexcept {
    return status == SA_SOLVER_CONVERGED || status == SA_SOLVER_SINGULARITY
        || status == SA_SOLVER_INDEFINITE_OPERATOR || status == SA_SOLVER_NONCONVERGENCE
        || status == SA_SOLVER_INCREMENT_LIMIT || status == SA_SOLVER_RESIDUAL_LIMIT;
}

[[nodiscard]] sa_status_code_v1 validate_sparse_restart_problem(
    const sa_sparse_linear_config_v1* const config,
    const sa_sparse_csr_matrix_v1* const matrix,
    const sa_buffer_view_v1* const right_hand_side,
    const sa_buffer_view_v1* const initial_guess,
    ...
    sa_error_buffer_v1* const error) {
    if (!pointer_is_aligned(config) || !pointer_is_aligned(matrix)
        || !pointer_is_aligned(right_hand_side)
        || (initial_guess != nullptr && !pointer_is_aligned(initial_guess))) {
        return report_error(
            error,
            SA_ERR_INVALID_ARGUMENT,
            "sparse linear restart problem descriptor is null or misaligned");
    }
    if (config->abi_version != SA_ABI_V1_10 || matrix->abi_version != SA_ABI_V1_10) {
        return report_error(
            error,
            SA_ERR_ABI_VERSION_MISMATCH,
            "sparse linear restart problem requires ABI v1.10");
    }
    if (config->struct_size < sizeof(sa_sparse_linear_config_v1)
        || matrix->struct_size < sizeof(sa_sparse_csr_matrix_v1)) {
        return report_error(
            error,
            SA_ERR_STRUCT_SIZE,
            "sparse linear restart problem struct_size is too small");
    }
    if (config->flags != 0U
        || std::any_of(
            std::begin(config->reserved), std::end(config->reserved), [](const auto value) {
        ...
        return report_error(
            error,
            SA_ERR_INVALID_ARGUMENT,
            "sparse linear restart problem reserved fields are not zero");
    }
    if (matrix->order == 0U || matrix->order > SA_SPARSE_LINEAR_MAX_ORDER
        || matrix->order > std::numeric_limits<std::size_t>::max() - 1U) {
        return report_error(
            error,
            SA_ERR_INVALID_ARGUMENT,
            "sparse linear restart order is outside the bounded domain");
    }
    order = matrix->order;
    nonzero_count = matrix->values.length;
    if (nonzero_count > SA_SPARSE_LINEAR_MAX_NONZEROS
        || matrix->column_indices.length != nonzero_count) {
        return report_error(
            error,
            SA_ERR_INVALID_ARGUMENT,
            "sparse linear restart nonzero dimensions are invalid");
    }
    ...
    const std::array lengths {order + 1U, nonzero_count, nonzero_count, order};
    ...
    const std::array labels {
        std::string_view {"sparse restart row-offset metadata is invalid"},
        std::string_view {"sparse restart column-index metadata is invalid"},
        std::string_view {"sparse restart value metadata is invalid"},
        std::string_view {"sparse restart right-hand-side metadata is invalid"},
    };
    for (std::size_t index = 0U; index < views.size(); ++index) {
        const auto status = validate_sparse_input_view(
            *views[index],
            lengths[index],
            types[index],
            widths[index],
    ...
    if (initial_guess != nullptr) {
        return validate_sparse_input_view(
            *initial_guess,
            initial_guess->length == 0U ? 0U : order,
            SA_ELEMENT_TYPE_F64,
            sizeof(double),
            SA_ABI_V1_10,
            "sparse restart initial-guess metadata is invalid",
            error);
    }
    return SA_OK;
}

[[nodiscard]] sa_status_code_v1 validate_sparse_restart_state(
    const sa_sparse_linear_state_v1* const state,
    const std::uint64_t order,
    const bool initial_output,
    sa_error_buffer_v1* const error) {
    if (!pointer_is_aligned(state)) {
        return report_error(
            error, SA_ERR_INVALID_ARGUMENT, "sparse linear restart state is null or misaligned");
    }
    if (state->abi_version != SA_ABI_V1_10) {
        return report_error(
            error, SA_ERR_ABI_VERSION_MISMATCH, "sparse linear state requires ABI v1.10");
    }
    if (state->struct_size < sizeof(sa_sparse_linear_state_v1)) {
        return report_error(
            error, SA_ERR_STRUCT_SIZE, "sparse linear state struct_size is too small");
    }
    if (state->reserved_u32 != 0U
        || std::any_of(
            std::begin(state->reserved), std::end(state->reserved), [](const auto value) {
        ...
        return report_error(
            error, SA_ERR_CHECKPOINT_MISMATCH, "sparse linear state reserved fields are not zero");
    }
    const std::array state_views {
        &state->solution,
        &state->residual,
    ...
    for (const auto* const view : state_views) {
        const auto status = validate_sparse_output_view(
            *view,
            order,
            SA_ABI_V1_10,
            "sparse linear state buffer metadata is invalid",
            error);
        if (status != SA_OK) {
            return status;
        }
        if (view->length != order) {
            return report_error(
                error,
                SA_ERR_CHECKPOINT_MISMATCH,
                "sparse linear state buffer length is not canonical");
        }
    }
    const bool base_metadata_valid = state->vector_length == order
        && state->execution_backend == SA_EXECUTION_BACKEND_CPU
        && state->fallback_count == 0U;
    if (!base_metadata_valid) {
        return report_error(
            error, SA_ERR_CHECKPOINT_MISMATCH, "sparse linear state metadata is invalid");
    }
    if (initial_output) {
        if (state->execution_status != SA_SPARSE_LINEAR_EXECUTION_ACTIVE
            || state->solver_status != SA_SOLVER_NONCONVERGENCE || state->iterations != 0U
            || state->initial_residual_inf != 0.0 || state->convergence_limit != 0.0
            || state->rho != 0.0 || state->last_increment_inf != 0.0) {
            return report_error(
                error,
                SA_ERR_INVALID_ARGUMENT,
                "sparse linear initial state metadata is not canonical");
        }
        return SA_OK;
    }
    const bool execution_valid =
        (state->execution_status == SA_SPARSE_LINEAR_EXECUTION_ACTIVE
         && state->solver_status == SA_SOLVER_NONCONVERGENCE)
        || (state->execution_status == SA_SPARSE_LINEAR_EXECUTION_TERMINAL
            && sparse_terminal_status_is_valid(state->solver_status));
    if (!execution_valid) {
        return report_error(
            error,
            SA_ERR_CHECKPOINT_MISMATCH,
            "sparse linear execution status is invalid");
    }
    return SA_OK;
}

[[nodiscard]] sa_status_code_v1 validate_sparse_restart_memory(
    const sa_sparse_linear_config_v1* const config,
    const sa_sparse_csr_matrix_v1* const matrix,
    const sa_buffer_view_v1* const right_hand_side,
    const sa_buffer_view_v1* const initial_guess,
    const sa_sparse_linear_state_v1* const state,
    const std::uint64_t order,
    const std::uint64_t nonzero_count,
    sa_error_buffer_v1* const error) {
    std::vector<MemoryRegion> descriptors {
        {config, sizeof(*config)},
        {matrix, sizeof(*matrix)},
        {right_hand_side, sizeof(*right_hand_side)},
        {state, sizeof(*state)},
    };
    if (initial_guess != nullptr) {
        descriptors.push_back({initial_guess, sizeof(*initial_guess)});
    }
    std::vector<MemoryRegion> inputs {
        {matrix->row_offsets.data, (order + 1U) * sizeof(std::uint64_t)},
        {matrix->column_indices.data, nonzero_count * sizeof(std::uint32_t)},
        {matrix->values.data, nonzero_count * sizeof(double)},
        {right_hand_side->data, order * sizeof(double)},
    };
    if (initial_guess != nullptr && initial_guess->length > 0U) {
        inputs.push_back({initial_guess->data, order * sizeof(double)});
    }
    const std::array outputs {
        MemoryRegion {state->solution.data, order * sizeof(double)},
        MemoryRegion {state->residual.data, order * sizeof(double)},
        MemoryRegion {state->direction.data, order * sizeof(double)},
        MemoryRegion {state->diagonal_inverse.data, order * sizeof(double)},
    };
    for (std::size_t left = 0U; left < descriptors.size(); ++left) {
        for (std::size_t right = left + 1U; right < descriptors.size(); ++right) {
            if (ranges_overlap(
                    ...
                    descriptors[right].extent)) {
                return report_error(
                    error, SA_ERR_INVALID_ARGUMENT, "sparse restart descriptors overlap");
            }
        }
    }
        ...
        for (const auto descriptor : descriptors) {
            if (ranges_overlap(
                ...
                return report_error(
                    error,
                    SA_ERR_INVALID_ARGUMENT,
                    "sparse restart input overlaps descriptor storage");
            }
        }
        for (std::size_t other = index + 1U; other < inputs.size(); ++other) {
            if (ranges_overlap(
                    inputs[index].data,
                    inputs[index].extent,
                    inputs[other].data,
                    inputs[other].extent)) {
                return report_error(
                    error, SA_ERR_INVALID_ARGUMENT, "sparse restart input buffers overlap");
            }
        }
    }
    for (std::size_t index = 0U; index < outputs.size(); ++index) {
        for (const auto descriptor : descriptors) {
            if (ranges_overlap(
                    outputs[index].data,
                    outputs[index].extent,
                    descriptor.data,
                    descriptor.extent)) {
                return report_error(
                    error,
                    SA_ERR_INVALID_ARGUMENT,
                    "sparse restart state buffer overlaps descriptor storage");
            }
        }
        for (const auto input : inputs) {
            if (ranges_overlap(
                ...
                return report_error(
                    error,
                    SA_ERR_INVALID_ARGUMENT,
                    "sparse restart state buffer overlaps input data");
            }
        }
        for (std::size_t other = index + 1U; other < outputs.size(); ++other) {
            if (ranges_overlap(
                    outputs[index].data,
                    outputs[index].extent,
                    outputs[other].data,
                    outputs[other].extent)) {
                return report_error(
                    error, SA_ERR_INVALID_ARGUMENT, "sparse restart state buffers overlap");
            }
        }
    }
    return SA_OK;
...
[[nodiscard]] structural::solver_cpu::CsrMatrixView sparse_native_matrix(
    const sa_sparse_csr_matrix_v1& matrix) {
    return {
        static_cast<std::size_t>(matrix.order),
        {static_cast<const std::uint64_t*>(matrix.row_offsets.data),
         static_cast<std::size_t>(matrix.order + 1U)},
        {static_cast<const std::uint32_t*>(matrix.column_indices.data),
         static_cast<std::size_t>(matrix.column_indices.length)},
        {static_cast<const double*>(matrix.values.data),
         static_cast<std::size_t>(matrix.values.length)},
    };
}

[[nodiscard]] structural::solver_cpu::SparseLinearConfig sparse_native_config(
    const sa_sparse_linear_config_v1& config) noexcept {
    return {
        config.max_iterations,
        config.absolute_residual_tolerance,
        config.relative_residual_tolerance,
...
void publish_sparse_restart_state(
    const structural::solver_cpu::SparseLinearExecutionState& native,
    sa_sparse_linear_state_v1& state) {
    const auto vector_bytes = native.solution.size() * sizeof(double);
    std::memcpy(state.solution.data, native.solution.data(), vector_bytes);
    std::memcpy(state.residual.data, native.residual.data(), vector_bytes);
    std::memcpy(state.direction.data, native.direction.data(), vector_bytes);
    std::memcpy(
        state.diagonal_inverse.data, native.diagonal_inverse.data(), vector_bytes);
    state.execution_status = static_cast<std::uint32_t>(native.execution_status);
    state.solver_status = static_cast<std::uint32_t>(native.solver_status);
    state.iterations = native.iterations;
    state.execution_backend = SA_EXECUTION_BACKEND_CPU;
    state.fallback_count = 0U;
    state.initial_residual_inf = native.initial_residual_inf;
    state.convergence_limit = native.convergence_limit;
    state.rho = native.rho;
    state.last_increment_inf = native.last_increment_inf;
    state.vector_length = native.solution.size();
}

[[nodiscard]] sa_status_code_v1 sparse_linear_begin_boundary(
    const sa_sparse_linear_config_v1* const config,
    const sa_sparse_csr_matrix_v1* const matrix,
    const sa_buffer_view_v1* const right_hand_side,
    const sa_buffer_view_v1* const initial_guess,
    sa_sparse_linear_state_v1* const state,
    sa_error_buffer_v1* const error) noexcept {
    return contain_boundary(
        error,
        [config, matrix, right_hand_side, initial_guess, state, error]() -> sa_status_code_v1 {
        std::uint64_t order = 0U;
        std::uint64_t nonzero_count = 0U;
        auto status = validate_sparse_restart_problem(
            config,
            matrix,
            right_hand_side,
            initial_guess,
        ...
        status = validate_sparse_restart_state(state, order, true, error);
        if (status != SA_OK) {
            return status;
        }
        status = validate_sparse_restart_memory(
            config,
            matrix,
            right_hand_side,
            initial_guess,
        ...
        const auto native_initial = std::span<const double> {
            ...
            static_cast<std::size_t>(initial_guess->length)};
        try {
            const auto native = structural::solver_cpu::begin_sparse_spd_pcg(
                sparse_native_matrix(*matrix),
                native_rhs,
                native_initial,
                sparse_native_config(*config));
            publish_sparse_restart_state(native, *state);
        } catch (const std::invalid_argument& exception) {
            return report_error(error, SA_ERR_INVALID_ARGUMENT, exception.what());
        }
        return SA_OK;
...
[[nodiscard]] sa_status_code_v1 sparse_linear_advance_boundary(
    const sa_sparse_linear_config_v1* const config,
    const sa_sparse_csr_matrix_v1* const matrix,
    const sa_buffer_view_v1* const right_hand_side,
    const std::uint32_t iteration_budget,
    ...
    return contain_boundary(
        error,
        [config, matrix, right_hand_side, iteration_budget, state, error]() -> sa_status_code_v1 {
        std::uint64_t order = 0U;
        std::uint64_t nonzero_count = 0U;
        auto status = validate_sparse_restart_problem(
            config,
        ...
        status = validate_sparse_restart_state(state, order, false, error);
        if (status != SA_OK) {
            return status;
        }
        status = validate_sparse_restart_memory(
        ...
        const auto native_matrix = sparse_native_matrix(*matrix);
        const auto native_config = sparse_native_config(*config);
        const auto native_rhs = std::span<const double> {
            static_cast<const double*>(right_hand_side->data), static_cast<std::size_t>(order)};
        try {
            structural::solver_cpu::validate_sparse_spd_problem(
                native_matrix, native_rhs, {}, native_config);
        } catch (const std::invalid_argument& exception) {
            return report_error(error, SA_ERR_INVALID_ARGUMENT, exception.what());
        }
        const auto copy_state_view = [order](const sa_mut_buffer_view_v1& view) {
            const auto* const begin = static_cast<const double*>(view.data);
            return std::vector<double>(begin, begin + static_cast<std::size_t>(order));
        };
        structural::solver_cpu::SparseLinearExecutionState native {
            static_cast<structural::solver_cpu::SparseLinearExecutionStatus>(
                state->execution_status),
            static_cast<structural::solver_cpu::SolverStatus>(state->solver_status),
            state->iterations,
            state->initial_residual_inf,
            state->convergence_limit,
            state->rho,
            state->last_increment_inf,
            copy_state_view(state->solution),
            copy_state_view(state->residual),
            copy_state_view(state->direction),
            copy_state_view(state->diagonal_inverse),
        };
        try {
            structural::solver_cpu::advance_sparse_spd_pcg(
                native_matrix, native_rhs, native_config, iteration_budget, native);
        } catch (const std::invalid_argument&) {
            return report_error(
                error,
                SA_ERR_CHECKPOINT_MISMATCH,
                "sparse linear restart state failed deterministic validation");
        }
        publish_sparse_restart_state(native, *state);
        return SA_OK;
        });
...
[[nodiscard]] sa_status_code_v1 get_api_impl(
    ...
    case SA_ABI_V1_14:
        ...
        break;
    case SA_ABI_V1_15:
        api_min_size = SA_API_V1_15_MIN_SIZE;
        break;
    default:
        return report_error(
    ...
    const bool model_ir_linear_buckling_enabled = request->abi_version >= SA_ABI_V1_15;
    const sa_api_v1 table {
        ...
        SA_CAPABILITY_BUFFER_VALIDATION
            ...
            | (model_ir_linear_reactions_enabled
                    ...
                    : UINT64_C(0))
            | (model_ir_linear_buckling_enabled
        ...
        model_ir_linear_buckling_enabled
            ? &model_ir_linear_buckling_assemble_boundary
            : nullptr,
    };
