...
#define SA_MODEL_IR_LINEAR_MAX_NESTED_COMBINATION_DEPTH UINT64_C(8)
#define SA_MODEL_IR_LINEAR_MAX_EXPANDED_COMBINATION_TERMS UINT64_C(64)
...
/*
 * v1.14 appends a reaction-only projection over the immutable v1.13 ModelIR linear source.
 * The exact sizes query returns the sorted homogeneous-constrained global DOF count. Execute
 * accepts one finite full-state displacement, evaluates the same stable-order element source,
 * and publishes constrained internal load, external load and reaction = internal - external.
 */
typedef struct sa_model_ir_linear_reaction_sizes_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint64_t global_dof_count;
    uint64_t constrained_dof_count;
    uint64_t model_identity_length;
    uint64_t reserved[3];
} sa_model_ir_linear_reaction_sizes_v1;

typedef struct sa_model_ir_linear_reaction_config_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    sa_string_view_v1 load_pattern_id;
    sa_buffer_view_v1 displacement;
    uint64_t flags;
    uint64_t reserved[2];
} sa_model_ir_linear_reaction_config_v1;

typedef struct sa_model_ir_linear_reaction_outputs_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    sa_mut_buffer_view_v1 constrained_dof_indices;
    sa_mut_buffer_view_v1 constrained_internal_force;
    sa_mut_buffer_view_v1 constrained_external_load;
    sa_mut_buffer_view_v1 reactions;
    sa_mut_buffer_view_v1 model_content_hash;
    sa_mut_buffer_view_v1 model_semantic_hash;
    sa_mut_buffer_view_v1 model_provenance_hash;
    uint64_t reserved[2];
} sa_model_ir_linear_reaction_outputs_v1;

typedef struct sa_model_ir_linear_reaction_result_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint64_t global_dof_count;
    uint64_t constrained_dof_count;
 ...
 */
typedef struct sa_dense_symmetric_matrix_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint64_t order;
    sa_buffer_view_v1 values;
    uint64_t reserved[2];
} sa_dense_symmetric_matrix_v1;

typedef struct sa_generalized_eigen_config_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint32_t mode_count;
    uint32_t maximum_sweeps;
    uint32_t flags;
    uint32_t reserved_u32;
    ...
    uint64_t reserved[2];
} sa_generalized_eigen_config_v1;

typedef struct sa_modal_outputs_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    sa_mut_buffer_view_v1 eigenvalue_rad2_per_s2;
    sa_mut_buffer_view_v1 omega_rad_per_s;
    sa_mut_buffer_view_v1 frequency_hz;
    sa_mut_buffer_view_v1 period_s;
    sa_mut_buffer_view_v1 mass_normalized_mode_shapes;
    sa_mut_buffer_view_v1 generalized_mass;
    sa_mut_buffer_view_v1 generalized_stiffness;
    sa_mut_buffer_view_v1 residual_relative_inf;
    uint64_t reserved[2];
} sa_modal_outputs_v1;

typedef struct sa_modal_result_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint32_t solver_status;
    uint32_t rigid_mode_count;
    uint32_t eigensolver_sweeps;
    uint32_t reserved_u32;
    ...
    uint64_t output_mode_count;
    uint64_t output_shape_length;
    uint32_t execution_backend;
    uint32_t fallback_count;
    uint64_t reserved[2];
} sa_modal_result_v1;

typedef struct sa_buckling_outputs_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    sa_mut_buffer_view_v1 load_factor;
    sa_mut_buffer_view_v1 stiffness_normalized_mode_shapes;
    sa_mut_buffer_view_v1 generalized_elastic_stiffness;
    sa_mut_buffer_view_v1 generalized_geometric_stiffness;
    sa_mut_buffer_view_v1 residual_relative_inf;
    uint64_t reserved[2];
} sa_buckling_outputs_v1;

typedef struct sa_buckling_result_v1 {
    uint32_t abi_version;
    uint32_t struct_size;
    uint32_t solver_status;
    uint32_t finite_positive_eigenvalue_count;
    uint32_t geometric_stiffness_positive_rank;
    uint32_t eigensolver_sweeps;
...
typedef sa_status_code_v1 (*sa_model_ir_linear_reaction_sizes_fn_v1)(
    const sa_model_ir_handle_v1* handle,
    sa_model_ir_linear_reaction_sizes_v1* sizes,
    sa_error_buffer_v1* error);

typedef sa_status_code_v1 (*sa_model_ir_linear_reactions_fn_v1)(
    const sa_model_ir_handle_v1* handle,
    const sa_model_ir_linear_reaction_config_v1* config,
    const sa_model_ir_linear_reaction_outputs_v1* outputs,
...
typedef sa_status_code_v1 (*sa_modal_solve_fn_v1)(
    ...
    const sa_dense_symmetric_matrix_v1* mass,
    const sa_buffer_view_v1* coordinate_recovery_scale,
    const sa_modal_outputs_v1* outputs,
    sa_modal_result_v1* result,
    sa_error_buffer_v1* error);

typedef sa_status_code_v1 (*sa_buckling_solve_fn_v1)(
    const sa_generalized_eigen_config_v1* config,
    const sa_dense_symmetric_matrix_v1* stiffness,
    const sa_dense_symmetric_matrix_v1* geometric_stiffness_per_unit_load,
    const sa_buffer_view_v1* coordinate_recovery_scale,
    const sa_buckling_outputs_v1* outputs,
