...
fn source_metadata(value: &Value) -> Result<Value, ModelIrContractError> {
    ...
    if let Some(member_loads) = row.get("member_distributed_loads") {
        let loads = member_loads
            .as_array()
            .ok_or_else(|| invariant_error("member_distributed_loads"))?
            .iter()
            .map(source_metadata)
            .collect::<Result<Vec<_>, _>>()?;
        metadata.insert("member_distributed_loads".to_owned(), Value::Array(loads));
    }
