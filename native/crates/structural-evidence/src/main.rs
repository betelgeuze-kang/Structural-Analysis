...
use structural_evidence::{
    build_evidence_bundle, canonical_receipt_json, check_evidence_sources, EvidenceBuildError,
...
fn main() -> ExitCode {
    ...
    match run(&arguments) {
        ...
        Err(CliError::Usage(detail)) => {
            print_error("evidence_usage_error", &detail);
            ExitCode::from(EXIT_USAGE)
        }
        Err(CliError::Evidence(error)) => {
...
enum CliError {
    Usage(String),
    Evidence(EvidenceBuildError),
}

impl From<EvidenceBuildError> for CliError {
    fn from(value: EvidenceBuildError) -> Self {
        Self::Evidence(value)
    }
}
...
fn run(arguments: &[OsString]) -> Result<String, CliError> {
    if arguments.len() == 1 && arguments[0] == "--version" {
        return canonicalize_model_ir_v2(&json!({
            "schema_version": "structural-evidence-version.v1",
    ...
    match command {
        "check" => {
            ...
            let receipt = check_evidence_sources(&required_path(&options, "--root")?)?;
        ...
        "build" => {
            ...
            let output = resolve_output(&root, required_path(&options, "--out")?);
            let generated_at = required_utf8(&options, "--generated-at")?;
            let receipt = build_evidence_bundle(&EvidenceBundleBuildRequest {
                source_root: &root,
                output: &output,
                generated_at,
            })?;
...
fn resolve_output(root: &Path, output: PathBuf) -> PathBuf {
...
fn print_error(code: &str, detail: &str) {
    let value = json!({
        "schema_version": "structural-evidence-error.v1",
        "code": code,
        "detail": detail,
    });
    match canonicalize_model_ir_v2(&value) {
        ...
        Err(_) => println!(
            "{{\"code\":\"evidence_error_encode_failed\",\"detail\":\"failed to encode evidence error\",\"schema_version\":\"structural-evidence-error.v1\"}}"
        ),
    }
}
...
fn usage() -> &'static str {
    "usage: structural-evidence check --root DIR | structural-evidence build --root DIR --out DIR --generated-at RFC3339"
}
