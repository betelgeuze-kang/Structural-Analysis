...
use structural_cli::{
    ...
    execute_model_ir_linear_external_comparison, execute_model_ir_modal_analysis_with_checkpoint,
    ...
    publish_model_ir_linear_analysis, publish_model_ir_linear_buckling_analysis,
    publish_model_ir_linear_external_comparison, publish_model_ir_modal_analysis,
...
const MAX_MODEL_LINEAR_RECOVERY_BYTES: u64 = 64 * 1024 * 1024;
const MAX_MODEL_MODAL_CHECKPOINT_BYTES: u64 = 6 * 1024 * 1024;
const MAX_MODEL_BUCKLING_CHECKPOINT_BYTES: u64 = 140 * 1024 * 1024;
...
fn run(arguments: &[OsString]) -> ExitCode {
    ...
    if let Some(command) = parse_model_buckling_analysis_arguments(arguments) {
        return run_model_buckling_analysis(&command);
    }
    ...
    if let Some(command) = parse_model_linear_comparison_arguments(arguments) {
        return run_model_linear_external_comparison(&command);
    }
...
#[derive(Clone, Debug, Eq, PartialEq)]
struct ModelBucklingAnalysisCommand {
    model_path: PathBuf,
    request_path: PathBuf,
    checkpoint_path: Option<PathBuf>,
    output_directory: PathBuf,
...
fn run_model_buckling_analysis(command: &ModelBucklingAnalysisCommand) -> ExitCode {
    let Ok(model_bytes) = read_bounded_regular_file(&command.model_path, MAX_MODEL_IR_BYTES) else {
        return model_buckling_analysis_input_failure("model_read_error", "/model");
    };
    let Ok(request_bytes) =
        read_bounded_regular_file(&command.request_path, MAX_MODEL_ANALYSIS_REQUEST_BYTES)
    else {
        return model_buckling_analysis_input_failure("request_read_error", "/request");
    };
    let checkpoint_bytes = if let Some(path) = command.checkpoint_path.as_ref() {
        let Ok(bytes) = read_bounded_regular_file(path, MAX_MODEL_BUCKLING_CHECKPOINT_BYTES) else {
            return model_buckling_analysis_input_failure("checkpoint_read_error", "/checkpoint");
        };
        Some(bytes)
    } else {
        None
    ...
    let outcome = match execute_model_ir_linear_buckling_analysis_with_checkpoint(
        ...
        Err(error) => {
            ...
            println!(
                "{}",
                json!({
                    "schema_version": "structural-model-ir-linear-buckling-analysis-failure.v1",
                    "code": "model_ir_linear_buckling_analysis_failed",
                    "detail": error.to_string()
                })
            );
            return ExitCode::from(exit);
    ...
    if let Err(error) =
        publish_model_ir_linear_buckling_analysis(&command.output_directory, &outcome)
    {
        println!(
            "{}",
            json!({
                "schema_version": "structural-model-ir-linear-buckling-analysis-failure.v1",
                "code": "model_ir_linear_buckling_publish_failed",
                "detail": error.to_string()
            })
        );
        return ExitCode::from(EXIT_FAILURE);
...
fn model_buckling_analysis_input_failure(code: &str, path: &str) -> ExitCode {
    println!(
        "{}",
        json!({
            "schema_version": "structural-model-ir-linear-buckling-analysis-failure.v1",
            "code": code,
            "path": path,
            "detail": "ModelIR linear-buckling input must be a bounded regular non-symlink file"
        })
    );
    ExitCode::from(EXIT_FAILURE)
}
...
fn run_model_modal_analysis(command: &ModelModalAnalysisCommand) -> ExitCode {
    ...
    let checkpoint_bytes = if let Some(path) = command.checkpoint_path.as_ref() {
        let Ok(bytes) = read_bounded_regular_file(path, MAX_MODEL_MODAL_CHECKPOINT_BYTES) else {
            return model_modal_analysis_input_failure("checkpoint_read_error", "/checkpoint");
        };
        Some(bytes)
    } else {
        None
    };
    let outcome = match execute_model_ir_modal_analysis_with_checkpoint(
        &model_bytes,
        &request_bytes,
        checkpoint_bytes.as_deref(),
    ) {
...
#[derive(Clone, Debug, Eq, PartialEq)]
struct ModelLinearComparisonCommand {
    result_ir_path: PathBuf,
    recovery_ir_path: PathBuf,
...
fn run_model_linear_external_comparison(command: &ModelLinearComparisonCommand) -> ExitCode {
    let result_ir = match read_bounded_regular_file(&command.result_ir_path, MAX_RESULT_IR_BYTES) {
        Ok(bytes) => bytes,
        Err(detail) => {
            return model_linear_comparison_input_failure(
                "result_ir_read_error",
                "/result_ir",
                &detail,
            );
        }
    ...
    let recovery_ir =
        match read_bounded_regular_file(&command.recovery_ir_path, MAX_MODEL_LINEAR_RECOVERY_BYTES)
        {
            Ok(bytes) => bytes,
            Err(detail) => {
                return model_linear_comparison_input_failure(
                    "recovery_ir_read_error",
                    "/recovery_ir",
                    &detail,
                );
            }
    ...
    let outcome = match execute_model_ir_linear_external_comparison(
        &result_ir,
        &recovery_ir,
        &external_result,
        &source_artifact,
        ...
        Err(error) => {
            ...
            return model_linear_comparison_failure(
                "model_ir_linear_comparison_failed",
                &error.to_string(),
                exit,
            );
        }
    ...
    if let Err(error) =
        publish_model_ir_linear_external_comparison(&command.output_directory, &outcome)
    {
        return model_linear_comparison_failure(
            "model_ir_linear_comparison_publish_failed",
            &error.to_string(),
            EXIT_FAILURE,
        );
    }
...
fn model_linear_comparison_failure(code: &str, detail: &str, exit: u8) -> ExitCode {
    println!(
        "{}",
        json!({
            "schema_version": "structural-native-model-ir-linear-comparison-failure.v1",
            "code": code,
            "detail": detail
        })
    );
...
fn model_linear_comparison_input_failure(code: &str, path: &str, detail: &str) -> ExitCode {
    println!(
        "{}",
        json!({
            "schema_version": "structural-native-model-ir-linear-comparison-failure.v1",
...
fn parse_model_linear_comparison_arguments(
    arguments: &[OsString],
) -> Option<ModelLinearComparisonCommand> {
    if arguments.len() < 8 || arguments[0] != "comparison" || arguments[1] != "model-linear" {
        return None;
    }
    let positional = [&arguments[2], &arguments[3], &arguments[4], &arguments[5]];
    if positional
        .iter()
        .any(|value| value.to_string_lossy().starts_with('-'))
    {
    ...
    Some(ModelLinearComparisonCommand {
        result_ir_path: PathBuf::from(&arguments[2]),
        recovery_ir_path: PathBuf::from(&arguments[3]),
        external_result_path: PathBuf::from(&arguments[4]),
        source_artifact_path: PathBuf::from(&arguments[5]),
...
fn parse_model_modal_analysis_arguments(
    ...
    let (positional_count, checkpoint_index) = if arguments.get(1)? == "model-modal-run" {
        (4, None)
    } else if arguments.get(1)? == "model-modal-resume" {
        (5, Some(4))
    } else {
        return None;
    };
    if arguments.len() != positional_count + 2
        || arguments[2..positional_count]
            .iter()
            .any(|value| value.to_string_lossy().starts_with('-'))
        || arguments[positional_count] != "--output-dir"
        || arguments[positional_count + 1]
            .to_string_lossy()
            .starts_with('-')
    {
        return None;
    ...
    Some(ModelModalAnalysisCommand {
        model_path: PathBuf::from(&arguments[2]),
        request_path: PathBuf::from(&arguments[3]),
        checkpoint_path: checkpoint_index.map(|index| PathBuf::from(&arguments[index])),
        output_directory: PathBuf::from(&arguments[positional_count + 1]),
    })
}

fn parse_model_buckling_analysis_arguments(
    arguments: &[OsString],
) -> Option<ModelBucklingAnalysisCommand> {
    if arguments.first()? != "analysis" {
        return None;
    }
    let (positional_count, checkpoint_index) = if arguments.get(1)? == "model-buckling-run" {
        (4, None)
    } else if arguments.get(1)? == "model-buckling-resume" {
        (5, Some(4))
    } else {
        return None;
    };
    ...
    Some(ModelBucklingAnalysisCommand {
        model_path: PathBuf::from(&arguments[2]),
        request_path: PathBuf::from(&arguments[3]),
        checkpoint_path: checkpoint_index.map(|index| PathBuf::from(&arguments[index])),
        output_directory: PathBuf::from(&arguments[positional_count + 1]),
...
mod tests {
    use super::{
        ...
        parse_model_analysis_arguments, parse_model_buckling_analysis_arguments,
        parse_model_linear_analysis_arguments, parse_model_linear_comparison_arguments,
        ...
        ModelAnalysisCommand, ModelBucklingAnalysisCommand, ModelLinearAnalysisCommand,
        ModelLinearComparisonCommand, ModelModalAnalysisCommand, PdfReportCommand,
        PdfReportLocaleV2, PdfReportProfile, SparseAnalysisCommand, SpectralAnalysisCommand,
    };
    use std::ffi::OsString;
    ...
    fn model_modal_arguments_require_exact_model_request_and_destination() {
        ...
        assert_eq!(
            ...
            Some(ModelModalAnalysisCommand {
                model_path: "model.json".into(),
                request_path: "modal.json".into(),
                checkpoint_path: Some("checkpoint.mmcp".into()),
    ...
    #[test]
    fn model_buckling_arguments_require_exact_model_request_checkpoint_and_destination() {
        assert_eq!(
            parse_model_buckling_analysis_arguments(&args(&[
                "analysis",
                "model-buckling-run",
                "model.json",
                "buckling.json",
        ...
        assert_eq!(
            ...
            Some(ModelBucklingAnalysisCommand {
                model_path: "model.json".into(),
                request_path: "buckling.json".into(),
                checkpoint_path: Some("checkpoint.mbcp".into()),
                output_directory: "resumed".into(),
            })
        );
        assert!(parse_model_buckling_analysis_arguments(&args(&[
            "analysis",
            "model-buckling-run",
            "model.json",
            "buckling.json",
    ...
    #[test]
    fn model_linear_comparison_arguments_require_result_and_recovery_paths() {
        assert_eq!(
            parse_model_linear_comparison_arguments(&args(&[
                "comparison",
                "model-linear",
                "result.json",
                "recovery.json",
            ...
            Some(ModelLinearComparisonCommand {
                result_ir_path: "result.json".into(),
                recovery_ir_path: "recovery.json".into(),
                external_result_path: "external.json".into(),
                source_artifact_path: "source.out".into(),
                executable_artifact_path: Some("solver.bin".into()),
                output_directory: "comparison".into(),
        ...
        assert!(parse_model_linear_comparison_arguments(&args(&[
            "comparison",
            "model-linear",
            "result.json",
            "recovery.json",
