...
fn member_distributed_load_model_bytes() -> Vec<u8> {
    let mut value: Value = serde_json::from_slice(&model_bytes()).expect("ModelIR fixture JSON");
    value["model_id"] = json!("engine-v2-frame-cantilever-member-load");
    value["load_patterns"][1]["nodal_loads"] = json!([]);
    ...
    canonicalize_model_ir_v2(&value)
        .expect("canonical member-load ModelIR")
        .into_bytes()
}
...
#[test]
fn frame3d_member_distributed_load_executes_recovers_and_restarts_exactly() {
    let model = member_distributed_load_model_bytes();
    let request = rebound_request_bytes(&model, "LC_WEAK", 100);
    let first = validate_model_ir_linear_analysis_compatibility(&model, &request)
        .expect("bounded member-load compatibility");
    let repeated = validate_model_ir_linear_analysis_compatibility(&model, &request)
        .expect("deterministic member-load compatibility");
    assert_eq!(first, repeated);

    let direct = execute_model_ir_linear_analysis(&model, &request, None, u32::MAX)
        .expect("member-load execution");
    assert!(direct.is_complete());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("member-load recovery IR"),
    )
    .expect("member-load recovery JSON");
    let external = recovery["active_external_load"]
        .as_array()
        .expect("member-load external load")
        .iter()
        .map(|value| value.as_f64().expect("finite member-load external load"))
        .collect::<Vec<_>>();
    assert_eq!(external.len(), 6);
    assert_eq!(external[0].to_bits(), 0.0_f64.to_bits());
    assert_eq!(external[1].to_bits(), (-1000.0_f64).to_bits());
    assert_eq!(external[2].to_bits(), 0.0_f64.to_bits());
    assert_eq!(external[3].to_bits(), 0.0_f64.to_bits());
    assert_eq!(external[4].to_bits(), 0.0_f64.to_bits());
    assert!((external[5] - (1000.0 / 3.0)).abs() <= 1.0e-12);
    assert_eq!(recovery["fallback_count"], 0);

    let element_values = recovery["recovery_values"]
        .as_array()
        .expect("member-load element recovery")
        .iter()
        .map(|value| value.as_f64().expect("finite member-load recovery"))
        .collect::<Vec<_>>();
    assert!((element_values[1] - 2000.0).abs() <= 1.0e-7);
    assert!((element_values[5] - 2000.0).abs() <= 1.0e-7);
    for value in element_values.iter().take(12).skip(6) {
        assert!(value.abs() <= 1.0e-7);
    }

    let reaction = parse_model_ir_linear_reaction_result_ir_v1(
        direct
            .reaction_result_ir_json()
            .expect("member-load reaction IR")
            .as_bytes(),
    )
    .expect("strict member-load reaction IR");
    assert!((reaction.result().reactions[1] - 2000.0).abs() <= 1.0e-7);
    assert!((reaction.result().reactions[5] - 2000.0).abs() <= 1.0e-7);
    assert_eq!(reaction.result().backend_receipt.fallback_count, 0);

    let partial = execute_model_ir_linear_analysis(&model, &request, None, 0)
        .expect("initial member-load checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &model,
        &request,
    ...
    .expect("resumed member-load execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
