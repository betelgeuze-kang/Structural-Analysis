...
use super::viewer_server::prepare_server;
use super::{
...
pub fn run_viewer_browser_smoke(
    ...
    let prepared = prepare_browser_smoke(root, mode)
        .map_err(|error| map_playwright_error(PlaywrightErrorDomain::Viewer, error))?;
    let execution = if dry_run {
        None
    } else {
        Some(
            execute_playwright(&prepared.playwright_plan)
                .map_err(|error| map_playwright_error(PlaywrightErrorDomain::Viewer, error))?,
        )
    };
    build_receipt(prepared, mode, execution.as_ref())
}
...
fn prepare_browser_smoke(
    ...
    let playwright_plan = PlaywrightPlan {
        ...
        server_route: PlaywrightServerRoute::Viewer(server_source),
    };
