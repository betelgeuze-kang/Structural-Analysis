...
#[test]
fn frontend_install_dry_run_is_deterministic_runtime_free_and_self_hashed() {
    let root = repository_root();
    let mut options = FrontendInstallOptions::new(root);
    options.dry_run = true;
    let first = run_frontend_install(&options).expect("frontend install dry-run");
    let second = run_frontend_install(&options).expect("repeat frontend install dry-run");
    assert_eq!(first, second);
    assert_eq!(first.execution_mode, "dry_run");
    assert_eq!(first.status, "planned");
    assert_eq!(first.logical_command, ["npm", "ci"]);
    ...
    assert!(!first.runtime_requirements.node_modules_mutation_expected);
    assert!(first
        .network_access_accounting
        .starts_with("not_instrumented"));
    assert!(first
        .filesystem_mutation_accounting
        .contains("node_modules_replaced"));
    assert!(first
        .environment_accounting
        .contains("identity_not_instrumented"));
    assert!(first.deterministic_receipt);
    let encoded = canonical_frontend_install_receipt_json(&first)
        .expect("canonical frontend install receipt");
    let value: Value = serde_json::from_str(&encoded).expect("frontend install receipt JSON");
    verify_receipt_hash(&value);
}

#[cfg(unix)]
#[test]
fn frontend_install_owns_exact_child_removes_node_options_and_preserves_contract() {
    let test = TestRoot::create();
    copy_contract_inventory(&test.0);
    let bin = write_fake_npm(
        &test.0,
        b"#!/bin/sh\nif [ \"${NODE_OPTIONS+x}\" = x ]; then exit 91; fi\nprintf '%s\\n' \"$*\" > \"$FRONTEND_INSTALL_TEST_LOG\"\nexit 0\n",
    );
    let log = test.0.join("frontend-install-invocation.log");
    let output = Command::new(env!("CARGO_BIN_EXE_structural-frontend-contract"))
        .args(["frontend-install", "--root"])
        .arg(&test.0)
        .env_clear()
        .env("PATH", &bin)
        .env("NODE_OPTIONS", "--inspect")
        .env("FRONTEND_INSTALL_TEST_LOG", &log)
        .output()
        .expect("run frontend install with fake npm");
    assert!(
        output.status.success(),
        "{}",
    ...
    assert_eq!(
        std::fs::read_to_string(log).expect("frontend install invocation log"),
        "ci\n"
    );
    let bytes = output.stdout.strip_suffix(b"\n").expect("one JSON line");
    let value: Value = serde_json::from_slice(bytes).expect("frontend install receipt JSON");
    assert_eq!(value["execution_mode"], "execute");
    assert_eq!(value["status"], "installed");
    assert_eq!(value["logical_command"], serde_json::json!(["npm", "ci"]));
    assert_eq!(value["direct_processes_spawned"], 1);
    assert_eq!(value["successful_exit_code"], 0);
    assert_eq!(
        value["runtime_requirements"]["node_modules_mutation_expected"],
        true
    );
    assert_eq!(
        value["node_options_disposition"],
...
#[cfg(unix)]
#[test]
fn frontend_install_rejects_package_mutation_after_child_success() {
    let test = TestRoot::create();
    copy_contract_inventory(&test.0);
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_structural-frontend-contract"))
        ...
        .env("PATH", &bin)
        .output()
        .expect("run mutating frontend install");
    assert_eq!(output.status.code(), Some(1));
    assert!(output.stderr.is_empty());
    let error: Value =
        serde_json::from_slice(output.stdout.strip_suffix(b"\n").expect("one JSON line"))
