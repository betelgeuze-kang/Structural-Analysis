...
const NODAL_LOAD_IDENTITY_EDIT_EXTENSION_KEY: &str =
...
const FIXED_CONSTRAINT_DOF_DELETE_EXTENSION_KEY: &str =
...
const NESTED_LINEAR_LOAD_COMBINATION_TERM_ADD_EXTENSION_KEY: &str =
    "structural-native:model-add-nested-linear-load-combination-term.v1";
const NESTED_LINEAR_LOAD_COMBINATION_TERM_INSERT_EXTENSION_KEY: &str =
    "structural-native:model-insert-nested-linear-load-combination-term.v1";
const NESTED_LINEAR_LOAD_COMBINATION_TERM_DELETE_EXTENSION_KEY: &str =
    "structural-native:model-delete-nested-linear-load-combination-term.v1";
const NESTED_LINEAR_LOAD_COMBINATION_TERM_REORDER_EXTENSION_KEY: &str =
...
const NODAL_LOAD_IDENTITY_EDIT_CLAIM_BOUNDARY: &str = "bounded_cpp_revalidated_existing_modelir_nodal_load_identity_replacement_to_distinct_globally_unique_stable_id_with_pattern_and_load_indices_analysis_type_node_components_source_extensions_and_unrelated_rows_preserved_without_unsupported_feature_cascade_and_with_pattern_roundtrip_degradation_not_target_component_pattern_creation_deletion_combination_constraint_topology_solver_visual_editing_engineering_acceptance_or_c6";
...
const NESTED_LINEAR_LOAD_COMBINATION_TERM_INSERT_CLAIM_BOUNDARY: &str = "bounded_cpp_revalidated_neutral_unreferenced_extension_free_acyclic_nested_linear_static_root_two_to_63_unique_typed_terms_insert_single_new_existing_pattern_or_combination_term_at_requested_final_index_zero_through_source_count_to_three_to_64_with_existing_relative_order_and_descendants_preserved_depth_eight_expanded_64_terms_not_existing_term_edit_removal_reorder_descendant_or_downstream_referenced_edit_direct_degradation_source_owned_roundtrip_unsupported_feature_cascade_general_solver_selection_visual_editing_engineering_acceptance_or_c6";
const NESTED_LINEAR_LOAD_COMBINATION_TERM_DELETE_CLAIM_BOUNDARY: &str = "bounded_cpp_revalidated_neutral_unreferenced_extension_free_acyclic_nested_linear_static_root_three_to_64_unique_typed_terms_remove_single_existing_pattern_or_combination_term_to_two_to_63_with_remaining_order_and_descendants_preserved_depth_eight_expanded_64_terms_not_combination_deletion_factor_reference_or_reorder_descendant_or_downstream_referenced_edit_direct_degradation_source_owned_roundtrip_unsupported_feature_cascade_general_solver_selection_visual_editing_engineering_acceptance_or_c6";
const NESTED_LINEAR_LOAD_COMBINATION_TERM_REORDER_CLAIM_BOUNDARY: &str = "bounded_cpp_revalidated_neutral_unreferenced_extension_free_acyclic_nested_linear_static_root_two_to_64_unique_typed_terms_move_single_existing_pattern_or_combination_term_to_distinct_in_range_final_index_with_count_factors_references_descendants_preserved_depth_eight_expanded_64_terms_not_combination_deletion_factor_reference_addition_removal_descendant_or_downstream_referenced_edit_source_owned_roundtrip_unsupported_feature_cascade_general_solver_selection_visual_editing_engineering_acceptance_or_c6";
...
/// Complete deterministic artifact pair produced by one bounded nodal-load identity edit.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelNodalLoadIdentityEditOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}
...
/// Complete deterministic artifact pair produced by one bounded fixed-constraint DOF deletion.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelFixedConstraintDofDeleteOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}
...
/// Complete deterministic artifact pair produced by one bounded direct-combination term deletion.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelLinearLoadCombinationTermDeleteOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}

/// Complete deterministic artifact pair produced by one bounded direct-combination term reorder.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelLinearLoadCombinationTermReorderOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}

/// Complete deterministic artifact pair produced by one bounded nested root-term addition.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelNestedLinearLoadCombinationTermAddOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}

/// Complete deterministic artifact pair produced by one bounded nested root-term insertion.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelNestedLinearLoadCombinationTermInsertOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}

/// Complete deterministic artifact pair produced by one bounded nested root-term deletion.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelNestedLinearLoadCombinationTermDeleteOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}

/// Complete deterministic artifact pair produced by one bounded nested root-term reorder.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelNestedLinearLoadCombinationTermReorderOutcomeV1 {
    pub model_ir_json: String,
    pub receipt_json: String,
}
...
/// Replace one existing nodal-load identity and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid or colliding identities, invalid source or edited semantics,
/// missing patterns or loads, no-op edits, unsupported-feature ownership, malformed retained
/// fields, or publication failure.
pub fn publish_model_nodal_load_identity_edit(
    source_path: &Path,
    load_pattern_id: &str,
    nodal_load_id: &str,
    replacement_nodal_load_id: &str,
    output_directory: &Path,
) -> Result<ModelNodalLoadIdentityEditOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = edit_model_nodal_load_identity(
        &source,
        load_pattern_id,
        nodal_load_id,
        replacement_nodal_load_id,
    )?;
    publish_new_directory(
        output_directory,
        &[
...
/// Delete one restrained DOF from an existing fixed constraint and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities or DOFs, invalid source or edited semantics, missing
/// constraints or restraints, deletion of the last restrained DOF, or publication failure.
pub fn publish_model_fixed_constraint_dof_delete(
    source_path: &Path,
    constraint_id: &str,
    dof: &str,
    output_directory: &Path,
) -> Result<ModelFixedConstraintDofDeleteOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = delete_model_fixed_constraint_dof(&source, constraint_id, dof)?;
    publish_new_directory(
        output_directory,
        &[
            ("model-ir.json", outcome.model_ir_json.as_bytes()),
...
/// Add one restrained DOF to an existing fixed constraint and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities, DOFs or values, invalid source or edited semantics,
/// missing constraints, already restrained or same-node overlapping DOFs, or publication failure.
pub fn publish_model_fixed_constraint_dof_add(
    source_path: &Path,
    constraint_id: &str,
    dof: &str,
    value_si: f64,
...
/// Append one pattern term to a bounded direct linear load combination and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities or factors, invalid source or edited semantics,
/// source-owned, extended, referenced, nested, unsupported-feature-owned or round-trip-owned
/// combinations, missing, nonlinear or duplicate patterns, 64-term sources, and publication
/// failures.
pub fn publish_model_direct_linear_load_combination_term_add(
    source_path: &Path,
    load_combination_id: &str,
    load_pattern_id: &str,
    factor: f64,
    output_directory: &Path,
) -> Result<ModelLinearLoadCombinationTermAddOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = add_model_direct_linear_load_combination_term(
        &source,
        load_combination_id,
        load_pattern_id,
        factor,
...
/// Delete one pattern term from a bounded direct linear load combination and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities, invalid source or edited semantics, source-owned,
/// extended, referenced, nested, unsupported-feature-owned or round-trip-owned combinations,
/// malformed or missing pattern terms, two-term sources, and publication failures.
pub fn publish_model_direct_linear_load_combination_term_delete(
    source_path: &Path,
    load_combination_id: &str,
    load_pattern_id: &str,
    output_directory: &Path,
) -> Result<ModelLinearLoadCombinationTermDeleteOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = delete_model_direct_linear_load_combination_term(
        &source,
        load_combination_id,
        load_pattern_id,
    )?;
...
/// Move one pattern term within a bounded direct linear load combination and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities or target indices, invalid source or edited semantics,
/// source-owned, extended, referenced, nested, unsupported-feature-owned or round-trip-owned
/// combinations, malformed or missing pattern terms, no-op moves, and publication failures.
pub fn publish_model_direct_linear_load_combination_term_reorder(
    source_path: &Path,
    load_combination_id: &str,
    load_pattern_id: &str,
    target_index: usize,
    output_directory: &Path,
) -> Result<ModelLinearLoadCombinationTermReorderOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = reorder_model_direct_linear_load_combination_term(
        &source,
        load_combination_id,
        load_pattern_id,
        target_index,
...
/// Append one typed root term to a bounded nested linear load combination and publish it
/// atomically.
///
/// # Errors
///
...
/// roots, missing, incompatible or duplicate typed references, 64-term roots, cycles,
/// out-of-profile expansion, and publication failures.
pub fn publish_model_nested_linear_load_combination_term_add(
    source_path: &Path,
    load_combination_id: &str,
    ...
    factor: f64,
    output_directory: &Path,
) -> Result<ModelNestedLinearLoadCombinationTermAddOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = add_model_nested_linear_load_combination_term(
        &source,
        load_combination_id,
        reference_kind,
        reference_id,
...
/// Insert one typed root term at a requested final index in a bounded nested linear load
/// combination and publish it atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities, factors or insertion indices, invalid source or
/// edited semantics, source-owned, extended, referenced, direct, unsupported-feature-owned or
/// round-trip-owned roots, missing, incompatible or duplicate typed references, 64-term roots,
/// cycles, out-of-profile expansion, and publication failures.
pub fn publish_model_nested_linear_load_combination_term_insert(
    source_path: &Path,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    ...
    target_index: usize,
    output_directory: &Path,
) -> Result<ModelNestedLinearLoadCombinationTermInsertOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = insert_model_nested_linear_load_combination_term(
        &source,
        load_combination_id,
        reference_kind,
        reference_id,
...
/// Delete one typed root term from a bounded nested linear load combination and publish it
/// atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities, invalid source or edited semantics, source-owned,
/// extended, referenced, direct, unsupported-feature-owned or round-trip-owned roots, malformed
/// or missing typed references, two-term roots, direct degradation, out-of-profile expansion, and
/// publication failures.
pub fn publish_model_nested_linear_load_combination_term_delete(
    source_path: &Path,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    output_directory: &Path,
) -> Result<ModelNestedLinearLoadCombinationTermDeleteOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = delete_model_nested_linear_load_combination_term(
        &source,
        load_combination_id,
        reference_kind,
        reference_id,
...
/// Move one typed root term within a bounded nested linear load combination and publish it
/// atomically.
///
/// # Errors
///
/// Rejects unsafe paths, invalid identities or target indices, invalid source or edited
/// semantics, source-owned, extended, referenced, direct, unsupported-feature-owned or
/// round-trip-owned roots, malformed or missing typed references, no-op moves, out-of-profile
/// expansion, and publication failures.
pub fn publish_model_nested_linear_load_combination_term_reorder(
    source_path: &Path,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    target_index: usize,
    output_directory: &Path,
) -> Result<ModelNestedLinearLoadCombinationTermReorderOutcomeV1, WorkbenchError> {
    let source = read_bounded_regular_file(source_path, MAX_MODEL_BYTES)?;
    let outcome = reorder_model_nested_linear_load_combination_term(
        &source,
        load_combination_id,
        reference_kind,
        reference_id,
...
/// Replace one existing nodal-load identity in memory.
///
/// # Errors
///
/// Rejects invalid or colliding identities, invalid source semantics, missing patterns or loads,
/// no-op edits, unsupported-feature ownership, malformed retained fields, schema drift, or edited
/// semantics rejected by C++.
pub fn edit_model_nodal_load_identity(
    source_bytes: &[u8],
    load_pattern_id: &str,
    nodal_load_id: &str,
    replacement_nodal_load_id: &str,
) -> Result<ModelNodalLoadIdentityEditOutcomeV1, WorkbenchError> {
    validate_nodal_load_identity_edit_request(
        source_bytes.len(),
        load_pattern_id,
        nodal_load_id,
        replacement_nodal_load_id,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let renamed = replace_nodal_load_identity(
        &mut edited,
        load_pattern_id,
        nodal_load_id,
        replacement_nodal_load_id,
    )?;
    bind_nodal_load_identity_edit_provenance(
        &mut edited,
        load_pattern_id,
        nodal_load_id,
        replacement_nodal_load_id,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the nodal-load identity-edited ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "nodal_load_identity_edit",
        "model_id": edited_validation.report.model_id,
        "load_pattern_id": load_pattern_id,
        "load_pattern_index": renamed.load_pattern_index,
        "analysis_type": renamed.analysis_type,
        "source_nodal_load_id": nodal_load_id,
        "replacement_nodal_load_id": replacement_nodal_load_id,
        "nodal_load_index": renamed.nodal_load_index,
        "node_id": renamed.node_id,
        "retained_components_si": renamed.components_si,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": NODAL_LOAD_IDENTITY_EDIT_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelNodalLoadIdentityEditOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}
...
#[derive(Clone, Debug, PartialEq)]
struct RemovedConstraintDofV1 {
    constraint_index: usize,
    constraint_type: String,
    node_id: String,
    removed_prescribed_value_explicit: bool,
    removed_prescribed_value_si: f64,
    retained_dofs: Value,
...
/// Delete one restrained DOF from an existing fixed constraint in memory.
///
/// # Errors
///
/// Rejects invalid identities or DOFs, invalid source semantics, missing constraints or
/// restraints, removal of the last DOF, malformed retained fields, schema drift, or edited
/// semantics rejected by C++.
pub fn delete_model_fixed_constraint_dof(
    ...
    validate_fixed_constraint_dof_delete_request(source_bytes.len(), constraint_id, dof)?;
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let removed = remove_fixed_constraint_dof(&mut edited, constraint_id, dof)?;
    bind_fixed_constraint_dof_delete_provenance(
        &mut edited,
        constraint_id,
        dof,
        &removed,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the fixed-constraint DOF-deleted ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "fixed_constraint_dof_delete",
        "model_id": edited_validation.report.model_id,
        "constraint_id": constraint_id,
        "constraint_index": removed.constraint_index,
        "constraint_type": removed.constraint_type,
        "node_id": removed.node_id,
        "removed_dof": dof,
        "removed_prescribed_value_explicit": removed.removed_prescribed_value_explicit,
        "removed_prescribed_value_si": removed.removed_prescribed_value_si,
        "retained_dofs": removed.retained_dofs,
        "retained_prescribed_values_si": removed.retained_prescribed_values_si,
        "retained_source_id": removed.retained_source_id,
        "retained_extensions": removed.retained_extensions,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        "source_semantic_hash": source_semantic_hash,
        "source_provenance_hash": source_provenance_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": FIXED_CONSTRAINT_DOF_DELETE_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelFixedConstraintDofDeleteOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}
...
#[derive(Clone, Debug, PartialEq)]
struct AddedConstraintDofV1 {
    constraint_index: usize,
    constraint_type: String,
    node_id: String,
    source_dofs: Value,
    source_prescribed_values_si: Value,
    edited_dofs: Value,
    edited_prescribed_values_si: Value,
    retained_source_id: Value,
    retained_extensions: Value,
}

/// Add one restrained DOF and explicit prescribed value to an existing fixed constraint in memory.
///
/// # Errors
///
/// Rejects invalid identities, DOFs or values, invalid source semantics, missing constraints,
/// already restrained or same-node overlapping DOFs, malformed retained fields, schema drift, or
/// edited semantics rejected by C++.
pub fn add_model_fixed_constraint_dof(
    ...
    validate_fixed_constraint_dof_add_request(source_bytes.len(), constraint_id, dof, value_si)?;
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let added = append_fixed_constraint_dof(&mut edited, constraint_id, dof, value_si)?;
    bind_fixed_constraint_dof_add_provenance(
        &mut edited,
        constraint_id,
        dof,
        value_si,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the fixed-constraint DOF-added ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "fixed_constraint_dof_add",
        "model_id": edited_validation.report.model_id,
        "constraint_id": constraint_id,
        "constraint_index": added.constraint_index,
        "constraint_type": added.constraint_type,
        "node_id": added.node_id,
        "added_dof": dof,
        "added_prescribed_value_si": value_si,
        "unit": constraint_value_unit(dof),
        "source_dofs": added.source_dofs,
        "source_prescribed_values_si": added.source_prescribed_values_si,
        "edited_dofs": added.edited_dofs,
        "edited_prescribed_values_si": added.edited_prescribed_values_si,
        "retained_source_id": added.retained_source_id,
        "retained_extensions": added.retained_extensions,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        "source_semantic_hash": source_semantic_hash,
        "source_provenance_hash": source_provenance_hash,
...
/// Append one new pattern term to a provenance-bound direct linear load combination.
///
/// # Errors
///
/// Rejects invalid identities or factors, invalid source semantics, source-owned, extended,
/// referenced, nested, unsupported-feature-owned or round-trip-owned combinations, malformed
/// direct terms, missing, nonlinear or duplicate patterns, 64-term sources, schema drift, or
/// edited semantics rejected by C++.
pub fn add_model_direct_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    load_pattern_id: &str,
    factor: f64,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_add = append_direct_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        load_pattern_id,
        factor,
    )?;
    bind_direct_linear_load_combination_term_add_provenance(
        &mut edited,
        load_combination_id,
        load_pattern_id,
        factor,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the term-extended ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "direct_linear_load_combination_term_add",
        "editing_profile": "unique_direct_linear_static_patterns_3_to_64",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_add.load_combination_index,
        "combination_type": "linear",
        "load_pattern_id": load_pattern_id,
        "term_index": term_add.term_index,
        "source_term_count": term_add.term_index,
        "term_count": term_add.edited_terms.as_array().map_or(0, Vec::len),
        "factor": factor,
        "source_terms": term_add.source_terms,
        "edited_terms": term_add.edited_terms,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        "source_semantic_hash": source_semantic_hash,
        "source_provenance_hash": source_provenance_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_ADD_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelLinearLoadCombinationTermAddOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}
...
/// # Errors
///
/// Rejects invalid identities or source semantics, source-owned, extended, referenced, nested,
/// unsupported-feature-owned or round-trip-owned combinations, malformed or missing direct terms,
/// two-term sources, schema drift, or edited semantics rejected by C++.
pub fn delete_model_direct_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    load_pattern_id: &str,
) -> Result<ModelLinearLoadCombinationTermDeleteOutcomeV1, WorkbenchError> {
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_delete = remove_direct_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        load_pattern_id,
    )?;
    bind_direct_linear_load_combination_term_delete_provenance(
        &mut edited,
        load_combination_id,
        load_pattern_id,
        &term_delete,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the term-reduced ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "direct_linear_load_combination_term_delete",
        "editing_profile": "unique_direct_linear_static_patterns_2_to_63",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_delete.load_combination_index,
        "combination_type": "linear",
        "load_pattern_id": load_pattern_id,
        "term_index": term_delete.term_index,
        "source_term_count": term_delete.source_terms.as_array().map_or(0, Vec::len),
        "term_count": term_delete.edited_terms.as_array().map_or(0, Vec::len),
        "removed_factor": term_delete.removed_factor,
        "source_terms": term_delete.source_terms,
        "edited_terms": term_delete.edited_terms,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        "source_semantic_hash": source_semantic_hash,
        "source_provenance_hash": source_provenance_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_DELETE_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelLinearLoadCombinationTermDeleteOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}

/// Move one existing pattern term to a distinct final index in a provenance-bound direct linear
/// load combination.
///
/// # Errors
///
/// Rejects invalid identities or target indices, invalid source semantics, source-owned,
/// extended, referenced, nested, unsupported-feature-owned or round-trip-owned combinations,
/// malformed or missing pattern terms, no-op moves, schema drift, or edited semantics rejected by
/// C++.
#[allow(clippy::too_many_lines)]
pub fn reorder_model_direct_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    load_pattern_id: &str,
    target_index: usize,
) -> Result<ModelLinearLoadCombinationTermReorderOutcomeV1, WorkbenchError> {
    validate_direct_linear_load_combination_term_reorder_request(
        source_bytes.len(),
        load_combination_id,
        load_pattern_id,
        target_index,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_reorder = move_direct_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        load_pattern_id,
        target_index,
    )?;
    bind_direct_linear_load_combination_term_reorder_provenance(
        &mut edited,
        load_combination_id,
        load_pattern_id,
        &term_reorder,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the reordered direct-combination ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "direct_linear_load_combination_term_reorder",
        "editing_profile": "unique_direct_linear_static_patterns_2_to_64",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_reorder.load_combination_index,
        "combination_type": "linear",
        "load_pattern_id": load_pattern_id,
        "factor": term_reorder.factor,
        "source_term_index": term_reorder.source_term_index,
        "target_term_index": term_reorder.target_term_index,
        "term_count": term_reorder.source_terms.as_array().map_or(0, Vec::len),
        "source_terms": term_reorder.source_terms,
        "edited_terms": term_reorder.edited_terms,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        "source_semantic_hash": source_semantic_hash,
        "source_provenance_hash": source_provenance_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_REORDER_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelLinearLoadCombinationTermReorderOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}

/// Append one typed term to a provenance-bound bounded acyclic nested linear load combination.
///
/// # Errors
///
/// Rejects invalid identities or factors, invalid source semantics, source-owned, extended,
/// referenced, direct, unsupported-feature-owned or round-trip-owned roots, malformed or
/// duplicate typed root terms, missing or incompatible new references, 64-term roots, cycles,
/// depth/expansion overflow, schema drift, or edited semantics rejected by C++.
#[allow(clippy::too_many_lines)]
pub fn add_model_nested_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    ...
    validate_nested_linear_load_combination_term_add_request(
        source_bytes.len(),
        load_combination_id,
        reference_id,
        factor,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_add = append_nested_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    bind_nested_linear_load_combination_term_add_provenance(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the term-extended nested-combination ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let source_expanded_pattern_count = term_add
        .source_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let edited_expanded_pattern_count = term_add
        .edited_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "nested_linear_load_combination_term_add",
        ...
        "factor": factor,
        "term_index": term_add.term_index,
        "source_term_count": term_add.source_expansion.root_terms.as_array().map_or(0, Vec::len),
        "term_count": term_add.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
        "source_terms": term_add.source_expansion.root_terms,
        "edited_terms": term_add.edited_expansion.root_terms,
        "source_combination_depth": term_add.source_expansion.max_depth,
        "source_expanded_term_count": term_add.source_expansion.expanded_term_count,
        "source_expanded_pattern_count": source_expanded_pattern_count,
        "source_expanded_pattern_terms": term_add.source_expansion.expanded_pattern_terms,
        "edited_combination_depth": term_add.edited_expansion.max_depth,
        "edited_expanded_term_count": term_add.edited_expansion.expanded_term_count,
        "edited_expanded_pattern_count": edited_expanded_pattern_count,
        "edited_expanded_pattern_terms": term_add.edited_expansion.expanded_pattern_terms,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": NESTED_LINEAR_LOAD_COMBINATION_TERM_ADD_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelNestedLinearLoadCombinationTermAddOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}

/// Insert one new typed root term at a requested final index in a provenance-bound bounded acyclic
/// nested linear load combination.
///
/// # Errors
///
/// Rejects invalid identities, factors or insertion indices, invalid source semantics,
/// source-owned, extended, referenced, direct, unsupported-feature-owned or round-trip-owned
/// roots, malformed or duplicate typed root terms, missing or incompatible new references,
/// 64-term roots, cycles, depth/expansion overflow, schema drift, or edited semantics rejected by
/// C++.
#[allow(clippy::too_many_lines)]
pub fn insert_model_nested_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_insert = insert_nested_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    bind_nested_linear_load_combination_term_insert_provenance(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the term-inserted nested-combination ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let source_expanded_pattern_count = term_insert
        .source_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let edited_expanded_pattern_count = term_insert
        .edited_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "nested_linear_load_combination_term_insert",
        "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_insert.load_combination_index,
        ...
        "factor": factor,
        "term_index": term_insert.term_index,
        "source_term_count": term_insert.source_expansion.root_terms.as_array().map_or(0, Vec::len),
        "term_count": term_insert.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
        "source_terms": term_insert.source_expansion.root_terms,
        "edited_terms": term_insert.edited_expansion.root_terms,
        "source_combination_depth": term_insert.source_expansion.max_depth,
        "source_expanded_term_count": term_insert.source_expansion.expanded_term_count,
        "source_expanded_pattern_count": source_expanded_pattern_count,
        "source_expanded_pattern_terms": term_insert.source_expansion.expanded_pattern_terms,
        "edited_combination_depth": term_insert.edited_expansion.max_depth,
        "edited_expanded_term_count": term_insert.edited_expansion.expanded_term_count,
        "edited_expanded_pattern_count": edited_expanded_pattern_count,
        "edited_expanded_pattern_terms": term_insert.edited_expansion.expanded_pattern_terms,
        "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
        "maximum_expanded_terms": MODEL_LINEAR_LOAD_COMBINATION_MAX_EXPANDED_TERMS_V1,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": NESTED_LINEAR_LOAD_COMBINATION_TERM_INSERT_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelNestedLinearLoadCombinationTermInsertOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}

/// Delete one existing typed root term from a provenance-bound bounded acyclic nested linear load
/// combination.
///
/// # Errors
///
/// Rejects invalid identities or source semantics, source-owned, extended, referenced, direct,
/// unsupported-feature-owned or round-trip-owned roots, malformed or missing typed root terms,
/// two-term roots, direct degradation, depth/expansion overflow, schema drift, or edited semantics
/// rejected by C++.
#[allow(clippy::too_many_lines)]
pub fn delete_model_nested_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
) -> Result<ModelNestedLinearLoadCombinationTermDeleteOutcomeV1, WorkbenchError> {
    validate_nested_linear_load_combination_term_delete_request(
        source_bytes.len(),
        load_combination_id,
        reference_id,
    )?;
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_delete = remove_nested_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    )?;
    bind_nested_linear_load_combination_term_delete_provenance(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the term-reduced nested-combination ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let source_expanded_pattern_count = term_delete
        .source_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let edited_expanded_pattern_count = term_delete
        .edited_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "nested_linear_load_combination_term_delete",
        "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_delete.load_combination_index,
        ...
        "removed_factor": term_delete.removed_factor,
        "term_index": term_delete.term_index,
        "source_term_count": term_delete.source_expansion.root_terms.as_array().map_or(0, Vec::len),
        "term_count": term_delete.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
        "source_terms": term_delete.source_expansion.root_terms,
        "edited_terms": term_delete.edited_expansion.root_terms,
        "source_combination_depth": term_delete.source_expansion.max_depth,
        "source_expanded_term_count": term_delete.source_expansion.expanded_term_count,
        "source_expanded_pattern_count": source_expanded_pattern_count,
        "source_expanded_pattern_terms": term_delete.source_expansion.expanded_pattern_terms,
        "edited_combination_depth": term_delete.edited_expansion.max_depth,
        "edited_expanded_term_count": term_delete.edited_expansion.expanded_term_count,
        "edited_expanded_pattern_count": edited_expanded_pattern_count,
        "edited_expanded_pattern_terms": term_delete.edited_expansion.expanded_pattern_terms,
        "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
        "maximum_expanded_terms": MODEL_LINEAR_LOAD_COMBINATION_MAX_EXPANDED_TERMS_V1,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": NESTED_LINEAR_LOAD_COMBINATION_TERM_DELETE_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelNestedLinearLoadCombinationTermDeleteOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}

/// Move one existing typed root term to a distinct final index in a provenance-bound bounded
/// acyclic nested linear load combination.
///
/// # Errors
///
/// Rejects invalid identities or target indices, invalid source semantics, source-owned,
/// extended, referenced, direct, unsupported-feature-owned or round-trip-owned roots, malformed
/// or missing typed root terms, no-op moves, depth/expansion overflow, schema drift, or edited
/// semantics rejected by C++.
#[allow(clippy::too_many_lines)]
pub fn reorder_model_nested_linear_load_combination_term(
    source_bytes: &[u8],
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    ...
    let source_input_sha256 = sha256_identity(source_bytes);
    let mut edited = source_document.value().clone();
    let term_reorder = move_nested_linear_load_combination_term(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    bind_nested_linear_load_combination_term_reorder_provenance(
        &mut edited,
        load_combination_id,
        reference_kind,
        reference_id,
    ...
    if !edited_validation.report.contract_valid || !edited_validation.report.semantics_valid {
        return Err(WorkbenchError::new(
            "workbench_model_edit_semantics_invalid",
            "native C++ validation rejected the reordered nested-combination ModelIR semantics",
        ));
    }
    let model_ir_json = edited_validation.snapshot.canonical_json().to_owned();
    let model_artifact = artifact_entry(
    ...
    let source_expanded_pattern_count = term_reorder
        .source_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let edited_expanded_pattern_count = term_reorder
        .edited_expansion
        .expanded_pattern_terms
        .as_array()
        .map_or(0, Vec::len);
    let receipt_json = canonical_self_hashed(json!({
        "schema_version": EDIT_SCHEMA_V1,
        "operation": "nested_linear_load_combination_term_reorder",
        "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
        "model_id": edited_validation.report.model_id,
        "load_combination_id": load_combination_id,
        "load_combination_index": term_reorder.load_combination_index,
        ...
        "source_term_index": term_reorder.source_term_index,
        "target_term_index": term_reorder.target_term_index,
        "term_count": term_reorder.source_expansion.root_terms.as_array().map_or(0, Vec::len),
        "source_terms": term_reorder.source_expansion.root_terms,
        "edited_terms": term_reorder.edited_expansion.root_terms,
        "source_combination_depth": term_reorder.source_expansion.max_depth,
        "source_expanded_term_count": term_reorder.source_expansion.expanded_term_count,
        "source_expanded_pattern_count": source_expanded_pattern_count,
        "source_expanded_pattern_terms": term_reorder.source_expansion.expanded_pattern_terms,
        "edited_combination_depth": term_reorder.edited_expansion.max_depth,
        "edited_expanded_term_count": term_reorder.edited_expansion.expanded_term_count,
        "edited_expanded_pattern_count": edited_expanded_pattern_count,
        "edited_expanded_pattern_terms": term_reorder.edited_expansion.expanded_pattern_terms,
        "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
        "maximum_expanded_terms": MODEL_LINEAR_LOAD_COMBINATION_MAX_EXPANDED_TERMS_V1,
        "source_input_sha256": source_input_sha256,
        "source_content_hash": source_content_hash,
        ...
        "blocking_feature_ids": edited_validation.report.blocking_feature_ids,
        "artifacts": [model_artifact],
        "claim_boundary": NESTED_LINEAR_LOAD_COMBINATION_TERM_REORDER_CLAIM_BOUNDARY,
    }))?;
    Ok(ModelNestedLinearLoadCombinationTermReorderOutcomeV1 {
        model_ir_json,
        receipt_json,
    })
}
...
#[derive(Clone, Debug, PartialEq)]
struct DirectLinearLoadCombinationTermDeleteV1 {
    load_combination_index: usize,
    term_index: usize,
    removed_factor: f64,
    source_terms: Value,
...
#[derive(Clone, Debug, PartialEq)]
struct DirectLinearLoadCombinationTermReorderV1 {
    load_combination_index: usize,
    source_term_index: usize,
    target_term_index: usize,
    factor: f64,
    source_terms: Value,
    edited_terms: Value,
}
...
#[derive(Clone, Debug, PartialEq)]
struct NestedLinearLoadCombinationTermAddV1 {
    load_combination_index: usize,
    term_index: usize,
...
#[derive(Clone, Debug, PartialEq)]
struct NestedLinearLoadCombinationTermInsertV1 {
    load_combination_index: usize,
    term_index: usize,
    source_expansion: ExpandedLinearLoadCombinationV1,
    edited_expansion: ExpandedLinearLoadCombinationV1,
...
#[derive(Clone, Debug, PartialEq)]
struct NestedLinearLoadCombinationTermDeleteV1 {
    load_combination_index: usize,
    term_index: usize,
    removed_factor: f64,
    source_expansion: ExpandedLinearLoadCombinationV1,
...
#[derive(Clone, Debug, PartialEq)]
struct NestedLinearLoadCombinationTermReorderV1 {
    load_combination_index: usize,
    source_term_index: usize,
    target_term_index: usize,
    factor: f64,
...
fn validate_nodal_load_identity_edit_request(
    source_length: usize,
    load_pattern_id: &str,
    nodal_load_id: &str,
    replacement_nodal_load_id: &str,
    ...
    validate_bounded_edit_identity(source_length, load_pattern_id, "load pattern")?;
    validate_bounded_edit_identity(0, nodal_load_id, "nodal load")?;
    validate_bounded_edit_identity(0, replacement_nodal_load_id, "replacement nodal load")?;
    let replacement_bytes = replacement_nodal_load_id.as_bytes();
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_edit_nodal_load_identity_replacement_invalid",
            "replacement nodal-load identity must satisfy the ModelIR stable-ID grammar",
        ));
    }
    if nodal_load_id == replacement_nodal_load_id {
        return Err(WorkbenchError::new(
            "workbench_model_edit_no_change",
            "replacement nodal-load identity is identical to the source identity",
        ));
    }
    Ok(())
}
...
fn validate_fixed_constraint_dof_delete_request(
    source_length: usize,
    constraint_id: &str,
    dof: &str,
) -> Result<(), WorkbenchError> {
    ...
    if !DOF_KEYS.contains(&dof) {
        return Err(WorkbenchError::new(
            "workbench_model_delete_fixed_constraint_dof_invalid",
            "deleted constraint DOF must be UX, UY, UZ, RX, RY, or RZ",
        ));
    }
    Ok(())
}

fn validate_fixed_constraint_dof_add_request(
    source_length: usize,
    constraint_id: &str,
    dof: &str,
    value_si: f64,
    ...
    if !DOF_KEYS.contains(&dof) {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_invalid",
            "added constraint DOF must be UX, UY, UZ, RX, RY, or RZ",
        ));
    }
    if !value_si.is_finite() {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_value_invalid",
            "added constraint prescribed value must be a finite SI value",
        ));
    }
    Ok(())
}
...
fn validate_direct_linear_load_combination_term_add_request(
    source_length: usize,
    load_combination_id: &str,
    load_pattern_id: &str,
    factor: f64,
    ...
    if !factor.is_finite() || factor == 0.0 {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_factor_invalid",
            "new direct load-combination term factor must be finite and non-zero",
        ));
    }
    Ok(())
}
...
fn validate_direct_linear_load_combination_term_delete_request(
    ...
    validate_bounded_edit_identity(source_length, load_combination_id, "load combination")?;
    validate_bounded_edit_identity(0, load_pattern_id, "deleted load-combination term pattern")?;
    Ok(())
}

fn validate_direct_linear_load_combination_term_reorder_request(
    source_length: usize,
    load_combination_id: &str,
    load_pattern_id: &str,
    target_index: usize,
    ...
    validate_bounded_edit_identity(
        0,
        load_pattern_id,
        "reordered load-combination term pattern",
    )?;
    if target_index >= MODEL_LINEAR_LOAD_COMBINATION_MAX_DIRECT_TERMS_V1 {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_target_index_invalid",
            "direct load-combination target index must be between zero and 63",
        ));
    }
    Ok(())
}

fn validate_nested_linear_load_combination_term_add_request(
    source_length: usize,
    load_combination_id: &str,
    reference_id: &str,
    factor: f64,
    ...
    validate_bounded_edit_identity(source_length, load_combination_id, "load combination")?;
    validate_bounded_edit_identity(0, reference_id, "new nested typed reference")?;
    if !factor.is_finite() || factor == 0.0 {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_factor_invalid",
            "new nested load-combination term factor must be finite and non-zero",
        ));
    }
    Ok(())
}

fn validate_nested_linear_load_combination_term_insert_request(
    source_length: usize,
    load_combination_id: &str,
    reference_id: &str,
    factor: f64,
    ...
    if !factor.is_finite() || factor == 0.0 {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_factor_invalid",
            "inserted nested load-combination term factor must be finite and non-zero",
        ));
    }
    if target_index >= MODEL_LINEAR_LOAD_COMBINATION_MAX_DIRECT_TERMS_V1 {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_target_index_invalid",
            "nested load-combination insertion index must be between zero and 63",
        ));
    }
    Ok(())
}

fn validate_nested_linear_load_combination_term_delete_request(
    source_length: usize,
    load_combination_id: &str,
    reference_id: &str,
) -> Result<(), WorkbenchError> {
    validate_bounded_edit_identity(source_length, load_combination_id, "load combination")?;
    validate_bounded_edit_identity(0, reference_id, "deleted nested typed reference")?;
    Ok(())
}

fn validate_nested_linear_load_combination_term_reorder_request(
    source_length: usize,
    load_combination_id: &str,
    reference_id: &str,
    target_index: usize,
    ...
    validate_bounded_edit_identity(source_length, load_combination_id, "load combination")?;
    validate_bounded_edit_identity(0, reference_id, "reordered nested typed reference")?;
    if target_index >= MODEL_LINEAR_LOAD_COMBINATION_MAX_DIRECT_TERMS_V1 {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_target_index_invalid",
            "nested load-combination target index must be between zero and 63",
        ));
    }
    Ok(())
}
...
#[allow(clippy::too_many_lines)]
fn replace_nodal_load_identity(
    model: &mut Value,
    load_pattern_id: &str,
    nodal_load_id: &str,
    replacement_nodal_load_id: &str,
) -> Result<RenamedNodalLoadV1, WorkbenchError> {
    if nodal_load_id == replacement_nodal_load_id {
        return Err(WorkbenchError::new(
            "workbench_model_edit_no_change",
            "replacement nodal-load identity is identical to the source identity",
    ...
    if load_patterns.iter().any(|pattern| {
        pattern
            ...
            .is_some_and(|loads| {
                loads.iter().any(|load| {
                    load.get("id").and_then(Value::as_str) == Some(replacement_nodal_load_id)
                })
            })
    }) {
        return Err(WorkbenchError::new(
            "workbench_model_edit_nodal_load_identity_replacement_exists",
            format!("ModelIR already has a nodal load with identity {replacement_nodal_load_id}"),
        ));
    }
    let load_pattern_index = load_patterns
        .iter()
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_edit_nodal_load_identity_pattern_missing",
                format!("ModelIR has no load pattern with identity {load_pattern_id}"),
            )
        })?;
    ...
    if load_pattern.get("index").and_then(Value::as_u64) != u64::try_from(load_pattern_index).ok() {
        return Err(WorkbenchError::new(
            "workbench_model_edit_nodal_load_identity_pattern_index_mismatch",
            "identity-edited nodal-load pattern index must match its contiguous position",
        ));
    }
    let analysis_type = load_pattern
        .get("analysis_type")
    ...
    let nodal_load_index = nodal_loads
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_edit_nodal_load_identity_load_missing",
                format!(
                    "load pattern {load_pattern_id} has no nodal load with identity {nodal_load_id}"
                ),
            )
    ...
    if nodal_load.get("index").and_then(Value::as_u64) != u64::try_from(nodal_load_index).ok() {
        return Err(WorkbenchError::new(
            "workbench_model_edit_nodal_load_identity_load_index_mismatch",
            "identity-edited nodal-load index must match its contiguous pattern-local position",
        ));
    }
    ...
    if unsupported_features.iter().any(|feature| {
        matches!(
            ...
            Some(id) if id == nodal_load_id || id == replacement_nodal_load_id
        )
    }) {
        return Err(WorkbenchError::new(
            "workbench_model_edit_nodal_load_identity_unsupported_feature_owned",
            "nodal-load identity editing refuses source or replacement ownership by an unsupported feature",
        ));
    }
    ...
    model
        ...
        .and_then(Value::as_array_mut)
        .and_then(|loads| loads.get_mut(nodal_load_index))
        .and_then(Value::as_object_mut)
        .ok_or_else(|| snapshot_error("nodal load"))?
        .insert("id".to_owned(), json!(replacement_nodal_load_id));
    Ok(RenamedNodalLoadV1 {
        load_pattern_index,
        nodal_load_index,
        analysis_type,
        node_id,
...
#[allow(clippy::too_many_lines)]
fn remove_fixed_constraint_dof(
    model: &mut Value,
    constraint_id: &str,
    dof: &str,
) -> Result<RemovedConstraintDofV1, WorkbenchError> {
    let constraints = model
        .get("constraints")
        .and_then(Value::as_array)
        .ok_or_else(|| snapshot_error("constraints"))?;
    let constraint_index = constraints
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_fixed_constraint_dof_constraint_missing",
                format!("ModelIR has no constraint with identity {constraint_id}"),
            )
        })?;
    let constraint = &constraints[constraint_index];
    if constraint.get("index").and_then(Value::as_u64) != u64::try_from(constraint_index).ok() {
        return Err(WorkbenchError::new(
            "workbench_model_delete_fixed_constraint_dof_index_mismatch",
            "edited constraint index must match its contiguous position",
        ));
    }
    let constraint_type = constraint
        .get("type")
    ...
    if constraint_type != "fixed_dofs" {
        return Err(WorkbenchError::new(
            "workbench_model_delete_fixed_constraint_dof_type_unsupported",
            "constraint DOF deletion accepts only a fixed_dofs constraint",
        ));
    }
    ...
    if dofs.len() <= 1 {
        return Err(WorkbenchError::new(
            "workbench_model_delete_fixed_constraint_dof_minimum_mask",
            "constraint DOF deletion must retain at least one restrained DOF",
        ));
    }
    let dof_index = dofs
        .iter()
        .position(|candidate| candidate.as_str() == Some(dof))
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_fixed_constraint_dof_not_restrained",
                format!("constraint {constraint_id} does not restrain DOF {dof}"),
            )
        })?;
    if dofs.iter().any(|candidate| candidate.as_str().is_none()) {
        return Err(snapshot_error("constraint dofs"));
    }
    ...
    let removed_prescribed_value_explicit = prescribed.contains_key(dof);
    let removed_prescribed_value_si = prescribed
        .get(dof)
    ...
    constraint
        ...
        .ok_or_else(|| snapshot_error("constraint prescribed_values_si"))?
        .remove(dof);
    Ok(RemovedConstraintDofV1 {
        constraint_index,
        constraint_type,
        node_id,
        removed_prescribed_value_explicit,
        removed_prescribed_value_si,
        retained_dofs: constraint["dofs"].clone(),
        retained_prescribed_values_si: constraint["prescribed_values_si"].clone(),
        retained_source_id,
        retained_extensions,
    })
}
...
#[allow(clippy::too_many_lines)]
fn append_fixed_constraint_dof(
    model: &mut Value,
    constraint_id: &str,
    dof: &str,
    value_si: f64,
) -> Result<AddedConstraintDofV1, WorkbenchError> {
    let constraints = model
        .get("constraints")
        .and_then(Value::as_array)
        .ok_or_else(|| snapshot_error("constraints"))?;
    ...
    if constraint.get("index").and_then(Value::as_u64) != u64::try_from(constraint_index).ok() {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_index_mismatch",
            "edited constraint index must match its contiguous position",
        ));
    }
    let constraint_type = constraint
    ...
    if constraint_type != "fixed_dofs" {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_type_unsupported",
            "constraint DOF addition accepts only a fixed_dofs constraint",
        ));
    }
    let node_id = constraint
        .get("node_id")
    ...
    if dofs.iter().any(|candidate| candidate.as_str() == Some(dof)) {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_already_restrained",
            format!("constraint {constraint_id} already restrains DOF {dof}"),
        ));
    }
    if constraints.iter().enumerate().any(|(index, candidate)| {
        index != constraint_index
            && candidate.get("node_id").and_then(Value::as_str) == Some(node_id.as_str())
            && candidate
                .get("dofs")
                .and_then(Value::as_array)
                .is_some_and(|candidate_dofs| {
                    candidate_dofs
                        .iter()
                        .any(|candidate_dof| candidate_dof.as_str() == Some(dof))
                })
    }) {
        return Err(WorkbenchError::new(
            "workbench_model_add_fixed_constraint_dof_overlap",
            format!("node {node_id} already has DOF {dof} restrained by another constraint"),
        ));
    }
    let prescribed = constraint
        .get("prescribed_values_si")
    ...
    let source_prescribed_values_si = Value::Object(prescribed.clone());
    let retained_source_id = constraint
        .get("source_id")
        .ok_or_else(|| snapshot_error("constraint source_id"))?
        .clone();
    ...
    constraint
        ...
        .and_then(Value::as_object_mut)
        .ok_or_else(|| snapshot_error("constraint prescribed_values_si"))?
        .insert(dof.to_owned(), json!(value_si));
    Ok(AddedConstraintDofV1 {
        constraint_index,
        constraint_type,
        node_id,
        source_dofs,
...
#[allow(clippy::too_many_lines)]
fn append_direct_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    load_pattern_id: &str,
    factor: f64,
) -> Result<DirectLinearLoadCombinationTermAddV1, WorkbenchError> {
    let load_combinations = model
        .get("load_combinations")
        .and_then(Value::as_array)
        .ok_or_else(|| snapshot_error("load_combinations"))?;
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_add_direct_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_index_mismatch",
            "term-extended load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_type_unsupported",
            "term addition accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_source_owned",
            "term addition accepts only a neutral combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_extensions_unsupported",
            "term addition accepts only a combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_add_direct_linear_load_combination_term_count_invalid",
                "term addition requires a direct source combination with between two and 63 terms",
            )
        })?;
    let source_terms = Value::Array(terms.clone());
    ...
    if new_pattern.get("analysis_type").and_then(Value::as_str) != Some("linear_static") {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_pattern_unsupported",
    ...
    for term in terms {
        if term.get("ref_kind").and_then(Value::as_str) != Some("load_pattern") {
            return Err(WorkbenchError::new(
                "workbench_model_add_direct_linear_load_combination_term_nested_unsupported",
                "direct term addition accepts load-pattern terms only",
            ));
        }
        let reference_id = term
            .get("ref_id")
        ...
        term.get("factor")
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_add_direct_linear_load_combination_term_source_factor_unsupported",
                    "source load-combination factors must be finite and non-zero",
                )
            })?;
        if direct_pattern_ids.contains(&reference_id) {
            return Err(WorkbenchError::new(
                "workbench_model_add_direct_linear_load_combination_term_source_pattern_duplicate",
        ...
        if !load_patterns.iter().any(|pattern| {
            ...
            return Err(WorkbenchError::new(
                "workbench_model_add_direct_linear_load_combination_term_source_pattern_unsupported",
                format!("combination term {reference_id} is not an existing linear_static pattern"),
            ));
        }
        direct_pattern_ids.push(reference_id);
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_unsupported_feature_owned",
            "term addition refuses a combination referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_direct_linear_load_combination_term_roundtrip_owned",
            "term addition refuses a combination with a direct round-trip mapping",
        ));
    }
    ...
    let edited_terms = {
        let combination = model
            ...
            .and_then(Value::as_array_mut)
            .and_then(|combinations| combinations.get_mut(load_combination_index))
            .ok_or_else(|| snapshot_error("term-extended load combination"))?;
        let terms = combination
            .get_mut("terms")
            .and_then(Value::as_array_mut)
            .ok_or_else(|| snapshot_error("term-extended load-combination terms"))?;
        terms.push(json!({
...
#[allow(clippy::too_many_lines)]
fn remove_direct_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    load_pattern_id: &str,
) -> Result<DirectLinearLoadCombinationTermDeleteV1, WorkbenchError> {
    let load_combinations = model
        .get("load_combinations")
        .and_then(Value::as_array)
        .ok_or_else(|| snapshot_error("load_combinations"))?;
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_direct_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_index_mismatch",
            "term-reduced load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_type_unsupported",
            "term deletion accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_source_owned",
            "term deletion accepts only a neutral combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_extensions_unsupported",
            "term deletion accepts only a combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        .and_then(Value::as_array)
        .filter(|terms| {
            terms.len() > MODEL_LINEAR_LOAD_COMBINATION_MIN_DIRECT_TERMS_V1
                && terms.len() <= MODEL_LINEAR_LOAD_COMBINATION_MAX_DIRECT_TERMS_V1
        })
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_direct_linear_load_combination_term_count_invalid",
                "term deletion requires a direct source combination with between three and 64 terms",
            )
        })?;
    let source_terms = Value::Array(terms.clone());
    let load_patterns = model
    ...
    for (term_index, term) in terms.iter().enumerate() {
        if term.get("ref_kind").and_then(Value::as_str) != Some("load_pattern") {
            return Err(WorkbenchError::new(
                "workbench_model_delete_direct_linear_load_combination_term_nested_unsupported",
                "direct term deletion accepts load-pattern terms only",
            ));
        }
        let reference_id = term
            .get("ref_id")
        ...
        let factor = term
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_delete_direct_linear_load_combination_term_source_factor_unsupported",
                    "source load-combination factors must be finite and non-zero",
                )
            })?;
        if direct_pattern_ids.contains(&reference_id) {
            return Err(WorkbenchError::new(
                "workbench_model_delete_direct_linear_load_combination_term_source_pattern_duplicate",
                "source direct load combination must reference unique load patterns",
            ));
        }
        if !load_patterns.iter().any(|pattern| {
            ...
            return Err(WorkbenchError::new(
                "workbench_model_delete_direct_linear_load_combination_term_source_pattern_unsupported",
                format!("combination term {reference_id} is not an existing linear_static pattern"),
            ));
        }
        if reference_id == load_pattern_id {
            selected = Some((term_index, factor));
        }
        direct_pattern_ids.push(reference_id);
    }
    let (term_index, removed_factor) = selected.ok_or_else(|| {
        WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_pattern_missing",
            format!("load pattern {load_pattern_id} does not occur in the combination"),
        )
    })?;
    if load_combinations
        .iter()
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_unsupported_feature_owned",
            "term deletion refuses a combination referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_direct_linear_load_combination_term_roundtrip_owned",
            "term deletion refuses a combination with a direct round-trip mapping",
        ));
    }
    ...
    let edited_terms = {
        let combination = model
            ...
            .and_then(Value::as_array_mut)
            .and_then(|combinations| combinations.get_mut(load_combination_index))
            .ok_or_else(|| snapshot_error("term-reduced load combination"))?;
        let terms = combination
            .get_mut("terms")
            .and_then(Value::as_array_mut)
            .ok_or_else(|| snapshot_error("term-reduced load-combination terms"))?;
        terms.remove(term_index);
        Value::Array(terms.clone())
    };
    Ok(DirectLinearLoadCombinationTermDeleteV1 {
        load_combination_index,
        term_index,
        removed_factor,
        source_terms,
...
#[allow(clippy::too_many_lines)]
fn move_direct_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    load_pattern_id: &str,
    target_index: usize,
) -> Result<DirectLinearLoadCombinationTermReorderV1, WorkbenchError> {
    let load_combinations = model
        .get("load_combinations")
        .and_then(Value::as_array)
        .ok_or_else(|| snapshot_error("load_combinations"))?;
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_reorder_direct_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_index_mismatch",
            "reordered load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_type_unsupported",
            "direct term reordering accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_source_owned",
            "direct term reordering accepts only a neutral combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_extensions_unsupported",
            "direct term reordering accepts only a combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_reorder_direct_linear_load_combination_term_count_invalid",
                "direct term reordering requires a source combination with between two and 64 terms",
            )
        })?;
    if target_index >= terms.len() {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_target_index_invalid",
            format!(
                "target index {target_index} is outside the {} direct terms",
                terms.len()
            ),
        ));
    }
    ...
    for (term_index, term) in terms.iter().enumerate() {
        if term.get("ref_kind").and_then(Value::as_str) != Some("load_pattern") {
            return Err(WorkbenchError::new(
                "workbench_model_reorder_direct_linear_load_combination_term_nested_unsupported",
                "direct term reordering accepts load-pattern terms only",
            ));
        }
        let reference_id = term
            .get("ref_id")
        ...
        let factor = term
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_reorder_direct_linear_load_combination_term_source_factor_unsupported",
                    "source load-combination factors must be finite and non-zero",
                )
            })?;
        if direct_pattern_ids.contains(&reference_id) {
            return Err(WorkbenchError::new(
                "workbench_model_reorder_direct_linear_load_combination_term_source_pattern_duplicate",
                "source direct load combination must reference unique load patterns",
            ));
        }
        if !load_patterns.iter().any(|pattern| {
            ...
            return Err(WorkbenchError::new(
                "workbench_model_reorder_direct_linear_load_combination_term_source_pattern_unsupported",
                format!("combination term {reference_id} is not an existing linear_static pattern"),
            ));
        }
        if reference_id == load_pattern_id {
    ...
    let (source_term_index, factor) = selected.ok_or_else(|| {
        WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_pattern_missing",
            format!("load pattern {load_pattern_id} does not occur in the combination"),
        )
    })?;
    if source_term_index == target_index {
        return Err(WorkbenchError::new(
            "workbench_model_edit_no_change",
            "direct load-combination term already occupies the requested target index",
        ));
    }
    if load_combinations
        .iter()
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_unsupported_feature_owned",
            "direct term reordering refuses a combination referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_roundtrip_owned",
            "direct term reordering refuses a combination with a direct round-trip mapping",
        ));
    }
    ...
    let edited_terms = {
        let terms = model
            ...
            .and_then(|combination| combination.get_mut("terms"))
            .and_then(Value::as_array_mut)
            .ok_or_else(|| snapshot_error("reordered direct load-combination terms"))?;
        let moved = terms.remove(source_term_index);
        terms.insert(target_index, moved);
        Value::Array(terms.clone())
    };
    let edited_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if edited_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_direct_linear_load_combination_term_nested_unsupported",
            "reordered direct load combination must remain pattern-only",
        ));
    }
    Ok(DirectLinearLoadCombinationTermReorderV1 {
        load_combination_index,
        source_term_index,
        target_term_index: target_index,
        factor,
        source_terms,
        edited_terms,
    })
...
#[allow(clippy::too_many_lines)]
fn append_nested_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    factor: f64,
) -> Result<NestedLinearLoadCombinationTermAddV1, WorkbenchError> {
    let source_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !source_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_direct_unsupported",
            "nested term addition requires a root with at least one load-combination term",
        ));
    }
    let load_combinations = model
        .get("load_combinations")
    ...
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_add_nested_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_index_mismatch",
            "term-extended nested load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_type_unsupported",
            "nested term addition accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_source_owned",
            "nested term addition accepts only a neutral root combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_extensions_unsupported",
            "nested term addition accepts only a root combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_add_nested_linear_load_combination_term_count_invalid",
                "nested term addition requires a root with between two and 63 terms",
            )
        })?;
    ...
    for term in terms {
        let source_reference_kind = match term.get("ref_kind").and_then(Value::as_str) {
            ...
            _ => {
                return Err(WorkbenchError::new(
                    "workbench_model_add_nested_linear_load_combination_term_reference_kind_unsupported",
                    "nested term addition accepts load_pattern or load_combination root terms only",
                ));
            }
        };
        ...
        term.get("factor")
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_add_nested_linear_load_combination_term_source_factor_unsupported",
        ...
        if typed_references.contains(&typed_reference) {
            return Err(WorkbenchError::new(
                "workbench_model_add_nested_linear_load_combination_term_source_reference_duplicate",
                "source nested load-combination root must contain unique typed references",
            ));
        }
        typed_references.push(typed_reference);
    }
    let new_typed_reference = format!("{}\u{0}{reference_id}", reference_kind.as_str());
    if typed_references.contains(&new_typed_reference) {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_reference_duplicate",
            format!(
                "{} {reference_id} already occurs in the root terms",
    ...
    match reference_kind {
        LinearLoadCombinationReferenceKindV1::LoadPattern => {
            ...
            let pattern = load_patterns
                .iter()
                .find(|pattern| pattern.get("id").and_then(Value::as_str) == Some(reference_id))
                .ok_or_else(|| {
                    WorkbenchError::new(
                        "workbench_model_add_nested_linear_load_combination_term_pattern_missing",
                        format!("ModelIR has no load pattern with identity {reference_id}"),
                    )
                })?;
            if pattern.get("analysis_type").and_then(Value::as_str) != Some("linear_static") {
                return Err(WorkbenchError::new(
                    "workbench_model_add_nested_linear_load_combination_term_pattern_unsupported",
                    format!("load pattern {reference_id} is not linear_static"),
                ));
            }
        }
        LinearLoadCombinationReferenceKindV1::LoadCombination => {
            let referenced = load_combinations
                .iter()
                .find(|candidate| {
                    candidate.get("id").and_then(Value::as_str) == Some(reference_id)
                })
                .ok_or_else(|| {
                    WorkbenchError::new(
                        "workbench_model_add_nested_linear_load_combination_term_reference_combination_missing",
                        format!("ModelIR has no load combination with identity {reference_id}"),
                    )
                })?;
            if referenced.get("combination_type").and_then(Value::as_str) != Some("linear") {
                return Err(WorkbenchError::new(
                    "workbench_model_add_nested_linear_load_combination_term_reference_combination_unsupported",
                    format!("load combination {reference_id} is not linear"),
                ));
            }
        }
    }
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_unsupported_feature_owned",
            "nested term addition refuses a root referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_roundtrip_owned",
            "nested term addition refuses a root with a direct round-trip mapping",
        ));
    }
    ...
    model
        ...
        .and_then(|combination| combination.get_mut("terms"))
        .and_then(Value::as_array_mut)
        .ok_or_else(|| snapshot_error("term-extended nested load-combination terms"))?
        .push(json!({
            "ref_id": reference_id,
            "ref_kind": reference_kind.as_str(),
            "factor": factor
        }));
    let edited_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !edited_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_add_nested_linear_load_combination_term_direct_unsupported",
            "term-extended load-combination root must remain nested",
        ));
    }
    Ok(NestedLinearLoadCombinationTermAddV1 {
        load_combination_index,
        term_index,
        source_expansion,
        edited_expansion,
...
#[allow(clippy::too_many_lines)]
fn insert_nested_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    factor: f64,
    target_index: usize,
) -> Result<NestedLinearLoadCombinationTermInsertV1, WorkbenchError> {
    let source_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !source_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_direct_unsupported",
            "nested term insertion requires a root with at least one load-combination term",
        ));
    }
    let load_combinations = model
        .get("load_combinations")
    ...
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_insert_nested_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_index_mismatch",
            "term-inserted nested load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_type_unsupported",
            "nested term insertion accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_source_owned",
            "nested term insertion accepts only a neutral root combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_extensions_unsupported",
            "nested term insertion accepts only a root combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_insert_nested_linear_load_combination_term_count_invalid",
                "nested term insertion requires a root with between two and 63 terms",
            )
        })?;
    if target_index > terms.len() {
        return Err(WorkbenchError::new(
    ...
    for term in terms {
        let source_reference_kind = match term.get("ref_kind").and_then(Value::as_str) {
            ...
            _ => {
                return Err(WorkbenchError::new(
                    "workbench_model_insert_nested_linear_load_combination_term_reference_kind_unsupported",
                    "nested term insertion accepts load_pattern or load_combination root terms only",
                ));
            }
        };
        let source_reference_id = term
        ...
        term.get("factor")
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_insert_nested_linear_load_combination_term_source_factor_unsupported",
                    "source nested load-combination factors must be finite and non-zero",
                )
            })?;
        let typed_reference = format!(
        ...
        if typed_references.contains(&typed_reference) {
            return Err(WorkbenchError::new(
                "workbench_model_insert_nested_linear_load_combination_term_source_reference_duplicate",
                "source nested load-combination root must contain unique typed references",
            ));
        }
        typed_references.push(typed_reference);
    ...
    match reference_kind {
        LinearLoadCombinationReferenceKindV1::LoadPattern => {
            ...
            if pattern.get("analysis_type").and_then(Value::as_str) != Some("linear_static") {
                return Err(WorkbenchError::new(
                    "workbench_model_insert_nested_linear_load_combination_term_pattern_unsupported",
                    format!("load pattern {reference_id} is not linear_static"),
                ));
            }
        }
        LinearLoadCombinationReferenceKindV1::LoadCombination => {
            let referenced = load_combinations
                ...
                .ok_or_else(|| {
                    WorkbenchError::new(
                        "workbench_model_insert_nested_linear_load_combination_term_reference_combination_missing",
                        format!("ModelIR has no load combination with identity {reference_id}"),
                    )
                })?;
            if referenced.get("combination_type").and_then(Value::as_str) != Some("linear") {
                return Err(WorkbenchError::new(
                    "workbench_model_insert_nested_linear_load_combination_term_reference_combination_unsupported",
                    format!("load combination {reference_id} is not linear"),
                ));
            }
        }
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_unsupported_feature_owned",
            "nested term insertion refuses a root referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_roundtrip_owned",
            "nested term insertion refuses a root with a direct round-trip mapping",
        ));
    }
    ...
    model
        ...
        .and_then(|combination| combination.get_mut("terms"))
        .and_then(Value::as_array_mut)
        .ok_or_else(|| snapshot_error("term-inserted nested load-combination terms"))?
        .insert(
            target_index,
            json!({
                "ref_id": reference_id,
    ...
    if !edited_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_insert_nested_linear_load_combination_term_direct_unsupported",
            "term-inserted load-combination root must remain nested",
        ));
    }
    Ok(NestedLinearLoadCombinationTermInsertV1 {
        load_combination_index,
        term_index: target_index,
        source_expansion,
        edited_expansion,
...
#[allow(clippy::too_many_lines)]
fn remove_nested_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
) -> Result<NestedLinearLoadCombinationTermDeleteV1, WorkbenchError> {
    let source_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !source_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_direct_unsupported",
            "nested term deletion requires a root with at least one load-combination term",
        ));
    }
    let load_combinations = model
        .get("load_combinations")
    ...
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_nested_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_index_mismatch",
            "term-reduced nested load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_type_unsupported",
            "nested term deletion accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_source_owned",
            "nested term deletion accepts only a neutral root combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_extensions_unsupported",
            "nested term deletion accepts only a root combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_delete_nested_linear_load_combination_term_count_invalid",
                "nested term deletion requires a root with between three and 64 terms",
            )
        })?;
    let mut typed_references = Vec::with_capacity(terms.len());
    let mut selected = None;
    for (term_index, term) in terms.iter().enumerate() {
        let source_reference_kind = match term.get("ref_kind").and_then(Value::as_str) {
            ...
            _ => {
                return Err(WorkbenchError::new(
                    "workbench_model_delete_nested_linear_load_combination_term_reference_kind_unsupported",
                    "nested term deletion accepts load_pattern or load_combination root terms only",
                ));
            }
        };
        let source_reference_id = term
        ...
        let factor = term
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_delete_nested_linear_load_combination_term_source_factor_unsupported",
                    "source nested load-combination factors must be finite and non-zero",
                )
            })?;
        let typed_reference = format!(
        ...
        if typed_references.contains(&typed_reference) {
            return Err(WorkbenchError::new(
                "workbench_model_delete_nested_linear_load_combination_term_source_reference_duplicate",
                "source nested load-combination root must contain unique typed references",
            ));
        }
    ...
    let (term_index, removed_factor) = selected.ok_or_else(|| {
        WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_reference_missing",
            format!(
                "{} {reference_id} does not occur in the root terms",
                reference_kind.as_str()
            ),
        )
    })?;
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_unsupported_feature_owned",
            "nested term deletion refuses a root referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_roundtrip_owned",
            "nested term deletion refuses a root with a direct round-trip mapping",
        ));
    }
    ...
    model
        ...
        .and_then(|combination| combination.get_mut("terms"))
        .and_then(Value::as_array_mut)
        .ok_or_else(|| snapshot_error("term-reduced nested load-combination terms"))?
        .remove(term_index);
    let edited_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !edited_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_delete_nested_linear_load_combination_term_direct_degradation",
            "term-reduced load-combination root must remain genuinely nested",
        ));
    }
    Ok(NestedLinearLoadCombinationTermDeleteV1 {
        load_combination_index,
        term_index,
        removed_factor,
        source_expansion,
...
#[allow(clippy::too_many_lines)]
fn move_nested_linear_load_combination_term(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    target_index: usize,
) -> Result<NestedLinearLoadCombinationTermReorderV1, WorkbenchError> {
    let source_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !source_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_direct_unsupported",
            "nested term reordering requires a root with at least one load-combination term",
        ));
    }
    let load_combinations = model
        .get("load_combinations")
    ...
    let load_combination_index = load_combinations
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_reorder_nested_linear_load_combination_term_combination_missing",
                format!("ModelIR has no load combination with identity {load_combination_id}"),
            )
        })?;
    let combination = &load_combinations[load_combination_index];
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_index_mismatch",
            "reordered nested load-combination index must match its contiguous position",
        ));
    }
    if combination.get("combination_type").and_then(Value::as_str) != Some("linear") {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_type_unsupported",
            "nested term reordering accepts only a linear load combination",
        ));
    }
    if !combination.get("source_id").is_some_and(Value::is_null) {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_source_owned",
            "nested term reordering accepts only a neutral root combination with null source_id",
        ));
    }
    if !combination
        .get("extensions")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_extensions_unsupported",
            "nested term reordering accepts only a root combination with empty extensions",
        ));
    }
    let terms = combination
        .get("terms")
        ...
        .ok_or_else(|| {
            WorkbenchError::new(
                "workbench_model_reorder_nested_linear_load_combination_term_count_invalid",
                "nested term reordering requires a root with between two and 64 terms",
            )
        })?;
    if target_index >= terms.len() {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_target_index_invalid",
            format!(
                "target index {target_index} is outside the {} root terms",
                terms.len()
            ),
        ));
    }
    ...
    for (term_index, term) in terms.iter().enumerate() {
        let source_reference_kind = match term.get("ref_kind").and_then(Value::as_str) {
            ...
            _ => {
                return Err(WorkbenchError::new(
                    "workbench_model_reorder_nested_linear_load_combination_term_reference_kind_unsupported",
                    "nested term reordering accepts load_pattern or load_combination root terms only",
                ));
            }
        };
        let source_reference_id = term
        ...
        let factor = term
            ...
            .ok_or_else(|| {
                WorkbenchError::new(
                    "workbench_model_reorder_nested_linear_load_combination_term_source_factor_unsupported",
                    "source nested load-combination factors must be finite and non-zero",
                )
            })?;
        let typed_reference = format!(
        ...
        if typed_references.contains(&typed_reference) {
            return Err(WorkbenchError::new(
                "workbench_model_reorder_nested_linear_load_combination_term_source_reference_duplicate",
                "source nested load-combination root must contain unique typed references",
            ));
        }
        if source_reference_kind == reference_kind && source_reference_id == reference_id {
    ...
    if source_term_index == target_index {
        return Err(WorkbenchError::new(
            "workbench_model_edit_no_change",
            "nested load-combination term already occupies the requested target index",
        ));
    }
    if load_combinations
        .iter()
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_referenced_by_combination",
            format!("load combination {load_combination_id} is referenced by another combination"),
        ));
    }
    if model
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_unsupported_feature_owned",
            "nested term reordering refuses a root referenced by an unsupported feature",
        ));
    }
    if model
        .get("roundtrip_map")
    ...
    {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_roundtrip_owned",
            "nested term reordering refuses a root with a direct round-trip mapping",
        ));
    }
    ...
    let terms = model
        ...
        .and_then(|combination| combination.get_mut("terms"))
        .and_then(Value::as_array_mut)
        .ok_or_else(|| snapshot_error("reordered nested load-combination terms"))?;
    let moved = terms.remove(source_term_index);
    terms.insert(target_index, moved);
    let edited_expansion = require_bounded_linear_load_combination(model, load_combination_id)?;
    if !edited_expansion.nested {
        return Err(WorkbenchError::new(
            "workbench_model_reorder_nested_linear_load_combination_term_direct_unsupported",
            "reordered load-combination root must remain genuinely nested",
        ));
    }
    Ok(NestedLinearLoadCombinationTermReorderV1 {
        load_combination_index,
        source_term_index,
        target_term_index: target_index,
        factor,
...
#[allow(clippy::too_many_arguments)]
fn bind_nodal_load_identity_edit_provenance(
    model: &mut Value,
    load_pattern_id: &str,
    nodal_load_id: &str,
    replacement_nodal_load_id: &str,
    renamed: &RenamedNodalLoadV1,
    source_content_hash: &str,
    source_semantic_hash: &str,
    source_provenance_hash: &str,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        NODAL_LOAD_IDENTITY_EDIT_EXTENSION_KEY,
        json!({
            "operation": "nodal_load_identity_edit",
            "load_pattern_id": load_pattern_id,
            "load_pattern_index": renamed.load_pattern_index,
            ...
            "retained_components_si": renamed.components_si.clone(),
...
#[allow(clippy::too_many_arguments)]
fn bind_fixed_constraint_dof_delete_provenance(
    model: &mut Value,
    constraint_id: &str,
    dof: &str,
    removed: &RemovedConstraintDofV1,
    source_content_hash: &str,
    source_semantic_hash: &str,
    source_provenance_hash: &str,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        FIXED_CONSTRAINT_DOF_DELETE_EXTENSION_KEY,
        json!({
            "operation": "fixed_constraint_dof_delete",
            "constraint_id": constraint_id,
            "constraint_index": removed.constraint_index,
            "constraint_type": removed.constraint_type.clone(),
            "node_id": removed.node_id.clone(),
            "removed_dof": dof,
            "removed_prescribed_value_explicit": removed.removed_prescribed_value_explicit,
            "removed_prescribed_value_si": removed.removed_prescribed_value_si,
            "retained_dofs": removed.retained_dofs.clone(),
            "retained_prescribed_values_si": removed.retained_prescribed_values_si.clone(),
            "retained_source_id": removed.retained_source_id.clone(),
            "retained_extensions": removed.retained_extensions.clone(),
            "source_content_hash": source_content_hash,
            "source_semantic_hash": source_semantic_hash,
            "source_provenance_hash": source_provenance_hash,
            "claim_boundary": FIXED_CONSTRAINT_DOF_DELETE_CLAIM_BOUNDARY
...
#[allow(clippy::too_many_arguments)]
fn bind_fixed_constraint_dof_add_provenance(
    model: &mut Value,
    constraint_id: &str,
    dof: &str,
    value_si: f64,
    added: &AddedConstraintDofV1,
    source_content_hash: &str,
    source_semantic_hash: &str,
    source_provenance_hash: &str,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        FIXED_CONSTRAINT_DOF_ADD_EXTENSION_KEY,
        json!({
            "operation": "fixed_constraint_dof_add",
            "constraint_id": constraint_id,
            "constraint_index": added.constraint_index,
            "constraint_type": added.constraint_type.clone(),
            "node_id": added.node_id.clone(),
            "added_dof": dof,
            "added_prescribed_value_si": value_si,
            "unit": constraint_value_unit(dof),
            "source_dofs": added.source_dofs.clone(),
            "source_prescribed_values_si": added.source_prescribed_values_si.clone(),
            "edited_dofs": added.edited_dofs.clone(),
            "edited_prescribed_values_si": added.edited_prescribed_values_si.clone(),
            "retained_source_id": added.retained_source_id.clone(),
            "retained_extensions": added.retained_extensions.clone(),
            "source_content_hash": source_content_hash,
            "source_semantic_hash": source_semantic_hash,
            "source_provenance_hash": source_provenance_hash,
            "claim_boundary": FIXED_CONSTRAINT_DOF_ADD_CLAIM_BOUNDARY
...
#[allow(clippy::too_many_arguments)]
fn bind_direct_linear_load_combination_term_add_provenance(
    model: &mut Value,
    load_combination_id: &str,
    load_pattern_id: &str,
    factor: f64,
    term_add: &DirectLinearLoadCombinationTermAddV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        DIRECT_LINEAR_LOAD_COMBINATION_TERM_ADD_EXTENSION_KEY,
        json!({
            "operation": "direct_linear_load_combination_term_add",
            "editing_profile": "unique_direct_linear_static_patterns_3_to_64",
            "load_combination_id": load_combination_id,
            ...
            "term_count": term_add.edited_terms.as_array().map_or(0, Vec::len),
            "factor": factor,
            "source_terms": term_add.source_terms.clone(),
            "edited_terms": term_add.edited_terms.clone(),
            "source_content_hash": source_hashes.content,
            "source_semantic_hash": source_hashes.semantic,
            "source_provenance_hash": source_hashes.provenance,
            "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_ADD_CLAIM_BOUNDARY
...
fn bind_direct_linear_load_combination_term_delete_provenance(
    ...
    load_combination_id: &str,
    load_pattern_id: &str,
    term_delete: &DirectLinearLoadCombinationTermDeleteV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        DIRECT_LINEAR_LOAD_COMBINATION_TERM_DELETE_EXTENSION_KEY,
        json!({
            "operation": "direct_linear_load_combination_term_delete",
            "editing_profile": "unique_direct_linear_static_patterns_2_to_63",
            "load_combination_id": load_combination_id,
            ...
            "term_count": term_delete.edited_terms.as_array().map_or(0, Vec::len),
            "removed_factor": term_delete.removed_factor,
            "source_terms": term_delete.source_terms.clone(),
            "edited_terms": term_delete.edited_terms.clone(),
            "source_content_hash": source_hashes.content,
            "source_semantic_hash": source_hashes.semantic,
            "source_provenance_hash": source_hashes.provenance,
            "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_DELETE_CLAIM_BOUNDARY
...
#[allow(clippy::too_many_arguments)]
fn bind_direct_linear_load_combination_term_reorder_provenance(
    model: &mut Value,
    load_combination_id: &str,
    load_pattern_id: &str,
    term_reorder: &DirectLinearLoadCombinationTermReorderV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        DIRECT_LINEAR_LOAD_COMBINATION_TERM_REORDER_EXTENSION_KEY,
        json!({
            "operation": "direct_linear_load_combination_term_reorder",
            "editing_profile": "unique_direct_linear_static_patterns_2_to_64",
            "load_combination_id": load_combination_id,
            ...
            "target_term_index": term_reorder.target_term_index,
            "term_count": term_reorder.source_terms.as_array().map_or(0, Vec::len),
            "source_terms": term_reorder.source_terms.clone(),
            "edited_terms": term_reorder.edited_terms.clone(),
            "source_content_hash": source_hashes.content,
            "source_semantic_hash": source_hashes.semantic,
            "source_provenance_hash": source_hashes.provenance,
            "claim_boundary": DIRECT_LINEAR_LOAD_COMBINATION_TERM_REORDER_CLAIM_BOUNDARY
...
#[allow(clippy::too_many_arguments)]
fn bind_nested_linear_load_combination_term_add_provenance(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    factor: f64,
    term_add: &NestedLinearLoadCombinationTermAddV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        NESTED_LINEAR_LOAD_COMBINATION_TERM_ADD_EXTENSION_KEY,
        json!({
            "operation": "nested_linear_load_combination_term_add",
            "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
            "load_combination_id": load_combination_id,
            ...
            "term_count": term_add.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
            "factor": factor,
            "source_terms": term_add.source_expansion.root_terms.clone(),
            "edited_terms": term_add.edited_expansion.root_terms.clone(),
            "source_combination_depth": term_add.source_expansion.max_depth,
            "source_expanded_term_count": term_add.source_expansion.expanded_term_count,
            "source_expanded_pattern_count": term_add
                .source_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "source_expanded_pattern_terms": term_add
                .source_expansion
                .expanded_pattern_terms
                .clone(),
            "edited_combination_depth": term_add.edited_expansion.max_depth,
            "edited_expanded_term_count": term_add.edited_expansion.expanded_term_count,
            "edited_expanded_pattern_count": term_add
                .edited_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "edited_expanded_pattern_terms": term_add
                .edited_expansion
                .expanded_pattern_terms
                .clone(),
            "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
...
#[allow(clippy::too_many_arguments)]
fn bind_nested_linear_load_combination_term_insert_provenance(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    factor: f64,
    term_insert: &NestedLinearLoadCombinationTermInsertV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        NESTED_LINEAR_LOAD_COMBINATION_TERM_INSERT_EXTENSION_KEY,
        json!({
            "operation": "nested_linear_load_combination_term_insert",
            "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
            "load_combination_id": load_combination_id,
            ...
            "term_count": term_insert.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
            "factor": factor,
            "source_terms": term_insert.source_expansion.root_terms.clone(),
            "edited_terms": term_insert.edited_expansion.root_terms.clone(),
            "source_combination_depth": term_insert.source_expansion.max_depth,
            "source_expanded_term_count": term_insert.source_expansion.expanded_term_count,
            "source_expanded_pattern_count": term_insert
                .source_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "source_expanded_pattern_terms": term_insert
                .source_expansion
                .expanded_pattern_terms
                .clone(),
            "edited_combination_depth": term_insert.edited_expansion.max_depth,
            "edited_expanded_term_count": term_insert.edited_expansion.expanded_term_count,
            "edited_expanded_pattern_count": term_insert
                .edited_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "edited_expanded_pattern_terms": term_insert
                .edited_expansion
                .expanded_pattern_terms
                .clone(),
            "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
...
#[allow(clippy::too_many_arguments)]
fn bind_nested_linear_load_combination_term_delete_provenance(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    term_delete: &NestedLinearLoadCombinationTermDeleteV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        NESTED_LINEAR_LOAD_COMBINATION_TERM_DELETE_EXTENSION_KEY,
        json!({
            "operation": "nested_linear_load_combination_term_delete",
            "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
            "load_combination_id": load_combination_id,
            ...
            "term_count": term_delete.edited_expansion.root_terms.as_array().map_or(0, Vec::len),
            "removed_factor": term_delete.removed_factor,
            "source_terms": term_delete.source_expansion.root_terms.clone(),
            "edited_terms": term_delete.edited_expansion.root_terms.clone(),
            "source_combination_depth": term_delete.source_expansion.max_depth,
            "source_expanded_term_count": term_delete.source_expansion.expanded_term_count,
            "source_expanded_pattern_count": term_delete
                .source_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "source_expanded_pattern_terms": term_delete
                .source_expansion
                .expanded_pattern_terms
                .clone(),
            "edited_combination_depth": term_delete.edited_expansion.max_depth,
            "edited_expanded_term_count": term_delete.edited_expansion.expanded_term_count,
            "edited_expanded_pattern_count": term_delete
                .edited_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "edited_expanded_pattern_terms": term_delete
                .edited_expansion
                .expanded_pattern_terms
                .clone(),
            "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
...
#[allow(clippy::too_many_arguments)]
fn bind_nested_linear_load_combination_term_reorder_provenance(
    model: &mut Value,
    load_combination_id: &str,
    reference_kind: LinearLoadCombinationReferenceKindV1,
    reference_id: &str,
    term_reorder: &NestedLinearLoadCombinationTermReorderV1,
    source_hashes: SourceModelHashesV1<'_>,
) -> Result<(), WorkbenchError> {
    bind_parameter_edit_provenance(
        model,
        NESTED_LINEAR_LOAD_COMBINATION_TERM_REORDER_EXTENSION_KEY,
        json!({
            "operation": "nested_linear_load_combination_term_reorder",
            "editing_profile": "acyclic_nested_linear_static_depth_8_expanded_terms_64",
            "load_combination_id": load_combination_id,
            ...
            "target_term_index": term_reorder.target_term_index,
            "term_count": term_reorder.source_expansion.root_terms.as_array().map_or(0, Vec::len),
            "source_terms": term_reorder.source_expansion.root_terms.clone(),
            "edited_terms": term_reorder.edited_expansion.root_terms.clone(),
            "source_combination_depth": term_reorder.source_expansion.max_depth,
            "source_expanded_term_count": term_reorder.source_expansion.expanded_term_count,
            "source_expanded_pattern_count": term_reorder
                .source_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "source_expanded_pattern_terms": term_reorder
                .source_expansion
                .expanded_pattern_terms
                .clone(),
            "edited_combination_depth": term_reorder.edited_expansion.max_depth,
            "edited_expanded_term_count": term_reorder.edited_expansion.expanded_term_count,
            "edited_expanded_pattern_count": term_reorder
                .edited_expansion
                .expanded_pattern_terms
                .as_array()
                .map_or(0, Vec::len),
            "edited_expanded_pattern_terms": term_reorder
                .edited_expansion
                .expanded_pattern_terms
                .clone(),
            "maximum_combination_depth": MODEL_LINEAR_LOAD_COMBINATION_MAX_NESTED_DEPTH_V1,
...
mod tests {
    ...
    use super::{
        append_direct_linear_load_combination_term, append_fixed_constraint_dof,
        append_linear_load_combination, append_nested_linear_load_combination,
        append_nested_linear_load_combination_term, append_node, constraint_value_unit,
    ...
    #[test]
    fn nodal_load_identity_edit_is_globally_unique_and_field_preserving() {
        validate_nodal_load_identity_edit_request(0, "LC1", "L1", "L1_RENAMED")
            .expect("valid nodal-load identity request");
        assert_eq!(
            validate_nodal_load_identity_edit_request(0, "LC1", "L1", "1_INVALID")
                .expect_err("invalid replacement stable ID")
                .code,
            "workbench_model_edit_nodal_load_identity_replacement_invalid"
        );
        assert_eq!(
            validate_nodal_load_identity_edit_request(0, "LC1", "L1", "L1")
            ...
            "load_patterns": [
                {
                    ...
                    "nodal_loads": [{
                        ...
                        "source_id": "generated:L1", "extensions": {"fixture": true}
                    }]
                },
                {
                    "id": "LC2", "index": 1, "analysis_type": "linear_static",
                    "nodal_loads": [{
                        "id": "L2", "index": 0, "node_id": "N3",
                        "components_si": {
        ...
        let source_load = model["load_patterns"][0]["nodal_loads"][0].clone();
        let mut edited = model.clone();
        let renamed = replace_nodal_load_identity(&mut edited, "LC1", "L1", "L1_RENAMED")
            .expect("rename existing nodal load");
        assert_eq!(renamed.load_pattern_index, 0);
        assert_eq!(renamed.nodal_load_index, 0);
        assert_eq!(renamed.analysis_type, "linear_static");
        assert_eq!(renamed.node_id, "N2");
        assert_eq!(renamed.components_si, source_load["components_si"]);
        assert_eq!(renamed.retained_source_id, source_load["source_id"]);
        assert_eq!(renamed.retained_extensions, source_load["extensions"]);
        assert_eq!(
            edited["load_patterns"][0]["nodal_loads"][0]["id"],
            "L1_RENAMED"
        );
        for key in [
            "index",
            ...
            assert_eq!(
                edited["load_patterns"][0]["nodal_loads"][0][key],
                source_load[key]
            );
        }
        ...
        assert_eq!(
            replace_nodal_load_identity(&mut model.clone(), "LC1", "L1", "L2")
                .expect_err("global collision")
                .code,
            "workbench_model_edit_nodal_load_identity_replacement_exists"
        );
        assert_eq!(
            replace_nodal_load_identity(&mut model.clone(), "MISSING", "L1", "L3")
                .expect_err("missing pattern")
                .code,
            "workbench_model_edit_nodal_load_identity_pattern_missing"
        );
        assert_eq!(
            replace_nodal_load_identity(&mut model.clone(), "LC1", "MISSING", "L3")
                .expect_err("missing load")
                .code,
            "workbench_model_edit_nodal_load_identity_load_missing"
        );
        let mut feature_owned = model;
        feature_owned["unsupported_features"] = json!([{"source_entity_id": "L1"}]);
        assert_eq!(
            replace_nodal_load_identity(&mut feature_owned, "LC1", "L1", "L3")
    ...
    #[test]
    fn fixed_constraint_dof_delete_preserves_row_and_retains_one_dof() {
        validate_fixed_constraint_dof_delete_request(0, "BC2", "UX")
            .expect("valid fixed-constraint DOF delete request");
        assert!(validate_fixed_constraint_dof_delete_request(0, "", "UX").is_err());
        assert_eq!(
            validate_fixed_constraint_dof_delete_request(0, "BC2", "QX")
        ...
        let model = json!({
            "constraints": [{
                ...
                "dofs": ["UX", "UY"], "prescribed_values_si": {"UX": 0.125},
                "source_id": "generated:BC2", "extensions": {"fixture": true}
            }]
        });
        let mut edited = model.clone();
        let removed = remove_fixed_constraint_dof(&mut edited, "BC2", "UX")
            .expect("delete explicit prescribed DOF");
        assert_eq!(removed.constraint_index, 0);
        assert_eq!(removed.constraint_type, "fixed_dofs");
        assert_eq!(removed.node_id, "N2");
        assert!(removed.removed_prescribed_value_explicit);
        assert_eq!(
            removed.removed_prescribed_value_si.to_bits(),
            0.125_f64.to_bits()
        );
        assert_eq!(removed.retained_dofs, json!(["UY"]));
        assert_eq!(removed.retained_prescribed_values_si, json!({}));
        assert_eq!(removed.retained_source_id, "generated:BC2");
        assert_eq!(removed.retained_extensions, json!({"fixture": true}));
        assert_eq!(edited["constraints"][0]["id"], "BC2");
        assert_eq!(edited["constraints"][0]["node_id"], "N2");

        let implicit = remove_fixed_constraint_dof(&mut model.clone(), "BC2", "UY")
            .expect("delete implicit-zero prescribed DOF");
        assert!(!implicit.removed_prescribed_value_explicit);
        assert_eq!(
            implicit.removed_prescribed_value_si.to_bits(),
            0.0_f64.to_bits()
        );
        assert_eq!(implicit.retained_dofs, json!(["UX"]));
        assert_eq!(implicit.retained_prescribed_values_si, json!({"UX": 0.125}));

        assert_eq!(
            remove_fixed_constraint_dof(&mut model.clone(), "MISSING", "UX")
                .expect_err("missing constraint")
                .code,
            "workbench_model_delete_fixed_constraint_dof_constraint_missing"
        );
        assert_eq!(
            remove_fixed_constraint_dof(&mut model.clone(), "BC2", "UZ")
                .expect_err("unrestrained DOF")
                .code,
            "workbench_model_delete_fixed_constraint_dof_not_restrained"
        );
        let mut minimum = model;
        minimum["constraints"][0]["dofs"] = json!(["UX"]);
        assert_eq!(
            remove_fixed_constraint_dof(&mut minimum, "BC2", "UX")
                .expect_err("last DOF")
                .code,
            "workbench_model_delete_fixed_constraint_dof_minimum_mask"
        );
    ...
    #[test]
    fn fixed_constraint_dof_add_appends_explicit_value_and_rejects_overlap() {
        validate_fixed_constraint_dof_add_request(0, "BC2", "UZ", 0.25)
            .expect("valid fixed-constraint DOF add request");
        assert!(validate_fixed_constraint_dof_add_request(0, "", "UZ", 0.25).is_err());
        assert_eq!(
            validate_fixed_constraint_dof_add_request(0, "BC2", "QX", 0.25)
                .expect_err("invalid DOF")
                .code,
            "workbench_model_add_fixed_constraint_dof_invalid"
        );
        assert_eq!(
            validate_fixed_constraint_dof_add_request(0, "BC2", "UZ", f64::NAN)
        ...
        let model = json!({
            "constraints": [
                ...
                {
                    "id": "BC3", "index": 1, "type": "fixed_dofs", "node_id": "N2",
                    "dofs": ["RZ"], "prescribed_values_si": {},
                    "source_id": null, "extensions": {}
                }
            ]
        });
        let mut edited = model.clone();
        let added = append_fixed_constraint_dof(&mut edited, "BC2", "UZ", 0.25)
            .expect("append unrestrained non-overlapping DOF");
        assert_eq!(added.constraint_index, 0);
        assert_eq!(added.constraint_type, "fixed_dofs");
        assert_eq!(added.node_id, "N2");
        assert_eq!(added.source_dofs, json!(["UX", "UY"]));
        assert_eq!(added.source_prescribed_values_si, json!({"UX": 0.125}));
        assert_eq!(added.edited_dofs, json!(["UX", "UY", "UZ"]));
        assert_eq!(
            added.edited_prescribed_values_si,
            json!({"UX": 0.125, "UZ": 0.25})
        );
        assert_eq!(added.retained_source_id, "generated:BC2");
        assert_eq!(added.retained_extensions, json!({"fixture": true}));
        assert_eq!(edited["constraints"][1], model["constraints"][1]);

        assert_eq!(
            append_fixed_constraint_dof(&mut model.clone(), "BC2", "UX", 0.0)
                .expect_err("already restrained DOF")
                .code,
            "workbench_model_add_fixed_constraint_dof_already_restrained"
        );
        assert_eq!(
            append_fixed_constraint_dof(&mut model.clone(), "BC2", "RZ", 0.0)
                .expect_err("same-node overlapping DOF")
                .code,
            "workbench_model_add_fixed_constraint_dof_overlap"
        );
        assert_eq!(
            append_fixed_constraint_dof(&mut model.clone(), "MISSING", "UZ", 0.0)
                .expect_err("missing constraint")
                .code,
            "workbench_model_add_fixed_constraint_dof_constraint_missing"
        );
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn direct_linear_load_combination_term_add_is_bounded_and_append_only() {
        validate_direct_linear_load_combination_term_add_request(
            0,
            "COMBO_SERVICE",
            "LC_AXIAL",
        ...
        .expect("valid direct term-add request");
        assert_eq!(
            validate_direct_linear_load_combination_term_add_request(
                0,
                "COMBO_SERVICE",
            ...
            .expect_err("zero new term factor")
            .code,
            "workbench_model_add_direct_linear_load_combination_term_factor_invalid"
        );
        ...
        let mut edited = model.clone();
        let result = append_direct_linear_load_combination_term(
            &mut edited,
            "COMBO_SERVICE",
            "LC_AXIAL",
            0.25,
        )
        .expect("append one unique direct pattern term");
        assert_eq!(result.load_combination_index, 0);
        assert_eq!(result.term_index, 2);
        assert_eq!(result.source_terms, model["load_combinations"][0]["terms"]);
        assert_eq!(result.edited_terms, edited["load_combinations"][0]["terms"]);
        assert_eq!(
            edited["load_combinations"][0]["terms"],
        ...
        assert_eq!(
            edited["load_combinations"][0]["terms"][0],
            model["load_combinations"][0]["terms"][0]
        );
        assert_eq!(
            edited["load_combinations"][0]["terms"][1],
            model["load_combinations"][0]["terms"][1]
        ...
        assert_eq!(
            append_direct_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_SERVICE",
                "LC_MISSING",
                0.25,
            )
            .expect_err("missing new pattern")
            .code,
            "workbench_model_add_direct_linear_load_combination_term_pattern_missing"
        );
        assert_eq!(
            ...
            .expect_err("nonlinear new pattern")
            .code,
            "workbench_model_add_direct_linear_load_combination_term_pattern_unsupported"
        );
        assert_eq!(
            ...
            .expect_err("duplicate new pattern")
            .code,
            "workbench_model_add_direct_linear_load_combination_term_pattern_duplicate"
        );
        let mut full = model.clone();
        full["load_combinations"][0]["terms"] = Value::Array(
            (0..64)
                .map(|index| {
                    json!({
                        "ref_id": format!("LC_{index}"),
                        "ref_kind": "load_pattern",
                        "factor": 1.0
                    })
                })
        ...
        assert_eq!(
            ...
            .expect_err("64-term source")
            .code,
            "workbench_model_add_direct_linear_load_combination_term_count_invalid"
        );
        let mut source_owned = model.clone();
        source_owned["load_combinations"][0]["source_id"] = json!("mgt:COMBO_SERVICE");
        assert_eq!(
            append_direct_linear_load_combination_term(
                &mut source_owned,
                "COMBO_SERVICE",
        ...
        let mut feature_owned = model.clone();
        feature_owned["unsupported_features"] = json!([{"source_entity_id": "COMBO_SERVICE"}]);
        assert_eq!(
            append_direct_linear_load_combination_term(
                &mut feature_owned,
                "COMBO_SERVICE",
        ...
        let mut roundtrip_owned = model.clone();
        roundtrip_owned["roundtrip_map"] = json!([{"model_ir_entity_id": "COMBO_SERVICE"}]);
        assert_eq!(
            append_direct_linear_load_combination_term(
                &mut roundtrip_owned,
                "COMBO_SERVICE",
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn direct_linear_load_combination_term_delete_is_bounded_and_order_preserving() {
        validate_direct_linear_load_combination_term_delete_request(
            0,
            "COMBO_SERVICE",
            "LC_STRONG",
        )
        .expect("valid direct term-delete request");
        assert!(validate_direct_linear_load_combination_term_delete_request(
            0,
            "COMBO_SERVICE",
            "",
        )
        ...
        let mut edited = model.clone();
        let result =
            remove_direct_linear_load_combination_term(&mut edited, "COMBO_SERVICE", "LC_STRONG")
                .expect("remove one existing direct pattern term");
        assert_eq!(result.load_combination_index, 0);
        assert_eq!(result.term_index, 1);
        assert_eq!(result.removed_factor.to_bits(), (-0.5_f64).to_bits());
        assert_eq!(result.source_terms, model["load_combinations"][0]["terms"]);
        assert_eq!(result.edited_terms, edited["load_combinations"][0]["terms"]);
        assert_eq!(
            edited["load_combinations"][0]["terms"],
        ...
        assert_eq!(
            edited["load_combinations"][0]["terms"][1],
            model["load_combinations"][0]["terms"][2]
        );

        assert_eq!(
            remove_direct_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_SERVICE",
                "LC_MISSING",
            )
            .expect_err("missing removed pattern")
            .code,
            "workbench_model_delete_direct_linear_load_combination_term_pattern_missing"
        );
        let mut minimum = model.clone();
        minimum["load_combinations"][0]["terms"]
            .as_array_mut()
            .expect("terms")
            .pop();
        assert_eq!(
            remove_direct_linear_load_combination_term(&mut minimum, "COMBO_SERVICE", "LC_STRONG",)
                .expect_err("two-term minimum")
                .code,
            "workbench_model_delete_direct_linear_load_combination_term_count_invalid"
        );
        let mut source_owned = model.clone();
        ...
        nested["load_combinations"][0]["terms"][0]["ref_kind"] = json!("load_combination");
        assert_eq!(
            remove_direct_linear_load_combination_term(&mut nested, "COMBO_SERVICE", "LC_STRONG",)
                .expect_err("nested combination")
                .code,
            "workbench_model_delete_direct_linear_load_combination_term_nested_unsupported"
        );
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn direct_linear_load_combination_term_reorder_is_bounded_and_order_only() {
        validate_direct_linear_load_combination_term_reorder_request(
            0,
            "COMBO_SERVICE",
            "LC_AXIAL",
        ...
        .expect("valid direct term-reorder request");
        assert_eq!(
            validate_direct_linear_load_combination_term_reorder_request(
                0,
                "COMBO_SERVICE",
            ...
            .expect_err("global target-index overflow")
            .code,
            "workbench_model_reorder_direct_linear_load_combination_term_target_index_invalid"
        );
        ...
        let mut edited = model.clone();
        let result =
            move_direct_linear_load_combination_term(&mut edited, "COMBO_SERVICE", "LC_AXIAL", 0)
                .expect("move an existing direct pattern term to the first position");
        assert_eq!(result.load_combination_index, 0);
        assert_eq!(result.source_term_index, 2);
        assert_eq!(result.target_term_index, 0);
        assert_eq!(result.factor.to_bits(), 0.25_f64.to_bits());
        assert_eq!(result.source_terms, model["load_combinations"][0]["terms"]);
        assert_eq!(result.edited_terms, edited["load_combinations"][0]["terms"]);
        assert_eq!(
            edited["load_combinations"][0]["terms"],
        ...
        assert_eq!(
            move_direct_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_SERVICE",
                "LC_AXIAL",
                2,
        ...
        assert_eq!(
            ...
            .expect_err("root-local target-index overflow")
            .code,
            "workbench_model_reorder_direct_linear_load_combination_term_target_index_invalid"
        );
        assert_eq!(
            ...
            .expect_err("missing source pattern")
            .code,
            "workbench_model_reorder_direct_linear_load_combination_term_pattern_missing"
        );
        ...
        let mut nested = model.clone();
        nested["load_combinations"][0]["terms"][0]["ref_kind"] = json!("load_combination");
        nested["load_combinations"][0]["terms"][0]["ref_id"] = json!("COMBO_SERVICE");
        assert_eq!(
            move_direct_linear_load_combination_term(&mut nested, "COMBO_SERVICE", "LC_AXIAL", 0,)
                .expect_err("nested combination")
                .code,
            "workbench_model_reorder_direct_linear_load_combination_term_nested_unsupported"
        );
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn nested_linear_load_combination_term_add_is_typed_bounded_and_append_only() {
        validate_nested_linear_load_combination_term_add_request(
            0,
            "COMBO_NESTED",
            "LC_STRONG",
        ...
        .expect("valid nested term-add request");
        assert_eq!(
            validate_nested_linear_load_combination_term_add_request(
                0,
                "COMBO_NESTED",
            ...
            .expect_err("zero nested term factor")
            .code,
            "workbench_model_add_nested_linear_load_combination_term_factor_invalid"
        );
        ...
        let mut pattern_added = model.clone();
        let result = append_nested_linear_load_combination_term(
            &mut pattern_added,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadPattern,
            "LC_STRONG",
        ...
        .expect("append one existing pattern term");
        assert_eq!(result.load_combination_index, 2);
        assert_eq!(result.term_index, 2);
        assert_eq!(
            result.source_expansion.root_terms,
            model["load_combinations"][2]["terms"]
        );
        assert_eq!(
            pattern_added["load_combinations"][2]["terms"],
            json!([
                {"ref_id": "COMBO_BASE", "ref_kind": "load_combination", "factor": 0.5},
                {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25},
        ...
        assert_eq!(
            pattern_added["load_combinations"][0],
            model["load_combinations"][0]
        );
        assert_eq!(
            result.edited_expansion.expanded_term_count, 4,
            "the repeated LC_STRONG path remains an explicit expanded contribution"
        );
        assert_eq!(
        ...
        let mut combination_added = model.clone();
        let combination_result = append_nested_linear_load_combination_term(
            &mut combination_added,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadCombination,
            "COMBO_ALTERNATE",
            0.25,
        )
        .expect("append one existing combination term");
        assert_eq!(combination_result.term_index, 2);
        assert_eq!(combination_result.edited_expansion.expanded_term_count, 5);
        assert_eq!(
            combination_result.edited_expansion.expanded_pattern_terms,
            json!([
                {"ref_id": "LC_WEAK", "ref_kind": "load_pattern", "factor": 0.8},
                {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": -0.2},
                {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25}
        ...
        assert_eq!(
            append_nested_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_NESTED",
                LinearLoadCombinationReferenceKindV1::LoadPattern,
                "LC_AXIAL",
            ...
            .expect_err("duplicate typed root reference")
            .code,
            "workbench_model_add_nested_linear_load_combination_term_reference_duplicate"
        );
        assert_eq!(
            ...
            .expect_err("missing appended pattern")
            .code,
            "workbench_model_add_nested_linear_load_combination_term_pattern_missing"
        );
        assert_eq!(
        ...
        let mut source_owned = model.clone();
        source_owned["load_combinations"][2]["source_id"] = json!("mgt:COMBO_NESTED");
        assert_eq!(
            append_nested_linear_load_combination_term(
                &mut source_owned,
                "COMBO_NESTED",
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn nested_linear_load_combination_term_insert_is_typed_bounded_and_positioned() {
        validate_nested_linear_load_combination_term_insert_request(
            0,
            "COMBO_NESTED",
            "LC_STRONG",
        ...
        .expect("valid nested term-insert request");
        assert_eq!(
            validate_nested_linear_load_combination_term_insert_request(
                0,
                "COMBO_NESTED",
            ...
            .expect_err("zero nested inserted factor")
            .code,
            "workbench_model_insert_nested_linear_load_combination_term_factor_invalid"
        );
        assert_eq!(
            ...
            .expect_err("global nested insertion-index overflow")
            .code,
            "workbench_model_insert_nested_linear_load_combination_term_target_index_invalid"
        );
        ...
        let mut pattern_inserted = model.clone();
        let result = insert_nested_linear_load_combination_term(
        ...
        .expect("insert one existing pattern term at the middle index");
        assert_eq!(result.load_combination_index, 2);
        assert_eq!(result.term_index, 1);
        assert_eq!(
            result.source_expansion.root_terms,
        ...
        assert_eq!(
            pattern_inserted["load_combinations"][2]["terms"],
            json!([
                {"ref_id": "COMBO_BASE", "ref_kind": "load_combination", "factor": 0.5},
                {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": 0.1},
                {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25}
        ...
        assert_eq!(
            pattern_inserted["load_combinations"][0],
            model["load_combinations"][0]
        );
        ...
        let mut combination_inserted = model.clone();
        let combination_result = insert_nested_linear_load_combination_term(
            &mut combination_inserted,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadCombination,
            "COMBO_ALTERNATE",
            0.25,
        ...
        .expect("insert one existing combination term at the front");
        assert_eq!(combination_result.term_index, 0);
        assert_eq!(combination_result.edited_expansion.expanded_term_count, 5);
        assert_eq!(
            combination_inserted["load_combinations"][2]["terms"][0],
            json!({
                "ref_id": "COMBO_ALTERNATE",
        ...
        assert_eq!(
            insert_nested_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_NESTED",
                LinearLoadCombinationReferenceKindV1::LoadPattern,
                "LC_STRONG",
            ...
            .expect_err("root-local nested insertion-index overflow")
            .code,
            "workbench_model_insert_nested_linear_load_combination_term_target_index_invalid"
        );
        assert_eq!(
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn nested_linear_load_combination_term_delete_is_typed_bounded_and_non_cascading() {
        validate_nested_linear_load_combination_term_delete_request(0, "COMBO_NESTED", "LC_AXIAL")
            .expect("valid nested term-delete request");
        assert!(
            validate_nested_linear_load_combination_term_delete_request(0, "", "LC_AXIAL").is_err()
        );
        assert!(
            validate_nested_linear_load_combination_term_delete_request(0, "COMBO_NESTED", "")
                .is_err()
        );
        ...
        let mut pattern_deleted = model.clone();
        let result = remove_nested_linear_load_combination_term(
            &mut pattern_deleted,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadPattern,
            "LC_AXIAL",
        )
        .expect("delete one existing typed pattern term");
        assert_eq!(result.load_combination_index, 2);
        assert_eq!(result.term_index, 1);
        assert_eq!(result.removed_factor.to_bits(), 0.25_f64.to_bits());
        assert_eq!(
            result.source_expansion.root_terms,
            model["load_combinations"][2]["terms"]
        );
        assert_eq!(
            pattern_deleted["load_combinations"][2]["terms"],
            json!([
                {"ref_id": "COMBO_BASE", "ref_kind": "load_combination", "factor": 0.5},
                {"ref_id": "COMBO_ALTERNATE", "ref_kind": "load_combination", "factor": 0.25},
                {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": 0.1}
        ...
        assert_eq!(
            pattern_deleted["load_combinations"][0],
            model["load_combinations"][0]
        );
        assert_eq!(
            pattern_deleted["load_combinations"][1],
            model["load_combinations"][1]
        );
        assert_eq!(result.edited_expansion.expanded_term_count, 5);
        assert_eq!(
            result.edited_expansion.expanded_pattern_terms,
            json!([
                {"ref_id": "LC_WEAK", "ref_kind": "load_pattern", "factor": 0.8},
        ...
        let mut combination_deleted = model.clone();
        let combination_result = remove_nested_linear_load_combination_term(
            &mut combination_deleted,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadCombination,
            "COMBO_BASE",
        )
        .expect("delete one existing typed combination term while retaining another");
        assert_eq!(combination_result.term_index, 0);
        assert_eq!(combination_result.edited_expansion.expanded_term_count, 4);
        assert_eq!(
            combination_result.edited_expansion.expanded_pattern_terms,
            json!([
                {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25},
                {"ref_id": "LC_WEAK", "ref_kind": "load_pattern", "factor": 0.2},
                {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": 0.150_000_000_000_000_02}
            ])
        );

        assert_eq!(
            remove_nested_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_NESTED",
                LinearLoadCombinationReferenceKindV1::LoadCombination,
                "LC_AXIAL",
            )
            .expect_err("typed reference is absent")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_reference_missing"
        );
        assert_eq!(
        ...
        let mut minimum = model.clone();
        minimum["load_combinations"][2]["terms"] = json!([
            {"ref_id": "COMBO_BASE", "ref_kind": "load_combination", "factor": 0.5},
            {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25}
        ]);
        assert_eq!(
            ...
            .expect_err("two-term source")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_count_invalid"
        );
        let mut direct_degradation = model.clone();
        direct_degradation["load_combinations"][2]["terms"] = json!([
            {"ref_id": "COMBO_BASE", "ref_kind": "load_combination", "factor": 0.5},
            {"ref_id": "LC_AXIAL", "ref_kind": "load_pattern", "factor": 0.25},
            {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": 0.1}
        ]);
        assert_eq!(
            remove_nested_linear_load_combination_term(
                &mut direct_degradation,
                "COMBO_NESTED",
                LinearLoadCombinationReferenceKindV1::LoadCombination,
                "COMBO_BASE",
            )
            .expect_err("degrades root to direct terms")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_direct_degradation"
        );
        ...
        assert_eq!(
            ...
            .expect_err("source-owned root")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_source_owned"
        );
        let mut extended = model.clone();
        extended["load_combinations"][2]["extensions"] = json!({"owner": "external"});
        assert_eq!(
            remove_nested_linear_load_combination_term(
                &mut extended,
                "COMBO_NESTED",
        ...
        assert_eq!(
            ...
            .expect_err("unsupported-feature-owned root")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_unsupported_feature_owned"
        );
        let mut roundtrip_owned = model.clone();
        ...
        assert_eq!(
            ...
            .expect_err("round-trip-owned root")
            .code,
            "workbench_model_delete_nested_linear_load_combination_term_roundtrip_owned"
        );
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn nested_linear_load_combination_term_reorder_is_typed_bounded_and_non_cascading() {
        validate_nested_linear_load_combination_term_reorder_request(
            0,
            "COMBO_NESTED",
            "LC_STRONG",
        ...
        .expect("valid nested term-reorder request");
        assert_eq!(
            validate_nested_linear_load_combination_term_reorder_request(
                0,
                "COMBO_NESTED",
        ...
        let mut edited = model.clone();
        let result = move_nested_linear_load_combination_term(
            &mut edited,
            "COMBO_NESTED",
            LinearLoadCombinationReferenceKindV1::LoadPattern,
            "LC_STRONG",
        ...
        .expect("move typed pattern term to the first root position");
        assert_eq!(result.load_combination_index, 1);
        assert_eq!(result.source_term_index, 1);
        assert_eq!(result.target_term_index, 0);
        assert_eq!(result.factor.to_bits(), 0.1_f64.to_bits());
        assert_eq!(
            result.source_expansion.root_terms,
            model["load_combinations"][1]["terms"]
        );
        assert_eq!(
            edited["load_combinations"][1]["terms"],
            json!([
                {"ref_id": "LC_STRONG", "ref_kind": "load_pattern", "factor": 0.1},
                {"ref_id": "COMBO_SERVICE", "ref_kind": "load_combination", "factor": 0.5}
            ])
        ...
        assert_eq!(result.edited_expansion.expanded_term_count, 3);
        assert_eq!(
        ...
        assert_eq!(
            move_nested_linear_load_combination_term(
                &mut model.clone(),
                "COMBO_NESTED",
                LinearLoadCombinationReferenceKindV1::LoadCombination,
                "COMBO_SERVICE",
        ...
        assert_eq!(
            ...
            .expect_err("missing typed reference")
            .code,
            "workbench_model_reorder_nested_linear_load_combination_term_reference_missing"
        );
        assert_eq!(
