...
fn run_frame_section_identity_cascade_edit(
    source: &Path,
    destination: &Path,
    section_id: &str,
    replacement_section_id: &str,
    ...
    run_workbench(&[
        text("model-edit-frame-section-identity-cascade"),
        source.as_os_str(),
        text("--section"),
        text(section_id),
        text("--new-section"),
...
fn run_truss_section_identity_edit(
    source: &Path,
    destination: &Path,
    section_id: &str,
    replacement_section_id: &str,
    ...
    run_workbench(&[
        text("model-edit-truss-section-identity"),
        source.as_os_str(),
        text("--section"),
        text(section_id),
        text("--new-section"),
...
fn run_element_identity_edit(
    ...
    run_workbench(&[
        text("model-edit-element-identity"),
        source.as_os_str(),
        text("--element"),
        text(element_id),
        text("--new-element"),
...
fn run_constraint_target_edit(
    ...
    run_workbench(&[
        text("model-edit-constraint-target"),
        source.as_os_str(),
        text("--constraint"),
        text(constraint_id),
        text("--node"),
...
fn run_fixed_constraint_dof_delete(
    source: &Path,
    destination: &Path,
    constraint_id: &str,
    dof: &str,
    ...
    run_workbench(&[
        text("model-delete-fixed-constraint-dof"),
        source.as_os_str(),
        text("--constraint"),
        text(constraint_id),
        text("--dof"),
...
fn run_fixed_constraint_dof_add(
    source: &Path,
    destination: &Path,
    constraint_id: &str,
    dof: &str,
    ...
    run_workbench(&[
        text("model-add-fixed-constraint-dof"),
        source.as_os_str(),
        text("--constraint"),
        text(constraint_id),
        text("--dof"),
...
fn run_nodal_load_identity_edit(
    ...
    run_workbench(&[
        text("model-edit-nodal-load-identity"),
        source.as_os_str(),
        text("--load-pattern"),
        text(load_pattern_id),
        text("--load"),
        text(nodal_load_id),
        text("--new-load"),
        text(replacement_nodal_load_id),
        text("--output-dir"),
        destination.as_os_str(),
    ])
}
...
fn run_linear_load_combination_identity_cascade_edit(
    source: &Path,
    destination: &Path,
    load_combination_id: &str,
    replacement_load_combination_id: &str,
    ...
    run_workbench(&[
        text("model-edit-linear-load-combination-identity-cascade"),
        source.as_os_str(),
        text("--load-combination"),
        text(load_combination_id),
        text("--new-load-combination"),
...
fn run_direct_linear_load_combination_term_insert(
    source: &Path,
    destination: &Path,
    load_combination_id: &str,
    load_pattern_id: &str,
    ...
    run_workbench(&[
        text("model-insert-linear-load-combination-term"),
        source.as_os_str(),
        text("--load-combination"),
        text(load_combination_id),
        text("--load-pattern"),
...
fn run_nested_linear_load_combination_term_add(
    source: &Path,
    destination: &Path,
    load_combination_id: &str,
    reference_kind: &str,
    ...
    run_workbench(&[
        text("model-add-nested-linear-load-combination-term"),
        source.as_os_str(),
        text("--load-combination"),
        text(load_combination_id),
        text("--ref-kind"),
...
#[test]
#[allow(clippy::too_many_lines)]
fn element_identity_edit_is_deterministic_fail_closed_restartable_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let source_bytes = std::fs::read(&source).expect("source ModelIR bytes");
    let source_validation = validate_model_bytes(&source_bytes).expect("C++-validated source");
    let source_model = source_validation.snapshot.value().clone();

    let first = temporary.0.join("element-identity-edit-first");
    let second = temporary.0.join("element-identity-edit-second");
    for destination in [&first, &second] {
        let output = run_element_identity_edit(&source, destination, "E1", "E1_RENAMED");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("published element-identity receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first element-identity artifact"),
            std::fs::read(second.join(artifact)).expect("second element-identity artifact")
        );
    }
    assert_eq!(
        std::fs::read(&source).expect("unchanged element source"),
        source_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("element identity-edited ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict element identity-edited ModelIR");
    let edited_validation =
        validate_model_bytes(&edited_bytes).expect("C++-validated identity-edited ModelIR");
    assert_eq!(
        edited_validation.snapshot.canonical_json().as_bytes(),
    ...
    let source_elements = source_model["elements"]
        .as_array()
        .expect("source elements");
    let edited_elements = edited.value()["elements"]
        .as_array()
        .expect("edited elements");
    assert_eq!(edited_elements.len(), source_elements.len());
    assert_eq!(edited_elements[0]["id"], "E1_RENAMED");
    for (key, value) in source_elements[0]
        .as_object()
        .expect("source element object")
    {
        if key != "id" {
            assert_eq!(&edited_elements[0][key], value);
        }
    }
    assert_eq!(&edited_elements[1..], &source_elements[1..]);
    for family in [
        "nodes",
        "materials",
        "sections",
    ...
    let mut retained_element = source_elements[0]
        .as_object()
        .expect("source element object")
        .clone();
    ...
    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-element-identity.v1")
        .expect("element identity provenance extension");
    assert_eq!(extension["operation"], "element_identity_edit");
    assert_eq!(extension["source_element_id"], "E1");
    assert_eq!(extension["replacement_element_id"], "E1_RENAMED");
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("element identity receipt"),
    )
    .expect("element identity receipt JSON");
    assert_eq!(receipt["operation"], "element_identity_edit");
    assert_eq!(receipt["source_element_id"], "E1");
    assert_eq!(receipt["replacement_element_id"], "E1_RENAMED");
    ...
    for (name, element_id, replacement, expected_code) in [
        ...
        let destination = temporary.0.join(format!("element-identity-{name}"));
        let rejected = run_element_identity_edit(&source, &destination, element_id, replacement);
        assert_eq!(rejected.status.code(), Some(1), "{name} status");
        assert!(
            String::from_utf8_lossy(&rejected.stdout).contains(expected_code),
            "{name} rejection: {}",
    ...
    let collision_source = temporary.0.join("element-identity-collision-source");
    ...
    let collision_destination = temporary.0.join("element-identity-collision-output");
    let collision = run_element_identity_edit(
    ...
    let existing = run_element_identity_edit(&source, &first, "E1", "E1_RENAMED");
    assert_eq!(existing.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&existing.stdout).contains("workbench_stage_destination_exists")
    );

    for (name, owned_value, expected_code) in [
        (
            "stage",
            serde_json::json!([{
                "id": "STAGE_E1",
        ...
        (
            "unsupported-feature",
            serde_json::json!([{
                "feature_id": "feature.element-identity-owned",
                "kind": "unsupported_solver_feature",
                "source_entity_id": "E1",
                "disposition": "blocked",
                "blocking": true,
        ...
        let field = match name {
            ...
            "unsupported-feature" => "unsupported_features",
            "roundtrip" => "roundtrip_map",
            _ => unreachable!(),
        };
        owned[field] = owned_value;
        let owned_path = temporary
            .0
            .join(format!("element-identity-{name}-source.json"));
        std::fs::write(
            &owned_path,
            canonicalize_model_ir_v2(&owned)
                .expect("canonical owned element source")
                .as_bytes(),
        )
        .expect("write owned element source");
        let destination = temporary.0.join(format!("element-identity-{name}-output"));
        let rejected = run_element_identity_edit(&owned_path, &destination, "E1", "E1_RENAMED");
        assert_eq!(rejected.status.code(), Some(1), "{name} status");
        assert!(
            String::from_utf8_lossy(&rejected.stdout).contains(expected_code),
            "{name} rejection: {}",
    ...
    let malformed_path = temporary.0.join("element-identity-malformed-source.json");
    let mut malformed = source_model;
    malformed["elements"][0]["index"] = serde_json::json!(7);
    std::fs::write(
        &malformed_path,
        canonicalize_model_ir_v2(&malformed)
            .expect("canonical malformed element source")
            .as_bytes(),
    )
    .expect("write malformed element source");
    let malformed_destination = temporary.0.join("element-identity-malformed-output");
    let malformed_rejection =
        run_element_identity_edit(&malformed_path, &malformed_destination, "E1", "E1_RENAMED");
    assert_eq!(malformed_rejection.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&malformed_rejection.stdout)
        .contains("workbench_model_edit_source_semantics_invalid"));
    assert!(!malformed_destination.exists());

    let request_directory = temporary.0.join("element-identity-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "element-identity-c5",
    ...
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("element identity request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("element identity direct execution");
    assert!(direct.is_complete());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("element identity direct recovery"),
    )
    .expect("element identity recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([6, 7, 8, 9, 10, 11])
    );
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("element identity initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
...
#[test]
#[allow(clippy::too_many_lines)]
fn constraint_target_edit_is_deterministic_cpp_revalidated_restartable_and_executable() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let member_directory = temporary.0.join("constraint-target-member-source");
    assert_success(&run_frame3d_member_add(
        &source,
        &member_directory,
        "N3",
    ...
    let load_directory = temporary.0.join("constraint-target-load-source");
    assert_success(&run_nodal_load_add(
        &member_directory.join("model-ir.json"),
        &load_directory,
        "LC_WEAK",
    ...
    let fixed_directory = temporary.0.join("constraint-target-fixed-source");
    assert_success(&run_fixed_constraint_add(
    ...
    let fixed_bytes = std::fs::read(&fixed_path).expect("constraint-target source bytes");
    let fixed_model = parse_model_ir_v2(&fixed_bytes).expect("strict constraint-target source");

    let first = temporary.0.join("constraint-target-first");
    let second = temporary.0.join("constraint-target-second");
    for destination in [&first, &second] {
        let output = run_constraint_target_edit(&fixed_path, destination, "BC_N3", "N2");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("constraint-target receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first constraint-target artifact"),
            std::fs::read(second.join(artifact)).expect("second constraint-target artifact")
        );
    }
    assert_eq!(
        std::fs::read(&fixed_path).expect("unchanged constraint-target source"),
        fixed_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("retargeted constraint ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict retargeted constraint ModelIR");
    let validation = validate_model_bytes(&edited_bytes).expect("C++ retargeted validation");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    for family in [
        ...
        assert_eq!(edited.value()[family], fixed_model.value()[family]);
    }
    let source_constraints = fixed_model.value()["constraints"]
        .as_array()
        .expect("source constraints");
    let edited_constraints = edited.value()["constraints"]
        .as_array()
        .expect("edited constraints");
    assert_eq!(source_constraints.len(), edited_constraints.len());
    ...
    assert_eq!(source_constraints[1]["node_id"], "N3");
    assert_eq!(edited_constraints[1]["node_id"], "N2");

    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-constraint-target.v1")
        .expect("constraint-target provenance extension");
    assert_eq!(extension["operation"], "constraint_target");
    ...
    assert_eq!(extension["previous_node_id"], "N3");
    assert_eq!(extension["edited_node_id"], "N2");
    assert_eq!(
        extension["dofs"],
        serde_json::json!(["UX", "UY", "UZ", "RX", "RY", "RZ"])
    );
    assert_eq!(
        extension["prescribed_values_si"],
        serde_json::json!({"UX": 0, "UY": 0, "UZ": 0, "RX": 0, "RY": 0, "RZ": 0})
    );
    assert_eq!(extension["retained_source_id"], Value::Null);
    assert_eq!(extension["retained_extensions"], serde_json::json!({}));
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("constraint-target receipt"),
    )
    .expect("constraint-target receipt JSON");
    assert_eq!(receipt["operation"], "constraint_target");
    assert_eq!(receipt["constraint_id"], "BC_N3");
    assert_eq!(receipt["constraint_index"], 1);
    assert_eq!(receipt["previous_node_id"], "N3");
    assert_eq!(receipt["edited_node_id"], "N2");
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    assert_eq!(receipt["analysis_ready"], true);
    assert_eq!(receipt["edited_content_hash"], edited.content_hash());
    assert_self_hashed_edit_receipt(&mut receipt);

    let request_directory = temporary.0.join("constraint-target-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "constraint-target-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("constraint-target request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("constraint-target direct execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("constraint-target direct recovery"),
    )
    .expect("constraint-target recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("constraint-target initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("constraint-target resumed execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (name, constraint, node, code) in [
        ...
        let destination = temporary.0.join(name);
        let rejected = run_constraint_target_edit(&fixed_path, &destination, constraint, node);
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }
    ...
    blocked["unsupported_features"] = serde_json::json!([{
        "feature_id": "feature.constraint-target-visible-not-runnable",
        "kind": "unsupported_solver_feature",
        "source_entity_id": null,
        "disposition": "blocked",
        "blocking": true,
        "detail": "Constraint target editing must preserve unsupported solver blockers.",
        "extensions": {}
    }]);
    blocked["roundtrip_map"] = serde_json::json!([
        {
    ...
    let blocked_source = temporary.0.join("blocked-constraint-target-source.json");
    std::fs::write(
        &blocked_source,
        canonicalize_model_ir_v2(&blocked)
            .expect("canonical blocked constraint-target source")
            .as_bytes(),
    )
    .expect("write blocked constraint-target source");
    let blocked_destination = temporary.0.join("blocked-constraint-target-output");
    ...
    let blocked_receipt: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("edit-receipt.json"))
            .expect("blocked constraint-target receipt"),
    )
    .expect("blocked constraint-target receipt JSON");
    assert_eq!(blocked_receipt["analysis_ready"], false);
    assert_eq!(
        blocked_receipt["blocking_feature_ids"],
        serde_json::json!(["feature.constraint-target-visible-not-runnable"])
    );
    let blocked_edited: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("model-ir.json"))
            .expect("blocked retargeted constraint ModelIR"),
    )
    .expect("blocked retargeted constraint JSON");
    assert_eq!(blocked_edited["constraints"][1]["node_id"], "N2");
    assert_eq!(
        blocked_edited["roundtrip_map"][0]["mapping_status"],
        "approximated"
    );
    ...
    let existing = run_constraint_target_edit(&fixed_path, &first, "BC_N3", "N2");
    assert_eq!(existing.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&existing.stdout).contains("workbench_stage_destination_exists")
    );
...
#[test]
#[allow(clippy::too_many_lines)]
fn fixed_constraint_dof_deletion_is_deterministic_restartable_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let member_directory = temporary.0.join("constraint-dof-delete-member-source");
    assert_success(&run_frame3d_member_add(
        &source,
        &member_directory,
        "N3",
    ...
    let load_directory = temporary.0.join("constraint-dof-delete-load-source");
    assert_success(&run_nodal_load_add(
        &member_directory.join("model-ir.json"),
        &load_directory,
        "LC_WEAK",
    ...
    let fixed_directory = temporary.0.join("constraint-dof-delete-fixed-source");
    assert_success(&run_fixed_constraint_add(
        &load_directory.join("model-ir.json"),
        &fixed_directory,
        "BC_N3",
    ...
    let target_directory = temporary.0.join("constraint-dof-delete-target-source");
    assert_success(&run_constraint_target_edit(
    ...
    let target_path = target_directory.join("model-ir.json");
    let target_bytes = std::fs::read(&target_path).expect("constraint-DOF delete source bytes");
    let target_model = parse_model_ir_v2(&target_bytes).expect("strict DOF-delete source");

    let first = temporary.0.join("constraint-dof-delete-first");
    let second = temporary.0.join("constraint-dof-delete-second");
    for destination in [&first, &second] {
        let output = run_fixed_constraint_dof_delete(&target_path, destination, "BC_N3", "RZ");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("constraint-DOF delete receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first constraint-DOF delete artifact"),
            std::fs::read(second.join(artifact)).expect("second constraint-DOF delete artifact")
        );
    }
    assert_eq!(
        std::fs::read(&target_path).expect("unchanged constraint-DOF delete source"),
        target_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("constraint-DOF-deleted ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict DOF-deleted ModelIR");
    let validation = validate_model_bytes(&edited_bytes).expect("C++ DOF-deleted validation");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    for family in [
        ...
        assert_eq!(edited.value()[family], target_model.value()[family]);
    }
    let source_constraints = target_model.value()["constraints"]
        .as_array()
        .expect("source constraints");
    let edited_constraints = edited.value()["constraints"]
        .as_array()
        .expect("edited constraints");
    assert_eq!(edited_constraints[0], source_constraints[0]);
    for key in ["id", "index", "type", "node_id", "source_id", "extensions"] {
        assert_eq!(edited_constraints[1][key], source_constraints[1][key]);
    }
    assert_eq!(
    ...
    assert_eq!(
        ...
        serde_json::json!(["UX", "UY", "UZ", "RX", "RY"])
    );
    assert_eq!(source_constraints[1]["prescribed_values_si"]["RZ"], 0);
    assert!(edited_constraints[1]["prescribed_values_si"]
        .get("RZ")
        .is_none());

    let extension = edited.value()["extensions"]
        .get("structural-native:model-delete-fixed-constraint-dof.v1")
        .expect("constraint-DOF delete provenance extension");
    assert_eq!(extension["operation"], "fixed_constraint_dof_delete");
    assert_eq!(extension["constraint_id"], "BC_N3");
    assert_eq!(extension["constraint_index"], 1);
    assert_eq!(extension["constraint_type"], "fixed_dofs");
    assert_eq!(extension["node_id"], "N2");
    assert_eq!(extension["removed_dof"], "RZ");
    assert_eq!(extension["removed_prescribed_value_explicit"], true);
    assert_eq!(extension["removed_prescribed_value_si"], 0);
    assert_eq!(
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("constraint-DOF delete receipt"),
    )
    .expect("constraint-DOF delete receipt JSON");
    assert_eq!(receipt["operation"], "fixed_constraint_dof_delete");
    assert_eq!(receipt["constraint_id"], "BC_N3");
    assert_eq!(receipt["removed_dof"], "RZ");
    assert_eq!(receipt["removed_prescribed_value_explicit"], true);
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    assert_eq!(receipt["analysis_ready"], true);
    assert_eq!(receipt["edited_content_hash"], edited.content_hash());
    assert_self_hashed_edit_receipt(&mut receipt);

    let request_directory = temporary.0.join("constraint-dof-delete-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "constraint-dof-delete-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("constraint-DOF delete request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("constraint-DOF delete direct execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("constraint-DOF delete direct recovery"),
    )
    .expect("constraint-DOF delete recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
    ...
    assert_eq!(
        recovery["active_external_load"],
        serde_json::json!([0, 0, -1000, 0, 0, 0, 0])
    );
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("constraint-DOF delete initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("constraint-DOF delete resumed execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (name, constraint, dof, code) in [
        (
            "constraint-dof-delete-missing-constraint",
            "MISSING",
            "RZ",
            "workbench_model_delete_fixed_constraint_dof_constraint_missing",
        ),
        (
            "constraint-dof-delete-not-restrained",
            "BC_N3",
            "RZ",
            "workbench_model_delete_fixed_constraint_dof_not_restrained",
        ),
    ] {
        let source_path = if name.ends_with("not-restrained") {
            first.join("model-ir.json")
        } else {
            target_path.clone()
        };
        let destination = temporary.0.join(name);
        let rejected = run_fixed_constraint_dof_delete(&source_path, &destination, constraint, dof);
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }

    let mut minimum = target_model.value().clone();
    minimum["constraints"][1]["dofs"] = serde_json::json!(["RZ"]);
    minimum["constraints"][1]["prescribed_values_si"] = serde_json::json!({"RZ": 0});
    let minimum_source = temporary
        .0
        .join("constraint-dof-delete-minimum-source.json");
    std::fs::write(
        &minimum_source,
        canonicalize_model_ir_v2(&minimum)
            .expect("canonical minimum-mask source")
            .as_bytes(),
    )
    .expect("write minimum-mask source");
    let minimum_destination = temporary.0.join("constraint-dof-delete-minimum-rejected");
    let minimum_rejected =
        run_fixed_constraint_dof_delete(&minimum_source, &minimum_destination, "BC_N3", "RZ");
    assert_eq!(minimum_rejected.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&minimum_rejected.stdout)
        .contains("workbench_model_delete_fixed_constraint_dof_minimum_mask"));
    assert!(!minimum_destination.exists());

    let mut blocked = target_model.value().clone();
    blocked["unsupported_features"] = serde_json::json!([{
        "feature_id": "feature.constraint-dof-delete-visible-not-runnable",
        "kind": "unsupported_solver_feature",
        "source_entity_id": null,
        "disposition": "blocked",
        "blocking": true,
        "detail": "Constraint DOF deletion must preserve unsupported solver blockers.",
        "extensions": {}
    }]);
    blocked["roundtrip_map"] = serde_json::json!([{
        "source_entity_id": "source:BC_N3",
    ...
    let blocked_source = temporary
        .0
        .join("blocked-constraint-dof-delete-source.json");
    std::fs::write(
        &blocked_source,
        canonicalize_model_ir_v2(&blocked)
            .expect("canonical blocked constraint-DOF delete source")
            .as_bytes(),
    )
    .expect("write blocked constraint-DOF delete source");
    let blocked_destination = temporary.0.join("blocked-constraint-dof-delete-output");
    ...
    let blocked_receipt: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("edit-receipt.json"))
            .expect("blocked constraint-DOF delete receipt"),
    )
    .expect("blocked constraint-DOF delete receipt JSON");
    assert_eq!(blocked_receipt["analysis_ready"], false);
    assert_eq!(
        blocked_receipt["blocking_feature_ids"],
        serde_json::json!(["feature.constraint-dof-delete-visible-not-runnable"])
    );
    let blocked_edited: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("model-ir.json"))
            .expect("blocked constraint-DOF-deleted ModelIR"),
    )
    .expect("blocked constraint-DOF-deleted JSON");
    assert_eq!(
        blocked_edited["roundtrip_map"][0]["mapping_status"],
        "approximated"
    );

    let existing = run_fixed_constraint_dof_delete(&target_path, &first, "BC_N3", "RZ");
    assert_eq!(existing.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&existing.stdout).contains("workbench_stage_destination_exists")
    );
...
#[test]
#[allow(clippy::too_many_lines)]
fn fixed_constraint_dof_addition_is_deterministic_restartable_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let member_directory = temporary.0.join("constraint-dof-add-member-source");
    assert_success(&run_frame3d_member_add(
        &source,
        &member_directory,
        "N3",
    ...
    let load_directory = temporary.0.join("constraint-dof-add-load-source");
    assert_success(&run_nodal_load_add(
        &member_directory.join("model-ir.json"),
        &load_directory,
        "LC_WEAK",
    ...
    let fixed_directory = temporary.0.join("constraint-dof-add-fixed-source");
    assert_success(&run_fixed_constraint_add(
        &load_directory.join("model-ir.json"),
        &fixed_directory,
        "BC_N3",
    ...
    let target_directory = temporary.0.join("constraint-dof-add-target-source");
    assert_success(&run_constraint_target_edit(
        &fixed_directory.join("model-ir.json"),
        &target_directory,
        "BC_N3",
    ...
    let delete_directory = temporary.0.join("constraint-dof-add-delete-source");
    assert_success(&run_fixed_constraint_dof_delete(
    ...
    let delete_path = delete_directory.join("model-ir.json");
    let delete_bytes = std::fs::read(&delete_path).expect("constraint-DOF add source bytes");
    let delete_model = parse_model_ir_v2(&delete_bytes).expect("strict DOF-add source");

    let first = temporary.0.join("constraint-dof-add-first");
    let second = temporary.0.join("constraint-dof-add-second");
    for destination in [&first, &second] {
        let output = run_fixed_constraint_dof_add(&delete_path, destination, "BC_N3", "RZ", "0");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("constraint-DOF add receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first constraint-DOF add artifact"),
            std::fs::read(second.join(artifact)).expect("second constraint-DOF add artifact")
        );
    }
    assert_eq!(
        std::fs::read(&delete_path).expect("unchanged constraint-DOF add source"),
        delete_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("constraint-DOF-added ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict DOF-added ModelIR");
    let validation = validate_model_bytes(&edited_bytes).expect("C++ DOF-added validation");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    for family in [
        ...
        assert_eq!(edited.value()[family], delete_model.value()[family]);
    }
    let source_constraints = delete_model.value()["constraints"]
        .as_array()
        .expect("source constraints");
    let edited_constraints = edited.value()["constraints"]
        .as_array()
    ...
    assert!(source_constraints[1]["prescribed_values_si"]
        .get("RZ")
        .is_none());
    assert_eq!(edited_constraints[1]["prescribed_values_si"]["RZ"], 0);

    let extension = edited.value()["extensions"]
        .get("structural-native:model-add-fixed-constraint-dof.v1")
        .expect("constraint-DOF add provenance extension");
    assert_eq!(extension["operation"], "fixed_constraint_dof_add");
    assert_eq!(extension["constraint_id"], "BC_N3");
    assert_eq!(extension["constraint_index"], 1);
    assert_eq!(extension["constraint_type"], "fixed_dofs");
    assert_eq!(extension["node_id"], "N2");
    assert_eq!(extension["added_dof"], "RZ");
    assert_eq!(extension["added_prescribed_value_si"], 0);
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("constraint-DOF add receipt"),
    )
    .expect("constraint-DOF add receipt JSON");
    assert_eq!(receipt["operation"], "fixed_constraint_dof_add");
    assert_eq!(receipt["constraint_id"], "BC_N3");
    assert_eq!(receipt["added_dof"], "RZ");
    assert_eq!(receipt["added_prescribed_value_si"], 0);
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    assert_eq!(receipt["analysis_ready"], true);
    assert_eq!(receipt["edited_content_hash"], edited.content_hash());
    assert_self_hashed_edit_receipt(&mut receipt);

    let request_directory = temporary.0.join("constraint-dof-add-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "constraint-dof-add-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("constraint-DOF add request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("constraint-DOF add direct execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("constraint-DOF add direct recovery"),
    )
    .expect("constraint-DOF add recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([12, 13, 14, 15, 16, 17])
    );
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("constraint-DOF add initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("constraint-DOF add resumed execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (name, source_path, constraint, dof, code) in [
        (
            "constraint-dof-add-missing-constraint",
            delete_path.clone(),
            "MISSING",
            "RZ",
        ...
        (
            "constraint-dof-add-already-restrained",
            first.join("model-ir.json"),
            "BC_N3",
            "RZ",
        ...
        let destination = temporary.0.join(name);
        let rejected =
            run_fixed_constraint_dof_add(&source_path, &destination, constraint, dof, "0");
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }

    let mut overlap = delete_model.value().clone();
    overlap["constraints"]
        .as_array_mut()
        .expect("overlap constraints")
        .push(serde_json::json!({
            "id": "BC_OVERLAP", "index": 2, "type": "fixed_dofs", "node_id": "N2",
            "dofs": ["RZ"], "prescribed_values_si": {"RZ": 0},
            "source_id": null, "extensions": {}
        }));
    let overlap_source = temporary.0.join("constraint-dof-add-overlap-source.json");
    std::fs::write(
        &overlap_source,
        canonicalize_model_ir_v2(&overlap)
            .expect("canonical overlap source")
            .as_bytes(),
    )
    .expect("write overlap source");
    let overlap_destination = temporary.0.join("constraint-dof-add-overlap-rejected");
    let overlap_rejected =
        run_fixed_constraint_dof_add(&overlap_source, &overlap_destination, "BC_N3", "RZ", "0");
    assert_eq!(overlap_rejected.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&overlap_rejected.stdout)
        .contains("workbench_model_add_fixed_constraint_dof_overlap"));
    assert!(!overlap_destination.exists());

    let mut blocked = delete_model.value().clone();
    blocked["unsupported_features"] = serde_json::json!([{
        "feature_id": "feature.constraint-dof-add-visible-not-runnable",
        "kind": "unsupported_solver_feature",
        "source_entity_id": null,
        "disposition": "blocked",
        "blocking": true,
        "detail": "Constraint DOF addition must preserve unsupported solver blockers.",
        "extensions": {}
    }]);
    blocked["roundtrip_map"] = serde_json::json!([{
        "source_entity_id": "source:BC_N3",
    ...
    let blocked_source = temporary.0.join("blocked-constraint-dof-add-source.json");
    std::fs::write(
        &blocked_source,
        canonicalize_model_ir_v2(&blocked)
            .expect("canonical blocked constraint-DOF add source")
            .as_bytes(),
    )
    .expect("write blocked constraint-DOF add source");
    let blocked_destination = temporary.0.join("blocked-constraint-dof-add-output");
    ...
    let blocked_receipt: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("edit-receipt.json"))
            .expect("blocked constraint-DOF add receipt"),
    )
    .expect("blocked constraint-DOF add receipt JSON");
    assert_eq!(blocked_receipt["analysis_ready"], false);
    assert_eq!(
        blocked_receipt["blocking_feature_ids"],
        serde_json::json!(["feature.constraint-dof-add-visible-not-runnable"])
    );
    let blocked_edited: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("model-ir.json"))
            .expect("blocked constraint-DOF-added ModelIR"),
    )
    .expect("blocked constraint-DOF-added JSON");
    assert_eq!(
        blocked_edited["roundtrip_map"][0]["mapping_status"],
        "approximated"
    );
...
#[test]
#[allow(clippy::too_many_lines)]
fn nodal_load_identity_edit_is_deterministic_globally_unique_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let source_bytes = std::fs::read(&source).expect("nodal-load identity source bytes");
    let source_model = validate_model_bytes(&source_bytes)
        .expect("C++ nodal-load identity source validation")
        .snapshot;

    let first = temporary.0.join("nodal-load-identity-first");
    let second = temporary.0.join("nodal-load-identity-second");
    for destination in [&first, &second] {
        let output = run_nodal_load_identity_edit(
            &source,
            destination,
            "LC_WEAK",
            "L_WEAK_N2",
        ...
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("nodal-load identity receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first nodal-load identity artifact"),
            std::fs::read(second.join(artifact)).expect("second nodal-load identity artifact")
        );
    }
    assert_eq!(
        std::fs::read(&source).expect("unchanged nodal-load identity source"),
        source_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("identity-edited nodal-load ModelIR");
    let edited =
        parse_model_ir_v2(&edited_bytes).expect("strict identity-edited nodal-load ModelIR");
    let validation =
        validate_model_bytes(&edited_bytes).expect("C++ nodal-load identity validation");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    for family in [
    ...
    for (source_pattern, edited_pattern) in source_patterns.iter().zip(edited_patterns) {
        if source_pattern["id"] == "LC_WEAK" {
            ...
            let source_load = &source_pattern["nodal_loads"][0];
            let edited_load = &edited_pattern["nodal_loads"][0];
            assert_eq!(source_load["id"], "L_WEAK_N2");
            assert_eq!(edited_load["id"], "L_WEAK_N2_RENAMED");
            for key in [
                "index",
                "node_id",
                "components_si",
    ...
    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-nodal-load-identity.v1")
        .expect("nodal-load identity provenance extension");
    assert_eq!(extension["operation"], "nodal_load_identity_edit");
    assert_eq!(extension["load_pattern_id"], "LC_WEAK");
    assert_eq!(extension["load_pattern_index"], 1);
    assert_eq!(extension["analysis_type"], "linear_static");
    assert_eq!(extension["source_nodal_load_id"], "L_WEAK_N2");
    assert_eq!(extension["replacement_nodal_load_id"], "L_WEAK_N2_RENAMED");
    assert_eq!(extension["nodal_load_index"], 0);
    assert_eq!(extension["node_id"], "N2");
    assert_eq!(
        extension["retained_components_si"],
        serde_json::json!({"FX": 0, "FY": -10000, "FZ": 0, "MX": 0, "MY": 0, "MZ": 0})
    );
    assert_eq!(extension["retained_source_id"], "generated:L_WEAK_N2");
    assert_eq!(extension["retained_extensions"], serde_json::json!({}));
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("nodal-load identity receipt"),
    )
    .expect("nodal-load identity receipt JSON");
    assert_eq!(receipt["operation"], "nodal_load_identity_edit");
    assert_eq!(receipt["source_nodal_load_id"], "L_WEAK_N2");
    assert_eq!(receipt["replacement_nodal_load_id"], "L_WEAK_N2_RENAMED");
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    assert_eq!(receipt["analysis_ready"], true);
    assert_eq!(receipt["edited_content_hash"], edited.content_hash());
    assert_self_hashed_edit_receipt(&mut receipt);

    let request_directory = temporary.0.join("nodal-load-identity-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "nodal-load-identity-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("nodal-load identity request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("nodal-load identity direct execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("nodal-load identity direct recovery"),
    )
    .expect("nodal-load identity recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([6, 7, 8, 9, 10, 11])
    );
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("nodal-load identity initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("nodal-load identity resumed execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (name, pattern, source_id, replacement_id, code) in [
        (
            "nodal-load-identity-missing-pattern",
            "MISSING",
            "L_WEAK_N2",
            "L_NEW",
        ...
        (
            "nodal-load-identity-missing-load",
            "LC_WEAK",
            "MISSING",
            "L_NEW",
        ...
        (
            "nodal-load-identity-global-collision",
            "LC_WEAK",
            "L_WEAK_N2",
            "L_AXIAL_N2",
        ...
        (
            "nodal-load-identity-invalid-stable-id",
            "LC_WEAK",
            "L_WEAK_N2",
            "1_INVALID",
        ...
        let destination = temporary.0.join(name);
        let rejected =
            run_nodal_load_identity_edit(&source, &destination, pattern, source_id, replacement_id);
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }
    ...
    let mut blocked = source_model.value().clone();
    blocked["unsupported_features"] = serde_json::json!([{
        "feature_id": "feature.nodal-load-identity-visible-not-runnable",
        "kind": "unsupported_solver_feature",
        "source_entity_id": null,
        "disposition": "blocked",
        "blocking": true,
        "detail": "Nodal-load identity editing must preserve unrelated blockers.",
        "extensions": {}
    }]);
    blocked["roundtrip_map"] = serde_json::json!([
        {
    ...
    let blocked_source = temporary.0.join("blocked-nodal-load-identity-source.json");
    std::fs::write(
        &blocked_source,
        canonicalize_model_ir_v2(&blocked)
            .expect("canonical blocked nodal-load identity source")
            .as_bytes(),
    )
    .expect("write blocked nodal-load identity source");
    let blocked_destination = temporary.0.join("blocked-nodal-load-identity-output");
    assert_success(&run_nodal_load_identity_edit(
        &blocked_source,
        &blocked_destination,
        "LC_WEAK",
        "L_WEAK_N2",
    ...
    let blocked_receipt: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("edit-receipt.json"))
            .expect("blocked nodal-load identity receipt"),
    )
    .expect("blocked nodal-load identity receipt JSON");
    assert_eq!(blocked_receipt["analysis_ready"], false);
    assert_eq!(
        blocked_receipt["blocking_feature_ids"],
        serde_json::json!(["feature.nodal-load-identity-visible-not-runnable"])
    );
    let blocked_edited: Value = serde_json::from_slice(
        &std::fs::read(blocked_destination.join("model-ir.json"))
            .expect("blocked identity-edited ModelIR"),
    )
    .expect("blocked identity-edited JSON");
    assert_eq!(
        blocked_edited["roundtrip_map"][0]["mapping_status"],
        "approximated"
    );
...
#[test]
#[allow(clippy::too_many_lines)]
fn frame_section_identity_cascade_edit_is_atomic_restartable_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let fixture =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let fixture_bytes = std::fs::read(&fixture).expect("frame-section cascade fixture bytes");
    let mut source_model = validate_model_bytes(&fixture_bytes)
        .expect("C++-validated frame-section cascade fixture")
        .snapshot
        .value()
        .clone();
    source_model["roundtrip_map"] = serde_json::json!([{
        ...
        "model_ir_entity_id": "S1",
        "mapping_status": "canonicalized",
        "extensions": {"structural-native:fixture": "section-cascade"}
    }]);
    let source_bytes = canonicalize_model_ir_v2(&source_model)
        .expect("canonical frame-section cascade source")
        .into_bytes();
    validate_model_bytes(&source_bytes).expect("C++-validated mapped frame-section cascade source");
    let source = temporary.0.join("frame-section-cascade-source.json");
    std::fs::write(&source, &source_bytes).expect("write frame-section cascade source");

    let first = temporary.0.join("frame-section-cascade-first");
    let second = temporary.0.join("frame-section-cascade-second");
    for destination in [&first, &second] {
        let output =
            run_frame_section_identity_cascade_edit(&source, destination, "S1", "S1_LINKED");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("published frame-section cascade receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first frame-section cascade artifact"),
            std::fs::read(second.join(artifact)).expect("second frame-section cascade artifact")
        );
    }
    assert_eq!(
        std::fs::read(&source).expect("unchanged frame-section cascade source"),
        source_bytes
    );

    let edited_bytes = std::fs::read(first.join("model-ir.json"))
        .expect("frame-section identity-cascaded ModelIR");
    let edited =
        parse_model_ir_v2(&edited_bytes).expect("strict frame-section identity-cascaded ModelIR");
    let validation = validate_model_bytes(&edited_bytes)
        .expect("C++-validated frame-section identity-cascaded ModelIR");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    assert_eq!(
    ...
    assert_eq!(edited.value()["sections"][0]["id"], "S1_LINKED");
    for key in [
        "index",
        "family_id",
        "parameter_set_version",
        ...
        assert_eq!(
            edited.value()["sections"][0][key],
            source_model["sections"][0][key]
        );
    }
    assert_eq!(edited.value()["elements"][0]["section_id"], "S1_LINKED");
    for key in [
        "id",
        "index",
        "element_type",
    ...
    assert_eq!(edited.value()["roundtrip_map"][0]["entity_kind"], "section");
    assert_eq!(
        edited.value()["roundtrip_map"][0]["model_ir_entity_id"],
        "S1_LINKED"
    );
    ...
    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-frame-section-identity-cascade.v2")
        .expect("frame-section cascade provenance extension");
    assert_eq!(
        extension["operation"],
        "frame_section_identity_cascade_edit"
    );
    assert_eq!(extension["source_section_id"], "S1");
    assert_eq!(extension["replacement_section_id"], "S1_LINKED");
    assert_eq!(extension["section_index"], 0);
    assert_eq!(extension["family_id"], "frame_3d");
    assert_eq!(extension["parameter_set_version"], "1");
    assert_eq!(extension["element_reference_count"], 1);
    assert_eq!(extension["roundtrip_reference_count"], 1);
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("frame-section cascade receipt"),
    )
    .expect("frame-section cascade receipt JSON");
    assert_eq!(receipt["operation"], "frame_section_identity_cascade_edit");
    assert_eq!(receipt["source_section_id"], "S1");
    assert_eq!(receipt["replacement_section_id"], "S1_LINKED");
    assert_eq!(receipt["section_index"], 0);
    assert_eq!(receipt["element_reference_count"], 1);
    assert_eq!(receipt["roundtrip_reference_count"], 1);
    assert_eq!(receipt["typed_reference_cascade_verified"], true);
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    ...
    for (name, section_id, replacement, code) in [
        ...
        let destination = temporary.0.join(format!("frame-section-cascade-{name}"));
        let rejected =
            run_frame_section_identity_cascade_edit(&source, &destination, section_id, replacement);
        assert_eq!(rejected.status.code(), Some(1), "{name} status");
        assert!(
            String::from_utf8_lossy(&rejected.stdout).contains(code),
            "{name} rejection: {}",
    ...
    let orphan_source = temporary.0.join("frame-section-cascade-orphan-source");
    assert_success(&run_frame_section_add(
        &fixture,
        &orphan_source,
        "S2",
    ...
    let orphan_destination = temporary.0.join("frame-section-cascade-orphan-output");
    let orphan_rejection = run_frame_section_identity_cascade_edit(
        &orphan_source.join("model-ir.json"),
        &orphan_destination,
        "S2",
        "S2_LINKED",
    ...
    assert_eq!(orphan_rejection.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&orphan_rejection.stdout)
        .contains("workbench_model_edit_frame_section_identity_cascade_unreferenced"));
    assert!(!orphan_destination.exists());

    for (name, owned_id) in [("source-owned", "S1"), ("replacement-owned", "S1_LINKED")] {
        let mut owned = source_model.clone();
        owned["unsupported_features"] = serde_json::json!([{
            "feature_id": format!("feature.frame-section-cascade-{name}"),
            "kind": "unsupported_solver_feature",
            "source_entity_id": owned_id,
            "disposition": "blocked",
            "blocking": true,
            "detail": "Frame-section identity remains externally owned.",
            "extensions": {}
        }]);
        let owned_path = temporary
            .0
            .join(format!("frame-section-cascade-{name}-source.json"));
        std::fs::write(
            &owned_path,
            canonicalize_model_ir_v2(&owned)
                .expect("canonical owned frame-section cascade source")
                .as_bytes(),
        )
        .expect("write owned frame-section cascade source");
        let destination = temporary
            .0
            .join(format!("frame-section-cascade-{name}-output"));
        let rejected =
            run_frame_section_identity_cascade_edit(&owned_path, &destination, "S1", "S1_LINKED");
        assert_eq!(rejected.status.code(), Some(1), "{name} status");
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(
            "workbench_model_edit_frame_section_identity_cascade_unsupported_feature_owned"
        ));
        assert!(!destination.exists());
    }
    let existing = run_frame_section_identity_cascade_edit(&source, &first, "S1", "S1_LINKED");
    assert_eq!(existing.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&existing.stdout).contains("workbench_stage_destination_exists")
    );

    let request_directory = temporary.0.join("frame-section-cascade-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "frame-section-cascade-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("frame-section cascade request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("frame-section cascade direct CPU execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("frame-section cascade direct recovery"),
    )
    .expect("frame-section cascade recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([6, 7, 8, 9, 10, 11])
    );
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("frame-section cascade initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("frame-section cascade resumed CPU execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
...
#[test]
#[allow(clippy::too_many_lines)]
fn truss_section_identity_edit_is_deterministic_fail_closed_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let base = repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let first_section = temporary.0.join("truss-section-identity-t1");
    assert_success(&run_truss_section_add(&base, &first_section, "T1", "0.005"));
    let member = temporary.0.join("truss-section-identity-member");
    assert_success(&run_truss3d_member_add(
        &first_section.join("model-ir.json"),
        &member,
        "N3",
        ["2", "1", "0"],
        "E2",
    ...
    let fixed = temporary.0.join("truss-section-identity-fixed");
    assert_success(&run_fixed_constraint_add(
    ...
    let second_section = temporary.0.join("truss-section-identity-source");
    ...
    let source = second_section.join("model-ir.json");
    let source_bytes = std::fs::read(&source).expect("truss-section identity source bytes");
    let source_model = validate_model_bytes(&source_bytes)
        .expect("C++ truss-section identity source validation")
        .snapshot;

    let first = temporary.0.join("truss-section-identity-first");
    let second = temporary.0.join("truss-section-identity-second");
    for destination in [&first, &second] {
        let output = run_truss_section_identity_edit(&source, destination, "T2", "T2_RENAMED");
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("truss-section identity receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first truss-section identity artifact"),
            std::fs::read(second.join(artifact)).expect("second truss-section identity artifact")
        );
    }
    assert_eq!(
        std::fs::read(&source).expect("unchanged truss-section identity source"),
        source_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("identity-edited truss-section ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict identity-edited truss section");
    let validation =
        validate_model_bytes(&edited_bytes).expect("C++ truss-section identity validation");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    for family in [
    ...
    assert_eq!(source_sections.len(), edited_sections.len());
    assert_eq!(edited_sections[0], source_sections[0]);
    assert_eq!(edited_sections[1], source_sections[1]);
    assert_eq!(edited_sections[2]["id"], "T2_RENAMED");
    for key in [
        "index",
        "family_id",
        "parameter_set_version",
        ...
        assert_eq!(edited_sections[2][key], source_sections[2][key]);
    }

    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-truss-section-identity.v1")
        .expect("truss-section identity provenance extension");
    assert_eq!(extension["operation"], "truss_section_identity_edit");
    assert_eq!(extension["source_section_id"], "T2");
    assert_eq!(extension["replacement_section_id"], "T2_RENAMED");
    assert_eq!(extension["section_index"], 2);
    assert_eq!(extension["family_id"], "truss_3d");
    assert_eq!(extension["parameter_set_version"], "1");
    assert_eq!(
        extension["retained_parameters_si"],
        source_sections[2]["parameters"]
    );
    assert_eq!(extension["retained_source_id"], Value::Null);
    assert_eq!(extension["retained_extensions"], serde_json::json!({}));

    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("truss-section identity receipt"),
    )
    .expect("truss-section identity receipt JSON");
    assert_eq!(receipt["operation"], "truss_section_identity_edit");
    assert_eq!(receipt["source_section_id"], "T2");
    assert_eq!(receipt["replacement_section_id"], "T2_RENAMED");
    assert_eq!(
        receipt["retained_parameters_si"],
    ...
    let request_directory = temporary.0.join("truss-section-identity-request");
    assert_success(&run_model_linear_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "truss-section-identity-c5",
        "LC_WEAK",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("truss-section identity request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("truss-section identity direct execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("truss-section identity direct recovery"),
    )
    .expect("truss-section identity recovery JSON");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([6, 7, 8, 9, 10, 11])
    );
    ...
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("truss-section identity initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("truss-section identity resumed execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (name, source_id, replacement_id, code) in [
        ...
        let destination = temporary.0.join(name);
        let rejected =
            run_truss_section_identity_edit(&source, &destination, source_id, replacement_id);
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }
    ...
    let write_model = |name: &str, value: &Value| {
        ...
        std::fs::write(
            &path,
            canonicalize_model_ir_v2(value)
                .expect("canonical truss-section identity rejection source")
                .as_bytes(),
        )
        .expect("write truss-section identity rejection source");
        path
    };
    let mut element_owned = source_model.value().clone();
    element_owned["elements"][1]["section_id"] = serde_json::json!("T2");
    let mut feature_owned = source_model.value().clone();
    feature_owned["unsupported_features"] = serde_json::json!([{
        "feature_id": "feature.truss-section-identity-owned",
        "kind": "unsupported_solver_feature", "source_entity_id": "T2",
        "disposition": "blocked", "blocking": true,
        "detail": "Truss-section identity is externally owned.", "extensions": {}
    }]);
    let mut roundtrip_owned = source_model.value().clone();
    roundtrip_owned["roundtrip_map"]
        .as_array_mut()
        .expect("round-trip rows")
        .push(serde_json::json!({
            "source_entity_id": "source:T2", "entity_kind": "section",
            "model_ir_entity_id": "T2", "mapping_status": "exact", "extensions": {}
        }));
    for (name, value, code) in [
        (
            "element-owned-truss-section.json",
            element_owned,
            "workbench_model_edit_truss_section_identity_referenced_by_element",
        ),
        (
            "feature-owned-truss-section.json",
            feature_owned,
            "workbench_model_edit_truss_section_identity_unsupported_feature_owned",
        ),
        (
            "roundtrip-owned-truss-section.json",
            roundtrip_owned,
            "workbench_model_edit_truss_section_identity_roundtrip_owned",
        ),
    ] {
        let owned_source = write_model(name, &value);
        let destination = temporary.0.join(format!("{name}.output"));
        let rejected =
            run_truss_section_identity_edit(&owned_source, &destination, "T2", "T2_RENAMED");
        assert_eq!(rejected.status.code(), Some(1));
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(code));
        assert!(!destination.exists());
    }

    let existing = run_truss_section_identity_edit(&source, &first, "T2", "T2_RENAMED");
    assert_eq!(existing.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&existing.stdout).contains("workbench_stage_destination_exists")
    );
...
#[test]
#[allow(clippy::too_many_lines)]
fn linear_load_combination_identity_cascade_is_atomic_restartable_and_cpu_executable() {
    let temporary = TestDirectory::create();
    let base = repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let direct = temporary.0.join("combination-cascade-direct");
    assert_success(&run_direct_linear_load_combination_add(
        &base,
        &direct,
        "COMBO_BASE",
    ...
    let nested = temporary.0.join("combination-cascade-nested");
    assert_success(&run_nested_linear_load_combination_add(
        &direct.join("model-ir.json"),
        &nested,
        "COMBO_PARENT",
        &[
            ["--combination-term", "COMBO_BASE", "1"],
            ["--pattern-term", "LC_AXIAL", "0.1"],
        ],
    ));
    let nested_bytes = std::fs::read(nested.join("model-ir.json"))
        .expect("combination cascade nested source bytes");
    let mut source_model = parse_model_ir_v2(&nested_bytes)
        .expect("strict combination cascade nested source")
        .value()
        .clone();
    source_model["roundtrip_map"] = serde_json::json!([{
        "source_entity_id": "source:COMBO_BASE",
        "entity_kind": "load_combination",
        "model_ir_entity_id": "COMBO_BASE",
        "mapping_status": "canonicalized",
        "extensions": {"structural-native:fixture": "combination-cascade-map"}
    }]);
    let source_bytes = canonicalize_model_ir_v2(&source_model)
        .expect("canonical combination cascade source")
        .into_bytes();
    let source_validation = validate_model_bytes(&source_bytes)
        .expect("C++-validated referenced combination cascade source");
    assert!(source_validation.report.contract_valid);
    assert!(source_validation.report.semantics_valid);
    assert!(source_validation.report.analysis_ready);
    let source = temporary.0.join("combination-cascade-source.json");
    std::fs::write(&source, &source_bytes).expect("write combination cascade source");

    let first = temporary.0.join("combination-cascade-first");
    let second = temporary.0.join("combination-cascade-second");
    for destination in [&first, &second] {
        let output = run_linear_load_combination_identity_cascade_edit(
            &source,
            destination,
            "COMBO_BASE",
            "COMBO_BASE_LINKED",
        ...
        assert_success(&output);
        let receipt = std::fs::read(destination.join("edit-receipt.json"))
            .expect("published combination cascade receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first combination cascade artifact"),
            std::fs::read(second.join(artifact)).expect("second combination cascade artifact")
        );
    }
    assert_eq!(
        std::fs::read(&source).expect("unchanged combination cascade source"),
        source_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("combination identity-cascaded ModelIR");
    let edited =
        parse_model_ir_v2(&edited_bytes).expect("strict combination identity-cascaded ModelIR");
    let validation = validate_model_bytes(&edited_bytes)
        .expect("C++-validated combination identity-cascaded ModelIR");
    assert!(validation.report.contract_valid);
    assert!(validation.report.semantics_valid);
    assert!(validation.report.analysis_ready);
    assert_eq!(
    ...
    for key in [
        ...
        assert_eq!(
            edited.value()["load_combinations"][0][key],
            source_model["load_combinations"][0][key]
        );
    }
    assert_eq!(
        edited.value()["load_combinations"][1]["terms"][0]["ref_id"],
        "COMBO_BASE_LINKED"
    );
    assert_eq!(
        edited.value()["load_combinations"][1]["terms"][1],
        source_model["load_combinations"][1]["terms"][1]
    );
    assert_eq!(
        edited.value()["roundtrip_map"][0]["model_ir_entity_id"],
        "COMBO_BASE_LINKED"
    ...
    let mut retained_combination = source_model["load_combinations"][0]
        .as_object()
    ...
    let retained_combination = Value::Object(retained_combination);
    let extension = edited.value()["extensions"]
        .get("structural-native:model-edit-linear-load-combination-identity-cascade.v2")
        .expect("combination cascade provenance extension");
    assert_eq!(
        extension["operation"],
        "linear_load_combination_identity_cascade_edit"
    );
    assert_eq!(extension["source_load_combination_id"], "COMBO_BASE");
    assert_eq!(
        extension["replacement_load_combination_id"],
        "COMBO_BASE_LINKED"
    );
    ...
    assert_eq!(extension["nested"], false);
    assert_eq!(extension["root_term_count"], 3);
    assert_eq!(extension["downstream_combination_reference_count"], 1);
    assert_eq!(extension["roundtrip_reference_count"], 1);
    assert_eq!(extension["typed_reference_cascade_verified"], true);
    assert_eq!(extension["target_and_downstream_expansion_preserved"], true);
    assert_eq!(
        extension["retained_combination_without_identity"],
        retained_combination
    );
    ...
    let mut receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("combination cascade edit receipt"),
    )
    .expect("combination cascade receipt JSON");
    assert_eq!(
        receipt["operation"],
        "linear_load_combination_identity_cascade_edit"
    );
    assert_eq!(receipt["source_load_combination_id"], "COMBO_BASE");
    assert_eq!(
        receipt["replacement_load_combination_id"],
        "COMBO_BASE_LINKED"
    );
    assert_eq!(receipt["load_combination_index"], 0);
    assert_eq!(receipt["nested"], false);
    ...
    assert_eq!(receipt["expanded_term_count"], 3);
    assert_eq!(receipt["expanded_pattern_count"], 3);
    assert_eq!(receipt["downstream_combination_reference_count"], 1);
    assert_eq!(receipt["roundtrip_reference_count"], 1);
    assert_eq!(receipt["typed_reference_cascade_verified"], true);
    assert_eq!(receipt["target_and_downstream_expansion_preserved"], true);
    assert_eq!(receipt["cpp_semantic_snapshot_verified"], true);
    assert_eq!(receipt["analysis_ready"], true);
    assert_eq!(receipt["blocking_feature_ids"], serde_json::json!([]));
    assert_eq!(receipt["edited_content_hash"], edited.content_hash());
    ...
    for (name, source_id, replacement_id, code) in [
        ...
        let destination = temporary.0.join(format!("combination-cascade-{name}"));
        let rejected = run_linear_load_combination_identity_cascade_edit(
            &source,
            &destination,
            source_id,
            replacement_id,
    ...
    let mut orphan = source_model.clone();
    orphan["load_combinations"] = serde_json::json!([source_model["load_combinations"][0]]);
    let orphan_source = temporary.0.join("combination-cascade-orphan-source.json");
    std::fs::write(
        &orphan_source,
        canonicalize_model_ir_v2(&orphan)
            .expect("canonical orphan combination source")
            .as_bytes(),
    )
    .expect("write orphan combination cascade source");
    let orphan_destination = temporary.0.join("combination-cascade-orphan-output");
    let orphan_rejection = run_linear_load_combination_identity_cascade_edit(
        &orphan_source,
        &orphan_destination,
        "COMBO_BASE",
        "COMBO_BASE_LINKED",
    ...
    assert_eq!(orphan_rejection.status.code(), Some(1));
    assert!(String::from_utf8_lossy(&orphan_rejection.stdout)
        .contains("workbench_model_edit_linear_load_combination_identity_cascade_unreferenced"));
    assert!(!orphan_destination.exists());

    for (name, owned_id) in [
        ("source-owned", "COMBO_BASE"),
        ("replacement-owned", "COMBO_BASE_LINKED"),
    ] {
        let mut owned = source_model.clone();
        owned["unsupported_features"] = serde_json::json!([{
            "feature_id": format!("feature.combination-cascade-{name}"),
            "kind": "unsupported_solver_feature",
            "source_entity_id": owned_id,
            "disposition": "blocked",
            "blocking": true,
            "detail": "Load-combination identity remains externally owned.",
            "extensions": {}
        }]);
        let owned_path = temporary
            .0
            .join(format!("combination-cascade-{name}-source.json"));
        std::fs::write(
            &owned_path,
            canonicalize_model_ir_v2(&owned)
                .expect("canonical owned combination cascade source")
                .as_bytes(),
        )
        .expect("write owned combination cascade source");
        let destination = temporary
            .0
            .join(format!("combination-cascade-{name}-output"));
        let rejected = run_linear_load_combination_identity_cascade_edit(
            &owned_path,
            &destination,
            "COMBO_BASE",
        ...
        assert_eq!(rejected.status.code(), Some(1), "{name} status");
        assert!(String::from_utf8_lossy(&rejected.stdout).contains(
            "workbench_model_edit_linear_load_combination_identity_cascade_unsupported_feature_owned"
        ));
        assert!(!destination.exists());
    }

    let existing = run_linear_load_combination_identity_cascade_edit(
        &source,
        &first,
        "COMBO_BASE",
        "COMBO_BASE_LINKED",
    ...
    let request_directory = temporary.0.join("combination-cascade-request");
    assert_success(&run_model_linear_combination_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "combination-cascade-c5",
        "COMBO_PARENT",
    ));
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("combination cascade request");
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("combination cascade direct CPU execution");
    assert!(direct.is_complete());
    assert!(!direct.is_terminal_failure());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("combination cascade direct recovery"),
    )
    .expect("combination cascade recovery JSON");
    assert_eq!(recovery["load_pattern_id"], "COMBO_PARENT");
    assert_eq!(
        recovery["active_dof_indices"],
        serde_json::json!([6, 7, 8, 9, 10, 11])
    );
    assert_eq!(
        recovery["active_external_load"],
        serde_json::json!([35000, -12000, 5000, 0, 0, 0])
    );
    assert_eq!(recovery["fallback_count"], 0);
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("combination cascade initialized checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("combination cascade resumed CPU execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
...
#[test]
#[allow(clippy::too_many_lines)]
fn direct_linear_load_combination_term_insert_executes_and_restarts_without_fallback() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let authored = temporary.0.join("term-insert-authored");
    assert_success(&run_linear_load_combination_add(
        &source,
        &authored,
        "COMBO_SERVICE",
    ...
    let authored_path = authored.join("model-ir.json");
    let authored_bytes = std::fs::read(&authored_path).expect("term-insert source ModelIR");
    let source_document = parse_model_ir_v2(&authored_bytes).expect("strict term-insert source");
    let first = temporary.0.join("term-insert-first");
    let second = temporary.0.join("term-insert-second");
    for destination in [&first, &second] {
        let output = run_direct_linear_load_combination_term_insert(
            &authored_path,
            destination,
            "COMBO_SERVICE",
            "LC_AXIAL",
        ...
        assert_success(&output);
        let receipt =
            std::fs::read(destination.join("edit-receipt.json")).expect("term-insert receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first term-insert artifact"),
            std::fs::read(second.join(artifact)).expect("second term-insert artifact")
        );
    }
    assert_eq!(
        std::fs::read(&authored_path).expect("unchanged term-insert source"),
        authored_bytes
    );

    let edited_bytes = std::fs::read(first.join("model-ir.json")).expect("term-inserted ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict term-inserted ModelIR");
    let validation = validate_model_bytes(&edited_bytes).expect("C++-validated term insertion");
    assert!(validation.report.analysis_ready);
    assert_eq!(
        edited.value()["load_combinations"][0]["terms"],
        serde_json::json!([
    ...
    let extension = edited.value()["extensions"]
        .get("structural-native:model-insert-direct-linear-load-combination-term.v1")
        .expect("direct term-insert provenance extension");
    assert_eq!(
        extension["operation"],
    ...
    let mut edit_receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("term-insert receipt"),
    )
    .expect("term-insert receipt JSON");
    assert_eq!(
        edit_receipt["operation"],
        "direct_linear_load_combination_term_insert"
    );
    ...
    let request_directory = temporary.0.join("term-insert-request");
    assert_success(&run_model_linear_combination_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "term-insert-c5",
    ...
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("term-insert request");
    let request_receipt: Value = serde_json::from_slice(
        &std::fs::read(request_directory.join("request-receipt.json"))
            .expect("term-insert request receipt"),
    )
    .expect("term-insert request receipt JSON");
    assert_eq!(
        request_receipt["schema_version"],
        "structural-native-model-linear-direct-combination-request-create-receipt.v2"
    );
    ...
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("term-inserted direct CPU execution");
    assert!(direct.is_complete());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("term-insert recovery"),
    )
    .expect("term-insert recovery JSON");
    assert_eq!(recovery["load_pattern_id"], "COMBO_SERVICE");
    assert_eq!(
        recovery["active_external_load"],
        serde_json::json!([25000, -12000, 5000, 0, 0, 0])
    ...
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("term-insert initial checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("term-insert resumed CPU execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (label, pattern, target_index, expected_code) in [
        ...
        let destination = temporary.0.join(format!("term-insert-{label}-rejected"));
        let rejected = run_direct_linear_load_combination_term_insert(
            &authored_path,
            &destination,
            "COMBO_SERVICE",
            pattern,
        ...
        assert!(!rejected.status.success(), "accepted {label} insertion");
        assert!(
            String::from_utf8_lossy(&rejected.stdout).contains(expected_code),
...
#[test]
#[allow(clippy::too_many_lines)]
fn nested_linear_load_combination_term_add_executes_and_restarts_without_fallback() {
    let temporary = TestDirectory::create();
    let source =
        repository_root().join("tests/fixtures/model_ir_v2/frame_cantilever_all_modes.json");
    let base = temporary.0.join("nested-term-add-base");
    assert_success(&run_linear_load_combination_add(
        &source,
        &base,
        "COMBO_BASE",
    ...
    let nested = temporary.0.join("nested-term-add-authored");
    assert_success(&run_nested_linear_load_combination_add(
        &base.join("model-ir.json"),
        &nested,
        "COMBO_NESTED",
    ...
    let nested_bytes = std::fs::read(&nested_path).expect("nested term-add source");
    let source_document = parse_model_ir_v2(&nested_bytes).expect("strict nested term-add source");
    let first = temporary.0.join("nested-term-add-first");
    let second = temporary.0.join("nested-term-add-second");
    for destination in [&first, &second] {
        let output = run_nested_linear_load_combination_term_add(
            &nested_path,
            destination,
            "COMBO_NESTED",
            "load_pattern",
        ...
        assert_success(&output);
        let receipt =
            std::fs::read(destination.join("edit-receipt.json")).expect("nested term-add receipt");
        assert_eq!(output.stdout, [receipt.as_slice(), b"\n"].concat());
    }
    for artifact in ["model-ir.json", "edit-receipt.json"] {
        assert_eq!(
            std::fs::read(first.join(artifact)).expect("first nested term-add artifact"),
            std::fs::read(second.join(artifact)).expect("second nested term-add artifact")
        );
    }
    assert_eq!(
        std::fs::read(&nested_path).expect("unchanged nested term-add source"),
        nested_bytes
    );

    let edited_bytes =
        std::fs::read(first.join("model-ir.json")).expect("nested term-extended ModelIR");
    let edited = parse_model_ir_v2(&edited_bytes).expect("strict nested term-extended ModelIR");
    let validation =
        validate_model_bytes(&edited_bytes).expect("C++-validated nested term addition");
    assert!(validation.report.analysis_ready);
    assert_eq!(
        edited.value()["load_combinations"][1]["terms"],
        serde_json::json!([
    ...
    let extension = edited.value()["extensions"]
        .get("structural-native:model-add-nested-linear-load-combination-term.v1")
        .expect("nested term-add provenance extension");
    assert_eq!(
        extension["operation"],
    ...
    let mut edit_receipt: Value = serde_json::from_slice(
        &std::fs::read(first.join("edit-receipt.json")).expect("nested term-add receipt"),
    )
    .expect("nested term-add receipt JSON");
    assert_eq!(
        edit_receipt["operation"],
        "nested_linear_load_combination_term_add"
    );
    ...
    let request_directory = temporary.0.join("nested-term-add-request");
    assert_success(&run_model_linear_combination_request_create(
        &first.join("model-ir.json"),
        &request_directory,
        "nested-term-add-c5",
    ...
    let request_bytes = std::fs::read(request_directory.join("analysis-request.json"))
        .expect("nested term-add request");
    let request_receipt: Value = serde_json::from_slice(
        &std::fs::read(request_directory.join("request-receipt.json"))
            .expect("nested term-add request receipt"),
    )
    .expect("nested term-add request receipt JSON");
    assert_eq!(
        request_receipt["schema_version"],
        "structural-native-model-linear-nested-combination-request-create-receipt.v3"
    );
    ...
    let direct = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, u32::MAX)
        .expect("nested term-added direct CPU execution");
    assert!(direct.is_complete());
    let recovery: Value = serde_json::from_str(
        direct
            .result_recovery_ir_json()
            .expect("nested term-add recovery"),
    )
    .expect("nested term-add recovery JSON");
    assert_eq!(recovery["load_pattern_id"], "COMBO_NESTED");
    assert_eq!(
        recovery["active_external_load"],
    ...
    let partial = execute_model_ir_linear_analysis(&edited_bytes, &request_bytes, None, 0)
        .expect("nested term-add initial checkpoint");
    assert!(!partial.is_complete());
    let resumed = execute_model_ir_linear_analysis(
        &edited_bytes,
        &request_bytes,
    ...
    .expect("nested term-add resumed CPU execution");
    assert!(resumed.is_complete());
    assert_eq!(resumed.result_ir_json(), direct.result_ir_json());
    assert_eq!(
        resumed.result_recovery_ir_json(),
    ...
    for (label, reference_kind, reference_id, expected_code) in cases {
        let destination = temporary
            .0
            .join(format!("nested-term-add-{label}-rejected"));
        let rejected = run_nested_linear_load_combination_term_add(
            &nested_path,
            &destination,
            "COMBO_NESTED",
            reference_kind,
    ...
    let direct_destination = temporary.0.join("nested-term-add-direct-rejected");
    let rejected = run_nested_linear_load_combination_term_add(
        &nested_path,
        &direct_destination,
        "COMBO_BASE",
...
fn ndtha_response_view_is_windowed_deterministic_hash_bound_and_terminal_gated() {
    ...
    assert!(String::from_utf8_lossy(&unsupported_displacements.stdout)
        .contains("workbench_profile_unsupported"));
    let unsupported_element_recovery = run_workbench(&[
        text("element-recovery-view"),
        text("--workspace"),
        workspace.as_os_str(),
    ]);
    assert_eq!(unsupported_element_recovery.status.code(), Some(1));
    assert!(
        String::from_utf8_lossy(&unsupported_element_recovery.stdout)
            .contains("workbench_profile_unsupported")
    );
