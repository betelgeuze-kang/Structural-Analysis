...
- Critical Triage is accepted when the source viewer exposes `structure-viewer-critical-triage.v1`, renders at least four top critical rows in the first right-rail viewport, reports source row count, high-severity count, max D/C ratio, D/C limit markers, drift contribution, status and recommended action chips, mirrors full-table selection state, customer-open package smoke requires the marker, and dense 1600x900 smoke reports zero triage overflow.
