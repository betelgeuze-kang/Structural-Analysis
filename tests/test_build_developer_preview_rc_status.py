...
def test_developer_preview_rc_status_aggregates_deliverables_without_promotion() -> None:
    ...
    assert not any(
        ...
        for blocker in medium_blockers
    )
    ...
    assert quantity_handoff["targets"]["medium_structural_models"]["current"] == 5
    assert quantity_handoff["targets"]["medium_structural_models"]["required"] == 5
    assert quantity_handoff["targets"]["medium_structural_models"]["remaining"] == 0
    ...
    assert medium_scorecard_handoff["local_candidate_artifact_count"] == 5
