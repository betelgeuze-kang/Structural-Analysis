...
def test_run_audit_review_decision_batch_apply_live(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        [
            ...
            str(preview_out),
            "--mgt-export-report",
            str(tmp_path / "midas_generator_33.optimized.export_report.json"),
            "--audit-review-followup-manifest",
            str(tmp_path / "midas_generator_33.optimized.audit_review_followup_manifest.json"),
            "--audit-review-resolution-manifest",
            str(tmp_path / "midas_generator_33.optimized.audit_review_resolution_manifest.json"),
            "--audit-review-resolution-dir",
            str(tmp_path / "midas_generator_33.optimized.audit_review_resolution_files"),
            "--out",
            str(out),
            "--apply-live",
        ],
