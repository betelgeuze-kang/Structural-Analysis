...
def test_snapshot_accepts_structural_scope_owner_decisions_csv_as_receipt_boundary(
    ...
    receipt_commit = _commit_all(tmp_path, "structural scope owner decisions csv")
...
def test_snapshot_accepts_developer_preview_final_gate_action_register_as_handoff(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "docs/developer_preview_final_gate_action_register.md",
        "# Developer Preview Final Gate Action Register\n\nOwner handoff only.\n",
    )
    handoff_commit = _commit_all(tmp_path, "developer preview action register")
...
def test_snapshot_public_benchmark_phase2_runner_change_does_not_stale_snapshot_leaf_receipts(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/materialize_public_benchmark_phase2_from_rows.py",
        "print('public benchmark phase2 row runner changed')\n",
    )
    _commit_all(tmp_path, "public benchmark phase2 runner change")
...
def test_snapshot_structural_roadmap_builder_change_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_structural_product_development_roadmap.py",
        "print('structural roadmap aggregation changed')\n",
    )
    _commit_all(tmp_path, "structural roadmap builder change")
...
def test_snapshot_phase6_parity_builder_change_only_stales_developer_preview_rc(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_phase6_linux_windows_parity_status.py",
        "print('phase6 parity receipt builder changed')\n",
    )
    _commit_all(tmp_path, "phase6 parity builder change")
...
def test_snapshot_g1_load_dependent_comparison_builder_change_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _write_text(
        ...
        "print('g1 load dependent comparison diagnostic changed')\n",
    )
    _commit_all(tmp_path, "g1 load dependent comparison builder change")
...
def test_snapshot_g1_assembly_contract_seed_builder_change_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_g1_assembly_contract_seed_report.py",
        "print('g1 assembly seed builder changed')\n",
    )
    _commit_all(tmp_path, "g1 assembly seed builder change")
...
def test_snapshot_g1_load_dependent_packet_builder_change_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_g1_load_dependent_near_null_packet.py",
        "print('g1 load dependent near null packet diagnostic changed')\n",
    )
    _commit_all(tmp_path, "g1 load dependent packet builder change")
...
def test_snapshot_pocketmd_topk_materializer_change_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/materialize_pocketmd_lite_topk_survival_report.py",
        "print('pocketmd lite top-k materializer changed')\n",
    )
    _commit_all(tmp_path, "pocketmd materializer change")
...
def test_snapshot_science_actual_closure_materializer_does_not_stale_snapshot_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/materialize_science_actual_closure_from_rows.py",
        "print('science actual closure materializer changed')\n",
    )
    _commit_all(tmp_path, "science actual materializer change")
...
def test_snapshot_license_status_fill_helper_change_only_stales_license_receipt(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/fill_license_status_from_approval.py",
        "print('license status approval fill policy changed')\n",
    )
    _commit_all(tmp_path, "license fill helper change")
...
def test_snapshot_source_boundary_planner_change_only_stales_independent_product(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/plan_source_boundary_cleanup.py",
        "print('source boundary planner changed')\n",
    )
    _commit_all(tmp_path, "source boundary planner change")
...
def test_snapshot_structural_scope_checker_change_only_stales_scope_audit(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/check_structural_scope_contamination.py",
        "print('structural scope audit policy changed')\n",
    )
    _commit_all(tmp_path, "structural scope checker change")
    ...
    assert (
        metadata_rows["structural_scope_contamination_audit"]["source_state_kind"]
        == "non_receipt_paths_changed"
    )
    assert (
        "stale_or_inconsistent:source_commit_mismatch:pm_release_gate_report"
...
def test_snapshot_structural_scope_owner_review_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _write_text(
        ...
        "print('structural scope owner review helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope owner review helper change")
...
def test_snapshot_structural_scope_owner_decision_plan_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_structural_scope_owner_decision_application_plan.py",
        "print('structural scope owner decision plan helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope owner decision plan helper change")
...
def test_snapshot_structural_scope_owner_decision_batch_merge_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/merge_structural_scope_owner_decision_batch.py",
        "print('structural scope owner decision batch merge helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope owner decision batch merge helper change")
...
def test_snapshot_structural_scope_owner_decision_template_fill_helper_scopes_to_application_plan(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/fill_structural_scope_owner_decisions_from_template.py",
        "print('structural scope owner decision template fill helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope owner decision template fill helper change")
...
def test_snapshot_structural_scope_origin_helper_does_not_stale_leaf_receipts(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_structural_scope_origin_report.py",
        "print('structural scope origin helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope origin helper change")
...
def test_snapshot_structural_scope_cleanup_impact_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_structural_scope_cleanup_impact_report.py",
        "print('structural scope cleanup impact helper changed')\n",
    )
    _commit_all(tmp_path, "structural scope cleanup impact helper change")
...
def test_snapshot_repo_hygiene_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/check_repo_hygiene.py",
        "print('repo hygiene boundary policy changed')\n",
    )
    _commit_all(tmp_path, "repo hygiene helper change")
...
def test_snapshot_workspace_organizer_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "implementation/phase1/organize_phase1_workspace.py",
        "print('workspace organizer policy changed')\n",
    )
    _commit_all(tmp_path, "workspace organizer helper change")
...
def test_snapshot_quality_gate_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/verify_quality_gate.py",
        "print('quality gate orchestration changed')\n",
    )
    _commit_all(tmp_path, "quality gate helper change")
...
def test_snapshot_retired_science_surface_seed_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_science_evidence_surface_seeds.py",
        "print('retired structural-scope compatibility guard changed')\n",
    )
    _commit_all(tmp_path, "retired science seed helper change")
...
def test_snapshot_license_status_intake_helper_does_not_stale_leaf_receipts(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_license_status_intake_packet.py",
        "print('license status intake helper changed')\n",
    )
    _commit_all(tmp_path, "license status intake helper change")
...
def test_snapshot_ci_streak_intake_helper_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_ci_streak_intake_packet.py",
        "print('ci streak intake helper changed')\n",
    )
    _commit_all(tmp_path, "ci streak intake helper change")
...
def test_snapshot_ux_observation_template_only_stales_ux_report(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "docs/templates/ux_new_user_observation.template.json",
        '{"participant_ref": "OWNER_INPUT_REQUIRED"}\n',
    )
    _commit_all(tmp_path, "ux observation template change")
    ...
    assert mismatch_blockers == [
        "stale_or_inconsistent:source_commit_mismatch:ux_new_user_observation_report"
    ]


def test_snapshot_ux_observation_fill_helper_only_stales_ux_report(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/fill_ux_new_user_observation_from_human_sample.py",
        "print('ux observation fill policy changed')\n",
    )
    _commit_all(tmp_path, "ux observation fill helper change")
...
def test_snapshot_windows_platform_replay_fill_helper_only_stales_dp_rc(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/fill_phase6_windows_platform_replay_receipt.py",
        "print('windows platform replay fill policy changed')\n",
    )
    _commit_all(tmp_path, "windows platform replay fill helper change")
...
def test_snapshot_source_of_truth_classification_doc_does_not_stale_leaf_receipts(
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "docs/source-of-truth-gap-classification.md",
        "classification doc changed\n",
    )
    _commit_all(tmp_path, "classification doc change")
...
def test_snapshot_source_of_truth_classification_builder_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_source_of_truth_gap_classification.py",
        "print('source-of-truth classification policy changed')\n",
    )
    _commit_all(tmp_path, "classification builder change")
...
def test_snapshot_hip_consistency_probe_generator_change_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
...
def test_snapshot_pm_release_gate_milestones_doc_does_not_stale_leaf_receipts(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "docs/pm-release-gate-milestones.md",
        "support bundle artifact count doc changed\n",
    )
...
def test_snapshot_medium_scorecard_receipt_builder_change_only_stales_dp_rc_artifact(
    tmp_path: Path,
) -> None:
    _init_git_repo(tmp_path)
    _write_stable_non_receipt_inputs(tmp_path)
    ...
    _commit_all(tmp_path, "receipt")
    _write_text(
        tmp_path / "scripts/build_phase3_medium_model_scorecard_readiness_receipt.py",
        "print('medium model scorecard receipt builder changed')\n",
    )
    _commit_all(tmp_path, "medium scorecard receipt builder change")
...
def test_snapshot_medium_scorecard_runner_change_only_stales_dp_rc_artifact(
    ...
    _write_text(
        ...
        "print('medium scorecard runner contract changed')\n",
    )
    _commit_all(tmp_path, "medium scorecard runner change")
...
def test_snapshot_phase6_parity_builder_change_only_stales_dp_rc_artifact(
    ...
    _write_text(
        tmp_path / "scripts/build_phase6_linux_windows_parity_status.py",
        "print('phase6 parity policy changed')\n",
    )
    _commit_all(tmp_path, "phase6 parity builder change")
