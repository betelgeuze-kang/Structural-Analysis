...
SCRIPT = ROOT / "scripts" / "check_native_sparse_linear_product.py"
SPEC = importlib.util.spec_from_file_location("check_native_sparse_linear_product", SCRIPT)
...
def test_repository_sparse_linear_c4_c5_contract_is_evidence_backed() -> None:
    report = product.check_sparse_linear_product(ROOT)
...
def test_contract_fails_closed_when_clean_environment_evidence_disappears(
    ...
    path = tmp_path / "native/crates/structural-cli/tests/sparse_linear_product_cli.rs"
    path.write_text(
    ...
    report = product.check_sparse_linear_product(tmp_path)
