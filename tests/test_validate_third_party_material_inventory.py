...
def test_permission_cannot_be_asserted_from_unreviewed_or_restricted_row() -> None:
    ...
    assert any(
        error.startswith("permission_asserted_without_approved_review:example-restricted-dataset")
...
def test_approved_or_permissioned_row_requires_evidence() -> None:
    ...
    report = validate_inventory(payload, schema=SCHEMA)
    assert report["contract_pass"] is False
    assert "permission_evidence_missing:example-reviewed-schema-source" in report[
        "contract_errors"
    ]
...
def test_redistribution_derivatives_and_training_require_use_permission() -> None:
    ...
    report = validate_inventory(payload, schema=SCHEMA)
    assert report["contract_pass"] is False
    assert "redistribution_requires_use:example-reviewed-schema-source" in report[
        "contract_errors"
    ]
    assert "derivative_works_requires_use:example-reviewed-schema-source" in report[
        "contract_errors"
    ]
    assert "training_requires_use:example-reviewed-schema-source" in report[
        "contract_errors"
    ]
...
def test_duplicate_material_and_path_credit_is_rejected() -> None:
    ...
    report = validate_inventory(payload, schema=SCHEMA)
    assert report["contract_pass"] is False
    assert "duplicate_material_id:example-restricted-dataset" in report[
        "contract_errors"
    ]
    assert any(
        error.startswith("duplicate_path_glob:examples/external/reviewed-schema-source.json")
        for error in report["contract_errors"]
    )
...
def test_approved_row_must_assert_at_least_one_bounded_permission() -> None:
    ...
    report = validate_inventory(payload, schema=SCHEMA)
    assert report["contract_pass"] is False
    assert "approved_row_asserts_no_permission:example-reviewed-schema-source" in report[
        "contract_errors"
    ]
