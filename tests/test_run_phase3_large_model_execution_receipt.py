...
SCRIPT_PATH = REPO_ROOT / "scripts/run_phase3_large_model_execution_receipt.py"
for candidate in (REPO_ROOT / "scripts", REPO_ROOT / "src"):
    if str(candidate) not in sys.path:
        sys.path.insert(0, str(candidate))

spec = importlib.util.spec_from_file_location("run_phase3_large_model_execution_receipt", SCRIPT_PATH)
...
def test_large_model_execution_receipt_runner_writes_pass_receipt_for_valid_attached_model(
    tmp_path: Path,
) -> None:
    ...
    payload = module.build_large_model_execution_receipt(
        ...
        result_out=result,
        report_out=report,
        runner_command="python3 scripts/run_phase3_large_model_execution_receipt.py --model attached-model.json",
    )

    assert payload["schema_version"] == "phase3-large-model-execution-receipt.v1"
    assert payload["contract_pass"] is True
    assert payload["large_model_execution_receipt_claim"] is True
    ...
    assert "satisfy the required 2/2 large-model quantity gate" in payload["claim_boundary"]


def test_large_model_execution_receipt_blocks_on_sha_mismatch_and_missing_review(
    tmp_path: Path,
) -> None:
    model = tmp_path / "attached-model.json"
    out = tmp_path / "receipt.json"
