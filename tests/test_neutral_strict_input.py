...
EXAMPLE = Path(__file__).resolve().parents[1] / "examples/public_rc_fiber_frame_cantilever.json"
...
@pytest.mark.parametrize("raw", [
    ...
    b'{"metadata":{"x":NaN}}', b'{"metadata":{"x":Infinity}}',
    b'{"metadata":{"x":-Infinity}}', b'{"metadata":{"x":1e9999}}',
    b'{"metadata":{"x":-1e9999}}', b'{"metadata":{"x":"\\ud800"}}',
    b'{"\\udfff":1}', b'{"x":"\xff"}', b'\xef\xbb\xbf{}',
    b'{"broken":', b'[]', b'null', b'', b'[' * 2000 + b']' * 2000,
])
...
def test_valid_input_preserves_canonical_payload_and_original_digest(kind):
    ...
    expected = loader._load_neutral_payload(
        payload, source_path="valid.json",
...
def test_repeated_keys_in_different_objects_and_numeric_metadata_remain_valid():
    ...
    payload["metadata"] = {"a": {"x": 0}, "b": {"x": -0.0},
                           "unicode": "\U0001F600", "integer": 2**60,
                           "label": "NaN", "finite": 1e300}
