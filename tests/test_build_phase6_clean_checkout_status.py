...
def test_phase6_clean_checkout_status_accepts_clean_clone_cleanup() -> None:
    ...
    assert payload["developer_preview_release_candidate_claim"] is True
    ...
    assert payload["git_clean_clone_gate"]["status"] == "ready"
    assert payload["git_clean_clone_gate"]["contract_pass"] is True
    assert payload["git_clean_clone_gate"]["git_clean_clone_executed"] is True
    assert payload["git_clean_clone_gate"]["blocker_count"] == 0
    assert payload["git_clean_clone_gate"]["blocker_counts"] == {}
    assert payload["release_control_cleanup_gate"]["status"] == "ready"
    assert payload["release_control_cleanup_gate"]["human_git_action_required"] is False
    ...
    assert grouping["groups"]["git_clean_clone_root"]["blockers"] == []
    ...
    assert grouping["groups"]["release_control_human_handoff"]["blockers"] == []
    assert grouping["groups"]["release_control_commit_set"]["blockers"] == []
