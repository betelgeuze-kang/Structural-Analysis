...
def options():
    return {
        "history_limits": design.FiberFrameHistoryLimits(0.1, 0.01),
        "material_limits": design.FiberFrameMaterialHistoryLimits(0.1, 1.0, 1.0),
        "prices": design.FiberFrameMaterialPrices(
            100.0, 2.0, "KRW", "2026-09-12", "synthetic, not a quote"
        ),
    }
...
def session(**kwargs):
    return reuse.RCControlResultSession(
        source_revision=SOURCE, scope_id="research", **kwargs
    )
    ...
    return [
        {
            ...
            "material_estimate": {
                "total": cost,
                "currency": "KRW",
                "scope": "synthetic-control",
                "price_table_hash": "sha256:" + "a" * 64,
            },
        }
...
def test_bound_agrees_with_every_possible_completion():
    costs = {"baseline": 4.0, "cheap": 2.0, "equal": 2.0, "costly": 7.0}
...
@pytest.mark.parametrize(
    "unknown_cost,confirmed,ties_known",
    [(1.0, False, None), (2.0, True, False), (3.0, True, True)],
)
def test_cheaper_unknown_blocks_but_ties_and_expensive_unknown_are_explicit(
    unknown_cost, confirmed, ties_known
):
    bound = search.finite_pool_cost_bound(
        pool({"baseline": 2.0, "other": unknown_cost}),
        {"baseline": True, "other": None},
    )
...
def test_zero_cost_ties_are_valid_without_division():
    b = search.finite_pool_cost_bound(
        pool({"baseline": 0.0, "tie": 0.0}), {"baseline": True, "tie": None}
    )
...
@pytest.mark.parametrize(
    "value", [True, False, None, "1", float("nan"), float("inf"), -1.0]
)
...
@pytest.mark.parametrize(
    "mode",
    ["empty", "duplicate", "missing-baseline", "missing-outcome", "extra-outcome"],
)
def test_pool_coverage_reject(mode):
    rows = pool({"baseline": 1.0})
...
def test_real_reprice_rescreen_and_immutable_originals(
    ...
    assert (
        first["new_work"]["known_counters"]["attempted_step_count"] == 8
    )  # preload + 3, twice
    assert first["fresh_reference_verification_this_call"] is True
    original = {
        role: (tmp_path / "fresh" / meta["path"]).read_bytes()
        for role, meta in first["row"]["artifacts"].items()
    }
    ...
    assert second["row"]["material_estimate"]["total"] - first["row"][
        "material_estimate"
    ]["total"] == pytest.approx(36.0)
    ...
    third = evaluate(
        ...
        options,
        tmp_path / "rescreen",
        history_limits=design.FiberFrameHistoryLimits(0.1, value * 0.5),
        allow_new_analysis=False,
    )
    ...
    second["row"]["performance"]["maximum_absolute_fiber_strain"] = 999.0
    fourth = evaluate(
        s, model, control_request, options, tmp_path / "again", allow_new_analysis=False
    )
...
@pytest.mark.parametrize(
    "change",
    ["load", "targets", "tolerance", "material", "section", "source-provenance"],
)
def test_changed_physics_needs_new_analysis(
    model, control_request, options, tmp_path, change
):
    s = session()
    evaluate(s, model, control_request, options, tmp_path / "first")
    if change == "load":
        control_request = replace(
            control_request, constant_nodal_loads=(("N2", -601.0, 0.0, 0.0),)
        )
    if change == "targets":
    ...
    if change == "tolerance":
        control_request = replace(
            ...
            solver_config=replace(
                control_request.solver_config,
                newton=replace(
                    control_request.solver_config.newton, residual_tolerance=2e-8
                ),
            ),
        )
    if change == "material":
        model.materials[0]["yield_stress_mpa"] = 251.0
    if change == "section":
        model = design.apply_fiber_frame_section_changes(model, candidate("wider", 0.5))
...
def test_forced_fresh_and_new_session_do_not_inherit_credit(
    model, control_request, options, tmp_path
):
    s = session()
    first = evaluate(s, model, control_request, options, tmp_path / "first")
    fresh = evaluate(
        s, model, control_request, options, tmp_path / "forced", fresh=True
    )
    ...
    with pytest.raises(reuse.NewAnalysisRequired):
        evaluate(
            ...
            control_request,
            options,
            tmp_path / "new-process-equivalent",
            allow_new_analysis=False,
        )
    s.clear()
    assert s.retained_bytes == 0
...
def test_scope_and_source_change_reject_before_output(
    ...
    with pytest.raises(ValueError, match="ownership"):
        s.evaluate(
            model,
            control_request,
            scope_id="another-user-label",
            output_directory=tmp_path / "foreign",
            **options,
        )
    assert not (tmp_path / "foreign").exists()
    monkeypatch.setattr(
        reuse, "_runtime_fingerprint", lambda: "changed-source-or-runtime"
    )
...
def test_small_cache_limit_does_not_truncate_or_claim_reuse(
    model, control_request, options, tmp_path
):
...
def test_lru_evicts_without_exceeding_bound(model, control_request, options, tmp_path):
    ...
    evaluate(
        s,
        design.apply_fiber_frame_section_changes(model, candidate("wide", 0.5)),
        control_request,
        options,
        tmp_path / "b",
    )
...
def test_failed_verification_is_exported_but_not_cached(
    model, control_request, options, tmp_path, monkeypatch
):
    s = session()
...
def test_real_search_matches_fresh_exhaustive_and_skips_expensive(
    model, control_request, options, tmp_path
):
    choices = (
        candidate("cheap", 0.35),
        candidate("middle", 0.45),
        candidate("costly", 0.5),
    )
    s = session()
    report = search.run_rc_control_cost_search(
        ...
        max_new_model_analyses=2,
        **options,
    )
    oracle = reference.compare_rc_control_designs(
        model,
        choices,
        control_request,
        source_revision=SOURCE,
        output_directory=tmp_path / "oracle",
        **options,
    )
    assert report["status"] == "pool_minimum_confirmed"
    assert (
        report["cost_bound"]["selected_candidate_id"]
    ...
    assert (
        sum(
            i["work"]["attempted_step_count"]
            for r in oracle["rows"]
            for i in r["invocations"]
        )
        == 32
    )
    assert report["cost_bound"]["unverified_response_candidate_ids"] == [
        "costly",
        "middle",
    ]
    ...
    again = search.run_rc_control_cost_search(
        model,
        choices,
        control_request,
        session=s,
        scope_id="research",
        output_directory=tmp_path / "again",
    ...
    assert (
        again["new_model_evaluations"] == 0 and again["reused_model_evaluations"] == 2
    )
    assert again["new_work"]["api_invocation_count"] == 0


def test_budget_does_not_call_unknown_cheaper_infeasible(
    model, control_request, options, tmp_path
):
    choices = (candidate("cheap", 0.35), candidate("costly", 0.5))
    report = search.run_rc_control_cost_search(
        model,
        choices,
        control_request,
...
def test_known_constraint_failure_and_unchecked_ties(
    model, control_request, options, tmp_path
):
    choices = (
        candidate("cheap", 0.35),
        design.FiberFrameDesignCandidate(
            "same", (design.FiberFrameSectionChange("RC1", cover_m=0.045),)
        ),
        candidate("expensive", 0.5),
    )
    s = session()
    ...
    report = search.run_rc_control_cost_search(
        ...
        session=s,
        scope_id="research",
        output_directory=tmp_path / "screened",
        max_new_model_analyses=0,
        **(
            options
            | {"history_limits": design.FiberFrameHistoryLimits(0.1, (a + b) / 2)}
        ),
    )
    assert report["status"] == "pool_minimum_confirmed"
...
def test_bad_budget_rejects_before_output(
    model, control_request, options, tmp_path, budget
):
    with pytest.raises(ValueError):
        search.run_rc_control_cost_search(
            model,
            (candidate("cheap", 0.35),),
            control_request,
            session=session(),
...
@pytest.mark.parametrize(
    "order",
    [("cheap",), ("cheap", "cheap"), ["cheap", "costly"], ("costly", "unknown")],
)
def test_invalid_schedule_rejects(model, control_request, options, tmp_path, order):
    with pytest.raises(ValueError):
        search.run_rc_control_cost_search(
            model,
            (candidate("cheap", 0.35), candidate("costly", 0.5)),
            control_request,
            session=session(),
            scope_id="research",
            output_directory=tmp_path / "bad",
...
def test_source_context_and_originals_are_frozen_in_plan(
    model, control_request, options, tmp_path
):
    report = search.run_rc_control_cost_search(
        model,
    ...
    assert plan["request"]["constant_nodal_loads"][0]["FX_kN"] == -600.0
    assert plan["source_revision_is_attestation"] is False
    assert report["claims"]["exhaustive_oracle_executed"] is False


def test_unknown_work_stops_scheduling_and_stays_unverified(
    model, control_request, options, tmp_path, monkeypatch
):
    ...
    result = search.run_rc_control_cost_search(
        ...
        session=session(),
        scope_id="research",
        output_directory=tmp_path / "stop",
        **options,
    )
    ...
    assert all(
        r["status"] == "not_run_after_unknown_numerical_work"
...
def experiment_file(path, options, concrete=None):
    ...
    prices = (
        options["prices"]
        if concrete is None
        else replace(options["prices"], concrete_per_m3=concrete)
    )
    value = {
        ...
        "candidates": [
            {
                "candidate_id": "cheap",
                "changes": [{"section_id": "RC1", "width_m": 0.35}],
            },
            {
                "candidate_id": "costly",
                "changes": [{"section_id": "RC1", "width_m": 0.5}],
            },
        ],
...
def test_cli_repricing_scenarios_share_one_global_budget(
    control_request, options, tmp_path, capsys
):
    req = tmp_path / "request.json"
    req.write_bytes(reference._bytes(control_request.to_dict()))
    first = experiment_file(tmp_path / "one.json", options)
    second = experiment_file(tmp_path / "two.json", options, 200.0)
    result = cli.main(
        [
            "--model",
...
def test_cli_rejects_ambiguous_model_before_output(
    control_request, options, tmp_path, capsys
):
    req = tmp_path / "request.json"
    req.write_bytes(reference._bytes(control_request.to_dict()))
...
def test_reused_result_does_not_suppress_fresh_scientific_comparison(
    model, control_request, options, tmp_path
):
    s = session()
    evaluate(s, model, control_request, options, tmp_path / "cache")
    report = reference.compare_rc_control_designs(
        model,
        (candidate("cheap", 0.35),),
        control_request,
        source_revision=SOURCE,
...
def test_concurrent_identical_evaluations_do_not_duplicate_solve(
    model, control_request, options, tmp_path
):
    ...
    with ThreadPoolExecutor(max_workers=2) as workers:
        futures = [
            workers.submit(
                evaluate, s, model, control_request, options, tmp_path / f"thread-{i}"
            )
            for i in range(2)
        ]
        results = [f.result() for f in futures]
    assert sorted(r["mode"] for r in results) == [
        "fresh_reference_and_replay",
        "verified_original_reused",
    ]
    assert sum(r["new_work"]["api_invocation_count"] for r in results) == 2


def test_unknown_work_is_not_cached(
    model, control_request, options, tmp_path, monkeypatch
):
    original = reference._evaluate_design_row
...
def test_unknown_work_alone_cannot_confirm_incumbent(
    model, control_request, options, tmp_path, monkeypatch
):
...
def test_publication_failure_never_admits_fresh_cache(
    model, control_request, options, tmp_path, monkeypatch
):
    ...
    with monkeypatch.context() as scoped:
        ...
        with pytest.raises(OSError, match="publication"):
            evaluate(
                s, model, control_request, options, tmp_path / "publication-failed"
            )
    assert s.retained_bytes == 0
    with pytest.raises(reuse.NewAnalysisRequired):
        evaluate(
...
def test_real_zero_price_baseline_confirms_minimum_without_tie_claim(
    model, control_request, options, tmp_path
):
    zero = replace(options["prices"], concrete_per_m3=0.0, rebar_per_kg=0.0)
    result = search.run_rc_control_cost_search(
        model,
        (candidate("alternative", 0.5),),
        control_request,
        session=session(),
        scope_id="research",
        output_directory=tmp_path / "zero-prices",
        **(options | {"prices": zero}),
    )
    assert result["new_model_evaluations"] == 1
...
@pytest.mark.parametrize(
    ...
    [
        ("fresh", 0),
        ("fresh", "false"),
        ("allow_new_analysis", 1),
        ("allow_new_analysis", None),
    ],
)
def test_execution_options_require_booleans(
    model, control_request, options, tmp_path, field, value
):
