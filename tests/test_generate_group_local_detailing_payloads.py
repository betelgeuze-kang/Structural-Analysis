...
def test_generate_group_local_detailing_payloads_projects_rows(tmp_path: Path) -> None:
    model = tmp_path / "model.json"
    dataset = tmp_path / "dataset.npz"
    changes = tmp_path / "changes.json"
    projection = tmp_path / "detailing_projection.json"
                ...
                "changes": [
                    {
                        ...
                        "before_detailing_quality": 0.60,
                        "after_detailing_quality": 0.72,
                        "zone_label": "transfer",
                        "story_band": 5,
                        "semantic_group": "",
                    }
    ...
    assert payload["summary"]["group_local_detailing_payload_available_count"] == 1
    row = payload["group_local_detailing_payloads"][0]
    ...
    assert row["baseline_detailing_quality"] == 0.60
    assert row["target_detailing_quality"] == 0.72
    assert row["review_priority"] == "medium"
    assert row["followup_type"] == "detailing_manual_review"


def test_generate_group_local_detailing_payloads_uses_signature_zone_member_fallback(tmp_path: Path) -> None:
    model = tmp_path / "model_fallback.json"
    dataset = tmp_path / "dataset_fallback.npz"
    changes = tmp_path / "changes_fallback.json"
    projection = tmp_path / "detailing_projection_fallback.json"
