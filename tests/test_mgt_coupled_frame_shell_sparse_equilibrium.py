"""Tests for coupled frame + shell bending/drilling sparse MGT equilibrium evidence."""
...
def test_mgt_coupled_frame_shell_sparse_equilibrium_is_ready(tmp_path: Path) -> None:
    out = tmp_path / "mgt_coupled_frame_shell_sparse_equilibrium.json"
    proc = subprocess.run(
        [
            sys.executable,
    ...
    assert payload["schema_version"] == "mgt-coupled-frame-shell-sparse-equilibrium.v1"
    assert payload["status"] == "ready"
    assert payload["coupled_frame_shell_sparse_equilibrium_ready"] is True
    assert payload["coupled_frame_shell_nonlinear_equilibrium"] is False
    assert payload["surface_shell_bending_drilling_coupled_ready"] is True
    ...
    assert mesh["coupled_stiffness_nnz"] > 1000000
    ...
    assert payload["runtime_metrics"]["backend"] == "scipy_sparse_spsolve_cpu_coupled_frame_shell"
