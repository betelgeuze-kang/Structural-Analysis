...
SCRIPT = ROOT / "scripts" / "check_native_nonlinear_ndtha_hip.py"
SPEC = importlib.util.spec_from_file_location(
    "check_native_nonlinear_ndtha_hip", SCRIPT
)
...
def _receipt(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "device_id": 0,
        "device_name": "synthetic nonlinear-NDTHA contract device",
        ...
        "solve_count": 11,
        "max_response_absolute_error": 0.0,
        "max_summary_absolute_error": 0.0,
        "h2d_bytes": 2104,
        "d2h_bytes": 4519,
        ...
        "peak_device_buffer_bytes": 1318,
        "vram_total_bytes": 16 * 1024 * 1024 * 1024,
        "vram_free_before_bytes": 15 * 1024 * 1024 * 1024,
        "vram_free_after_alloc_bytes": 15 * 1024 * 1024 * 1024 - 1318,
        "fallback_count": 0,
        "fp64": True,
        "deterministic": True,
...
def _copy_contract(tmp_path: Path) -> None:
    relatives = {
        *hip.REQUIRED_TOKENS,
        "native/cpp/src/hip/nonlinear_ndtha_hip.hpp",
...
def test_repository_nonlinear_ndtha_hip_source_bound_candidate(
    tmp_path: Path,
) -> None:
    receipt = tmp_path / "receipt.json"
    _write_receipt(receipt, _receipt(ROOT))

    report = hip.check_native_nonlinear_ndtha_hip(
        ROOT,
...
def test_nonlinear_ndtha_hip_receipt_fails_closed_on_identity_or_residency_drift(
    tmp_path: Path,
) -> None:
    ...
    payload["device_resident_step_state"] = False
    payload["host_step_control_transfer_count"] = 1
    receipt = tmp_path / "receipt.json"
    ...
    assert "nonlinear_ndtha_hip_kernel_source_sha256_mismatch" in report["blockers"]
    assert "nonlinear_ndtha_hip_receipt_value_invalid:fallback_count" in report[
        "blockers"
    ]
    assert (
        "nonlinear_ndtha_hip_receipt_value_invalid:device_resident_step_state"
        in report["blockers"]
    )
    assert (
        "nonlinear_ndtha_hip_receipt_value_invalid:host_step_control_transfer_count"
        in report["blockers"]
    )


def test_nonlinear_ndtha_hip_promotion_requires_protected_runner_context(
    tmp_path: Path,
    monkeypatch,
) -> None:
    receipt = tmp_path / "receipt.json"
    ...
    blocked = hip.check_native_nonlinear_ndtha_hip(
        ROOT,
        receipt_path=receipt,
        device_lib_path=DEVICE_LIB_PATH,
    ...
    assert "nonlinear_ndtha_hip_approval_not_github_actions" in blocked["blockers"]

    for key, value in environment.items():
        monkeypatch.setenv(key, value)
    approved = hip.check_native_nonlinear_ndtha_hip(
        ROOT,
        receipt_path=receipt,
        device_lib_path=DEVICE_LIB_PATH,
        require_approved_runner=True,
