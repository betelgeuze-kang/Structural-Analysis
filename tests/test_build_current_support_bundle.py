...
def _prepare_source_mocks(monkeypatch: pytest.MonkeyPatch) -> dict[str, object]:
    ...
    fixture_file = (
        current_support.DEFAULT_CLIENT_FIXTURE / "model.json"
    ).as_posix()
...
def test_current_builder_closes_53_of_53_without_promoting_child_statuses(
    ...
    assert "human new-user observation" in payload["claim_boundary"][
        "not_granted"
    ]
    assert "product code signing or platform notarization" in payload[
        "claim_boundary"
    ]["not_granted"]
