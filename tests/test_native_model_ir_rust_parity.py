    ...
    cycle["load_combinations"] = [
        {
            ...
            "terms": [{"ref_id": "CB", "ref_kind": "load_combination", "factor": 1.0}],
            "source_id": None,
            "extensions": {},
        },
        {
            ...
            "index": 1,
            "combination_type": "linear",
            "terms": [{"ref_id": "CA", "ref_kind": "load_combination", "factor": 1.0}],
            "source_id": None,
            "extensions": {},
        },
    ]
...
def test_rust_cpp_roundtrip_and_semantic_issue_sets_match_python_oracle(
    ...
    baseline = load_json_object_strict(ROOT / FIXTURES[0])
    cases = [(path.stem, load_json_object_strict(ROOT / path)) for path in FIXTURES]
    ...
    for index, (name, payload) in enumerate(cases):
        ...
        python_issue_set = {(issue.code, issue.path) for issue in python_report.issues}

        assert (
            native_report["model_ir_schema_version"] == python_report.schema_version
        ), name
