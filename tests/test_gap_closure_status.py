...
def test_report_gap_closure_status(tmp_path: Path) -> None:
    ...
    assert (
        payload["sections"]["ai_freeze_boundary"]["autonomous_ai_engine_claim"] is False
    )
    assert (
        payload["sections"]["ai_freeze_boundary"]["surrogate_truth_claim_frozen"]
        is True
    )
    ...
    assert (
        payload["sections"]["ai_freeze_boundary"]["production_pareto_policy_claim"]
        is False
    )
    assert (
        "does not prove autonomous structural-design AI"
        in payload["sections"]["ai_freeze_boundary"]["claim_boundary"]
    )
    assert payload["artifacts"]["ai_freeze_boundary_status"].endswith(
    ...
    assert (
        payload["full_gap_ledger_summary"]["nonclosed_claim_boundary_missing_count"]
        == 0
    )
    nonclosed_rows = [
        row for row in payload["ledger_requirements"] if not row["closed"]
    ]
