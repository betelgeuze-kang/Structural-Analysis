...
class _InvalidFullStepProblem:
    case_id = "invalid_full_step_then_valid_backtrack"
    ...
    def assemble(
        ...
        displacement = float(free_displacements_m[0])
        if displacement > 0.75:
            raise ValueError("invalid_geometry_trial")
        return np.array([2.0 * displacement - 1.0]), np.array([[1.0 if displacement == 0.0 else 2.0]])


def test_vector_line_search_rejects_invalid_trial_and_backtracks() -> None:
    problem = _InvalidFullStepProblem()
    ...
    assert solution.status == "ready"
    assert solution.free_displacements_m.tolist() == pytest.approx([0.5])
    first_search = solution.line_search_history[0]
    assert first_search["selected_alpha"] == 0.5
    assert first_search["attempts"][0]["invalid_trial"] is True
    assert first_search["attempts"][0]["invalid_trial_reason"] == "ValueError"
    assert first_search["attempts"][0]["accepted"] is False
    assert problem.committed_state == "unchanged"


class _AllVectorTrialsInvalidProblem:
    case_id = "all_vector_trials_invalid"
    ...
    def assemble(
        ...
        if float(free_displacements_m[0]) != 0.0:
            raise ValueError("unsupported_constitutive_path")
        return np.array([-1.0]), np.array([[1.0]])


def test_vector_line_search_does_not_accept_zero_step_when_all_trials_invalid() -> None:
    solution = newton_raphson_vector(
        _AllVectorTrialsInvalidProblem(),
        config=NewtonRaphsonConfig(
            line_search_alphas=(1.0, 0.5),
        ),
    )
    ...
    assert solution.free_displacements_m.tolist() == [0.0]
    assert solution.convergence_history[-1]["line_search_alpha"] == 0.0
    assert solution.convergence_history[-1]["accepted"] is False
    assert all(
        attempt["invalid_trial"] is True
        for attempt in solution.line_search_history[-1]["attempts"]
    )


class _AllScalarTrialsInvalidProblem:
    case_id = "all_scalar_trials_invalid"
...
def test_scalar_line_search_does_not_accept_zero_step_when_all_trials_invalid() -> None:
    solution = newton_raphson_scalar(
        _AllScalarTrialsInvalidProblem(),
        config=NewtonRaphsonConfig(
            line_search_alphas=(1.0, 0.5),
        ),
    )
