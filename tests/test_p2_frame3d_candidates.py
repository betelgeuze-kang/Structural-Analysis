...
import structural_analysis.elements as element_api
...
def test_frame3d_reference_kernels_are_exported_from_element_namespace() -> None:
    assert element_api.COROTATIONAL_FRAME3D_PROFILE == COROTATIONAL_FRAME3D_PROFILE
    assert element_api.TimoshenkoFrame3DSection is TimoshenkoFrame3DSection
    assert element_api.TorsionWarpingProperties is TorsionWarpingProperties
...
def test_timoshenko_cantilever_matches_bending_plus_shear_closed_form() -> None:
    ...
    expected = 10.0 * length**3 / (
        3.0 * props.e_n_per_m2 * props.iz_m4
...
def test_corotational_frame3d_force_and_tangent_derive_from_same_energy() -> None:
    ...
    for index in (0, 1, 3, 7, 10):
        step = 5.0e-7 * (max(response.initial_length_m, 1.0) if index % 6 < 3 else 1.0)
