...
def repository(root, scope="research", **kwargs):
    service = DurableJobService(
        root,
        tenant_tokens={"lab": TOKEN, "other": TOKEN + "2"},
...
def test_reopen_reprices_and_rescreens_without_numerical_work(
    model, control_request, options, tmp_path
):
    first = evaluate(
        session(tmp_path / "store"), model, control_request, options, tmp_path / "fresh"
    )
    ...
    for role, ref in first["row"]["artifacts"].items():
        assert (tmp_path / "fresh" / ref["path"]).read_bytes() == (
            tmp_path / "reopened" / second["row"]["artifacts"][role]["path"]
        ).read_bytes()
    ...
    assert (
        json.loads((tmp_path / "reopened/persistence.json").read_bytes())["admitted"]
        is True
    )
...
def test_capacity_refuses_admission_without_breaking_valid_solve(
    model, control_request, options, tmp_path
):
    repo = repository(tmp_path / "store", max_bytes=1)
    s = reuse.RCControlResultSession(
        source_revision=SOURCE, scope_id="research", repository=repo
    )
    result = evaluate(s, model, control_request, options, tmp_path / "out")
    assert result["row"]["full_reference_verification_pass"] is True
    assert (
        json.loads((tmp_path / "out/persistence.json").read_bytes())["admitted"]
        is False
    )
    with pytest.raises(reuse.NewAnalysisRequired):
        evaluate(
...
def test_output_publication_failure_does_not_register(
...
def test_failed_blob_publication_rolls_back_catalog(
    model, control_request, options, tmp_path, monkeypatch
):
    ...
    with sqlite3.connect(tmp_path / "store/jobs.sqlite3") as db:
        assert (
            db.execute("SELECT COUNT(*) FROM rc_verified_result_index_v1").fetchone()[0]
            == 0
        )
    assert list(
        (tmp_path / "store/blobs").rglob("*")
    )  # Orphans are not trusted index entries.
    with pytest.raises(reuse.NewAnalysisRequired):
        evaluate(
            session(tmp_path / "store"),
            model,
...
def test_corrupt_original_blocks_reuse_without_new_solver_call(
    model, control_request, options, tmp_path, monkeypatch
):
    repo = repository(tmp_path / "store")
    first = evaluate(
...
def test_tenant_and_scope_boundaries(model, control_request, options, tmp_path):
    ...
    with pytest.raises(JobServiceError):
        RCResultRepository(
            repo._service,
            tenant_id="lab",
            authorization_token="wrong-token-over-16",
            scope_id="research",
        )
    other = RCResultRepository(
        repo._service,
        tenant_id="other",
    ...
    assert TOKEN not in "".join(
        p.read_text(errors="ignore") for p in (tmp_path / "first").rglob("*.json")
    )
...
def test_local_cli_owner_cannot_be_reconfigured(tmp_path):
    ...
    with pytest.raises(ValueError, match="authorization"):
        open_local_rc_repository(
            root,
            tenant_id="lab",
            authorization_token=TOKEN + "changed",
            scope_id="research",
        )
    with pytest.raises(ValueError, match="ownership"):
        open_local_rc_repository(
            root, tenant_id="other", authorization_token=TOKEN, scope_id="research"
        )
...
def test_subprocess_cli_restart_has_zero_new_work(control_request, options, tmp_path):
    ...
    prefix = [
        sys.executable,
        "-m",
        "structural_analysis.benchmark.rc_control_local_search_cli",
        "--model",
    ...
    for suffix, budget in (("first", "2"), ("second", "0")):
        result = subprocess.run(
            prefix
            + ["--output", str(tmp_path / suffix), "--max-new-model-analyses", budget],
            env=env,
            capture_output=True,
            text=True,
            timeout=30,
...
"""
    process = subprocess.Popen(
        [sys.executable, "-c", child, str(tmp_path / "store"), TOKEN],
...
def test_forced_replay_retains_first_persistent_original(
    model, control_request, options, tmp_path
):
    repo = repository(tmp_path / "store")
    s = reuse.RCControlResultSession(
...
def test_separate_sessions_serialize_same_key(
    model, control_request, options, tmp_path
):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            executor.submit(
                evaluate, s, model, control_request, options, tmp_path / str(i)
            )
            for i, s in enumerate((one, two))
        ]
        results = [future.result(timeout=30) for future in futures]
    assert sorted(result["mode"] for result in results) == [
        "fresh_reference_and_replay",
        "verified_original_reused",
    ]
    assert (
        sum(
            result["new_work"]["known_counters"]["attempted_step_count"]
...
def test_real_child_exit_during_persistence_leaves_no_catalog_entry(
    ...
    open_local_rc_repository(
        store,
        tenant_id="local-research",
        authorization_token=TOKEN,
        scope_id="local-research",
    )
    code = """
import os,sys
from structural_analysis.execution.job_service import DurableJobService
...
"""
    ...
    env = os.environ | {"STRUCTURAL_RC_STORE_TOKEN": TOKEN}
    result = subprocess.run(
        [sys.executable, "-c", code, *args, "--output", str(tmp_path / "crashed")],
        env=env,
        capture_output=True,
        text=True,
        timeout=30,
    ...
    assert (
        summary["scenarios"][0]["cost_bound"]["pool_minimum_feasible_estimate"] is None
    )


def test_persistent_rescreen_and_tenant_report_no_new_credit(
    model, control_request, options, tmp_path
):
    first = evaluate(
        session(tmp_path / "store"), model, control_request, options, tmp_path / "first"
    )
    ...
    result = evaluate(
        ...
        options,
        tmp_path / "rescreen",
        history_limits=FiberFrameHistoryLimits(0.1, limit),
        allow_new_analysis=False,
    )
...
def test_symlink_store_is_rejected(tmp_path):
    ...
    with pytest.raises(ValueError, match="symlink"):
        open_local_rc_repository(
            link, tenant_id="lab", authorization_token=TOKEN, scope_id="research"
        )
...
def test_owner_failure_before_publish_leaves_no_partial_authority(
...
def test_first_owner_race_never_replaces_winner(
    tmp_path, monkeypatch, same_credentials
):
    from concurrent.futures import ThreadPoolExecutor
