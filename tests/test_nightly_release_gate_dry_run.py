...
def test_nightly_release_gate_dry_run_includes_global_authority(tmp_path: Path) -> None:
    ...
    ndtha_residual_step = next(step for step in steps if step.get("step") == "ndtha_residual_gate")
    assert "--max-fallback-rate 0.05" in str(ndtha_residual_step.get("command", ""))
    assert "--strict-recommended-residual-hard-fail" in str(ndtha_residual_step.get("command", ""))
    ...
    ssi_evidence_step = next(step for step in steps if step.get("step") == "ssi_boundary_gate_evidence")
    assert "materialize-evidence" in str(ssi_evidence_step.get("command", ""))
    assert "implementation/phase1/release_evidence/performance/ssi_boundary_gate_report.json" in str(
        ssi_evidence_step.get("command", "")
    )
    ...
    contact_evidence_step = next(step for step in steps if step.get("step") == "contact_readiness_gate_evidence")
    assert "implementation/phase1/release_evidence/performance/contact_readiness_report.json" in str(
        contact_evidence_step.get("command", "")
    )
    gpu_audit_evidence_step = next(step for step in steps if step.get("step") == "gpu_bottleneck_audit_evidence")
    assert "implementation/phase1/release_evidence/performance/gpu_bottleneck_audit_report.json" in str(
        gpu_audit_evidence_step.get("command", "")
    )
    foundation_evidence_step = next(step for step in steps if step.get("step") == "foundation_soil_link_evidence")
    ...
    commercial_readiness_evidence_step = next(
        step for step in steps if step.get("step") == "commercial_readiness_gate_evidence"
    )
    assert "materialize-evidence" in str(commercial_readiness_evidence_step.get("command", ""))
    assert "implementation/phase1/release_evidence/commercial/commercial_readiness_report.json" in str(
        commercial_readiness_evidence_step.get("command", "")
    )
