...
test.beforeAll(async () => {
  server = spawn('python3', ['-B', 'tests/frontend/job_transport_server.py', '--failure-history-checkpoint'], {
...
for (const width of [1280, 390]) test(`reviews injected prior failure while preserving changed-checkpoint retry success at ${width}`, async ({ page }, info) => {
  ...
  await history.getByLabel('Previous attempt').fill('1')
  ...
  await expect(page.getByRole('heading', { name: 'Failed attempt 3', exact: true })).toHaveCount(0)
  ...
  await history.getByLabel('Previous attempt').fill('3')
...
for (const kind of ['missing', 'partial', 'wrong_digest', 'wrong_checkpoint'] as const) {
  test(`rejects ${kind} historical binding without hiding the current result`, async ({ page }) => {
    await page.route('**/failure-diagnostics/1', async route => {
      const response = await route.fetch(), headers = response.headers()
      if (kind === 'missing' || kind === 'partial') delete headers['x-structural-diagnostic-sha256']
      if (kind === 'missing') delete headers['x-structural-diagnostic-checkpoint']
      if (kind === 'wrong_digest') headers['x-structural-diagnostic-sha256'] = 'sha256:' + '0'.repeat(64)
      if (kind === 'wrong_checkpoint') headers['x-structural-diagnostic-checkpoint'] = job.checkpoint.content_hash
    ...
    await history.getByLabel('Previous attempt').fill('1')
    await history.getByRole('button', { name: 'Review previous attempt' }).click()
    await expect(history).toHaveAttribute('data-failure-history', 'invalid')
    await expect(page.locator('[data-job-result-ir]')).toHaveAttribute('data-job-result-ir', 'verified')
    await expect(page.locator('[data-job-service]')).toHaveAttribute('data-job-status', 'succeeded')
  })
}
