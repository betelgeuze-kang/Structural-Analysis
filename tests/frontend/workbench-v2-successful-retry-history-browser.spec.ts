...
test.beforeAll(async () => {
  server = spawn('python3', ['-B', 'tests/frontend/job_transport_server.py', '--failure-history-success'], {
...
for (const width of [1280, 390]) test(`reviews injected prior failure while preserving actual retry success at ${width}`, async ({ page }, info) => {
  ...
  await expect(page.getByRole('heading', { name: 'Failed attempt 2', exact: true })).toHaveCount(0)
