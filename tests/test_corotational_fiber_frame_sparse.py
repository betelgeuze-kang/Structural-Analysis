...
def test_native_coo_csr_assembly_matches_dense_and_is_deterministic() -> None:
    ...
    assert max(receipt.metrics.values()) <= 1.0e-13
    assert receipt.to_manifest()["receipt_hash"] == receipt.receipt_hash
    ...
    assert first.csr_values_kn_per_m.flags.writeable is False
    with pytest.raises(ValueError):
        first.csr_values_kn_per_m[0] = 0.0

    caller_copy = first.jacobian_csr
    caller_copy.data[0] = 0.0
    assert first.jacobian_csr.data[0] != 0.0
...
def test_sparse_load_path_uses_native_assembly_and_matches_dense_solution() -> None:
    ...
    for sparse_step, dense_step in zip(sparse.steps, dense.steps, strict=True):
        np.testing.assert_allclose(
            sparse_step.trial_assembly.residual_kn,
            dense_step.trial_assembly.residual_kn,
            rtol=1.0e-10,
            atol=1.0e-11,
        )
        np.testing.assert_allclose(
            sparse_step.trial_assembly.reactions_global,
            dense_step.trial_assembly.reactions_global,
            rtol=1.0e-10,
            atol=1.0e-10,
        )
