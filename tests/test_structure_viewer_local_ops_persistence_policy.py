...
TEMP_WORKFLOW = (
    ROOT
    / ".github/workflows/codex-temp-local-ops-persistence-current-patch.yml"
)
...
const manifestDegraded = prepareViewerLocalOpsStateForStorage({
  lastIngestPreview: previewWithPayload(null, {
    ...
    manifest: {
      projects: [{
        marker: largeManifestMarker,
        payload: 'y'.repeat(VIEWER_PROJECT_BUNDLE_MAX_SERIALIZED_BYTES + 1),
      }],
    },
  }),
...
const importDegraded = prepareViewerLocalOpsStateForStorage({
  lastImportPreview: {
    ...
    local_state: {
      reviewNotes: {
        large: 'z'.repeat(VIEWER_PROJECT_BUNDLE_MAX_SERIALIZED_BYTES + 1),
      },
    },
  },
});

const safeStoredText = JSON.stringify({reviewNotes: {safe: 'preserve'}});
let invalidWriteCount = 0;
const invalidWriteResult = writeViewerLocalOpsState({
  reviewNotes: {
    huge: 'q'.repeat(VIEWER_PROJECT_BUNDLE_MAX_SERIALIZED_BYTES + 1),
  },
}, {
  storageGet: () => safeStoredText,
  storageSet: () => { invalidWriteCount += 1; },
});

const existingReceipts = receiptRows(
        ...
        solver_receipts: [
          {
            ...
            receipt_path: 'updated.json',
          },
          {
            project_id: 'project',
            drawing_id: 'drawing',
            member_id: 'new-1',
            receipt_path: 'new.json',
          },
        ],
      }],
    }],
...
const allowedInspection = inspectViewerProjectBundleLocalState(
  allowedReceiptResult,
);

const blockedPreview = previewWithPayload(rawPayload, {
  blocked_issues: [{issue: 'blocked input'}],
});
const blockedCurrent = {reviewNotes: {safe: 'keep'}};
const blockedResult = mergeViewerEvidenceIngestPreview(
  blockedCurrent,
  blockedPreview,
);
...
console.log(JSON.stringify({
  ...
  small: {
    ...
    markerCount,
    hasNestedPayload: Object.prototype.hasOwnProperty.call(
      smallPrepared.state.lastIngestPreview,
      'renderable_payload',
    ),
    hasNormalizedRows: Object.prototype.hasOwnProperty.call(
      smallPrepared.state.lastIngestPreview,
      'normalized_rows',
    ),
  ...
  receiptAllowed: {
    ...
    hasNestedPayload: Object.prototype.hasOwnProperty.call(
      allowedReceiptResult.lastIngestPreview,
      'renderable_payload',
    ),
    markerCount: JSON.stringify(allowedReceiptResult)
      .split('RAW_RENDERABLE_MARKER').length - 1,
  },
  blocked: {
    sameObject: blockedResult === blockedCurrent,
    state: blockedResult,
  },
}));
"""
    ...
    assert "SECRET_LARGE_RAW_PAYLOAD" not in raw_degraded["serialized"]
    ...
    assert "SECRET_LARGE_MANIFEST" not in manifest_degraded["serialized"]
    ...
    assert "SECRET_IMPORT_PAYLOAD" not in import_degraded["serialized"]
    ...
    assert payload["blocked"] == {
        "sameObject": True,
        "state": {"reviewNotes": {"safe": "keep"}},
    }


def test_temporary_local_ops_patch_workflow_is_absent() -> None:
