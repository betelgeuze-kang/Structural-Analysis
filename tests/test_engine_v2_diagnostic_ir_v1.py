...
def test_manifest_architecture_base_must_match_compiled_architecture(
    sources,
) -> None:
    manifest = deepcopy(_receipt(sources).to_manifest())
    provenance = manifest["source_provenance"]
    assert provenance["compiled_architecture"] == "gfx1030"
    assert provenance["runtime_architecture_base"] == "gfx1030"
    validate_diagnostic_ir_v1_manifest(manifest)

    provenance["compiled_architecture"] = "gfx1030"
    provenance["runtime_architecture_base"] = "gfx1100"
    ...
    with pytest.raises(DiagnosticIRV1Error) as caught:
        validate_diagnostic_ir_v1_manifest(manifest)
    assert caught.value.code == "diagnostic_ir_v1_architecture_invalid"
