...
def test_actual_counterbalanced_repeats_reuse_fit_and_preserve_complete_paths(
    ...
    assert result["assembly_timing_recording"] is True
    assert ("line_search_assembly_reuse" in result) is reuse
    plan = json.loads((root / "plan.json").read_bytes())
    assert plan["assembly_timing_recording"] is True
    assert (
    ...
    for fold in result["folds"]:
        ...
        for arm in ("reference", "secant", "proposal", "fresh-reference"):
            ...
            assert sum(summary["phase_wall_ns"].values()) == sum(
                inv["newton_assembly_work"]["wall_ns"] for inv in invocations
            )
            assert sum(summary["phase_wall_ns"].values()) <= path["wall_ns"]
            assert (
                sum(
                    inv["newton_assembly_work"].get("line_search_reuse_hit_count", 0)
                    for inv in invocations
...
@pytest.mark.parametrize("timing,work", [(True, False), (1, True), (None, True)])
def test_invalid_assembly_timing_rejects_before_training_or_output(
    tmp_path, timing, work
):
