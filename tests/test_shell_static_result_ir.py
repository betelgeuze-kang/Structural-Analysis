...
def _result():
    document = load_model_ir_v2(
        Path("tests/fixtures/model_ir_v2/shell_square_linear_static.json")
    )
    payload = document.to_dict()
    load = np.zeros(24)
    load[14] = load[20] = -1_000.0
    ...
    return create_shell_result_ir(
        ...
        solver_result_hash=solved.result_hash,
        stiffness_hash=solved.stiffness_hash,
        load_hash=solved.load_hash,
        terminal_checkpoint_hash=solved.checkpoint.checkpoint_hash,
        solver_id="dense.direct.shell3.v1",
        node_ids=model.node_ids,
        element_ids=model.element_ids,
        displacement_global=solved.displacement_global,
        reaction_global_n_nm=solved.reaction_global_n_nm,
