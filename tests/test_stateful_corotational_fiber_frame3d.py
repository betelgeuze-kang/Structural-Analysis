...
def test_distributed_fiber_member_same_parent_global_tangent() -> None:
    ...
    assert type(center.axial_material_response) is StatefulCorotationalFiberFrame3DResponse
...
def test_distributed_mixed_steel_concrete_fibers_couple_axial_and_bending() -> None:
    ...
    minus[6] -= step
    finite_difference = (
        element.integrate(plus, parent, reference_section=reference).internal_force_global
        - element.integrate(
            minus,
            parent,
            reference_section=reference,
