...
def test_detached_receipt_rejects_coherently_rehashed_semantic_forgery(
    diagnostic_family: _DiagnosticFamily,
) -> None:
    ...
    for field_name in ("case_id", "case_receipt_hash"):
        duplicate = rehash_row(
            rows[1],
            **{field_name: getattr(rows[0], field_name)},
        )
        with pytest.raises(HipFgmresModelFamilyDiagnosticIRV1Error) as duplicate_error:
            validate_hip_fgmres_model_family_diagnostic_ir_receipt_v1(
                reissue((rows[0], duplicate, rows[2])),
            )
        assert duplicate_error.value.code == (
            "hip_fgmres_family_diagnostic_ir_v1_duplicate_case"
        )
        assert duplicate_error.value.path == "/observations"
