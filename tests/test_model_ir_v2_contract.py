...
def test_schema_is_draft_2020_12_and_golden_fixture_is_analysis_ready() -> None:
    ...
    assert canonicalize_model_ir_v2(document.to_dict()) == canonicalize_model_ir_v2(payload)
...
def test_non_finite_number_is_blocked_even_when_jsonschema_accepts_number_type() -> None:
    payload = _payload()
...
def test_blocking_unsupported_feature_is_valid_but_not_analysis_ready(tmp_path: Path) -> None:
    payload = _payload()
    payload["unsupported_features"] = [
        {
            "feature_id": "UF1",
...
def test_validation_cli_emits_machine_readable_claim_bounded_report(tmp_path: Path) -> None:
    ...
    assert report["schema_version"] == "structural-analysis-model-ir-validation-report.v1"
    assert report["model_ir_schema_version"] == MODEL_IR_V2_SCHEMA_VERSION
    assert report["contract_valid"] is True
    assert report["analysis_ready"] is True
    assert report["claim_boundary"] == "model_ir_contract_validation_not_solver_readiness"
