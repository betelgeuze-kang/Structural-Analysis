...
def test_build_register_prioritizes_structural_scope_cleanup_plan(tmp_path: Path) -> None:
    ...
    assert any(
        "build_structural_scope_cleanup_impact_report.py" in command
        for command in row["reproduction_commands"]
    )
    assert any(
    ...
    assert any(
        "build_structural_scope_cleanup_impact_report.py --fail-blocked" in command
        for command in row["verification_commands"]
    )
    assert any(
    ...
    assert "structural_scope_cleanup_impact_report" in row["evidence_artifacts"]
