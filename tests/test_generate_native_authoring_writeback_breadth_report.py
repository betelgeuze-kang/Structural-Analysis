...
SCRIPT = Path("implementation/phase1/generate_native_authoring_writeback_breadth_report.py")
...
def test_build_native_authoring_writeback_breadth_report_surfaces_family_coverage(tmp_path: Path) -> None:
    out = tmp_path / "release" / "authoring" / "portfolio" / "native_authoring_writeback_breadth_report.json"
    payload = build_native_authoring_writeback_breadth_report(
        ...
        family_tracks_report={
            "track_rows": [
                {"family_id": "sample_tower", "release_ready": True, "registry_ready": True},
                {"family_id": "steel_braced_frame", "release_ready": True, "registry_ready": True},
            ]
        },
    ...
    assert "Native authoring writeback breadth: PASS" in payload["summary_line"]
    assert payload["family_rows"][0]["writeback_breadth_status"] == "broad"
    assert payload["family_rows"][1]["mesh_breadth_status"] == "targeted"


def test_generate_native_authoring_writeback_breadth_report_cli(tmp_path: Path) -> None:
    portfolio = tmp_path / "portfolio.json"
    family_tracks = tmp_path / "family_tracks.json"
    runtime_submission = tmp_path / "runtime_submission.json"
    out = tmp_path / "native_authoring_writeback_breadth_report.json"
    ...
    _write_json(
        family_tracks,
        {"track_rows": [{"family_id": "composite_podium", "release_ready": True, "registry_ready": True}]},
    )
    _write_json(
        runtime_submission,
        {
    ...
    assert "Native authoring writeback breadth: PASS" in proc.stdout
