...
def test_reported_agent_is_not_a_qualified_solver(monkeypatch):
    ...
    monkeypatch.setattr(amd,'_command',lambda command:{'status':'completed','output_bytes':30,
        'output_sha256':'a'*64,'text':'  Name: gfx1030\n Name: gfx1030\n Name: cpu\nsecret-not-for-report'})
    result=amd.diagnose_amd_environment()
...
@pytest.mark.parametrize('code,timeout,limit,expected',[
    ...
    ("import time;time.sleep(4)",.1,100,'timeout'),
    ("print('x'*100000)",2,10,'output_limit'),
])
def test_subprocess_bounds(code,timeout,limit,expected):
    start=time.monotonic()
    result=amd._command([sys.executable,'-c',code],timeout=timeout,maximum_bytes=limit)
