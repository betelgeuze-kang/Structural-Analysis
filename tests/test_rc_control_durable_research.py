...
def test_reopen_reprice_and_rescreen_preserve_originals(
    ...
    for role in first["row"]["artifacts"]:
        assert (
            tmp_path / "first" / first["row"]["artifacts"][role]["path"]
        ).read_bytes() == (
            tmp_path / "second" / second["row"]["artifacts"][role]["path"]
        ).read_bytes()
    ...
    third = evaluate(
        ...
        options,
        tmp_path / "third",
        history_limits=strict,
        allow_new_analysis=False,
    )
    assert (
        third["row"]["full_reference_verification_pass"]
        and not third["row"]["selection_eligible"]
    )
...
def test_chunk_boundary_pause_resume_matches_fresh(
    model, control_request, options, tmp_path
):
    args = dict(chunk_target_count=2, max_chunks_per_call=1)
    first = evaluate(
        session(tmp_path / "store", **args),
        model,
        control_request,
        options,
        tmp_path / "prefix",
    ...
    fresh = evaluate(
        session(tmp_path / "other"), model, control_request, options, tmp_path / "fresh"
    )

    def payload(row, root):
        return json.loads(
            (root / row["row"]["artifacts"]["result"]["path"]).read_bytes()
        )["api_result"]

    assert (
        payload(final, tmp_path / "final")["response_history"]
        == payload(fresh, tmp_path / "fresh")["response_history"]
    )


def test_second_interpreter_reuses_without_calling_worker(
    model, control_request, options, tmp_path
):
    root = tmp_path / "store"
    ...
    input_file.write_text(
        json.dumps(
            {
                "model": model.to_dict(),
                "request": control_request.to_dict(),
                "history": asdict(options["history_limits"]),
                "material": asdict(options["material_limits"]),
                "prices": asdict(options["prices"]),
            }
        )
    )
...
def test_corrupt_result_rejected_before_export(
    model, control_request, options, tmp_path
):
...
def test_export_failure_keeps_only_completed_durable_result(
...
def test_cancelled_job_is_not_automatically_retried(
    model, control_request, options, tmp_path
):
    ...
    with pytest.raises(JobServiceError):
        service.cancel_job(
            first["job"]["job_id"], tenant_id="lab", authorization_token="wrong-secret"
        )
    service.cancel_job(
        first["job"]["job_id"], tenant_id="lab", authorization_token=TENANT
    )
...
def test_candidate_search_reuses_after_session_recreation(
    model, control_request, options, tmp_path
):
    candidates = (candidate("cheap", 0.35), candidate("costly", 0.5))

    def run(s, out, budget):
        return run_rc_control_cost_search(
            model,
            candidates,
            control_request,
            session=s,
...
def test_failed_worker_cannot_publish_a_reusable_result(
    model, control_request, options, tmp_path, monkeypatch
):
    ...
    def failed(service, claim, **kwargs):
        service.fail_job(
            claim.job.job_id,
            worker_id="local-rc",
            authorization_token=WORKER,
            lease_token=claim.lease_token,
            error_code="test_failure",
            retriable=False,
        )
        return service.get_job(
            claim.job.job_id, tenant_id="lab", authorization_token=TENANT
        )
...
def test_bad_chunk_limits(name, value, tmp_path):
    with pytest.raises(ValueError):
        session(tmp_path / "unused", **{name: value})
...
def test_bounded_parallel_batch_real_models(model, control_request, options, tmp_path):
    ...
    wider = d.apply_fiber_frame_section_changes(model, candidate("wide", 0.5))
    result = run_local_rc_batch(
        {"one": model, "two": wider},
        control_request,
        session=session(tmp_path / "store"),
        scope_id="lab",
    ...
    assert (
        sum(
            r["new_work"]["known_counters"]["attempted_step_count"]
...
def test_batch_stop_and_budget_leave_unrequested(
    model, control_request, options, tmp_path
):
    ...
    assert (
        ...
        and result["records"]["one"]["status"] == "unrequested"
    )
    result = run_local_rc_batch(
        {"one": model},
        control_request,
        ...
        max_requested_models=0,
        **options,
    )
    assert result["requested_count"] == 0


def test_durable_cli_cross_invocation(
    model, control_request, options, tmp_path, monkeypatch, capsys
):
    ...
    assert (
        main(
            args
            + ["--output", str(tmp_path / "first"), "--max-new-model-analyses", "2"]
        )
        == 0
    )
    capsys.readouterr()
    assert (
        main(
            args
            + ["--output", str(tmp_path / "second"), "--max-new-model-analyses", "0"]
        )
        == 0
    )
...
def test_lost_lease_without_numerical_reservation_can_be_reclaimed(
    model, control_request, options, tmp_path, monkeypatch
):
    import sqlite3
    ...
    with pytest.raises(Halt):
        evaluate(s, model, control_request, options, tmp_path / "killed")
...
def test_abandoned_numerical_reservation_is_not_retried(
    model, control_request, options, tmp_path, monkeypatch
):
    import sqlite3
...
def test_simultaneous_same_physics_does_not_double_charge_work(
    model, control_request, options, tmp_path
):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = [
            pool.submit(
                evaluate, s, model, control_request, options, tmp_path / f"run-{i}"
            )
    ...
    assert (
        sum(
            r["new_work"]["known_counters"].get("attempted_step_count", 0)
            for r in results
        )
        == 8
    )
