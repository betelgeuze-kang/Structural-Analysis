...
def test_native_distribution_capability_is_bounded_c5():
    ...
    assert "frozen v1 through v14 receipts" in distribution["claim"]
...
def test_native_deployment_capability_is_bounded_c5() -> None:
    ...
    assert "distribution v15 E2E" in deployment["claim"]
    assert "v5 self-hashed local_rootfs_diagnostic_c5 receipt" in deployment["claim"]
    assert "frozen v1 through v4 rootfs receipts" in deployment["claim"]
