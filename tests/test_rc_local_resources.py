...
def run(model, request, options, path, **kwargs):
    return search.run_rc_control_cost_search(
        model,
        (candidate("cheap", 0.35), candidate("expensive", 0.5)),
        request,
        session=reuse.RCControlResultSession(
            source_revision=SOURCE, scope_id="research"
        ),
        scope_id="research",
...
def test_cancel_before_numerical_execution(model, control_request, options, tmp_path):
    ...
    assert all(
        row["status"] == "not_run_after_cooperative_stop" for row in result["records"]
    )


def test_stop_after_baseline_preserves_verified_result_and_unresolved_cheaper(
    ...
    result = run(
        model, control_request, options, tmp_path / "partial", stop_requested=stop
    )
    assert result["status"] == "cancelled_between_models"
...
def test_small_wall_budget_stops_only_between_models(
    model, control_request, options, tmp_path
):
    result = run(
        model, control_request, options, tmp_path / "time", maximum_wall_seconds=1e-12
    )
    assert result["status"] == "wall_budget_exhausted_between_models"
    assert result["new_model_evaluations"] == 0
    plan = json.loads((tmp_path / "time/plan.json").read_bytes())
    assert (
        plan["wall_budget_scope"] == "cooperative_between_models_not_a_solver_timeout"
    )
...
def test_amd_sysfs_is_inventory_not_kernel_evidence(tmp_path):
    ...
    assert report["amd_display_devices"] == [
        {"card": "card1", "vendor_id": "0x1002", "device_id": "0x73bf"}
    ]
