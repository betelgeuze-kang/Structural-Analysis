...
SCRIPT = ROOT / "scripts" / "check_native_model_ir_linear_jobs.py"
SPEC = importlib.util.spec_from_file_location("check_native_model_ir_linear_jobs", SCRIPT)
assert SPEC is not None and SPEC.loader is not None
checker = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(checker)


def test_repository_contract_closes_only_bounded_c5_composition() -> None:
    report = checker.check_model_ir_linear_jobs(ROOT)
    assert report["contract_pass"] is True, report["blockers"]
    assert report["durable_job_gate"] == "C5"
    assert report["service_api_gate"] == "C5"
...
def test_checker_fails_closed_on_missing_evidence_and_gate_drift(
    ...
    payload["capabilities"]["modelir_linear_service_api"]["cutover_gate"] = "C6"
    manifest_destination.write_text(json.dumps(payload), encoding="utf-8")
    (tmp_path / "native/crates/structural-runtime/tests/model_ir_linear_durable_job.rs").unlink()

    report = checker.check_model_ir_linear_jobs(tmp_path)
